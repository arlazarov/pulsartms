...
internal static class FuelStationDiagnosis
{
  ...
  private static readonly Guid DispatchId = Guid.Parse(
    "c0cf7891-4822-45ac-a1f1-347aa47b45ae"
  );
  ...
  public static async Task RunAsync(NpgsqlConnection connection)
  {
    ...
    )
    {
      ...
      {
        Console.WriteLine("No bounded matching truck fuel snapshot found.");
        return;
      }
      snapshot =
        ...
        ?? throw new InvalidOperationException(
          "The bounded snapshot is invalid."
        );
      baseline =
        JsonSerializer
          ...
          ?.Baseline
        ?? throw new InvalidOperationException(
          "The snapshot has no saved baseline."
        );
    }
    if (
      snapshot.TruckId != TruckId
      || snapshot.RootDispatchId != DispatchId
      ...
      throw new InvalidOperationException(
        "The snapshot exceeds the diagnostic scope or geometry bound."
      );
    ...
    )
    {
      ...
      while (await rows.ReadAsync(ct))
        if (!rows.IsDBNull(5) && !rows.IsDBNull(6))
          stations.Add(
            new(
              rows.GetGuid(0),
              rows.GetString(1),
              ...
              rows.GetString(3),
              rows.GetString(4),
              new((double)rows.GetDecimal(5), (double)rows.GetDecimal(6))
            )
          );
    }
    if (stations.Count != 1)
    {
      Console.WriteLine(
        $"Expected one exact station; found {stations.Count} bounded matches."
      );
      return;
    }
      ...
      while (await rows.ReadAsync(ct))
        quotes.Add(
          new(
            ...
            rows.GetString(1),
            rows.GetDecimal(2),
            rows.GetFieldValue<DateOnly>(3),
            rows.GetFieldValue<DateOnly>(4)
          )
        );
    }
    ...
    var price =
      quotes
        .Where(q =>
          q.Currency.Equals("USD", StringComparison.OrdinalIgnoreCase)
          && q.Product.Contains("diesel", StringComparison.OrdinalIgnoreCase)
          && !q.Product.Contains("reefer", StringComparison.OrdinalIgnoreCase)
        )
        .Select(q => (double?)q.Cash)
        .Min() ?? 1;
    var priced = new PricedFuelStation(
      new()
      {
      ...
      .Legs.Select(
        (leg, index) =>
        {
          ...
          return new
          {
            ...
            stop.Stop.Job,
            stop.Stop.Point,
            role = stop.DispatchId == DispatchId ? "current" : "future",
            finalLeg = index == baseline.Legs.Count - 1,
            matchedBaselineMiles = along,
            geographicAwayMiles = match.Away,
            withinSearchRadius = match.Away <= FuelAccessEstimate.NearbyMiles,
            endpointFilterRejects = along < 1 || along >= baseline.Miles - 1,
            estimatedRoundTripMiles = FuelAccessEstimate.DistanceMiles(
              match.Away
            ) * 2,
            occurrenceIncluded = occurrences.Any(candidate =>
              candidate.LegIndex == index
            ),
          };
        }
      )
    ...
    var profile = JsonSerializer.Deserialize<TruckRouteProfile>(
    ...
    if (profile is not null && profile.UseIfta == plan.UsesIfta)
    {
      var quote = FuelRegionGrid
        .Prices(
          [
            new(
              ...
              "",
              "US",
              (decimal)station.Point.Latitude,
              (decimal)station.Point.Longitude,
              quotes
                .Select(q => new FuelDiscountDto(
                  q.Currency,
                  q.Product,
                  q.Cash,
                  q.Cash,
        ...
        .SingleOrDefault();
      var selected = plan.Stops.SingleOrDefault(stop =>
        stop.Name == "LOVES #399"
      );
      if (quote is not null && selected is not null && profile.Mpg is > 0)
      {
        ...
        var grossSavings =
          selected.BuyGallons
          * (selected.EconomicUsdPerGallon - quote.EconomicUsd);
        comparison = new
        {
          basis = "Same saved purchase quantity, comparing estimated access only; not a complete replacement itinerary",
          quantityGallons = selected.BuyGallons,
          selectedEconomicUsdPerGallon = selected.EconomicUsdPerGallon,
          targetEconomicUsdPerGallon = quote.EconomicUsd,
          grossSavingsUsd = grossSavings,
          targetAccessMiles = targetMiles,
          selectedAccessMiles = selected.DetourMiles,
          extraAccessMiles = extraMiles,
          targetAccessMinutes = targetMinutes,
          selectedAccessMinutes = selected.DetourMinutes,
          extraAccessMinutes = extraMinutes,
          extraFuelGallons = extraGallons,
          extraFuelCostUsd = fuelCost,
          extraDrivingCostUsd = timeCost,
          additionalEstimatedAccessCostUsd = fuelCost + timeCost,
          netEstimatedSavingsUsd = grossSavings - fuelCost - timeCost,
          commonStopCostUsd = profile.StopCostUsd,
          stopCostDifferenceUsd = 0,
    ...
    Console.WriteLine(
      JsonSerializer.Serialize(
        new
        {
          ...
          snapshotSearchRadiusMiles = plan.EstimatedStationAccess
          && plan.SelectionVersion == 21
            ? 2
            : (double?)null,
          plan.DispatchIds,
          baselineMiles = baseline.Miles,
          plan.RemainingMiles,
          plan.StartingGallons,
          plan.PurchaseCostUsd,
          plan.EconomicCostUsd,
          plan.ExpectedFutureFuelCostUsd,
          station,
          currentCashQuotes = quotes,
          estimatedPriceComparison = comparison,
          matches = legs,
          occurrences = occurrences.Select(candidate => new
          {
            candidate.LegIndex,
