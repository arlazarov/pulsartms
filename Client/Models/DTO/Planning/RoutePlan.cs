...
public sealed class RoutePlan
{
  ...
  public int NextStopLegCount => Math.Clamp(Stops.FindIndex(s => s.Id == Tracking.NextStopId) + (FromCurrentPosition ? 1 : 0), 0, Route.Legs.Count);
  public double NextStopRouteMiles => Route.Legs.Take(NextStopLegCount).Sum(l => l.Miles);
  public double? NextStopDistance => Tracking.NextStopId.HasValue ? Route.Legs.Take(NextStopLegCount).LastOrDefault()?.Miles ?? 0 : null;
  public double? RemainingToNextStop(double? progress) => Tracking.NextStopId.HasValue && progress.HasValue
    ? Math.Max(0, NextStopRouteMiles - progress.Value) : null;
}
