...
public partial class Settings : IDisposable
{
    ...
    private async Task LoadAsync()
    {
        _loading = true;
        _error = null;
        var result = await Api.GetAsync<PlanningSettingsState>("api/settings/planning", _lifetime.Token);
        if (_disposed) return;
        _loading = false;
    ...
    private async Task SaveAsync()
    {
        ...
        _saved = false;
        _error = null;
        var result = await Api.PutAsync<PlanningSettingsUpdate, PlanningSettingsState>("api/settings/planning", new(_form, _revision), _lifetime.Token);
        if (_disposed) return;
        _saving = false;
        if (!result.Success || result.Response is null) { _error = result.ErrorMessage; return; }
        _revision = result.Response.Revision;
