...
public partial class DispatchNumberSettings : IDisposable
{
  ...
  [CascadingParameter(Name = "DispatchSettingsChanged")]
  public Action<DispatchSettingsState>? SettingsChanged { get; set; }
  ...
  private string Example =>
    LoadNumberDisplay.Format(1373, _form.LoadNumberPrefix?.Trim());
  ...
  private async Task SaveAsync()
  {
    ...
    _saved = false;
    _error = null;
    var request = new DispatchSettingsUpdate(
      _form.LoadNumberPrefix?.Trim() ?? "",
      _revision
    );
    var result = await Api.PutAsync<
      DispatchSettingsUpdate,
      DispatchSettingsState
    >("api/settings/dispatch", request, _lifetime.Token);
    if (_disposed)
      return;
    _saving = false;
    if (!result.Success || result.Response is null)
  ...
  private sealed class PrefixForm
  {
    [StringLength(
      16,
      ErrorMessage = "Use at most 16 characters for the prefix."
    )]
