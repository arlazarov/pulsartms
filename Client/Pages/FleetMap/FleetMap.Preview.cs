...
public partial class FleetMap
{
  ...
  private async Task<AutomaticPlanningResult?> LoadSavedPreviewAsync(Guid truckId)
  {
    using var request = CancellationTokenSource.CreateLinkedTokenSource(_lifetime.Token);
    _previewRequest = request;
    try
    {
      return await PlanningCache.ReadPreviewAsync(truckId, request.Token)
    ...
    finally { if (ReferenceEquals(_previewRequest, request)) _previewRequest = null; }
  }
}
