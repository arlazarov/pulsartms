...
class Events {
  ...
  fire(name, values = {}) {
    const event = {target: this, pointerId: 1, button: 0, isPrimary: true, clientX: 20, clientY: 20,
      detail: 1, prevented: false, stopped: false, ...values,
      preventDefault() { this.prevented = true; }, stopPropagation() { this.stopped = true; }};
    for (const callback of [...this.listeners.get(name) ?? []]) callback(event);
    return event;
  }
  get listenerCount() { return [...this.listeners.values()].reduce((count, entries) => count + entries.size, 0); }
}
...
class Element extends Events {
  ...
  append(child) { child.parent = this; child.ownerDocument = this.ownerDocument; this.children.push(child); return child; }
  ...
  matches(selector) {
    ...
    if (selector === '[data-reorder-key]') return this.dataset.reorderKey !== undefined;
    if (selector === '[data-reorder-handle]') return this.dataset.reorderHandle !== undefined;
    if (selector === '[data-reorder-surface]') return this.dataset.reorderSurface !== undefined;
    return false;
  }
  ...
  set scrollLeft(value) { this._scrollLeft = Math.max(0, Math.min(this.scrollWidth - this.clientWidth, value)); }
  get scrollTop() { return this._scrollTop; }
  set scrollTop(value) { this._scrollTop = Math.max(0, Math.min(this.scrollHeight - this.clientHeight, value)); }
  getBoundingClientRect() {
    const left = this.parent?.scrollLeft ?? 0;
    const top = this.parent?.scrollTop ?? 0;
    return {left: this.rect.left - left, right: this.rect.right - left,
      top: this.rect.top - top, bottom: this.rect.bottom - top};
  }
}
...
function fixture(t, horizontal = false) {
  ...
  const items = ['a', 'b', 'c'].map((key, index) => {
    ...
    row.rect = horizontal ? {left: index * 200, right: index * 200 + 180, top: 0, bottom: 100}
  ...
  const calls = [];
  const connection = attachReorderList(surface, {invokeMethodAsync(...args) { calls.push(args); return Promise.resolve(); }});
  t.after(() => connection.dispose());
  return {document, view, surface, items, frames, calls, connection,
    down(index = 0, values = {}) { return surface.fire('pointerdown', {target: items[index].icon, ...values}); },
    move(values = {}) { return document.fire('pointermove', values); },
    up(values = {}) { return document.fire('pointerup', values); },
...
test('pointer drag decorates source and target and sends one ordered move without mutating DOM', async t => {
  ...
  assert.deepEqual(f.surface.children.map(item => item.dataset.reorderKey), ['a', 'b', 'c']);
  assert.equal(f.items.every(item => item.row.classes.size === 0), true);
...
test('a short tap retains normal click behavior and does not reorder', async t => {
  ...
  assert.equal(f.surface.fire('click', {target: f.items[0].icon}).prevented, false);
});
...
test('drag suppresses the resulting click but does not suppress keyboard or the next pointer click', async t => {
  ...
  f.move({clientY: 275});
  f.up({clientY: 275});
  assert.equal(f.surface.fire('click', {target: f.items[0].icon, detail: 0}).prevented, false);
  assert.equal(f.surface.fire('click', {target: f.items[0].icon}).prevented, true);
  f.down();
  f.up();
  assert.equal(f.surface.fire('click', {target: f.items[0].icon}).prevented, false);
  await Promise.resolve();
...
test('only enabled primary-pointer handles start a drag, leaving other controls and scrolling alone', t => {
  ...
  for (const apply of [() => { first.handle.disabled = true; },
  ...
  for (const values of [{button: 2}, {isPrimary: false}, {target: first.control}]) {
...
for (const cancel of ['Escape', 'pointercancel', 'lostpointercapture', 'blur', 'dispose']) {
  test(`${cancel} cancels the drag and releases all transient listeners and frames`, async t => {
    const f = fixture(t);
    f.down();
    f.move({clientY: 275});
    assert.equal(f.frames.size, 1);
    if (cancel === 'Escape') assert.equal(f.document.fire('keydown', {key: 'Escape'}).prevented, true);
    else if (cancel === 'blur') f.view.fire('blur');
    else if (cancel === 'dispose') { f.connection.dispose(); f.connection.dispose(); }
...
test('outside drops and adjacent positions that leave order unchanged do not invoke Blazor', async t => {
  const f = fixture(t);
  for (const [source, startY, targetY] of [[0, 20, 110], [1, 120, 70], [0, 20, 350]]) {
...
test('arrow keys on enabled handles prevent scrolling without blocking Blazor or ordinary keyboard navigation', t => {
  ...
  for (const values of [{target: f.items[0].handle, key: 'Tab'}, {target: f.items[0].handle, key: 'Enter'},
    {target: f.items[0].control, key: 'ArrowDown'}]) {
    assert.equal(f.surface.fire('keydown', values).prevented, false);
  }
  f.items[0].handle.disabled = true;
  assert.equal(f.surface.fire('keydown', {target: f.items[0].handle, key: 'ArrowDown'}).prevented, false);
});
