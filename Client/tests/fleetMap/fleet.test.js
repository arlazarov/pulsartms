...
test('coordinates reject invalid values and preserve zero', () => {
  ...
  assert.equal(
    createTruckPoint({ latitude: 0, longitude: 0, updatedAt: 'bad' }),
    null,
  );
});
...
test('playback crosses north by the shortest heading and interpolates speed', () => {
  const points = [
    { latitude: 0, longitude: 0, gpsTime: 1000, heading: 350, speed: 10 },
...
test('playback keeps a full polling buffer and resumes without flying after an outage', () => {
  assert.equal(truckPlaybackDelay, 90000);
  ...
  assert.equal(advancePlaybackTime(90000, 130000, 110000, 100), 90100);
  assert.equal(advancePlaybackTime(90000, 130000, 110000, 10000), 110000);
  assert.equal(advancePlaybackTime(110000, 130000, 110010, 100), 110010);
  assert.equal(advancePlaybackTime(110000, 130000, 109000, 100), 110000);
});

test('duplicate telemetry timestamps are replaced and memory stays bounded', () => {
  const points = Array.from({ length: 500 }, (_, n) => ({
    gpsTime: n,
    latitude: n,
  }));
  const merged = mergeTruckPoints(
    points,
    [{ gpsTime: 499, latitude: 7 }],
    null,
    0,
  );
...
test('station selection respects date and coordinate validity', () => {
  ...
  const stations = [0, 100].map(latitude => ({
    latitude,
    longitude: 0,
    discounts,
...
test('station price bands preserve ten-cent differences despite an outlier', () => {
  ...
  assert.notEqual(
    priceColor(3, stats, palette),
    priceColor(3.1, stats, palette),
  );
  assert.notEqual(
    priceColor(3.1, stats, palette),
    priceColor(3.2, stats, palette),
  );
  assert.notEqual(
    priceColor(3.2, stats, palette),
    priceColor(3.3, stats, palette),
  );
...
test('widely separated expensive station prices do not collapse to the same red', () => {
  ...
  assert.notEqual(
    priceColor(6, stats, palette),
    priceColor(7.7, stats, palette),
  );
...
test('global scale keeps a nineteen-cent price difference visually distinct', () => {
  ...
  const colorDistance = first.reduce(
    (sum, value, index) => sum + Math.abs(value - second[index]),
    0,
  );
  assert.ok(
    colorDistance >= 120,
    `expected strong color distance, received ${colorDistance}`,
  );
});
...
test('new telemetry blends from the displayed position without snapping or rotating the long way', async () => {
  ...
  assert.equal(blendTruckPosition(from, to, 0.25).latitude, 40.5);
  assert.equal(blendTruckPosition(from, to, 0.5).latitude, 41);
  assert.equal(blendTruckPosition(from, to, 0.5).heading, 0);
