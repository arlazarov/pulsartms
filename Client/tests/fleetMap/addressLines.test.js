...
test('stop addresses separate the street from the city, region, postal code and country', () => {
  ...
  assert.deepEqual(
    ...
    {
      street: '308 Springhill Farm Rd, building 3',
      locality: 'Fort Mill, SC 29715, US',
    },
  );
  assert.deepEqual(
    addressLines('100 Main St, Unit 4, Toronto, ON, M5V 2T6, Canada'),
  ...
  assert.deepEqual(addressLines('100 Main St, Toronto, ON M5V 2T6, CA'), {
    street: '100 Main St',
    locality: 'Toronto, ON M5V 2T6, CA',
  });
});
...
test('unstructured or incomplete addresses are preserved rather than inventing a street or locality', () => {
  for (const value of [
    ...
    'Rome, NY, 13440, US',
    '100 Main St, Toronto',
    '100 Main St, Building 3, SC, 29715, US',
    '100 Main St, Rome, XX, 13440, US',
