...
function fixture(t, formatDistance) {
  ...
  globalThis.document.createElementNS = (_, tagName) =>
...
const plan = (stops = [stop()]) => ({
  ...
  route: { legs: stops.map(() => ({ miles: 100 })) },
});
const rows = content =>
  (content.children ?? []).flatMap(child => [child, ...rows(child)]);
const row = (content, className) =>
  rows(content).find(node => node.className?.split(' ').includes(className));
const fieldValue = (content, className) =>
  row(content, className).children.find(node => node.tagName === 'dd')
    .children[0].textContent;

test('changing units refreshes an open stop without replacing markers or losing selection', t => {
  let unit = 'miles';
  const { stops, markers, state, calls } = fixture(t, miles =>
    distanceLabel(miles, unit),
  );
  stops.setPlan(plan());
  stops.setProgress(20);
  markers[0].onSelect();
  assert.equal(fieldValue(state.shown, 'fleet-route-popup__distance'), '80 mi');
  unit = 'kilometers';
  stops.refreshDistances();
  assert.equal(
    fieldValue(state.shown, 'fleet-route-popup__distance'),
    '129 km',
  );
  unit = 'both';
  stops.refreshDistances();
...
test('current equipment stop shows the confirmed action and state without calling it pickup', t => {
...
test('arrival fuel is always visible, uses exact stop identity and clears invalidated balances', t => {
  ...
  assert.ok(
    row(
      row(state.shown, 'fleet-route-popup__information'),
      'fleet-route-popup__fuel',
    ),
  );
  assert.equal(
    row(
      row(state.shown, 'fleet-route-popup__location'),
      'fleet-route-popup__fuel',
    ),
    undefined,
  );
        ...
        stops: [
          {
            ...
            appointment: '2026-09-10T13:00:00-04:00',
            lateMinutes: 60,
            hours: {
              cycleVerified,
              cycleAtArrivalMinutes: -80,
              cycleAfterStopMinutes: -200,
              alternatives: [],
            },
          },
        ],
...
test('an unavailable current stop keeps one compact ETA field without internal maintenance diagnostics', t => {
  ...
  assert.equal(
    rows(state.shown).filter(node =>
      node.className?.split(' ').includes('fleet-route-popup__eta'),
    ).length,
    1,
  );
  assert.ok(
    rows(state.shown).every(
      node =>
        ![node.textContent, node.title, node['aria-label']].includes(reason),
    ),
  );
...
test('both current stops show and copy authoritative load and order numbers', async t => {
  ...
  stops.setPlan({
    ...plan([stop(), stop({ id: 'delivery', job: 'Drop Off' })]),
    dispatchId,
  });
  stops.setLoadReference({
    dispatchId,
    loadNumber: 1373,
    loadLabel: 'AMF1373',
...
test('presentation prefix updates only popup text and copies the original numeric identity', async t => {
  ...
  assert.equal(
    ...
    '1373',
    'unknown settings never invent a prefix',
  );
  for (const loadLabel of [
    'AMF1373',
    'ZX-1373',
    '1373',
    '<img src=x onerror=alert(1)>1373',
  ]) {
  ...
  stops.setLoadReference({
    dispatchId,
    loadNumber: 1373,
    loadLabel: '<img src=x onerror=alert(1)>1373',
  });
...
test('late reference updates reuse unchanged content, reject another load and never reopen closed stops', t => {
  ...
  assert.equal(
    calls.creates,
    creates,
    'unchanged metadata must not rebuild the card',
  );
  stops.setLoadReference({
    ...reference,
  ...
  assert.equal(
    state.shown,
    content,
    'late details from another dispatch cannot replace the active identity',
  );
  stops.close();
  ...
  assert.equal(
    row(state.shown, 'fleet-map-route-info__load'),
    undefined,
    'another dispatch never inherits identifiers',
  );
  stops.setLoadReference(reference);
  assert.equal(row(state.shown, 'fleet-map-route-info__load'), undefined);
});
...
test('current pickup and delivery link only to the authoritative dispatch and preserve popup caching', t => {
  ...
  assert.equal(
    state.shown,
    content,
    'reopening retains the same link and content',
  );
  stops.setPlan({
    ...route,
    dispatchId: '44444444-4444-4444-4444-444444444444',
...
test('current stop shows only its appointment reference under the address and updates without leaking notes', t => {
  ...
  assert.ok(
    row(state.shown, 'fleet-route-popup__distance')
      .className.split(' ')
      .includes('fleet-route-popup__section-start'),
  );
  assert.equal(state.shown.children[0].children.at(-1), reference);
  assert.ok(
    rows(state.shown).every(
      child =>
        !/DL654321|42845601|Service for Load|confirmation number/.test(
          child.textContent ?? '',
        ),
    ),
  );
  const original = state.shown,
    creates = calls.creates;
  stops.setPlan(
    plan([stop({ notes: `${notes} Other unrelated information.` })]),
  );
  assert.equal(state.shown, original);
  assert.equal(
    calls.creates,
    creates,
    'unrelated note edits must not regenerate the popup',
  );
  stops.setPlan(plan([stop({ notes, job: 'Drop Off' })]));
  assert.deepEqual(
    row(state.shown, 'fleet-route-popup__reference').children.map(
  ...
  assert.ok(
    rows(state.shown).every(
      child =>
        !/PU123456|42845601|Service for Load/.test(child.textContent ?? ''),
    ),
  );
  stops.setPlan(
    plan([
  ...
  assert.equal(
    calls.opens,
    1,
    'reference changes update the open card without reselecting it',
  );
});
...
test('current stops keep numbered pickup and delivery circles without persistent text labels', t => {
  ...
  const route = plan([
    stop(),
    stop({ id: 'delivery', job: 'Delivery', name: 'Customer' }),
  ]);
  stops.setPlan(route);
  stops.setProgress(20);
  stops.setEtas(
  ...
  assert.ok(
    markers.every(
      marker => marker.map && typeof marker.onSelect === 'function',
    ),
  );
  assert.equal(
    calls.creates,
    0,
    'unselected current stops do not create popup content',
  );

  markers[1].onSelect();
  assert.equal(
    fieldValue(state.shown, 'fleet-route-popup__eta'),
    'ETA Sep 10, 02:00 PM local',
  );
  assert.equal(
    fieldValue(state.shown, 'fleet-route-popup__distance'),
    '180 mi · 290 km',
  ...
  assert.equal(
    calls.labelWrites,
    0,
    'polling must not generate or update hidden stop labels',
  );
});
...
test('current-stop details show the local appointment window, exact ETA status and remaining miles and kilometers', t => {
  const { stops, markers, calls, state } = fixture(t);
  const eta = {
    text: 'ETA Sep 9, 03:45 AM local · Late',
    arrivalText: 'Sep 9, 03:45 AM',
    statusText: 'Late',
    tone: 'danger',
  };
  stops.setPlan(
  ...
  assert.equal(
    calls.creates,
    0,
    'unselected stops do not create details content',
  );
  ...
  assert.equal(calls.opens, 1);
  assert.equal(
    state.shown.className,
    'fleet-route-popup fleet-route-popup--stop',
  );
  assert.deepEqual(
    state.shown.children.map(section => section.className),
  ...
  assert.equal(
    row(state.shown, 'fleet-route-popup__status').textContent,
    'Late',
  );
  assert.ok(
    row(state.shown, 'fleet-route-popup__eta')
      .className.split(' ')
      .includes('fleet-route-popup__eta--danger'),
  );
  assert.equal(
    fieldValue(state.shown, 'fleet-route-popup__distance'),
    '80 mi · 129 km',
  ...
  assert.ok(
    rows(state.shown).every(
      node =>
        !/STEELCOILS|Commodity|Service for Load/.test(node.textContent ?? ''),
    ),
  );
});

test('five load occurrences keep exact appointments and repeat context without merging coincident stops', t => {
  const { stops, markers, state, calls } = fixture(t);
  const webster = {
    address: '675 Basket Rd, Webster, NY 14580, US',
    point: { latitude: 43.2, longitude: -77.4 },
  };
  const routeStops = [
    stop({ id: 'webster-1', ...webster, scheduledTime: '02:00:00' }),
    stop({
      id: 'amsterdam',
      address: '100 Amsterdam Rd, Amsterdam, NY 12010, US',
      scheduledTime: '11:00:00',
    }),
    stop({ id: 'webster-2', ...webster, scheduledTime: '13:00:00' }),
    stop({ id: 'webster-3', ...webster, scheduledTime: '14:00:00' }),
    stop({
      id: 'delivery',
      job: 'Delivery',
  ...
  for (const [index, time, visit] of [
    ...
    [2, '01:00 PM', 'Visit 2 of 3'],
    [3, '02:00 PM', 'Visit 3 of 3'],
  ]) {
    ...
    assert.equal(
      row(state.shown, 'fleet-route-popup__visit').title,
  ...
  assert.equal(
    calls.creates,
    creates,
    'unchanged visit metadata reuses its content',
  );
  stops.setPlan({
    ...route,
    referenceStops: routeStops,
  ...
  assert.equal(
    markers.length,
    5,
    'progress updates do not recreate retained markers',
  );
  assert.equal(calls.opens, opens, 'progress must not activate another visit');
  assert.deepEqual(
    route,
    original,
    'presentation does not alter route geometry or metadata',
  );
});

test('same coordinates without matching known addresses never invent a repeat-visit claim', t => {
  const { stops, markers, state } = fixture(t);
  stops.setPlan(
    plan([
      stop({ id: 'one', address: '' }),
      stop({ id: 'two', address: '' }),
      stop({ id: 'three', address: '1 Main St, Webster, NY' }),
      stop({ id: 'four', address: '1 Main St, Amsterdam, NY' }),
      stop({ id: 'five', address: '675 Basket Rd' }),
      stop({ id: 'six', address: '675 Basket Rd' }),
    ]),
  );
  for (const marker of markers) {
    marker.onSelect();
    assert.equal(row(state.shown, 'fleet-route-popup__visit'), undefined);
  }
});
...
test('current-stop popup keeps previous content unchanged through pending grace and replaces it when ready', t => {
  ...
  const etaStop = {
    ...
    arrival: '2026-09-10T14:00:00-04:00',
    timeZoneId: 'America/Toronto',
    appointment: '2026-09-10T11:00:00-04:00',
    lateMinutes: 185,
  };
  const pending = {
  ...
  markers[0].onSelect();
  const completeContent = state.shown,
    completeCreates = calls.creates;
  setEta(pending);
  assert.equal(state.shown, completeContent);
  assert.equal(
    calls.creates,
    completeCreates,
    'entering pending does not regenerate unchanged content',
  );
  assert.equal(
    row(state.shown, 'fleet-route-popup__status').textContent,
    'Late',
  ...
  assert.ok(
    !rows(state.shown).some(node => /Updating|Previous/.test(node.textContent)),
  );
  ...
  assert.equal(
    calls.creates,
    creates,
    'unchanged pending display must not regenerate the card',
  );
  setEta(pending, now + 15 * 60_000);
  assert.equal(
    row(state.shown, 'fleet-route-popup__status--previous'),
    undefined,
  );
  assert.equal(fieldValue(state.shown, 'fleet-route-popup__eta'), '—');
  setEta(pending);
  setEta({
    ...
    routeUpdatePending: false,
    stops: [
      { ...etaStop, arrival: '2026-09-10T10:00:00-04:00', lateMinutes: 0 },
    ],
  });
  assert.equal(
    row(state.shown, 'fleet-route-popup__status--previous'),
  ...
  assert.equal(
    calls.opens,
    1,
    'refresh updates the selected card without another selection',
  );
});
...
test('selected details refresh only changed display values and never coordinate another card during refresh', t => {
  ...
  Object.assign(unchanged.stops[0], {
    scheduledTime: '00:00:59',
    notes: ' Gate B ',
  });
  ...
  assert.equal(
    calls.shows.length,
    1,
    'unchanged display text does not replace or show content',
  );
  ...
  assert.ok(
    row(state.shown, 'fleet-route-popup__eta')
      .className.split(' ')
      .includes('fleet-route-popup__eta--success'),
  );
  assert.equal(
    calls.shows.length,
    3,
    'a changed tone refreshes even when the ETA text is identical',
  );
  stops.setEtas(
    new Map([['pickup', { ...eta, tone: 'success', statusText: 'On time' }]]),
  );
  assert.equal(
    row(state.shown, 'fleet-route-popup__status').textContent,
    'On time',
  ...
  assert.equal(
    calls.shows.length,
    4,
    'a changed status refreshes independently from arrival text and tone',
  );
  const updated = structuredClone(unchanged);
  Object.assign(updated.stops[0], {
    scheduledTime: '04:30:00',
  ...
  assert.ok(
    rows(state.shown).some(node => node.textContent === '456 Main Street'),
  );
  assert.equal(calls.shows.length, 5);
  assert.equal(
    calls.opens,
    1,
    'only explicit stop selection coordinates other cards',
  );
  ...
  assert.equal(
    state.shown,
    retained,
    'reopening unchanged details reuses the cached content',
  );
...
test('ETA expiry and unknown distance stay explicit, and a closed or passed stop never reopens on updates', t => {
  ...
  stops.setEtas(
    new Map([
      ['pickup', { text: 'ETA Sep 9, 03:45 AM local', tone: 'success' }],
    ]),
  );
  markers[0].onSelect();
  stops.setEtas(new Map());
  ...
  assert.equal(fieldValue(state.shown, 'fleet-route-popup__eta'), '—');
  assert.equal(
    row(state.shown, 'fleet-route-popup__eta').className,
    'fleet-route-popup__field fleet-route-popup__eta',
  );
  ...
  assert.equal(
    calls.shows.length,
    count,
    'different unknown progress values have the same display',
  );
  ...
  stops.setEtas(
    new Map([['pickup', { text: 'ETA Sep 9, 04:00 AM local', tone: 'eta' }]]),
  );
  stops.setPlan(route);
  assert.equal(state.shown, null);
...
test('reference-stop details keep unknown distance and render provider text as text', t => {
  ...
  assert.equal(fieldValue(state.shown, 'fleet-route-popup__eta'), '—');
  assert.ok(
    rows(state.shown).every(node =>
      ['div', 'strong', 'dl', 'dt', 'dd', 'span'].includes(node.tagName),
    ),
  );
  markers[1].onSelect();
...
test('current popup renders signed hours and explicit alternatives with metadata-only updates', t => {
  ...
  const update = value =>
    stops.setEtas(
      new Map([['pickup', stopEtaLabels(value, now).get('load:pickup')]]),
    );
  stops.setPlan(plan());
  update(eta);
  ...
  assert.ok(
    rows(state.shown).some(node =>
      node.className?.includes('stop-hours__value--danger'),
    ),
  );
  assert.equal(markers.length, 1);
  assert.equal(calls.opens, 1);
