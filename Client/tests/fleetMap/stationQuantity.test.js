...
test('station quantity uses country and converts US gallons to liters for Canada', () => {
  ...
  assert.equal(stationQuantity(50, {country:'US',region:'CA'}), '50 US gal');
...
test('full refill hides quantities in both countries', async () => {
  const { stationPurchase } = await import('../../Scripts/fleetMap/stations/stationQuantity.js');
  assert.equal(stationPurchase({ gallons:86, full:true }, {country:'US'}), 'Fill up');
  assert.equal(stationPurchase({ gallons:50, full:true }, {country:'CA'}), 'Fill up');
  assert.equal(stationPurchase({ gallons:50, full:false }, {country:'US'}), 'Buy 50 US gal');
});
