...
test('station editing is explicit, uses occurrence identity and disappears without a selected truck', t => {
  ...
  const element = () => ({children: [], listeners: {}, append(...nodes) { this.children.push(...nodes); },
    replaceChildren(...nodes) { this.children = nodes; }, setAttribute() {},
    addEventListener(name, handler) { this.listeners[name] = handler; },
    removeEventListener(name) { delete this.listeners[name]; }, remove() {}});
  ...
  popup.update(data);
  const actions = popup.element.children.find(node => node.className === 'fleet-station-popup__actions');
  ...
  assert.deepEqual(calls[0], {stationId: 'station-id', name: 'A station', beforeStopId: null, addNew: true});
  popup.update({...data, fuel: {visits: [{number: 3, beforeStopId: 'leg-stop', gallons: 20}]}});
  assert.equal(actions.children[0].textContent, 'Edit fuel plan');
  assert.equal(popup.element.className, 'fleet-station-popup fleet-station-popup--planned fleet-station-popup--single');
  assert.equal(popup.element.children.find(node => node.className === 'fleet-station-popup__plan-label').textContent, 'Fuel stop 3');
  ...
  actions.children[0].listeners.click({stopPropagation() {}});
  assert.equal(calls.length, 3, 'a formerly visible action must not edit after deselection');
  popup.dispose();
...
test('unchanged polling does not mutate popup content or trigger InfoWindow layout', () => {
  ...
  globalThis.document = { createElement() {
    return new Proxy({ children: [], append(...nodes) { this.children.push(...nodes); },
      setAttribute() {}, addEventListener() {}, removeEventListener() {}, remove() {} },
    { set(target, name, value) { writes++; target[name] = value; return true; } });
  }, createElementNS() { return this.createElement(); } };
  const popup = createStationPopup();
  assert.equal(popup.element.children.filter(node => node.className === 'fleet-station-popup__close').length, 0,
    'The details-card shell owns the sole close control');
  const data = { station: { name: 'Test', address: 'Street' },
    discount: { retailPrice: 5, discountPrice: 4, priceAfterIfta: 3, savings: 1 } };
  popup.update(data);
  writes = 0;
...
test('return visits display distinct numbers quantities and distances without polling churn', () => {
  let replacements = 0;
  globalThis.document = { createElement() {
    return { children: [], append(...nodes) { this.children.push(...nodes); },
      replaceChildren(...nodes) { replacements++; this.children = nodes; },
      setAttribute() {}, addEventListener() {}, removeEventListener() {}, remove() {} };
  } };
  const popup = createStationPopup();
  const data = { station: { name: 'LOVES #706', address: 'Street', country: 'US' }, discount: {},
    fuel: { visits: [{ number: 1, gallons: 66, miles: 369, arrivalGallons: 44, departureGallons: 110, tankGallons: 200 },
      { number: 2, gallons: 164, full: true, miles: 927, arrivalGallons: 26, departureGallons: 190, tankGallons: 200 }] } };
  globalThis.document.createElementNS = () => globalThis.document.createElement();
  popup.update(data);
  const visits = popup.element.children.find(node => node.className === 'fleet-station-popup__visits');
  assert.equal(visits.hidden, false);
  assert.deepEqual(visits.children.map(row => row.children[0].children[0].children[0].textContent), ['1', '2']);
  assert.deepEqual(visits.children.map(row => row.children[0].children[0].children[1].textContent), ['Fuel stop', 'Fuel stop']);
  assert.equal(popup.element.children.find(node => node.className === 'fleet-station-popup__plan-label').textContent, 'Fuel stops 1, 2');
  assert.equal(visits.children[0].children[0].children[1].textContent, '369 mi · 594 km');
  assert.equal(visits.children[1].children[0].children[1].textContent, '927 mi · 1492 km');
  ...
  popup.update(data);
  assert.equal(replacements, 2, 'changed arrival updates the gauge even if purchase and miles are unchanged');
...
test('station address uses separate street and locality lines while copying the original full address', async t => {
  ...
  let copied;
  globalThis.document = { createElement() {
    return { children: [], listeners: {}, append(...nodes) { this.children.push(...nodes); },
  ...
  const data = { station: { name: 'LOVES', address: fullAddress }, discount: {} };
  popup.update(data);
  const address = popup.element.children.find(node => node.className === 'fleet-station-popup__address');
  ...
  popup.update({ ...data, station: { ...data.station, address: 'Warehouse entrance' } });
...
test('planned purchase cards show server USD totals even at Canadian stations and hide unavailable totals', t => {
  ...
  const data = { station: { name: 'Canadian fuel', country: 'CA' }, discount: { currency: 'CAD', unit: 'L' },
    fuel: { visits: [visit] } };
  popup.update(data);
  const visits = popup.element.children.find(node => node.className === 'fleet-station-popup__visits');
  const actions = popup.element.children.find(node => node.className === 'fleet-station-popup__actions');
  const cost = actions.children.find(node => node.className === 'fleet-station-popup__cost');
  assert.equal(cost.children[1].textContent, '≈ $287.64 USD');
  assert.equal(actions.hidden, false, 'the cost remains visible without edit permission');
  assert.equal(actions.children[0].hidden, true);
  assert.equal(visits.children[0].children.length, 2, 'a single visit has one cost footer, not a duplicate inside the gauges');
  ...
  popup.update({ ...data, fuel: { visits: [visit, { ...visit, number: 2, purchaseCostUsd: 230 }] } });
  assert.equal(cost.hidden, true, 'return visits do not fabricate an aggregate client total');
  assert.deepEqual(visits.children.map(row => row.children.find(node => node.className === 'fleet-station-popup__visit-cost').children[1].textContent),
