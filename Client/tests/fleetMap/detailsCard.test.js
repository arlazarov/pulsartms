...
test('fixed popup reuses content, closes explicitly and releases listeners without querying station markup', t => {
  const oldDocument = globalThis.document,
    oldGoogle = globalThis.google;
  t.after(() => {
    globalThis.document = oldDocument;
    globalThis.google = oldGoogle;
  });
  const node = () => ({
    children: [],
    events: new Map(),
    ...
    querySelector() {
      throw new Error('Popup must not know station markup');
    },
  });
