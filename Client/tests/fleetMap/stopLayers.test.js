...
test('one stop update preserves other GPU data; hover only reorders opaque pairs', () => {
  let allocations = 0;
  class Layer { constructor(props) { this.props = props; allocations++; } }
  const build = createSceneLayers({ ScatterplotLayer: Layer, PathLayer: Layer, IconLayer: Layer, TextLayer: Layer });
  const input = { lines: [], stationData: [], stationsVisible: false, distanceData: [], vehicles: [] };
  const stops = new Set(Array.from({ length: 100 }, (_, id) => ({ id,
    position: [-80 + id * .01, 40], number: String(id + 1), job: id % 2 ? 'Delivery' : 'Pickup', transientLabel: id > 0 })));
  ...
  assert.deepEqual(first.slice(-2).map(layer => layer.props.id), currentPair);
  const next = [...stops][1];
  next.highlighted = true;
  snapshot = snapshotStops(stops, snapshot.stopData);
  allocations = 0;
  const hovered = build({ ...input, stopData: snapshot.stopData });
  assert.equal(allocations, 0, 'highlight changes drawing order without rebuilding data or layers');
  assert.deepEqual(hovered.slice(-2).map(layer => layer.props.id), ['route-stop-1-points', 'route-stop-1-numbers']);
  ...
  assert.equal(hovered.at(-1).props.getSize, 15, 'compact numbers remain readable');
  assert.equal(hovered.at(-1).props.pickable, true);
  assert.equal(hovered.at(-1).props.getText(hovered.at(-1).props.data[0]), '2');
  assert.equal(hovered.at(-1).props.background, false, 'text width cannot stretch the circle');
  assert.deepEqual(hovered.at(-1).props.getColor, [255, 255, 255, 255]);
  assert.deepEqual(hovered.at(-1).props.getPixelOffset(hovered.at(-1).props.data[0]), [0, 0]);
  next.number = '101';
  snapshot = snapshotStops(stops, snapshot.stopData);
  allocations = 0;
...
test('one- and two-digit stop circles share fixed geometry and preserve individual picking', () => {
  ...
  const stopData = snapshotStops([
    {id: 1, position: [-80, 40], number: '1', job: 'Pickup', color: currentRouteColor},
    {id: 2, position: [-80, 40], number: '12', job: 'Delivery', color: [32, 122, 99, 255]},
  ]).stopData;
  const selectStop = () => {};
  const layers = build({lines: [], stationData: [], stationsVisible: false, distanceData: [], vehicles: [], stopData, selectStop});
  for (const stop of stopData) {
    const prefix = `route-stop-${stop.id}`;
    const circle = layers.find(layer => layer.props.id === `${prefix}-points`).props;
    const digits = layers.find(layer => layer.props.id === `${prefix}-numbers`).props;
    const anchor = layers.find(layer => layer.props.id === `${prefix}-anchor`).props;
    assert.equal(circle.getSize, 34);
    assert.equal(typeof circle.getIcon, 'function', 'packed atlas frames require an accessor');
    assert.equal(circle.getIcon(stop), 'circle');
    assert.equal(circle.iconMapping.circle.width, circle.iconMapping.circle.height);
...
test('a stop card status change invalidates label colors without rebuilding marker geometry', () => {
  const stop = { id: 'stop', position: [-80, 40], number: '1', job: 'Pickup',
    distance: 'Company\nETA Sep 9, 09:00 AM local', distanceTones: ['heading', 'eta'] };
  const stops = new Set([stop]);
  const initial = snapshotStops(stops);
  const unchanged = snapshotStops(stops, initial.stopData, initial.distanceData);
  ...
  const confirmed = snapshotStops(stops, initial.stopData, initial.distanceData);
  assert.equal(confirmed.stopData, initial.stopData, 'ETA color is independent of the numbered marker');
  ...
  stop.job = 'Delivery';
  const delivery = snapshotStops(stops, confirmed.stopData, confirmed.distanceData);
  assert.notEqual(delivery.distanceData, confirmed.distanceData, 'job changes update the card accent even if text is unchanged');
