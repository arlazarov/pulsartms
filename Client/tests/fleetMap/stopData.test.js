...
test('current stops cover future stops until a future stop is highlighted', () => {
  ...
  assert.deepEqual(snapshotStops(stops).stopData.map(s => s.number), ['2', '1']);
  next.highlighted = true;
  assert.deepEqual(snapshotStops(stops).stopData.map(s => s.number), ['1', '2']);
  next.highlighted = false;
  assert.deepEqual(snapshotStops(stops).stopData.map(s => s.number), ['2', '1']);
});

test('hovered next-load label preserves current distance labels without rebuilding stop geometry', () => {
  const current = { position: [1, 2], number: '1', distance: 'Current distance' };
  const next = { position: [1, 2], number: '4', distance: null, transientLabel: true, onHover() {} };
  const stops = new Set([current, next]);
  ...
  assert.deepEqual(hovered.distanceData.map(x => x.text), ['Current distance', 'Next load']);
  assert.equal(hovered.distanceData[1].transient, true);
  next.distance = null;
  assert.deepEqual(snapshotStops(stops).distanceData.map(x => x.text), ['Current distance']);
});
