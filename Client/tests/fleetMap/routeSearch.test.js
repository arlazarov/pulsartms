...
import { lowerBound, segmentRange } from '../../Scripts/fleetMap/geometry/routeSearch.js';

test('segment window matches exhaustive search including duplicate distances and boundaries', () => {
  for (const values of [[], [0], [0, 0, 1, 1, 2, 5, 10], Array.from({ length: 1001 }, (_, i) => i / 10)]) {
    for (const center of [-20, 0, 1, 5, 10, 50, 100, 200]) {
      ...
      for (let i = 1; i < values.length; i++) {
        if (values[i] >= center - 5 && values[i - 1] <= center + 5) expected.push(i);
      }
...
test('large routes search only the nearby window using logarithmic index lookups', () => {
  let reads = 0;
  const values = new Proxy(Array.from({ length: 100001 }, (_, i) => i / 100), {
