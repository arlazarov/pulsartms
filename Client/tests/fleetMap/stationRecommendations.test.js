...
test('recommendations keep rings on the scene and distances only in the selected popup', async () => {
  ...
  const element = () => ({
    ...
    addEventListener(name, callback) { this.listeners[name] = callback; }, removeEventListener() {}, remove() {},
  });
  ...
  globalThis.google = { maps: {
    ...
    WebGLOverlayView: class {
      constructor() { overlay = this; this.redraws = 0; }
    ...
    marker: { AdvancedMarkerElement: class {
      constructor(options) { Object.assign(this, options); this.listeners = {}; recommendations.push(this); }
  ...
  const map = {
    ...
    setOptions(options) { this.cursor = options.draggableCursor; },
    addListener(name, callback) { mapListeners[name] = callback; return { remove: () => delete mapListeners[name] }; },
  };
  let opened = 0;
  const layer = createStationLayer(map, () => { opened++; }, () => {
    overlay = {map, redraws: 0};
    return {setPoint(id, position, color, recommended, selected, label) { points.set(id, {recommended, selected, label}); }, removePoint(id) { points.delete(id); }, redraw() { overlay.redraws++; },
      setVisible() {}, hitTest(p) { return p?.lat === 40 ? p.lng === -79 ? 'a' : p.lng === -78 ? 'b' : null : null; },
      dispose() { overlay.map = null; }};
  }, popupFixture);
  const quote = { currency: 'USD', unit: 'US gal', product: 'Diesel', discountPrice: 3, priceAfterIfta: 2.5,
    effectiveFrom: '2026-09-05', effectiveTo: '2026-09-05' };
  const stations = ['a', 'b'].map(id => ({ id, name: id, latitude: 40, longitude: id === 'a' ? -79 : -78,
    discounts: [quote], cashDiscount: quote, iftaDiscount: quote }));
  ...
  assert.equal(recommendations.length, 0, 'recommendations must not create DOM markers');
  ...
  await layer.setRecommended([{ id: 'b', gallons: 30, routeMile: 100, miles: 10 }]);
  await layer.setVisible(true);
  ...
  const distance = infoWindow.content.children.find(node => node.textContent === '10 mi · 16 km away');
  ...
  assert.equal(overlay.redraws, redrawsBeforeProgress, 'distance-only changes do not redraw station markers');
  layer.setProgress(100.6);
  assert.equal(points.get('b').recommended, false);
  assert.equal(points.get('b').label, undefined);
  assert.equal(distance.hidden, true);
  const pendingRender = layer.setStations(Array.from({length: 64}, (_, i) => ({...stations[0], id: `pending-${i}`})), '2026-09-05', false);
  layer.dispose();
  ...
  assert.equal(overlay.redraws, redrawsAfterDispose, 'late calls must not touch a disposed renderer');
});
