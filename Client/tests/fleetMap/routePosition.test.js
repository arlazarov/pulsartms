...
test('route position interpolates progress within the selected search window', () => {
  assert.deepEqual(
    routePosition({ latitude: 0, longitude: 0.5 }, path, cumulative, 1, 3),
    { segment: 1, miles: 5 },
  );
  assert.deepEqual(
    routePosition({ latitude: 0, longitude: 1.5 }, path, cumulative, 2, 3),
    { segment: 2, miles: 15 },
  );
  assert.equal(
    routePosition({ latitude: 0, longitude: 0.5 }, path, cumulative, 2, 3),
    null,
  );
});
...
test('route position rejects distant points and empty search windows', () => {
  assert.equal(
    routePosition({ latitude: 1, longitude: 0.5 }, path, cumulative, 1, 3),
    null,
  );
  assert.equal(
    routePosition({ latitude: 0, longitude: 0.5 }, path, cumulative, 1, 1),
    null,
  );
});
