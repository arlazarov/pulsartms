...
function fixture(t) {
  ...
  const overlays = new Map(), listeners = new Map(), observers = [];
  let reads = 0;
  class Observer {
    nodes = new Set();
    constructor(callback) { this.callback = callback; observers.push(this); }
    observe(node, options) { this.nodes.add(node); this.options = options; }
    unobserve(node) { this.nodes.delete(node); }
    disconnect() { this.nodes.clear(); this.disconnected = true; }
  }
  const stage = {querySelector: selector => overlays.get(selector)};
  const view = {ResizeObserver: Observer, MutationObserver: Observer,
    addEventListener: (name, callback) => listeners.set(name, callback),
    removeEventListener: name => listeners.delete(name)};
  const element = {parentElement: stage, ownerDocument: {defaultView: view},
    getBoundingClientRect: () => { reads++; return rect(80, 100, 1000, 600); }};
  const map = {getZoom: () => 5, getProjection: () => ({
  ...
  return {camera, map, stage, observers, listeners, get reads() { return reads; },
    overlay(selector, bounds) {
      const node = {getBoundingClientRect: () => { reads++; return bounds(); }};
...
test('no overlays retain the exact camera target and ordinary numeric route padding', t => {
  const {camera} = fixture(t), target = {lat: 40, lng: -79};
...
test('inspector top inset follows actual side clearance without repeated style writes', t => {
  const f = fixture(t), styles = new Map();
  let clearance = 100, writes = 0;
  const info = f.overlay('.fleet-map-info-reserved', () => rect(80 + clearance,
    100 + Math.min(12, Number.parseFloat(styles.get('--map-inspector-side-gap') ?? '0')),
    1000 - clearance * 2, 160));
  info.style = {getPropertyValue: name => styles.get(name), setProperty: (name, value) => { styles.set(name, value); writes++; }};
...
test('top truck info and right fuel card share one unoccluded camera region', t => {
  ...
  f.camera.refresh();
  assert.deepEqual(f.camera.center({lat: 40, lng: -79}), {lat: 40 - 80 / 32, lng: -79 + 200 / 32});
  assert.deepEqual(f.camera.padding(55), {left: 55, right: 455, top: 215, bottom: 55});
  const reads = f.reads;
  for (let index = 0; index < 1000; index++) {
    f.camera.center({lat: 40 + index / 1000, lng: -79});
    f.camera.padding(55);
  }
  assert.equal(f.reads, reads, 'telemetry camera movement only uses cached layout');
});

test('the existing hidden info wrapper is observed before first selection and later changes refresh once', t => {
  const f = fixture(t);
  let bounds = rect(0, 0, 0, 0), changes = 0;
  ...
  assert.deepEqual(f.camera.center({lat: 40, lng: -79}), {lat: 37.5, lng: -79});
  f.observers[0].callback();
  assert.equal(changes, 1, 'unchanged observer notifications do not repeat camera adjustments');
  bounds = rect(0, 0, 0, 0);
  f.observers[0].callback();
...
test('only direct overlay insertion is observed, removed nodes and all listeners are released', t => {
  const f = fixture(t), [resize, mutation] = f.observers;
  ...
  const reads = f.reads, target = {lat: 40, lng: -79};
  mutation.callback(); resize.callback(); f.camera.refresh();
...
test('fully obscured or unavailable projection safely retains the original target', t => {
  const f = fixture(t), target = {lat: 40, lng: -79};
