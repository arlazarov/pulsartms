...
function fixture() {
  ...
  const inspector = createDockedDetails(host, (kind, version) =>
    changes.push([kind, version]),
  );
  const stop = inspector.popupFactory('stop')(
    {},
    { onClose: () => stop.hide() },
  );
  const fuel = inspector.popupFactory('fuel')(
    {},
    { onClose: () => fuel.hide() },
  );
...
test('an editor suspends every inspector owner until explicitly resumed', () => {
  const { host, inspector, stop, fuel, changes } = fixture();
  inspector.activate('fuel');
  fuel.show({ node: 'previous' });
  inspector.setSuspended(true);
  const revision = changes.at(-1)[1];
  inspector.activate('stop');
  stop.show({ node: 'clicked stop' });
  inspector.activate('fuel');
  fuel.show({ node: 'station polling' });
  inspector.setMode('truck');
  inspector.setMode('next-stop');
  inspector.setSuspended(true);
  assert.deepEqual(host.children, []);
  assert.equal(inspector.mode, 'closed');
  assert.equal(changes.at(-1)[1], revision);
  inspector.setSuspended(false);
  fuel.show({ node: 'late polling' });
  assert.deepEqual(
    host.children,
    [],
    'resuming cannot replay an obsolete native owner',
  );
  inspector.setMode('truck');
  inspector.activate('stop');
  stop.show({ node: 'new selection' });
  assert.deepEqual(host.children, [{ node: 'new selection' }]);
  inspector.dispose();
  inspector.setSuspended(true);
  assert.deepEqual(host.children, []);
});
...
test('docked cards reuse only the explicit shared host, with no shell or copied content', () => {
  const { host, inspector, stop, changes } = fixture(),
    content = { node: 'existing stop HTML' };
  stop.show(content);
  assert.deepEqual(
    host.children,
    [],
    'polling cannot open an inactive inspector',
  );
  inspector.activate('stop');
  stop.show(content);
  stop.show(content);
  ...
  assert.equal(
    host.writes,
    2,
    'active metadata refresh replaces only native content',
  );
...
test('late show, hide and disposal from an old owner cannot erase or reopen another card', () => {
  ...
  stop.show({ node: 'stop' });
  inspector.activate('fuel');
  const content = { node: 'fuel' };
  fuel.show(content);
  stop.hide();
  stop.show({ node: 'late stop' });
  stop.dispose();
  assert.deepEqual(host.children, [content]);
  ...
  assert.equal(
    inspector.mode,
    'truck',
    'disposed owners cannot acquire the shared host',
  );
});
...
test('explicit Back, Close and native Escape restore only departing inspector focus without scrolling', () => {
  ...
  assert.equal(
    focused.length,
    0,
    'polling keeps the current keyboard location',
  );
