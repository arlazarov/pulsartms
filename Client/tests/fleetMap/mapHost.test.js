...
function container() {
  return {
    ownerDocument: { createElement: () => ({ remove() { this.parent = null; } }) },
    append(host) { host.parent = this; },
  };
}

test('provider host is bounded and reuse does not move the camera before fleet data arrives', () => {
  let creations = 0, host;
  const map = { setOptions(options) { this.options = options; } };
  const mount = createMapHost(node => { creations++; host = node; return map; });
  const first = mount(container(), {});
  assert.throws(() => mount(container(), {}), /already mounted/);
  first.release();
  assert.equal(host.parent, null);
  const target = container();
  const second = mount(target, { center: {lat: 41.5, lng: -87.5}, zoom: 5, mapId: 'id', renderingType: 'VECTOR', mapTypeId: 'roadmap' });
  first.release();
...
function startup(t) {
  ...
  const mount = createMapHost(node => {
    ...
    return {setOptions() {}, addListener(name, callback) {
...
test('first camera remains hidden until idle and polling cannot rehide it', t => {
  ...
  first.initialCamera(initial => { assert.equal(initial, false); fitted++; });
  assert.equal(host.className, 'fleet-map-host');
  assert.equal(listeners.size, 0);
...
test('disposed startup callbacks cannot reveal a later mount and dispose clears wait resources', t => {
  const {mount, first, listeners, timers, host} = startup(t);
  first.initialCamera(() => {});
  const oldIdle = listeners.get('idle'), oldFallback = [...timers.values()][0];
  first.release();
  assert.equal(listeners.size, 0);
  assert.equal(timers.size, 0);
  const second = mount(container(), {});
  oldIdle(); oldFallback(); first.show();
  assert.match(host.className, /initializing/);
  assert.throws(() => second.initialCamera(() => { throw new Error('Camera failed'); }), /Camera failed/);
  assert.equal(host.className, 'fleet-map-host');
  assert.equal(listeners.size, 0);
  assert.equal(timers.size, 0);
  second.release();
