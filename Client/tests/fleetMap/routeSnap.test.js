...
test('nearby same-direction route corrects only visual coordinates', () => {
  ...
  const offset = matchRoute(position, path, [0, 1], 0.5);
...
test('does not snap opposing, distant, stopped or out-of-progress positions', () => {
  for (const p of [
    { ...position, heading: 270 },
    { ...position, speed: 0 },
    { ...position, latitude: 40 + 60 / 111320 },
  ])
    assert.equal(matchRoute(p, path, [0, 1], 0.5), null);
...
test('correction fades out at distance boundary', () => {
  ...
  const result = matchRoute(p, path, [0, 1], 0.5);
...
test('smooth correction, gradual release and isolated route state', () => {
  ...
  const match = () => {
    calls++;
    return enabled ? { latitude: -0.0001, longitude: 0 } : null;
  };
  let result;
  for (let now = 0; now <= 1000; now += 10)
    result = snap(position, 'a', match, now);
