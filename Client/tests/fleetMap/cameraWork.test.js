...
function startupFixture(t, cameraViewport) {
  const previousDocument = globalThis.document, previousGoogle = globalThis.google;
  globalThis.document = {hidden: false, addEventListener() {}, removeEventListener() {}};
  globalThis.google = {maps: {LatLngBounds: class {
    points = [];
    extend(point) { this.points.push(point); }
    isEmpty() { return this.points.length === 0; }
  }}};
  const domEvents = new Map(), cameras = [], initial = [];
  const map = {getDiv: () => ({addEventListener: (name, callback) => domEvents.set(name, callback),
      removeEventListener: name => domEvents.delete(name)}),
    addListener: () => ({remove() {}}),
    fitBounds: bounds => cameras.push({fit: bounds.points}),
    moveCamera: value => cameras.push(value), panTo: center => cameras.push({pan: center}), setZoom() {}};
  const markers = new Map();
  const marker = () => ({render() {}, update(value) { markers.set(value.truckId, this); }, setSelected() {},
    setVisible(value) { this.visible = value; }, dispose() { this.disposed = true; }});
  const layer = createTruckLayer(map, undefined, undefined, undefined, marker, undefined, (change, wait) => {
    initial.push({hasChange: Boolean(change), wait});
    change?.(true);
  }, cameraViewport);
  t.after(() => { layer.dispose(); globalThis.document = previousDocument; globalThis.google = previousGoogle; });
  const trucks = [{truckId: 'a', truckExternalId: 'first', latitude: 40, longitude: -80},
...
test('editing hides other trucks across telemetry refreshes and restores the current layer preference', t => {
  ...
  layer.setTrucks([...trucks, {...trucks[1], truckId: 'c', truckExternalId: 'third'}], []);
  assert.equal(markers.get('b').visible, false);
  ...
  assert.ok([...markers.values()].every(marker => marker.visible && !marker.disposed));
...
test('first fleet snapshot fits once while telemetry refresh cannot move the camera again', t => {
  ...
  layer.setTrucks(trucks, []);
  assert.deepEqual(cameras, [{fit: [{lat: 40, lng: -80}, {lat: 45, lng: -75}]}]);
...
test('deep-link truck is the initial camera target without an intermediate fleet fit', t => {
  ...
  assert.equal(initial[0].wait, false, 'moveCamera is immediate and does not wait for an optional idle event');
  layer.setTrucks(trucks, []);
  assert.equal(cameras.length, 1);
});
...
test('a late query focus after empty telemetry selects without undoing the user camera', t => {
  ...
  assert.equal(layer.focusTruck('a', undefined, true), true, 'a different explicit query target may focus');
  assert.equal(cameras.length, 1);
  assert.equal(layer.focusTruck('b'), true, 'explicit search remains an intentional camera action');
...
test('manual focus and Follow use cached free-map coordinates, without undoing user gestures or route fits', t => {
  let offset = 0;
  const viewport = {center: position => ({...position, lat: position.lat + offset}), padding: value => value};
  ...
  assert.deepEqual(cameras.at(-1), {center: {lat: 43, lng: -75}}, 'first rendered info panel adjusts the focused truck');
  const beforeGesture = cameras.length;
  domEvents.get('pointerdown')();
  offset = -3;
  layer.refreshViewport();
  assert.equal(cameras.length, beforeGesture, 'layout updates do not undo a user camera change');
  layer.focusTruck('b');
  layer.clearViewportFocus();
  const beforeFit = cameras.length;
  offset = -4;
  layer.refreshViewport();
  assert.equal(cameras.length, beforeFit, 'route fits supersede temporary manual centering');
  layer.setFollow('b', true);
  assert.deepEqual(cameras.at(-1), {zoom: 15, center: {lat: 41, lng: -75}});
  offset = -5;
  layer.refreshViewport();
  assert.deepEqual(cameras.at(-1), {center: {lat: 40, lng: -75}}, 'stationary Follow responds to actual overlay layout changes');
  layer.dispose();
...
test('follow continues between telemetry polls until buffered movement is exhausted', t => {
  const saved = { document: globalThis.document, raf: globalThis.requestAnimationFrame,
    cancel: globalThis.cancelAnimationFrame, now: Date.now, performance: globalThis.performance };
  let now = 1800000000000, tick = 0, frame;
  globalThis.document = { hidden: false, addEventListener() {}, removeEventListener() {} };
  ...
  const centers = [];
  const layer = createTruckLayer({ addListener() { return { remove() {} }; },
    moveCamera({center}) { centers.push(center); }, getZoom: () => 15 },
  () => {}, () => {}, (_, p) => p,
  () => ({ render() {}, update() {}, setVisible() {}, setSelected() {}, dispose() {} }));
  t.after(() => { layer.dispose(); globalThis.document = saved.document;
    globalThis.requestAnimationFrame = saved.raf; globalThis.cancelAnimationFrame = saved.cancel;
    Date.now = saved.now; globalThis.performance = saved.performance; });
  layer.setVisible(false);
  const point = (age, longitude) => ({ truckId: 'id', truckExternalId: 'ext', latitude: 40,
    longitude, updatedAt: new Date(now - age).toISOString(), speed: 60, heading: 90 });
  layer.setTrucks([point(60000, -79.9)], [point(90000, -80)]);
  layer.setVisible(true);
  layer.setFollow('id', true);
  for (let i = 0; i < 120; i++) {
    assert.equal(typeof frame, 'function', 'buffered movement must keep requesting frames');
    now += 16; tick += 16;
    const next = frame; frame = null; next();
  }
  assert.ok(centers.length > 100);
  assert.ok(centers.at(-1).lng > centers[0].lng);
  layer.setFollow('id', false);
  const count = centers.length;
  now += 16; tick += 16; frame();
  assert.equal(centers.length, count, 'disabled follow does not move the camera');
  layer.dispose();
  layer.dispose();
...
test('follow uses displayed coordinates, zooms once and stops on drag, hide and deselect', t => {
  const previous = globalThis.document;
  globalThis.document = { hidden: false, addEventListener() {}, removeEventListener() {} };
  const events = new Map(), centers = [], changes = [], positions = [], zooms = [];
  let zoom = 5;
  const map = {
    ...
    setZoom(value) { zoom = value; zooms.push(value); events.get('zoom_changed')?.(); },
    getZoom() { return zoom; },
  };
  const marker = {render() {}, update() {}, setSelected() {}, setVisible() {}, dispose() {}};
  const layer = createTruckLayer(map, () => {}, (id, position) => positions.push(position),
    (_, p) => ({...p, latitude: p.latitude + .001}), () => marker, v => changes.push(v));
  t.after(() => { layer.dispose(); globalThis.document = previous; });
  layer.setTrucks([], []);
  assert.equal(layer.setFollow('missing', true), false);
  assert.equal(layer.isFollowing(), false);
  // Starting hidden avoids the unrelated initial fleet fit.
  layer.setVisible(false);
  layer.setTrucks([{truckId: 'id', truckExternalId: 'external', latitude: 40, longitude: -80,
    updatedAt: new Date(Date.now() - 90000).toISOString(), speed: 60}], []);
  layer.setVisible(true);
  ...
  assert.equal(centers.length, count, 'idle does not create a camera feedback loop');
  map.setZoom(17);
  assert.equal(layer.isFollowing(), false);
  ...
  assert.deepEqual(changes.slice(changeCount), [true], 'resume emits one state change');
  map.setZoom(13.5);
  assert.equal(changes.at(-1), false, 'fractional manual zoom ends follow immediately');
    ...
    const layer = createTruckLayer({
      getDiv() { return {
        addEventListener(name, fn) {
          assert.equal(domEvents.has(name), false, `duplicate ${name} listener`);
      ...
      addListener(name, fn) { events.set(name, fn); return { remove() { events.delete(name); } }; },
      getBounds() { measurements++; return null; }
    });
    assert.equal(events.has('bounds_changed'), false, 'camera motion does not suspend route progress');
