...
import { selectStationPrices, comparisonPrice, priceStatistics } from '../../Scripts/fleetMap/stations/stationPrices.js';
import { createStationLayer } from '../../Scripts/fleetMap/stations/stationLayer.js';

const date = '2026-09-09';
const quote = (overrides = {}) => ({ effectiveFrom: date, effectiveTo: date, currency: 'USD', unit: 'US gal',
  product: 'Diesel', discountPrice: 3, priceAfterIfta: 2.5, ...overrides });
const station = (overrides = {}) => ({ id: 'one', latitude: 40, longitude: -79, discounts: [], ...overrides });
...
test('map displays the server-selected road diesel rather than a newer cheaper non-road product', () => {
  ...
  const discounts = [quote({product: 'DEF', discountPrice: .1}), quote({product: 'Reefer Diesel', discountPrice: .2}), road];
  const item = selectStationPrices([station({discounts, cashDiscount: road, iftaDiscount: road})], date)[0];
...
test('IFTA chooses its ready server quote and never calculates tax or converts Canadian units', () => {
  const cash = quote({currency: 'CAD', unit: 'L', discountPrice: 1.4, priceAfterIfta: null});
  const net = quote({currency: 'CAD', unit: 'L', discountPrice: 1.5, priceAfterIfta: 1.1});
  const data = [station({discounts: [cash, net], cashDiscount: cash, iftaDiscount: net})];
...
test('ambiguous or legacy-unselected quotes keep a neutral station instead of an invalid cheapest price', () => {
  for (const selected of [{cashDiscount: null, iftaDiscount: null}, {}]) {
    const item = selectStationPrices([station({...selected, discounts: [quote(), quote({currency: 'CAD', unit: 'L', discountPrice: 1})]})], date)[0];
...
test('missing IFTA retains known cash details but an unavailable comparison', () => {
  ...
  const item = selectStationPrices([station({discounts: [cash], cashDiscount: cash, iftaDiscount: null})], date, true)[0];
...
test('expired selections cannot replace a current quote and bad positions remain excluded', () => {
  ...
  const current = quote();
  const data = station({discounts: [current], cashDiscount: expired, iftaDiscount: expired});
  assert.equal(comparisonPrice(selectStationPrices([data], date)[0].discount, false), null);
  assert.equal(selectStationPrices([{...data, latitude: 100}], date).length, 0);
  assert.equal(selectStationPrices([station({discounts: [expired], cashDiscount: expired})], date).length, 0);
});
...
test('IFTA toggles replace the selected popup quote without another station request', async t => {
  ...
  const element = () => ({ children: [], style: {setProperty() {}}, classList: {add() {}, toggle() {}},
  ...
  t.after(() => { globalThis.document = previousDocument; globalThis.getComputedStyle = previousStyle; });
  let select, popup, shown = 0, displayedPrice;
  const layer = createStationLayer({}, () => {}, (_map, onSelect) => {
    select = onSelect;
    return {setPoint(...args) { displayedPrice = args[7]; }, removePoint() {}, redraw() {}, setVisible() {}, dispose() {}};
  }, () => ({show(content) { popup = content; shown++; }, hide() {}, dispose() {}}));
  ...
  await layer.setStations([station({discounts: [cash, net], cashDiscount: cash, iftaDiscount: net})], date, false);
  ...
  assert.equal(displayedPrice, 2.5, 'GPU circle receives the same server-selected IFTA comparison, not a client tax formula');
