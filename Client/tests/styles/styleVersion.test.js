...
test('CSS address changes with its contents and remains stable for an unchanged build', () => {
  const source = '<link href="css/main.css?v=old" rel="stylesheet"><script src="_framework/blazor.webassembly#[.{fingerprint}].js"></script>';
  ...
  assert.ok(first.endsWith('<script src="_framework/blazor.webassembly#[.{fingerprint}].js"></script>'));
...
test('split-toolchain releases stamp CSS in the Node phase and skip Node during the dotnet phase', () => {
  const project = readFileSync(new URL('../../Client.csproj', import.meta.url), 'utf8');
