...
import {show, close, dispose} from '../../Scripts/shared/loadDialog.js';

function fixture(t) {
  const previous = globalThis.document;
  const previousWindow = globalThis.window;
  const opener = {isConnected: true, calls: [], getAttribute() {return 'load-1';}, focus(options) {this.calls.push(options);}};
  const htmlClasses = new Set(), bodyClasses = new Set(), properties = new Map(), controlled = [];
  globalThis.window = {scrollY: 500, scrollTo(x, y) {this.scrollY = y;}};
  globalThis.document = {activeElement: opener,
    documentElement: {classList: {add: value => htmlClasses.add(value), remove: value => htmlClasses.delete(value)}},
    ...
    getElementById: id => controlled.find(element => element.id === id), querySelectorAll: () => controlled};
  class Dialog extends EventTarget {
    isConnected = true;
    open = false;
    shows = 0;
    listeners = new Set();
    addEventListener(name, callback) {super.addEventListener(name, callback); this.listeners.add(callback);}
    removeEventListener(name, callback) {super.removeEventListener(name, callback); this.listeners.delete(callback);}
  ...
  t.after(() => {dispose(dialog); globalThis.document = previous; globalThis.window = previousWindow;});
...
test('close returns focus to the same Papers identity when polling recreates its opener', t => {
  ...
  const replacement = {isConnected: true, calls: [], getAttribute() {return 'load-1';}, focus(options) {this.calls.push(options);}};
...
test('only a complete backdrop gesture closes the modal, not content clicks or drags', t => {
  ...
  show(dialog);
  dialog.pointer('pointerdown', 150, 150); dialog.pointer('click', 150, 150);
  assert.equal(dialog.open, true);
  dialog.pointer('pointerdown', 150, 150); dialog.pointer('click', 50, 50);
  assert.equal(dialog.open, true);
  dialog.pointer('pointerdown', 50, 50); dialog.pointer('click', 50, 50);
...
test('unmount closes an open modal and releases handlers without focusing a removed opener', t => {
  ...
  show(dialog);
  opener.isConnected = false;
  dispose(dialog); dispose(dialog); close(dialog);
  assert.equal(dialog.open, false);
  assert.equal(dialog.listeners.size, 0);
