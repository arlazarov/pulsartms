...
const session = (id = randomUUID(), label = 'first') => ({Id: id, AccessToken: `access-${label}`, RefreshToken: `refresh-${label}`});

async function environment(run) {
  const original = Object.fromEntries(['localStorage', 'navigator'].map(name => [name, Object.getOwnPropertyDescriptor(globalThis, name)]));
  ...
  try { await run({values, locks, names}); }
  finally {
...
test('legacy token pairs migrate once under the same cross-tab lock as new login', async () => {
  await environment(async ({values, names}) => {
    ...
    const next = session();
    await Promise.all([storage.readSession(), storage.setSession(JSON.stringify(next))]);
...
test('a queued stale refresh from another tab cannot overwrite or clear a newer login', async () => {
  await environment(async ({locks}) => {
    ...
    let release;
    const held = locks.request('amftms:auth-session', () => new Promise(resolve => { release = resolve; }));
    await Promise.resolve();
    const login = storage.setSession(JSON.stringify(next));
    const replaced = storage.replaceSession(JSON.stringify(first), JSON.stringify(session(first.Id, 'rotated')));
...
test('refresh comparison uses both tokens while logout follows the same login identity', async () => {
  await environment(async () => {
    ...
    await storage.setSession(JSON.stringify(first));
    assert.equal(await storage.replaceSession(JSON.stringify(first), JSON.stringify(rotated)), true);
    assert.equal(await storage.replaceSession(JSON.stringify(first), null), false);
    assert.equal(await storage.clearSession(first.Id), true);
    assert.equal(await storage.readSession(), null);
    assert.throws(() => storage.replaceSession(JSON.stringify(first), JSON.stringify(session())), /cannot change/);
  });
});
...
test('session identifiers have a canonical form shared with the C# envelope', async () => {
  await environment(async ({values}) => {
    ...
    values.set('auth_session', JSON.stringify({...original, Id: original.Id.toUpperCase()}));
...
test('malformed envelopes fail closed without resurrecting legacy tokens', async () => {
  await environment(async ({values}) => {
    for (const json of ['{', '{}', 'null', '[]', JSON.stringify({Id: 'invalid', AccessToken: 'a', RefreshToken: 'r'}),
      JSON.stringify({...session(), AccessToken: ''}), JSON.stringify({...session(), RefreshToken: null})]) {
...
test('a browser without Web Locks cannot perform unsafe storage mutations or reads', async () => {
  await environment(async ({values}) => {
    ...
    assert.throws(() => storage.setSession(JSON.stringify(session())), /requires Web Locks/);
