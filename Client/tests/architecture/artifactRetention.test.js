...
import {beginRun, policy, prune, selectExpired} from '../../../scripts/artifacts.mjs';

const now = 2000000000000;
const old = now - 10 * 86400000;
const row = (id, time, extra = {}) => ({id, kind:'release', time, bytes:40, ...extra});

test('artifact policy targets one GiB and keeps two latest runs per kind', () => {
  assert.equal(policy.maxBytes, 1024 ** 3);
  const entries = [row('one', old), row('two', old+1), row('three', old+2), row('browser', old, {kind:'browser-ui'})];
  assert.deepEqual(selectExpired(entries, now).remove.map(x=>x.id), ['one']);
});

test('size pruning is oldest first without evicting protected or recent output', () => {
  const entries = [row('old',old), row('pin',old+1,{protected:true}), row('new',now-100), row('latest',now)];
  assert.deepEqual(selectExpired(entries,now,{...policy,maxBytes:1}).remove.map(x=>x.id), ['old']);
});

test('an age threshold never overrides active runs or the latest retained results', () => {
  const entries = [row('active',old,{protected:true}),row('two',old+1),row('three',old+2)];
  assert.deepEqual(selectExpired(entries,now).remove, []);
});

test('failed attempts cannot evict the last successful result', () => {
  const entries = [row('good',old,{success:true}),row('failure',old+1),row('failure2',old+2)];
  assert.deepEqual(selectExpired(entries,now,{...policy,maxBytes:1}).remove,[]);
});

function fixture(t) {
  const directory = fs.mkdtempSync(path.join(tmpdir(),'amftms-artifact-test-'));
...
function age(run, time = old) {
  run.finish();
  const marker = path.join(run.directory,'.amftms-artifact.json');
  const data = JSON.parse(fs.readFileSync(marker));
  fs.writeFileSync(marker,JSON.stringify({...data,pid:0,createdAt:time,completedAt:time}));
}

function requiresLsof(t) {
  if (spawnSync('lsof',['-nP','-Fn'],{encoding:'utf8',maxBuffer:64*1024**2}).status === 0) return true;
  t.skip('Open-file inventory unavailable; production cleanup also fails closed.');
  return false;
}
...
test('dry run is read-only; apply removes only owned expired trees', t => {
  ...
  const {pool} = fixture(t);
  const expired = beginRun('release',{pool}); age(expired);
  for(let i=1;i<=2;i++) age(beginRun('release',{pool}),old+i);
  const unknown = path.join(pool,'unrecognized'); fs.mkdirSync(unknown);
...
test('keep markers, live owners, open files and linked trees survive pruning', t => {
  if (!requiresLsof(t)) return;
  const {directory,pool} = fixture(t);
  const pinned = beginRun('release',{pool}); age(pinned);
  fs.writeFileSync(path.join(pinned.directory,'.keep'),'');
  const active = beginRun('release',{pool});
  const opened = beginRun('release',{pool}); age(opened);
  const fd = fs.openSync(path.join(opened.directory,'open.log'),'w'); t.after(()=>fs.closeSync(fd));
  const linked = beginRun('release',{pool}); age(linked);
  ...
  for(const run of [pinned,active,opened,linked]) assert.ok(fs.existsSync(run.directory));
});

test('invalid marker metadata cannot authorize deletion', t => {
  if (!requiresLsof(t)) return;
  const {pool} = fixture(t);
  const run = beginRun('release',{pool}); age(run);
  fs.writeFileSync(path.join(run.directory,'.amftms-artifact.json'),JSON.stringify({version:1,id:'elsewhere',kind:'release',createdAt:old}));
  assert.deepEqual(prune({pool,now,apply:true}).removed,[]);
...
test('all browser artifact producers use managed defaults with explicit override support', () => {
  ...
  const producers = ['uiSmoke.mjs','fuelEditorSmoke.mjs','nativeInspectorSmoke.mjs','stopDetailsSmoke.mjs',
    'mapStartupSmoke.mjs','mapMarkersSmoke.mjs','stationPopupSmoke.mjs','stopCardsSmoke.mjs','hoursForecastSmoke.mjs','mapLifecycle.mjs'];
  for(const name of producers) {
    const source = fs.readFileSync(new URL(name,browser),'utf8');
    assert.match(source,/browserOutput\('[a-z-]+', process\.env\.[A-Z_]+\)/,name);
    assert.doesNotMatch(source,/writeFile\(`test-results|screenshot\(\{path: 'test-results/,name);
  }
});
