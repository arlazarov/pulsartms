...
test('history guard rejects private paths and secret configuration without echoing values', () => {
  ...
  const result = secretFindings(
    'Server/API/appsettings.json',
    JSON.stringify({ Provider: { ApiKey: 'fixture-value' } }),
  );
  assert.ok(result.length);
  assert.ok(result.every(message => !message.includes('fixture-value')));
  assert.deepEqual(
    secretFindings(
      'Client/wwwroot/appsettings.example.json',
      '{"GoogleMaps":{"ApiKey":""}}',
    ),
    [],
  );
...
test('history guard recognizes private key and provider token signatures', () => {
  assert.ok(
    secretFindings('source.txt', ['-----BEGIN ', 'PRIVATE KEY-----'].join(''))
      .length,
  );
