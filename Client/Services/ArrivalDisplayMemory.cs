...
{
  public static readonly TimeSpan PendingDisplayGrace = TimeSpan.FromMinutes(
    15
  );
  ...
  )
  {
    ...
    if (DispatchId != dispatchId)
      Stop = null;
    DispatchId = dispatchId;
    if (stop is not null)
      Stop = stop;
    // A pending response is not a replacement for an already complete stop
    // snapshot.
    if (
      Matches(eta)
      && (
        ...
        || !eta!.RouteUpdatePending
          && (
            changedStopForecast is null
            || eta.CalculatedAt > changedStopForecast
          )
      )
      && (
        previous is null
        || eta!.CalculatedAt >= previous.CalculatedAt
  ...
  public DispatchEta? PreviousDuringUpdate(
    DispatchEta? eta,
    DateTime now,
    ...
    && now < previous.ValidUntil.ToUniversalTime().Add(PendingDisplayGrace)
      ? previous
      : null;

  public DispatchEta? Display(
    DispatchEta? eta,
    DateTime now,
    bool refreshing = false
  )
  {
    if (completed || awaitingChangedStop)
      return null;
    if (PreviousDuringUpdate(eta, now, refreshing) is { } retained)
      return retained;
    if (!Matches(eta))
      return
        Stop is not null
        && eta is { Stops.Count: 0 }
        && eta.ValidUntil.ToUniversalTime() > now
        ? eta
        : null;
    var latest =
      previous is not null && previous.CalculatedAt > eta!.CalculatedAt
        ? previous
        : eta;
    return latest?.ValidUntil.ToUniversalTime() > now ? latest : null;
  }

  private static bool SameStop(PlanStop first, PlanStop second) =>
    ...
    && first.ScheduledDate2 == second.ScheduledDate2
    && first.ScheduledTime2 == second.ScheduledTime2;

  private bool Matches(DispatchEta? eta) =>
    Stop is { } stop
    && eta?.Stops.FirstOrDefault() is { } first
