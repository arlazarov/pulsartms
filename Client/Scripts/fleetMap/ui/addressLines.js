...
const unitPattern = /^(?:apt|apartment|unit|suite|ste|building|bldg|floor|#)(?:\b|\s|\d)/i;
...
export function addressLines(address) {
  ...
  return (countryPattern.test(country) ? split(parts, parts.length - 2, country) : null)
...
function split(parts, end, country) {
  ...
  if (!regions.has(region.toUpperCase()) || end < 1 || !/\p{L}/u.test(city) || /^\d/.test(city)
    || regions.has(city.toUpperCase()) || countryPattern.test(city) || postalPattern.test(city)
    || regionPattern.test(city) || unitPattern.test(city)) return null;
  return { street: parts.slice(0, end).join(', '),
    locality: [city, [region, postal].filter(Boolean).join(' '), country].filter(Boolean).join(', ') };
}
