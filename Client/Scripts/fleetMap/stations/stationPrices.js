...
const unavailableDiscount = Object.freeze({ currency: '', unit: '', product: 'Diesel',
  retailPrice: null, discountPrice: null, priceAfterIfta: null, savings: null });
...
export function selectStationPrices(stations, date, useIfta = false) {
  return (Array.isArray(stations) ? stations : []).flatMap(station => {
    ...
    if (!position) return [];
    const active = value => value && date >= value.effectiveFrom && date <= value.effectiveTo;
    const selected = useIfta ? station.iftaDiscount ?? station.cashDiscount : station.cashDiscount;
    if (!(station.discounts || []).some(active) && !active(selected)) return [];
    const comparison = useIfta ? station.iftaComparison : station.cashComparison;
    return [{ station, discount: active(selected)
      ? comparison?.date === date ? { ...selected, comparison } : selected : unavailableDiscount, position }];
  });
}
...
export function priceStatistics(items, useIfta) {
  ...
  for (const { discount } of items) {
    ...
    const stats = groups.get(key) || { min: price, max: price, total: 0, count: 0, prices: [] };
  ...
  for (const stats of groups.values()) {
    ...
    stats.priceTiers = [...new Set(stats.prices.map(value => Math.round(value * 100)))];
    stats.pricePositions = pricePositions(stats.priceTiers);
    const middle = Math.floor(stats.prices.length / 2);
    stats.median = stats.prices.length % 2
      ? stats.prices[middle]
...
export function priceColor(price, stats, palette) {
  ...
  const percentile = stats.pricePositions?.get(tier) ?? .5;
  ...
  return percentile <= .5
    ...
    : interpolateHsl(palette.middle, palette.high, (percentile - .5) * 2);
}

function pricePositions(tiers) {
  const positions = new Map();
  if (tiers.length === 0) return positions;
  if (tiers.length === 1) { positions.set(tiers[0], .5); return positions; }
  const cumulative = [0];
  for (let index = 1; index < tiers.length; index++) {
    cumulative.push(cumulative[index - 1] + Math.pow(Math.max(1, tiers[index] - tiers[index - 1]), .2));
  }
  const total = cumulative.at(-1);
  tiers.forEach((tier, index) => positions.set(tier, cumulative[index] / total));
  return positions;
}
...
function interpolateHsl(from, to, ratio) {
  ...
  return hslToRgb(hue, start[1] + (end[1] - start[1]) * amount,
    start[2] + (end[2] - start[2]) * amount).join(',');
}
