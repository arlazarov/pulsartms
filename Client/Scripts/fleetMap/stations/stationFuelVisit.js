...
export function fuelGaugeValue(gallons, tankGallons) {
  if (!Number.isFinite(gallons) || gallons < 0 || !Number.isFinite(tankGallons)
    || tankGallons <= 0 || gallons > tankGallons) return null;
...
export function createFuelGauge(label, percent, quantity) {
  ...
  group.setAttribute('aria-label', `${label}: ${percent === null ? '' : `${percent}%, `}${quantity}`);
  const dial = node('span', 'driver-hours__dial fleet-fuel-visit__dial');
  const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', '0 0 64 64');
  svg.setAttribute('aria-hidden', 'true');
  for (const [className, fill] of [['driver-hours__track', null], ['driver-hours__arc', percent]]) {
    const circle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
    for (const [name, value] of Object.entries({ class: className, cx: '32', cy: '32', r: '28', pathLength: '100' }))
      circle.setAttribute(name, value);
    if (className === 'driver-hours__arc') circle.setAttribute('stroke-dasharray', `${fill ?? 0} 100`);
    svg.append(circle);
  }
  dial.append(svg, node('strong', 'fleet-fuel-visit__percent', percent === null ? '—' : `${percent}%`));
  group.append(node('span', 'fleet-fuel-visit__label', label), dial,
...
export function createFuelVisit(visit, station, discount, showCost = true) {
  ...
  name.append(node('span', 'fleet-fuel-visit__number', String(visit.number)),
    node('span', 'fleet-fuel-visit__title', 'Fuel stop'));
  const distance = node('span', 'fleet-fuel-visit__distance',
    Number.isFinite(visit.miles) ? distanceLabel(visit.miles) : '');
  if (Number.isFinite(visit.miles)) distance.setAttribute('aria-label', `${distanceLabel(visit.miles)} away`);
  ...
  action.append(node('span', 'fleet-fuel-visit__label', visit.full ? 'Fill up' : 'Buy'),
    node('strong', 'fleet-fuel-visit__buy', stationQuantity(visit.gallons, station, unit)));
  action.append(arrow);
  const levels = node('div', 'fleet-fuel-visit__levels');
  levels.append(gauge('On arrival', visit.arrivalGallons, visit.tankGallons, station, unit), action,
    gauge('After fueling', visit.departureGallons, visit.tankGallons, station, unit));
  row.append(heading, levels);
  ...
  if (showCost && costLabel) {
    ...
    cost.append(node('span', 'fleet-fuel-visit__label', 'Estimated purchase'),
