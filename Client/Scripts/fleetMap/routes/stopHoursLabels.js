...
export function stopHoursLabels(stop, eta, now) {
  ...
  const rows = [
    {
      ...
      title:
        hours.cycleAtArrivalMinutes != null
          ? 'Estimated cycle remaining on arrival'
          : hours.currentCycleMinutes != null
  ...
  if (
    baseline?.recapVerified &&
    typeof baseline.nextRecapMinutes === 'number' &&
    Number.isSafeInteger(baseline.nextRecapMinutes) &&
    baseline.nextRecapMinutes > 0 &&
    ...
    if (when)
      rows.push({
        label: 'Next recap',
        value: when,
        credit: signedDuration(baseline.nextRecapMinutes),
        recap: true,
        tone: 'neutral',
      });
  }
  if (hours.cycleVerified) {
    ...
    for (const alternative of hours.alternatives ?? []) {
      ...
      const known =
        typeof alternative.lateMinutes === 'number' &&
        Number.isSafeInteger(alternative.lateMinutes) &&
        alternative.lateMinutes >= 0 &&
      ...
      const late =
        typeof alternative.lateMinutes === 'number'
          ? signedDuration(alternative.lateMinutes).replace(/^\+/, '')
          : '';
      rows.push({
        label: 'With recap',
        value: when,
