const maximumInput = 4096, maximumReferences = 4;
...
const dateOrOtherId = /^(?:\d{1,4}[-/_]\d{1,2}[-/_]\d{1,4}|\d{1,2}[-/]\d{1,2}|\d{1,4}(?:am|pm)|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[-_]?\d{1,2}[-_]?\d{2,4}|\d{1,2}[-_]?(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*[-_]?\d{2,4}|(?:bol|load|order)[-_]?\d.*)$/i;

function kind(value) {
  const normalized = String(value ?? '').replace(/[^a-z]/gi, '').toLowerCase();
  const pickup = normalized.includes('pickup') || ['pu', 'shipper'].includes(normalized);
  const delivery = normalized.includes('delivery') || normalized.includes('dropoff') || ['del', 'deliver', 'receiver'].includes(normalized);
...
function endsClause(notes, index, limit) {
  ...
  return !['.', ','].includes(notes[index]) || index + 1 === notes.length || !/[a-z0-9]/i.test(notes[index + 1]);
}
...
function isReference(value) {
  ...
  const formats = [[value.slice(0, 4), value.slice(4, 6), value.slice(6)],
    [value.slice(4), value.slice(0, 2), value.slice(2, 4)], [value.slice(4), value.slice(2, 4), value.slice(0, 2)]];
  return !formats.some(parts => {
    const [year, month, day] = parts.map(Number), date = new Date(Date.UTC(year, month - 1, day));
...
export function stopAppointmentReference(notes, job) {
  ...
  const input = notes.slice(0, maximumInput), selectedKind = kind(job), values = [];
  for (const match of input.matchAll(reference)) {
    const [, qualifier, value] = match;
    if (qualifier && kind(qualifier) !== selectedKind || !isReference(value)
      || !endsClause(notes, match.index + match[0].length, input.length)) continue;
