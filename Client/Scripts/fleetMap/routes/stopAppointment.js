...
export function stopAppointment(stop) {
  ...
  const format = start && end && start.getUTCFullYear() !== end.getUTCFullYear() ? yearFormat : dayFormat;
  const firstTime = clock(stop.scheduledTime), lastTime = clock(stop.scheduledTime2);
  const sameDay = start?.getTime() === end?.getTime();
  const first = [start ? format.format(start) : '', firstTime].filter(Boolean).join(' · ');
  const last = [end && !sameDay ? format.format(end) : '', sameDay && firstTime === lastTime ? '' : lastTime]
    .filter(Boolean).join(' · ');
  return [first, last && last !== first ? last : ''].filter(Boolean).join(' – ') || '—';
}
