...
export function routeDetailIndices(path, zoom, anchors = []) {
  ...
  const scale = Math.max(0.087, Math.cos(latitude));
  const tolerance = Math.min(
    1000,
    Math.max(10, (0.6 * 156543.03 * scale) / 2 ** zoom),
  );
  const boundaries = [...new Set([0, ...anchors, path.length - 1])]
  ...
  while (pending.length) {
    ...
    for (let i = start + 1; i < end; i++) {
      const x = (path[i].lng - a.lng) * scale * 111320,
        y = (path[i].lat - a.lat) * 111320;
