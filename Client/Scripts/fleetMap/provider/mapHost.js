export function createMapHost(createMap) {
  ...
  return (element, options) => {
    ...
    if (!cached) {
      ...
      element.append(host);
      try { cached = { host, map: createMap(host, options) }; }
      ...
      const { mapId, renderingType, center, zoom, ...mutableOptions } = options;
      cached.map.setOptions(mutableOptions);
    }
    mounted = true;
    let released = false;
    let revealed = false, idleListener = null, fallback = null;
    ...
    return {
      ...
      initialCamera(change, waitForIdle = true) {
        if (released) return;
        if (revealed) { change?.(false); return; }
        if (!change) { show(); return; }
        stopWaiting();
        if (waitForIdle) {
