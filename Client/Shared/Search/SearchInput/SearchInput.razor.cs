...
public partial class SearchInput
{
  ...
  [Parameter]
  public IReadOnlyList<SearchSuggestion> Options { get; set; } = [];
  ...
  private async Task KeyAsync(KeyboardEventArgs args)
  {
    ...
    if (args.Key is "ArrowDown" or "ArrowUp")
    {
      ...
      if (Options.Count > 0)
        _active =
          args.Key == "ArrowDown"
            ? (_active + 1) % Options.Count
            : (_active <= 0 ? Options.Count - 1 : _active - 1);
    }
    else if (args.Key == "Enter")
    {
      if (ShowOptions && _active >= 0 && _active < Options.Count)
        await ChooseAsync(Options[_active].Value);
      else
      {
        _open = false;
        await Enter.InvokeAsync();
      }
    }
  }
}
