...
public partial class DispatchLoadStop
{
  ...
  [Parameter]
  public EventCallback OpenDetails { get; set; }
  ...
  [Parameter]
  public RenderFragment? AdditionalDetails { get; set; }

  [Parameter]
  public bool HasAdditionalDetails { get; set; } = true;

  [Parameter]
  public RenderFragment? StopActions { get; set; }
  ...
  private DispatchEta? DisplayEstimate =>
    Completed || Stop.DriverOnly
      ? null
      : Memory.Display(StopEstimate, Clock.GetUtcNow().UtcDateTime, Refreshing);
  private bool HasDisplayEstimate => DisplayEstimate?.Stops.Count > 0;
  private bool HasHoursForecast =>
    DisplayEstimate?.Stops.FirstOrDefault()?.Hours is not null;
  private bool HasLegacyCycle =>
    DisplayEstimate
      ?.Stops.FirstOrDefault()
      ?.CycleAfterDeparture?.RemainingMinutes
      is >= 0;
  private string CycleRemaining =>
    DispatchCycleDisplay.Remaining(
      DisplayEstimate?.Stops.FirstOrDefault()?.CycleAfterDeparture
    );
  private bool Completed =>
    !Stop.DriverOnly && CompletedLoad || Stop.IsCompleted;
  ...
  private bool HasDetails =>
    !CollapsibleDetails && HasStreet
    || !string.IsNullOrWhiteSpace(Stop.StopNo)
    || AppointmentReferences.Count > 0
    || HasDisplayEstimate
    || HasAdditionalDetails && AdditionalDetails is not null;
  ...
  protected override void OnParametersSet()
  {
    ...
    {
      Job = Job,
      ScheduledDate = Stop.ScheduledDate ?? FallbackDate,
      ScheduledTime = Stop.ScheduledTime,
      ScheduledDate2 = Stop.ScheduledDate2,
      ScheduledTime2 = Stop.ScheduledTime2,
    };
    var match =
      DispatchId != Guid.Empty && Stop.Id != Guid.Empty
        ? Eta?.Stops.FirstOrDefault(estimate =>
          estimate.DispatchId == DispatchId && estimate.StopId == Stop.Id
        )
        : null;
    StopEstimate =
      Completed || Stop.DriverOnly || Eta is null
        ? null
        : Eta with
        {
