...
public partial class TruckCamera
{
  ...
  private sealed record CameraImage(
    string Status,
    string? Url,
  ...
  protected override async Task OnAfterRenderAsync(bool firstRender)
  {
    ...
    _dialogModule ??= await JS.InvokeAsync<IJSObjectReference>(
      "import",
      "./js/generated/shared/cameraDialog.js"
    );
    if (_open)
      await _dialogModule.InvokeVoidAsync("show", _dialog);
  }
  ...
  {
    _image = null;
    _message = "Image unavailable or expired. Request another snapshot.";
  }
  ...
  private async Task RefreshAsync()
  {
    ...
    bool IsFresh(CameraImage? image) =>
      image is { Url: not null, CapturedAt: not null }
    ...
    try
    {
      ...
      for (var attempt = 0; attempt < 60; attempt++)
      {
        ...
        var result = await Api.GetAsync<CameraImage>(
          $"api/fleet/trucks/{TruckId}/camera/{started.Response}",
          request.Token
        );
        request.Token.ThrowIfCancellationRequested();
        if (!result.Success)
        ...
        if (
          result.Response is { Url: not null, CapturedAt: not null } received
          && (
            _image?.CapturedAt is null
            || received.CapturedAt > _image.CapturedAt
          )
        )
          _image = received;
        if (IsFresh(_image))
    ...
    {
      if (_open && ReferenceEquals(_request, request))
        _message = "Samsara did not deliver a new snapshot. Try Refresh.";
    }
    catch (Exception ex) when (ex is HttpRequestException or JsonException)
    {
      if (ReferenceEquals(_request, request))
        _message = "Could not load the camera snapshot.";
    }
    finally
    {
      if (ReferenceEquals(_request, request))
