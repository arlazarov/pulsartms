...
export function stampStyleVersion(index, css) {
  ...
  if ([...index.matchAll(pattern)].length !== 1) throw new Error('Expected one main stylesheet link.');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const [index, css] = await Promise.all([readFile(indexPath, 'utf8'), readFile(new URL('../wwwroot/css/main.css', import.meta.url))]);
