...
public static class FleetAssignmentSync
{
  ...
  )
  {
    ...
    var trucksById = trucks.Where(x => x.IsActive && !string.IsNullOrWhiteSpace(x.ExternalId))
      .ToDictionary(x => x.ExternalId, StringComparer.OrdinalIgnoreCase);
    var driversById = dbContext.Drivers.Local
      .Where(x => x.IsActive && !string.IsNullOrWhiteSpace(x.ExternalId))
      .ToDictionary(x => x.ExternalId, StringComparer.OrdinalIgnoreCase);
    var trailersById = dbContext.Trailers.Local
      .Where(x => x.IsActive && !string.IsNullOrWhiteSpace(x.ExternalId))
      .ToDictionary(x => x.ExternalId, StringComparer.OrdinalIgnoreCase);
    ...
    var current = assignments
      .Where(x => !x.IsPassenger && x.StartTime <= snapshotTime
      ...
      .SelectMany(group =>
      {
        var latest = group.Where(x => x.StartTime == group.Max(y => y.StartTime)).ToList();
        return latest.Select(x => x.VehicleExternalId).Distinct(StringComparer.OrdinalIgnoreCase).Count() == 1
          ? latest.Take(1) : [];
      })
    ...
    foreach (var group in current)
    {
      ...
      if (!trucksById.TryGetValue(assignment.VehicleExternalId, out var truck)
    ...
    var trailersByDriver = trailerAssignments
      .Where(x => x.StartTime <= snapshotTime && (x.EndTime is null || x.EndTime > snapshotTime)
        && !string.IsNullOrWhiteSpace(x.DriverExternalId)
        && !string.IsNullOrWhiteSpace(x.TrailerExternalId))
      .GroupBy(x => x.DriverExternalId, StringComparer.OrdinalIgnoreCase)
      .ToDictionary(x => x.Key,
        x => x.Select(y => y.TrailerExternalId).Distinct(StringComparer.OrdinalIgnoreCase).ToList(),
        StringComparer.OrdinalIgnoreCase);

    var candidates = selected
      .Where(x => trailersByDriver.TryGetValue(x.DriverExternalId, out var ids) && ids.Count == 1)
      .Select(x => new { Truck = trucksById[x.VehicleExternalId], TrailerId = trailersByDriver[x.DriverExternalId][0] })
      .GroupBy(x => x.TrailerId, StringComparer.OrdinalIgnoreCase);

    foreach (var group in candidates)
    {
      if (group.Count() != 1 || !trailersById.TryGetValue(group.Key, out var trailer))
        continue;
    ...
    {
      var driverId = driverTargets.TryGetValue(truck.Id, out var d) ? d : (Guid?)null;
      var trailerId = trailerTargets.TryGetValue(truck.Id, out var t) ? t : (Guid?)null;
      if (truck.DriverId != driverId) { truck.Driver = null; truck.DriverId = null; }
      if (truck.TrailerId != trailerId) { truck.Trailer = null; truck.TrailerId = null; }
    }
    var count = await dbContext.SaveChangesAsync(cancellationToken);
    foreach (var truck in trucks)
    {
      truck.DriverId = driverTargets.TryGetValue(truck.Id, out var d) ? d : null;
      truck.TrailerId = trailerTargets.TryGetValue(truck.Id, out var t) ? t : null;
    }
    return count;
  }
}
