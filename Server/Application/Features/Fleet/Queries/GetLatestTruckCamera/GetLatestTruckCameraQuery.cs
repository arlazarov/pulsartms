...
public record GetLatestTruckCameraQuery(Guid TruckId)
  : IRequest<RequestResponse<CameraImage>>;

public sealed class GetLatestTruckCameraHandler(
  IAppDbContext db,
...
) : IRequestHandler<GetLatestTruckCameraQuery, RequestResponse<CameraImage>>
{
  public async Task<RequestResponse<CameraImage>> Handle(
    GetLatestTruckCameraQuery request,
  ...
  )
  {
    ...
    try
    {
      return RequestResponse<CameraImage>.Ok(
        await provider.LatestAsync(vehicle, ct)
      );
    }
