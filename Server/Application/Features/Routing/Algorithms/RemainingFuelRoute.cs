...
{
  public static TruckRoute? TryRead(
    RoutePlan plan,
  ...
  )
  {
    ...
    if (
      ...
      || plan.InputsChanged
      || remaining.Count == 0
      || plan.Route.Legs.Count != plan.Stops.Count
    )
      return null;
    ...
    if (leg.Points.Count < 2 || !double.IsFinite(leg.Miles) || leg.Miles < 0)
      return null;
    var geometry = new RouteGeometry(new() { Legs = [leg] });
    var match = geometry.Match(position);
    // The result starts on the saved road; callers opting into a wider match
    // must account for origin access separately.
    if (match.Away > maximumMatchMiles)
      return null;
    var atEndpoint =
      match.Point == leg.Points[^1]
      || Math.Abs(leg.Miles - match.Along) <= 1e-8;
    if (leg.Miles == 0 && leg.Points.Any(point => point != leg.Points[^1]))
      return null;
    if (atEndpoint)
    {
      // Keep the mandatory stop and its leg identity until actual tracking
      // advances it.
      var endpoint = leg.Points[^1];
      return Remaining(new(0, 0, [endpoint, endpoint]));
    }
    if (match.Along >= leg.Miles)
      return null;
    var points = new List<RoutePoint> { match.Point };
    var lengths = leg
      .Points.Zip(leg.Points.Skip(1), RouteGeometry.Distance)
      .ToArray();
    ...
    TruckRoute Remaining(RouteLeg first)
    {
      ...
      return new()
      {
        CalculatedAt = plan.Route.CalculatedAt,
        Legs = legs,
        Miles = legs.Sum(x => x.Miles),
        Seconds = legs.Sum(x => x.Seconds),
        Points = legs.SelectMany((x, i) => i == 0 ? x.Points : x.Points.Skip(1))
          .ToList(),
