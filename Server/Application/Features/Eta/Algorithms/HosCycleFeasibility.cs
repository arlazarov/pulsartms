...
public sealed class HosCycleFeasibility
{
  ...
  public int? DrivingShortfallMinutes =>
    Verified ? MinutesCeiling(peakDrivingDebt) : null;

  public HosCycleFeasibility(
    DateTimeOffset now,
  ...
  )
  {
    ...
    anchorAt = now;
    timeline = CreateCycleTimeline(history, now);
    ...
    if (rule is not null)
    {
      ...
      if (
        clocks.CycleMs is >= 0 and { } cycle
        && cycle / 3600000d <= rule.Hours
      )
      {
        anchorHours = cycle / 3600000d;
        historicalDuty = double.IsFinite(used) ? used : 0;
        Verified = true;
        RecapVerified =
          double.IsFinite(used)
          && Math.Abs(rule.Hours - used - anchorHours) <= .25;
      }
      if (Verified && BalanceHours(now) is null)
        Verified = false;
    }
    if (!Verified)
    {
      RecapVerified = false;
      UnavailableReason =
        "Current ELD cycle or usable cycle history is unavailable.";
    }
  }

  private static HosTimeline? CreateCycleTimeline(
    HosHistory? history,
    DateTimeOffset now
  )
  {
    var complete = HosTimeline.Create(history, now);
    if (complete is not null || history is null || history.Periods.Count == 0)
      return complete;
    var first = history.Periods.Min(x => x.Start);
    if (first <= history.From || first >= history.Through)
      return null;
    // Validate the observed suffix without inventing duty for missing days.
    // Recap still requires coverage of the applicable cycle window.
    return HosTimeline.Create(history with { From = first }, now);
  }
  ...
  public void Enter(string nextCountry)
  {
    ...
    if (
      Verified
      && RecapVerified
      && nextRule is not null
      && nextCountry is "US" or "CA"
    )
    {
      ...
      // A border must not award hours: both histories must reconcile to the
      // same ELD anchor.
      if (
        double.IsFinite(nextUsed)
        && Math.Abs(nextRule.Hours - nextUsed - anchorHours) <= .25
        && (
          nextCountry != "CA"
          || nextRule.Days != 14
          || double.IsFinite(timeline.DutySinceDailyRest(anchorAt))
        )
      )
      {
        country = nextCountry;
    ...
    RecapVerified = false;
    UnavailableReason =
      "Cycle credits across different jurisdictions need verification.";
  }
  ...
  {
    if (!RecapVerified)
      return anchorHours - timeline!.Hours(anchorAt, at, p => p.Duty);
    ...
    if (beginning <= anchorAt && windowStart < anchorAt)
    {
      ...
      // Reconcile against ELD without releasing unexplained duty early or
      // carrying it past its home day.
      carried = Math.Min(
        eldUsed,
        remaining + Math.Max(0, eldUsed - historicalDuty)
      );
    }
    var projectedFrom = beginning > anchorAt ? beginning : anchorAt;
    return rule!.Hours
      - carried
      - timeline.Hours(projectedFrom, at, p => p.Duty);
  }

  public int? BalanceMinutes(DateTimeOffset at) =>
    BalanceHours(at) is { } balance ? SignedMinutes(balance) : null;

  public DateTimeOffset? NextHomeDay(DateTimeOffset at) =>
    Verified ? timeline!.NextDay(at) : null;

  public void Observe(DateTimeOffset start, DateTimeOffset end, string status)
  {
    if (timeline is null || end <= start)
      return;
    if (!Verified || status != "driving")
    {
      timeline.Add(start, end, status);
      return;
    }
    var cursor = start;
    while (cursor < end)
  ...
  public void CreditRestart(DateTimeOffset at)
  {
    if (!Verified || timeline!.OngoingRest(at) + Epsilon < rule!.RestartHours)
      throw new InvalidOperationException(
        "A cycle scenario cannot credit an unfinished restart."
      );
    restartAt = at;
    anchorAt = at;
  ...
  public DateTimeOffset? NextUsableRecap(DateTimeOffset at)
  {
    ...
    for (var day = 0; day < rule!.Days; day++)
    {
      ...
      if (country == "CA" && rule.Days == 14)
      {
        ...
        available = Math.Min(
          available,
          rest >= 24 ? 70 : 70 - timeline.DutySinceDailyRest(at)
        );
      }
  ...
  {
    if (BalanceHours(at) is not { } balance)
      return null;
    if (!RecapVerified)
      return new(Math.Max(0, SignedMinutes(balance)), null, null, null, false);
    var available = balance;
    var boundary = at;
    for (var day = 0; day < rule!.Days; day++)
    {
      boundary = timeline!.NextDay(boundary);
      if (BalanceHours(boundary) is not { } future)
        break;
      var returning = SignedMinutes(future - available);
      if (returning > 0)
        return new(
          Math.Max(0, SignedMinutes(balance)),
          boundary,
          returning,
          timeline.Source.TimeZoneId,
          true
        );
      available = future;
    }
  ...
  private static int SignedMinutes(double hours) =>
    (int)
      Math.Clamp(Math.Floor(hours * 60 + Epsilon), int.MinValue, int.MaxValue);

  private static int MinutesCeiling(double hours) =>
    (int)
      Math.Min(int.MaxValue, Math.Ceiling(Math.Max(0, hours) * 60 - Epsilon));
}

internal sealed class CycleScenarioUnavailableException(
  string reason = "A verified cycle alternative is unavailable."
) : Exception(reason);
