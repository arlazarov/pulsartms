...
public sealed class EtaRouteTimingCache(int capacity = 128, int maximumRetainedUnits = 32768)
{
  ...
  private readonly object[] gates = Enumerable.Range(0, 16).Select(_ => new object()).ToArray();
  ...
  public int RetainedUnits { get { lock (sync) return retainedUnits; } }

  public EtaRouteTiming GetOrCreate(Guid planId, int version, TruckRoute route, IRouteRegionLookup regions)
  {
    var key = new Key(planId, version);
    if (Find(key, route) is { } cached) return cached;
    lock (gates[(uint)key.GetHashCode() % gates.Length])
    {
      if (Find(key, route) is { } ready) return ready;
      var timing = EtaRouteTiming.Compile(route, regions);
      lock (sync)
      {
        if (entries.TryGetValue(key, out var previous)) Remove(previous);
        if (timing.RetainedUnits > maximumRetainedUnits) return timing;
        while (recent.Last is { } oldest && (entries.Count >= capacity || retainedUnits + timing.RetainedUnits > maximumRetainedUnits))
          Remove(oldest);
  ...
  private EtaRouteTiming? Find(Key key, TruckRoute route)
  {
    lock (sync)
    {
      // A plan version owns immutable geometry; cheap leg metadata catches accidental key reuse.
      if (!entries.TryGetValue(key, out var node) || !node.Value.Timing.Matches(route)) return null;
      recent.Remove(node);
