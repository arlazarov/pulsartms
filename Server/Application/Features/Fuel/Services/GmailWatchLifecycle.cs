...
public sealed class GmailWatchLifecycle(
...
)
{
  ...
  )
  {
    ...
    if (
      !register
      && (
        state is null || state.NextRenewalAt > now && state.NextRecoveryAt > now
      )
    )
      return new();
    ...
    try
    {
      ...
      if (register || state.NextRenewalAt <= now)
      {
        ...
        // Reserve the retry before the provider request so process restarts
        ...
        try
        {
          ...
          if (expiration <= now.AddMinutes(15))
            throw new InvalidOperationException(
              "Gmail watch expiration is missing or too soon."
            );
          ...
          var next =
            expiration.AddHours(-6) < now.AddDays(1)
              ? expiration.AddHours(-6)
              : now.AddDays(1);
          state.NextRenewalAt =
            next > now.AddMinutes(15) ? next : now.AddMinutes(15);
        }
      ...
      )
      {
        ...
        try
        {
          ...
          if (!imported.Success)
            throw new InvalidOperationException(
              "Fuel notification recovery failed."
            );
  ...
  private static TimeSpan RetryDelay(int failures) =>
    TimeSpan.FromMinutes(Math.Min(360, 15 * Math.Pow(2, failures - 1)));
}
