...
public sealed class ReadCache(IOptions<SynchronizationOptions> options) : IReadCache, IDisposable
{
  ...
  private static T Restore<T>(Cached value) => value.Board is { } board ? (T)(object)board : value.Route is { } route
    ? (T)(object)Copy(route) : JsonSerializer.Deserialize<T>(value.Json!, Json)!;
  ...
  public async Task<T> GetAsync<T>(string group, string key, Func<Task<T>> load, TimeSpan? lifetime = null)
  {
    ...
    try
    {
      ...
      var entry = result is DispatchBoardIndex board ? new Cached(null, null, board) : result is Domain.Entities.Dispatch.DispatchRoutePlan route
        ? new Cached(null, Copy(route)) : new Cached(JsonSerializer.SerializeToUtf8Bytes(result, Json), null);
      ...
      if (size <= 8 * 1024 * 1024 && version == generations.Get(group))
        bounded.Set(cacheKey, entry, new MemoryCacheEntryOptions
