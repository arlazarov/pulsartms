...
public sealed class UserRoleService(AppDbContext db) : IUserRoleService
{
  ...
  )
  {
    if (
      ...
      return null;
    var roles = await db
      .UserClaims.Where(c => c.UserId == identityId && c.ClaimType == RoleClaim)
  ...
  )
  {
    ...
    var roles = await db
      .UserClaims.AsNoTracking()
      .Where(claim =>
        identityIds.Contains(claim.UserId) && claim.ClaimType == RoleClaim
      )
      .Select(claim => new { claim.UserId, claim.ClaimValue })
      .ToListAsync(ct);
    var byIdentity = roles.ToLookup(
      claim => claim.UserId,
      claim => claim.ClaimValue
    );
    return users.ToDictionary(
      user => user.Id,
      user => Resolve(byIdentity[user.IdentityUserId].ToArray())
    );
  }
  ...
  )
  {
    ...
    if (db.Database.IsNpgsql())
      await db.Database.ExecuteSqlInterpolatedAsync(
        $"SELECT \"Id\" FROM \"AspNetUsers\" WHERE \"Id\" = {identityId} FOR UPDATE",
        ct
      );
    var claims = await db
      .UserClaims.Where(c => c.UserId == identityId && c.ClaimType == RoleClaim)
