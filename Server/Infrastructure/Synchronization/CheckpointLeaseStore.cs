...
internal sealed class CheckpointLeaseStore(AppDbContext db, Guid id)
{
  ...
  )
  {
    ...
    return await db
        ...
        .ExecuteUpdateAsync(
          s =>
            s.SetProperty(x => x.Owner, owner)
              .SetProperty(x => x.LeaseUntil, now.AddMinutes(3)),
          ct
        ) == 1;
  }
  ...
  public async Task<bool> RenewAsync(
    ...
    await db
      .SynchronizationCheckpoints.Where(x =>
        x.Id == id && x.Owner == owner && x.LeaseUntil > now
      )
      .ExecuteUpdateAsync(
        s => s.SetProperty(x => x.LeaseUntil, now.AddMinutes(3)),
        ct
      ) == 1;

  public Task<string?> ReadAsync(CancellationToken ct) =>
    db
      .SynchronizationCheckpoints.Where(x => x.Id == id)
  ...
  public async Task SaveAsync(string owner, string json, CancellationToken ct)
  {
    ...
    var changed = await db
      ...
      .ExecuteUpdateAsync(
        s =>
          s.SetProperty(x => x.StateJson, json)
            .SetProperty(x => x.UpdatedAt, now),
        ct
      );
    if (changed != 1)
      throw new InvalidOperationException(
        "Background checkpoint lease was lost."
      );
  }

  public Task ReleaseAsync(string owner, CancellationToken ct) =>
    db
      .SynchronizationCheckpoints.Where(x => x.Id == id && x.Owner == owner)
      .ExecuteUpdateAsync(
        s =>
          s.SetProperty(x => x.LeaseUntil, DateTime.UtcNow)
            .SetProperty(x => x.Owner, ""),
        ct
      );
}
