...
public sealed class GmailWatchStore(AppDbContext db) : IGmailWatchStore
{
  ...
  public Task SaveAsync(string owner, GmailWatchState state, CancellationToken ct) =>
  ...
  public async Task<GmailWatchState?> ReadAsync(CancellationToken ct)
  {
    ...
    var state = json is null ? null : JsonSerializer.Deserialize<GmailWatchState>(json, Json);
