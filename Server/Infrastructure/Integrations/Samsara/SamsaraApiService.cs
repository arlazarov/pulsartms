...
public class SamsaraApiService : BaseApiService
{
  ...
  public Task<System.Text.Json.JsonElement> RequestCameraImageAsync(string vehicleId, DateTimeOffset time, CancellationToken ct)
    => PostAsync<object, System.Text.Json.JsonElement>("cameras/media/retrieval", new {
      vehicleId, startTime = time.ToString("O"), endTime = time.ToString("O"),
      mediaType = "image", inputs = new[] { "dashcamRoadFacing" }
    }, ct);

  public SamsaraApiService(HttpClient httpClient, IIntegrationCredentials credentials)
    : base(httpClient)
  {
  ...
  protected override async Task PrepareRequestAsync(HttpRequestMessage request, CancellationToken cancellationToken)
  {
    ...
    if (destination.Scheme != Uri.UriSchemeHttps || destination.Host != "api.samsara.com"
      || destination.Port != 443 || destination.UserInfo.Length != 0)
      throw new InvalidOperationException("Samsara request destination is not allowed.");
    request.RequestUri = destination;
    var values = await _credentials.GetAsync(IntegrationProviderCatalog.Samsara, cancellationToken);
    var token = values.Get("apiKey");

    if (string.IsNullOrWhiteSpace(token))
    {
      throw new InvalidOperationException("Samsara API token is not configured.");
    }

    request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
  }

  public Task<IReadOnlyList<SamsaraHosHistory>> GetHosHistoryAsync(string driverId, DateTimeOffset from, DateTimeOffset to, CancellationToken ct)
    => GetDataAsync<SamsaraHosHistory>("fleet/hos/logs?driverIds=" + Uri.EscapeDataString(driverId)
      + "&startTime=" + Uri.EscapeDataString(from.ToString("O")) + "&endTime=" + Uri.EscapeDataString(to.ToString("O")), ct);

  public Task<IReadOnlyList<SamsaraHosClock>> GetHosClocksAsync(CancellationToken ct)
  ...
  public async Task<IReadOnlyList<SamsaraTrailerAssignment>> GetTrailerAssignmentsAsync(
  ...
  )
  {
    ...
    var firstBatch = true;
    foreach (var batch in driverIds.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct().Chunk(50))
    {
      if (!firstBatch)
        await Task.Delay(TimeSpan.FromMilliseconds(250), cancellationToken);
      firstBatch = false;
      var ids = string.Join(",", batch.Select(Uri.EscapeDataString));
      assignments.AddRange(await GetDataAsync<SamsaraTrailerAssignment>(
        $"driver-trailer-assignments?driverIds={ids}", cancellationToken));
    }
    return assignments;
  }
  ...
  public async Task<SamsaraResponse<SamsaraStatsFeed>> GetStatsFeedAsync(string? cursor, CancellationToken ct)
  {
    ...
    return await GetAsync<SamsaraResponse<SamsaraStatsFeed>>(url, ct)
      ?? throw new InvalidOperationException("Samsara returned an empty telemetry feed.");
  }
  ...
  )
  {
    ...
    while (true)
    {
      var response = await GetAsync<SamsaraResponse<T>>(url, cancellationToken)
        ?? throw new InvalidOperationException("Samsara returned an empty response.");
      ...
      if (string.IsNullOrWhiteSpace(cursor) || !cursors.Add(cursor))
        throw new InvalidOperationException("Samsara returned an invalid pagination cursor.");
      url = endpoint + (endpoint.Contains('?') ? "&" : "?") + "after=" + Uri.EscapeDataString(cursor);
      await Task.Delay(TimeSpan.FromMilliseconds(250), cancellationToken);
    }
  }
}
