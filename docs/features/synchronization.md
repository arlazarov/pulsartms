...
- All browsers use the server's current telemetry snapshot when background synchronization is enabled. They do not each poll Samsara. The feed cursor is stored together with the telemetry it represents. High-frequency location history is requested centrally for the fleet and kept in memory for two minutes. Each published snapshot carries a new `Revision`; `GET /api/fleet/locations` returns it as a weak `ETag` with `Cache-Control: private, no-cache`, so a browser poll whose `If-None-Match` still matches receives `304` and no body is serialized. Browsers revalidate telemetry automatically; the Client sends no conditional headers for it. The Client adds `wait=25`: when the presented validator still matches, the API holds the request until the next published snapshot or the wait elapses (capped by `Synchronization:LocationWaitSecondsMax`, default 25 seconds, below the 60-second Firebase Hosting rewrite timeout), so a quiet fleet costs one held request per wait instead of a poll every ten seconds and a new snapshot reaches open tabs within a second. Each held poll occupies one Cloud Run request slot for its duration; keep the service `--concurrency` above the expected number of open tabs. `GET /api/fleet/locations/stream` offers the same snapshots as server-sent events (one `locations` event per revision, a comment while unchanged) for hosts that pass streamed responses through, such as a direct Cloud Run domain; the Client does not use it because Firebase Hosting rewrites buffer streams. Planning reads (`GET /api/fleet/trucks/{truckId}/planning` and `GET /api/dispatch/{id}/planning/automatic`) have no cheap revision, so the API serializes the body once, tags it with a weak `ETag` derived from a digest of the body and answers a matching `If-None-Match` with `304`; the handler still runs. `PlanningDisplayCache` keeps that tag with each entry, sends it itself with `cache: no-store`, and on `304` reuses its copy for another lifetime instead of deserializing an identical plan. `POST` on the same planning URLs remains for browsers still running an older Client. The dispatch board polls `?points=false` and receives current positions without the location history; the map keeps the full snapshot. Map, board and planning polls run every 10 seconds while the tab is visible and every `PageVisibilityPacing.HiddenInterval` (60 seconds) while it is hidden; the next visible tick follows within the visible interval. Each history point carries only the truck's external ID, coordinates, speed, heading and GPS time; unit, driver, trailer, address and fuel appear once per truck in `Trucks`.
...
`Hosting:Role` splits the process when one instance is not enough. `All` (default) serves requests and runs every worker. `Workers` does the same and is the intended owner of the lease. `Api` serves requests only: no ETA refresh, truck history, planning refresh, base route or Gmail worker is hosted, and the synchronization worker never takes the lease; it re-reads the owner's checkpoint every `FollowSeconds` (default 5) and republishes the snapshot only when a vehicle timestamp changed, so validators keep matching. An `Api` instance therefore sees positions up to `CheckpointSeconds` plus `FollowSeconds` old and carries no high-frequency trail points; lower `CheckpointSeconds` (minimum 10) on the `Workers` service when a split is deployed. `Database:ApplyMigrations` should be on for one service only.
