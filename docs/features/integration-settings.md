...
Until an Admin explicitly saves a replacement, integrations continue reading the
existing server configuration: `TorqueAI:ApiKey`, `Samsara:ApiToken` and
...
key ring and a versioned, provider-specific purpose. Each storage operation uses
its own short-lived database context. Unique provider identity and revision
...
The additive `IntegrationCredentialSettings` table must exist before enabling the
new API. No credential-value migration or backfill is needed. Deploy the compatible
API before the Client. An older API ignores saved overrides; rolling back it can
