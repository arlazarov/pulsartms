...
- Meter `AMFTMS.Application`, histogram `amftms.request.duration` (milliseconds), tags `request`, `outcome`. When `OTEL_EXPORTER_OTLP_ENDPOINT` is set, the API exports this meter plus ASP.NET Core, HttpClient and .NET runtime metrics over OTLP (`TelemetryExport`); the standard `OTEL_EXPORTER_OTLP_*` variables select protocol and headers. Without the endpoint nothing is registered. Traces are intentionally not exported because HTTP client spans would carry provider URLs whose query strings can hold keys. Slow requests and routing requests also emit `RequestTiming` logs, so diagnosing them does not depend on a collector. See [load testing](load-testing.md) for the polling scenario that these metrics are meant to observe.
