...
public class HosCycleForecastTests
{
  ...
  public void RecapUsesDriverHomeDayIncludingDst(
    string nowText,
    int dayStart,
    string recapText
  )
  ...
  public void ConfiguredCycleDeterminesWhichDaysDutyReturns(
    int days,
    int hours,
    int remaining,
    int returning
  )
  {
    var history = History(
      Now,
      day => day == new DateTime(2026, 9, 2) ? 2 : 1
    ) with
    {
      UsCycle = new(days, hours, 34),
    };
  ...
  public void MismatchedAndStaleHistoryKeepRecapUnknown()
  {
    ...
    AssertUnknownRecap(
      new HosTravelClock(Now, Clocks(1), "US", history).SnapshotCycle()
    );
    AssertUnknownRecap(
      new HosTravelClock(
        Now,
  ...
  private static HosTravelClock Clock(
    DateTimeOffset now,
    HosHistory history,
    string country = "US",
    HosCycleMode mode = HosCycleMode.Observe
  )
  {
  ...
  private static DriverHosClocks Clocks(double cycleHours) =>
    new()
    {
      DriveMs = 11 * 3600000L,
      ShiftMs = 14 * 3600000L,
  ...
  private static HosHistory History(
    DateTimeOffset now,
    Func<DateTime, double>? dutyHours = null
  )
  {
    var start = new DateTimeOffset(
      now.UtcDateTime.Date.AddDays(-16),
      TimeSpan.Zero
    );
