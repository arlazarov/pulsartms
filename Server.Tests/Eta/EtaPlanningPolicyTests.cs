...
public class EtaPlanningPolicyTests
{
  ...
  private static DriverHosClocks Clocks(double drive = 11, double shift = 14) => new()
    { DriveMs = (long)(drive * 3600000), ShiftMs = (long)(shift * 3600000), CycleMs = 70 * 3600000L, BreakMs = 8 * 3600000L };

  [Fact]
  public void TenHourPlanningLimitAddsRestAndPtiBeforeTheNextShift()
  {
    var clock = new HosTravelClock(Now, Clocks(), "US", planning: new() { DrivingHoursPerShift = 10 });
    clock.Drive(11, "US"); clock.CompletePlannedBreak();
  ...
  public void FuelDoesNotReplaceTheSeparatePlannedBreak()
  {
    ...
    clock.Drive(7, "US"); clock.Fuel(); clock.Drive(3, "US");
  ...
  {
    var clock = new HosTravelClock(Now, Clocks(drive: 2, shift: 5), "US", planning: new() { DrivingHoursPerShift = 10 });
    clock.Drive(2, "US");
    Assert.Equal(.25, clock.PreTripHours);
  ...
  {
    var clock = new HosTravelClock(Now, Clocks(), "US", planning: new());
    clock.Drive(2, "US"); clock.CompletePlannedBreak(); clock.CompletePlannedBreak();
  ...
  public void DefaultAllowsElevenHoursButNotBeyondEldAvailability()
  {
    ...
    Assert.Equal(.5, clock.RestHours);
    var limited = new HosTravelClock(Now, Clocks(drive: 1), "US", planning: new());
    limited.Drive(2, "US");
