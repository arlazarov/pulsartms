...
public class HosDutyStatusTests
{
  ...
  public void ResetCountdownUsesCurrentCountryAndCanadianCycle(string country, int days, int restart, int target, int remaining)
  {
    var history = History() with { CanadaCycle = new(days, days == 7 ? 70 : 120, restart), Periods =
      [new(Now.AddHours(-8), Now.AddMinutes(-368), "driving"), new(Now.AddMinutes(-368), Now, "sleeperBerth")] };
  ...
  {
    var history = History();
    Assert.Null(HosDutyStatus.Read(history, Clocks("onDuty"), Now, "US")!.CycleResetRemainingMinutes);
    Assert.Null(HosDutyStatus.Read(history with { Through = Now.AddMinutes(-4) }, Clocks(), Now, "US")!.CycleResetRemainingMinutes);
    Assert.Null(HosDutyStatus.Read(history with { Periods = history.Periods.Where((_, i) => i != 2).ToArray() }, Clocks(), Now, "US")!.CycleResetRemainingMinutes);
  }

  [Fact]
  public void CompletedResetHasZeroRemainingTime()
  {
    var history = History() with { Periods =
      [new(Now.AddHours(-40), Now.AddHours(-36), "driving"), new(Now.AddHours(-36), Now, "sleeperBerth")] };
    Assert.Equal(0, HosDutyStatus.Read(history, Clocks(), Now, "US")!.CycleResetRemainingMinutes);
  }
  private static HosHistory History() => new(Now.AddDays(-16), Now, "Etc/UTC", 0, new(8, 70, 34), null,
  ...
  private static DriverHosClocks Clocks(string status = "sleeperBerth") => new()
    { CurrentDutyStatus = status, UpdatedAt = Now.UtcDateTime, DriveMs = 0, ShiftMs = 0, CycleMs = 54 * 3600000L, BreakMs = 8 * 3600000L };
  ...
  {
    var h = History();
    h = h with { Periods = h.Periods.Select((p, i) => i == 2 ? p with { Status = status } : p).ToArray() };
  ...
  {
    var h = History();
    Assert.Null(HosDutyStatus.Read(h with { Periods = h.Periods.Where((_, i) => i != 2).ToArray() }, Clocks(), Now)!.RestMinutes);
    Assert.Null(HosDutyStatus.Read(h with { Through = Now.AddMinutes(-4) }, Clocks(), Now)!.RestMinutes);
  ...
  {
    Assert.Equal("sleeperBerth", SamsaraHosHistoryProvider.NormalizeStatus("sleeperBed"));
    Assert.True(new HosPeriod(Now.AddHours(-1), Now, "personalConveyance").Rest);
  }

  [Fact]
  public void OngoingRestFinishesBeforeDrivingEvenWhenSomeDriveTimeRemains()
  {
    var clocks = Clocks(); clocks.DriveMs = 4 * 3600000L; clocks.ShiftMs = 6 * 3600000L;
  ...
  {
    var clocks = Clocks(); clocks.CycleMs = 3600000L;
    var clock = new HosTravelClock(Now, clocks, "US");
    clock.CompleteOngoingDailyRest(HosDutyStatus.Read(History(), clocks, Now));
  ...
  {
    var clock = new HosTravelClock(Now, new() { DriveMs = 5 * 3600000L, ShiftMs = 8 * 3600000L, CycleMs = 54 * 3600000L, BreakMs = 8 * 3600000L }, "US");
    clock.CompleteOngoingDailyRest(new("offDuty", Now.AddHours(-1), Now.AddHours(-1), Now));
    clock.Drive(1, "US");
