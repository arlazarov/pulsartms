...
public sealed class EtaSleeperPlanningTests
{
    ...
    public void FacilityServiceAdvancesDepartureWithoutSpendingCycle()
    {
        ...
        var clock = new HosTravelClock(Now, HosForecastFixture.Clocks(Now, history), "US", history, planning: new());
        clock.Drive(1, "US");
    ...
    public void DrivingDaySpendsElevenHoursDrivingAndTwentyMinutesOverheadDespiteMultipleStops()
    {
        ...
        Assert.Equal(40 * 60 - 11 * 60 - 20, clock.CycleFeasibility.BalanceMinutes(clock.Now));
    ...
    {
        var history = HosForecastFixture.History(Now, cycleHours: 20);
        var clock = new HosTravelClock(Now, HosForecastFixture.Clocks(Now, history, drive: 0, shift: 0),
            "US", history, planning: new());
