...
public sealed class EtaRouteTimingTests
{
  ...
  {
    var lookup = new CountingRegions(point => point.Longitude is > -80.7 and < -80.3 ? new("CA", "Etc/UTC", false) : new("US", "Etc/UTC", false));
    var timing = EtaRouteTiming.Compile(Route(), lookup);
    var leg = Assert.Single(timing.Legs);
    Assert.True(timing.HasCompleteTravelTimes);
    Assert.Equal(["US", "CA", "US"], leg.Segments.Select(segment => segment.Country));
  ...
  public void PreservesLegTimingBoundariesAndUnsupportedRegions()
  {
    ...
    var lookup = new CountingRegions(point => point.Longitude < -80 ? new("US", "Etc/UTC", false) : new("CA", "Etc/UTC", true));
    var timing = EtaRouteTiming.Compile(route, lookup);
    ...
    var unknown = EtaRouteTiming.Compile(Route(), new CountingRegions(_ => new("", "Etc/UTC", false)));
    Assert.False(Assert.Single(Assert.Single(unknown.Legs).Segments).IsSupported);
  }
  ...
  public void SamplingKeepsInteriorVerticesAndAtMostTwoMilesPerSample()
  {
    ...
    var route = new TruckRoute { Legs = [new(10, 1200, [new(35, -82), new(35, -81), new(35, -80)])] };
    var timing = EtaRouteTiming.Compile(route, new CountingRegions(point =>
    {
      sampled.Add(point);
  ...
  public async Task ConcurrentRequestsCompileTheSameVersionOnce()
  {
    ...
    var id = Guid.NewGuid();
    var route = Route();
    var results = await Task.WhenAll(Enumerable.Range(0, 12).Select(_ => Task.Run(() => cache.GetOrCreate(id, 1, route, lookup))));
  ...
  public void OversizedProfilesAreReturnedWithoutExceedingCacheBound()
  {
    ...
    var result = cache.GetOrCreate(Guid.NewGuid(), 1, Route(), new CountingRegions());
  ...
  {
    var route = Route();
    route.Legs[0] = route.Legs[0] with { Points = [new(35, -81), new(35, -81)] };
  ...
  {
    var route = new TruckRoute { Legs = [new(0, 0, [new(35, -81), new(35, -81)])] };
    var timing = EtaRouteTiming.Compile(route, new CountingRegions());
    Assert.True(timing.HasCompleteTravelTimes);
    Assert.Empty(Assert.Single(timing.Legs).Segments);
  ...
  private static TruckRoute Route() => new()
  {
    Miles = 100, Seconds = 7200,
