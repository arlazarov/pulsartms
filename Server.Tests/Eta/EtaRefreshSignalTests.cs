...
public sealed class EtaRefreshSignalTests
{
  ...
  public void StaleCleanupCannotRemoveANewerConcurrentResult()
  {
    ...
    var older = new EtaMemory.Entry("older", new(now, now.AddMinutes(2), [], null, []));
    var newer = new EtaMemory.Entry("newer", new(now.AddSeconds(1), now.AddMinutes(2), [], null, []));
  ...
  public async Task RepeatedSnapshotMissesCoalesceUntilTheInputRevisionChanges()
  {
    ...
    await memory.WaitForRefreshAsync(default).WaitAsync(TimeSpan.FromSeconds(1));
    using var cancelled = new CancellationTokenSource();
    var waiting = memory.WaitForRefreshAsync(cancelled.Token);
    for (var i = 0; i < 10; i++) memory.Demand(id, "first", DateTime.UtcNow);
  ...
  public async Task NewDemandWakesImmediatelyAndRepeatedViewsDoNotAccelerateRetries()
  {
    ...
    using var cancelled = new CancellationTokenSource();
    var waiting = memory.WaitForRefreshAsync(cancelled.Token);
    for (var i = 0; i < 10; i++) memory.View(id, DateTime.UtcNow);
    Assert.False(waiting.IsCompleted);
    cancelled.Cancel();
  ...
  {
    var memory = new EtaMemory();
    for (var i = 0; i < 10; i++) memory.RequestRefresh();
    await memory.WaitForRefreshAsync(default).WaitAsync(TimeSpan.FromSeconds(1));
    using var cancelled = new CancellationTokenSource();
    var waiting = memory.WaitForRefreshAsync(cancelled.Token);
    Assert.False(waiting.IsCompleted);
  ...
  {
    var memory = new EtaMemory();
    var plan = new RoutePlan { Id = Guid.NewGuid(), DispatchId = Guid.NewGuid(), Version = 2 };
    var state = new RoutePlanningState(new(), plan, null, null, null, true);
    var service = new EtaService(null!, null!, new RouteRegionLookup(), memory,
      new PlanningTestServices.NoHos(), Options.Create(new EtaPlanningOptions()));
    Assert.Null(service.GetCached(state));
    await memory.WaitForRefreshAsync(default).WaitAsync(TimeSpan.FromSeconds(1));
    memory.Results[plan.DispatchId] = new("old", new(DateTime.UtcNow, DateTime.UtcNow.AddMinutes(2), [], null, []), "old");
    Assert.Null(service.GetCached(state));
    await memory.WaitForRefreshAsync(default).WaitAsync(TimeSpan.FromSeconds(1));
    using var cancelled = new CancellationTokenSource();
    var waiting = memory.WaitForRefreshAsync(cancelled.Token);
