...
public sealed class EtaPerformanceTests(ITestOutputHelper output)
{
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void WarmCalculationReusesRegionsAndAllocationsDoNotGrowWithGeometry(
    bool withHistory
  )
  {
    ...
    WriteMeasurement(
      "Sparse synthetic single-country lookup (2 points)",
      sparse
    );
    WriteMeasurement(
      "Dense synthetic single-country lookup (10,001 points)",
      dense
    );
    ...
    if (withHistory)
      Assert.Contains(
        dense.Result.Assumptions,
        assumption =>
          assumption.StartsWith(
            "Verified HOS history:",
    ...
    Assert.True(
      dense.WarmBytesPerCalculation <= sparse.WarmBytesPerCalculation + 4096,
  ...
  public void WarmCalculationReusesRealLocalTimezoneLookups()
  {
    ...
    WriteMeasurement(
      "Real local GeoTimeZone lookup (1,001 points, no network)",
      measured
    );
  ...
  public void CycleAlternativeReplaysReuseCompiledDenseGeometry()
  {
    ...
    Assert.NotNull(
      Assert.Single(dense.Result.Stops).Hours!.FirstCycleShortageAt
    );
    Assert.Contains(
      dense.Result.Stops[0].Hours!.Alternatives,
  ...
  private Measurement Measure(
    int pointCount,
    bool withHistory,
    bool realRegions = false,
    bool cycleShortage = false
  )
  {
    var points = Enumerable
      ...
      .Select(i => new RoutePoint(
        35,
        -81 + (realRegions ? 1d : 10d) * i / (pointCount - 1)
      ))
      .ToList();
    var plan = new RoutePlan
    {
      ...
      Route = new()
      {
        Miles = 600,
        Seconds = 36_000,
        Legs = [new(600, 36_000, points)],
      },
    };
    var state = new RoutePlanningState(
      new(),
      plan,
      new(0, 600, 36_000, 0, false, false, Start.UtcDateTime, points[0]),
      50,
      Start.UtcDateTime,
      true
    );
    var regions = new CountingRegions(
      points[0],
      points[^1],
      realRegions ? new RouteRegionLookup() : null
    );
    ...
    if (cycleShortage)
    {
      ...
      plan.Stops[0] = plan.Stops[0] with
      {
    ...
    for (var i = 0; i < 3; i++)
      GC.KeepAlive(
        service.Calculate(state, clocks, Start.UtcDateTime, history)
      );
    regions.Reset();
    ...
    var warmMilliseconds =
      Stopwatch.GetElapsedTime(started).TotalMilliseconds / Iterations;
    var warmBytes =
    ...
    return new(
      ...
      warmMilliseconds,
      regions.Lookups,
      regions.GeometryLookups
    );
  }

  private void WriteMeasurement(string name, Measurement value) =>
    output.WriteLine(
  ...
  ) : IRouteRegionLookup
  {
    ...
    {
      Lookups++;
      if (point != initial && point != destination)
        GeometryLookups++;
  ...
  private sealed record Measurement(
    ...
    int ColdLookups,
    long WarmBytesPerCalculation,
    double WarmMillisecondsPerCalculation,
    int WarmLookups,
    int WarmGeometryLookups
  );
}
