...
{
  private static readonly DateTime Now = new(2026, 9, 8, 18, 0, 0, DateTimeKind.Utc);
  ...
  public async Task ForecastSurvivesANewContextAndPreservesStopIdentityAndLocalTimes()
  {
    ...
    Assert.True(await new EtaForecastStore(fixture.Db).SaveAsync([snapshot], default));
    await using var another = new AppDbContext(fixture.Options);
    var saved = Assert.Single(await new EtaForecastStore(another).ReadAsync([fixture.Current.Id], default));
    ...
    Assert.Equal(snapshot.Forecast.CycleAtCalculation, saved.Forecast.CycleAtCalculation);
    var expectedStop = Assert.Single(snapshot.Forecast.Stops);
    var actualStop = Assert.Single(saved.Forecast.Stops);
    Assert.Equal(expectedStop with { Hours = null }, actualStop with { Hours = null });
    Assert.Equal(expectedStop.Hours! with { Alternatives = [] }, actualStop.Hours! with { Alternatives = [] });
    Assert.Equal(expectedStop.Hours.Alternatives, actualStop.Hours.Alternatives);
    Assert.Empty(await new EtaForecastStore(another).ReadAsync([Guid.NewGuid()], default));
  }
  ...
  public async Task SnapshotIdentitySurvivesDatabaseTimestampPrecision()
  {
    ...
    Assert.True(await store.SaveAsync([fixture.Snapshot(fixture.Current.Id, Now.AddTicks(7))], default));
    var row = await fixture.Db.Set<DispatchEtaForecast>().AsNoTracking().SingleAsync();
    var snapshot = Assert.Single(await store.ReadAsync([fixture.Current.Id], default));
  ...
  public async Task OlderSavedForecastWithoutCycleFieldsDoesNotInventHours()
  {
    await using var fixture = await Fixture.CreateAsync();
    var store = new EtaForecastStore(fixture.Db);
    Assert.True(await store.SaveAsync([fixture.Snapshot(fixture.Current.Id, Now)], default));
  ...
  public async Task OlderBatchCannotOverwriteNewerForecastOrLeavePartialInserts()
  {
    ...
    Assert.False(await store.SaveAsync([
      ...
      fixture.Snapshot(fixture.Current.Id, Now.AddMinutes(-1))], default));
    Assert.Single(await store.ReadAsync([fixture.Current.Id, fixture.Future.Id], default));
    Assert.True(await store.SaveAsync([
      fixture.Snapshot(fixture.Future.Id, Now.AddMinutes(1)),
      fixture.Snapshot(fixture.Current.Id, Now.AddMinutes(1))], default));
    Assert.Equal(2, (await store.ReadAsync([fixture.Current.Id, fixture.Future.Id], default)).Count);
  }
  ...
  public async Task InvalidPayloadIsUnavailableAndCancelledSaveDoesNotWrite()
  {
    ...
    cancelled.Cancel();
    await Assert.ThrowsAnyAsync<OperationCanceledException>(() => store.SaveAsync([fixture.Snapshot(fixture.Current.Id, Now)], cancelled.Token));
  ...
  private sealed class Fixture(SqliteConnection connection, AppDbContext db,
    DbContextOptions<AppDbContext> options, Truck truck, Load current, Load future) : IAsyncDisposable
  {
    ...
    {
      var arrival = new DateTimeOffset(at).ToOffset(TimeSpan.FromHours(-4)).AddHours(2);
      var stop = new StopEta(Guid.NewGuid(), arrival, "America/New_York", arrival.AddMinutes(-30), 30, 120, 0)
        { DispatchId = dispatchId, ServiceStart = arrival, Departure = arrival.AddHours(2),
          CycleAfterDeparture = new(750, arrival.AddDays(1), 480, "America/New_York", true),
          Hours = new(-60, -180, 60, arrival.AddHours(-1), true,
            [new("recap", arrival.AddHours(8), arrival.AddHours(10), 510, 300, arrival.AddHours(-1), arrival.AddHours(7))], null) };
      return new(dispatchId, truck.Id, current.Id, new string('a', 64), "driver-test",
        new(at, at.AddMinutes(2), [stop], null, ["Estimated"])
          { CycleAtCalculation = new(990, arrival.AddHours(12), 185, "America/New_York", true) });
    }
    ...
    public static async Task<Fixture> CreateAsync()
    {
      ...
      var current = new Load { Id = Guid.Parse("00000000-0000-0000-0000-000000000002"), LoadNumber = 1, TruckId = truck.Id };
      var future = new Load { Id = Guid.Parse("00000000-0000-0000-0000-000000000001"), LoadNumber = 2, TruckId = truck.Id };
      db.Trucks.Add(truck);
