...
public class CurrentUserTests
{
  ...
  public void ControllersDoNotAccessPersistenceOrClaims()
  {
    ...
    foreach (
      var file in Directory.GetFiles(
        Path.Combine(directory.FullName, "Server/API/Controllers"),
        "*.cs"
      )
    )
    {
  ...
  public async Task MePreservesExistingWireContract(bool authenticated)
  {
    ...
    services.AddMediatR(options =>
      options.RegisterServicesFromAssemblyContaining<GetCurrentUserQuery>()
    );
  ...
  public void ContextDoesNotTrustAnonymousClaimsOrRetainAnotherRequest()
  {
    ...
    accessor.HttpContext = new DefaultHttpContext
    {
      User = new ClaimsPrincipal(
        new ClaimsIdentity([new Claim(ClaimTypes.NameIdentifier, "one")])
      ),
    };
    Assert.Null(current.IdentityUserId);
  ...
  public async Task ProfileRequiresAuthenticatedActiveUser(
    bool authenticated,
    string? identity,
    bool active,
    int status
  )
  {
    ...
    var handler = new GetCurrentUserHandler(
      fixture.Db,
      new Caller(authenticated, identity),
      roles
    );
  ...
  public async Task SelfProtectionRunsInsideUpdateHandlerBeforeAnyIdentityMutation(
    string? role,
    bool? active
  )
  {
    ...
    var handler = new UpdateUserHandler(
      fixture.Db,
      null!,
      new UserRoleService(fixture.Db),
      new Caller(true, "self"),
      reads
    );
  ...
  public async Task SelfDeletionIsBlockedInsideHandler()
  {
    await using var fixture = await Fixture.Create();
    using var reads = TestCache.Create();
    var result = await new DeleteUserHandler(
      fixture.Db,
      null!,
      new Caller(true, "self"),
      reads
  ...
  public async Task EditingOwnNameIsStillAllowed()
  {
    await using var fixture = await Fixture.Create();
    using var reads = TestCache.Create();
    var result = await new UpdateUserHandler(
      fixture.Db,
      null!,
      new UserRoleService(fixture.Db),
      new Caller(true, "self"),
  ...
  ) : IAsyncDisposable
  {
    ...
    public static async Task<Fixture> Create()
    {
      ...
      db.Set<AppUser>()
        .Add(new AppUser { Id = "self", UserName = "test@example.com" });
      var user = new User
      {
        Id = Guid.NewGuid(),
        IdentityUserId = "self",
