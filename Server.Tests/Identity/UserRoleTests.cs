...
public class UserRoleTests
{
  ...
  {
    var connectionString = $"Data Source=roles-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
    ...
    Task Assign(string role) => Task.Run(async () =>
    {
      await start.Task;
    ...
    Assert.Single(await db.UserClaims.Where(claim => claim.ClaimType == "amftms:role").ToListAsync());
    var roles = new UserRoleService(db);
  ...
  public async Task UniqueApplicationRoleDoesNotRestrictOtherMultiValueClaims()
  {
    ...
    db.UserClaims.AddRange(
      new() { UserId = user.IdentityUserId, ClaimType = "department", ClaimValue = "one" },
      new() { UserId = user.IdentityUserId, ClaimType = "department", ClaimValue = "two" });
    await db.SaveChangesAsync();
    await new UserRoleService(db).SetAsync(user.IdentityUserId, "Admin");
    db.UserClaims.Add(new() { UserId = user.IdentityUserId, ClaimType = "amftms:role", ClaimValue = "Dispatch" });
    await Assert.ThrowsAsync<DbUpdateException>(() => db.SaveChangesAsync());
    Assert.Equal(2, await db.UserClaims.CountAsync(claim => claim.ClaimType == "department"));
  }
  ...
  public async Task LegacyDuplicateClaimsFailClosedAndExplicitAssignmentRepairsThem()
  {
    ...
    await db.Database.EnsureCreatedAsync();
    var user = await AddUserAsync(db);
    await db.Database.ExecuteSqlRawAsync("DROP INDEX \"UX_AspNetUserClaims_ApplicationRole\"");
    db.UserClaims.AddRange(
      new() { UserId = user.IdentityUserId, ClaimType = "amftms:role", ClaimValue = "Admin" },
      new() { UserId = user.IdentityUserId, ClaimType = "amftms:role", ClaimValue = "Admin" });
    await db.SaveChangesAsync();
    var roles = new UserRoleService(db);
  ...
  {
    var identity = new AppUser { Id = Guid.NewGuid().ToString(), UserName = "role-test@example.test" };
    var user = new User { Id = Guid.NewGuid(), IdentityUserId = identity.Id, Name = "Role test", Email = "role-test@example.test" };
  ...
  public async Task MissingRolesCannotGrantAdministrationAndExplicitRolesPersist()
  {
    ...
    await using var db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseSqlite(connection).Options);
    await db.Database.EnsureCreatedAsync();
    var identity = new AppUser { Id = "legacy", UserName = "legacy@example.com" };
    var user = new User { Id = Guid.NewGuid(), IdentityUserId = identity.Id, Name = "Legacy", Email = "legacy@example.com" };
    db.Set<AppUser>().Add(identity);
    db.Users.Add(user);
    await db.SaveChangesAsync();
    var roles = new UserRoleService(db);
    ...
    Assert.Single(await db.UserClaims.ToListAsync());
    await Assert.ThrowsAsync<ArgumentException>(() => roles.SetAsync(identity.Id, "Owner"));
    user.IsActive = false;
    await db.SaveChangesAsync();
