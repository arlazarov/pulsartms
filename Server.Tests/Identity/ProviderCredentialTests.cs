...
public class ProviderCredentialTests
{
  ...
  public async Task RotationUsesLatestSnapshotWithoutChangingSharedHeaders(string provider)
  {
    ...
    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", "unchanged-default");
    ...
    Assert.Equal("unchanged-default", client.DefaultRequestHeaders.Authorization.Parameter);
  ...
  public async Task ConcurrentRequestsKeepTheirOwnAuthorization(string provider)
  {
    ...
    var firstRequest = await requests.Reader.ReadAsync().AsTask().WaitAsync(TimeSpan.FromSeconds(5));
    credentials.Values = Values("second-key");
    var second = call(CancellationToken.None);
    var secondRequest = await requests.Reader.ReadAsync().AsTask().WaitAsync(TimeSpan.FromSeconds(5));
    try
    {
      Assert.NotSame(firstRequest, secondRequest);
      Assert.Equal("first-key", firstRequest.Headers.Authorization?.Parameter);
      Assert.Equal("second-key", secondRequest.Headers.Authorization?.Parameter);
  ...
  public async Task ResolverFailureNeverSendsAnOutboundRequest(string provider)
  {
    ...
    var expected = new InvalidOperationException("Credential resolution unavailable.");
    using var cancellation = new CancellationTokenSource();
    var resolver = new DelegateCredentials((id, ct) =>
    {
    ...
    var error = await Assert.ThrowsAsync<InvalidOperationException>(() => CreateCall(provider, client, resolver)(cancellation.Token));
  ...
  public async Task MissingTokenFailsBeforeSending(string provider)
  {
    ...
    await Assert.ThrowsAsync<InvalidOperationException>(() => call(CancellationToken.None));
    Assert.Equal(0, requests);
  }
  ...
  public async Task SamsaraCameraPostResolvesFreshCredentials()
  {
    ...
    await service.RequestCameraImageAsync("vehicle", DateTimeOffset.UnixEpoch, CancellationToken.None);
    Assert.Equal(IntegrationProviderCatalog.Samsara, Assert.Single(credentials.Requests));
    Assert.Null(client.DefaultRequestHeaders.Authorization);
  }
  ...
  public async Task SamsaraRejectsOtherOriginsBeforeResolvingCredentials(string url)
  {
    var requests = 0;
    ...
    var error = await Assert.ThrowsAsync<InvalidOperationException>(() => service.GetAsync<object>(url));
  ...
  public async Task TorquePaginationResolvesCredentialsForEachPage()
  {
    ...
    using var client = new HttpClient(new Handler((request, _) =>
    {
      ...
      return Task.FromResult(Response("""{"data":[{"loadNumber":1358}],"totalCount":2,"itemsPerPage":1}"""));
    }));
    await CreateCall(IntegrationProviderCatalog.Torque, client, credentials)(CancellationToken.None);
    Assert.Equal(["first-page", "second-page"], received);
    Assert.Equal(2, credentials.Requests.Count);
  }

  private static Func<CancellationToken, Task> CreateCall(string provider, HttpClient client, IIntegrationCredentials credentials)
  {
    if (provider == IntegrationProviderCatalog.Samsara)
    {
      var service = new SamsaraApiService(client, credentials);
      return async ct => { await service.GetDriversAsync(ct); };
    }
    ...
    return async ct => { await torque.GetDispatchesAsync(new(2026, 9, 1), new(2026, 9, 1), ct); };
  }

  private static IntegrationCredentialValues Values(string key) => new([KeyValuePair.Create("apiKey", key)]);
  private static HttpResponseMessage Response(string body = """{"data":[],"totalCount":0,"itemsPerPage":500}""") =>
    new(HttpStatusCode.OK) { Content = new StringContent(body) };
  private sealed class Handler(Func<HttpRequestMessage, CancellationToken, Task<HttpResponseMessage>> send) : HttpMessageHandler
  {
    protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) => send(request, cancellationToken);
  }
  private sealed class DelegateCredentials(Func<string, CancellationToken, Task<IntegrationCredentialValues>> get) : IIntegrationCredentials
  {
    public Task<IntegrationCredentialValues> GetAsync(string provider, CancellationToken ct) => get(provider, ct);
  }
}
