...
public sealed class FuelSearchGeometryTests
{
  ...
  public void CoarseBoundsRefineToTheDetailedRoadAcrossCurvesAndRepeatedLegs()
  {
    ...
    Assert.InRange(search.BlockCount, 1, FuelSearchGeometry.TargetBlockCount + route.Legs.Count);
    ...
    for (var leg = 0; leg < route.Legs.Count; leg++)
    {
      ...
      for (var i = 0; i < 20; i++)
      {
        ...
        var point = onRoad with { Latitude = onRoad.Latitude + random.NextDouble() * .2 - .1 };
        Equal(detailedLeg.Match(point), search.MatchLeg(leg, point));
        Equal(original.Match(point, leg * 1500), search.Match(point, leg * 1500));
      }
    }
    foreach (var mile in new[] { -1d, 0, 150.5, 1499.5, 1500, 3000, 5999, 6001 })
      Assert.True(RouteGeometry.Distance(original.At(mile), search.At(mile)) < 1e-7);
    Assert.Equal(6000, route.Miles);
    Assert.Equal(360492, route.Seconds);
    Assert.All(route.Legs, leg => { Assert.Equal(1500, leg.Miles); Assert.Equal(90123, leg.Seconds); });
  ...
  public void BoundsRemainConservativeAtHighLatitudesAndAcrossTheDateLine()
  {
    ...
    var points = Enumerable.Range(0, 4001).Select(i => new RoutePoint(70 + Math.Sin(i / 20d),
      i < 2000 ? 170 + i / 200d : -180 + (i - 2000) / 200d)).ToList();
    var route = new TruckRoute { Miles = 1400, Legs = [new(1400, 100000, points)] };
    var exact = new RouteGeometry(route);
    var search = new FuelSearchGeometry(route);
    for (var i = 0; i < 60; i++)
    {
      var point = new RoutePoint(65 + random.NextDouble() * 10, -180 + random.NextDouble() * 360);
  ...
  public void ZeroLengthLegAndMandatoryBoundariesKeepTheirOrder()
  {
    ...
    var route = new TruckRoute { Miles = 200, Seconds = 12000,
      Legs = [new(100, 6000, [start, turn]), new(0, 0, [turn, turn]), new(100, 6000, [turn, start])] };
    var exact = new RouteGeometry(route);
    var search = new FuelSearchGeometry(route);
    ...
    var price = new PricedFuelStation(new() { StationId = Guid.NewGuid(), Point = turn }, 3, 3);
    var visits = FuelRouteOccurrences.Create(route, [price], new(), geometry: search);
  ...
  public void CandidateOccurrencesMatchDetailedPerLegProjectionWithoutChangingVisitIdentity()
  {
    ...
    var prices = Enumerable.Range(1, 20).Select(i => new PricedFuelStation(new()
    {
      StationId = Guid.NewGuid(), Point = new(40 + Math.Sin(i * Math.PI * .8) * .1, -105 + i * .9), Name = $"Station {i}"
    }, 3 + i * .01, 3 + i * .01)).ToList();
    ...
    for (var leg = 0; leg < route.Legs.Count; leg++)
    {
      ...
      foreach (var price in prices)
      {
        ...
        expected.Add(new(price.Station, leg * 1500 + match.Along, match.Away, 0,
          price.CashUsd, price.EconomicUsd) { LegIndex = leg });
      }
    }
    Assert.Equal(expected.Count, actual.Count);
    foreach (var candidate in actual)
    {
      var before = Assert.Single(expected, x => x.VisitKey == candidate.VisitKey);
    ...
    var selected = FuelRouteSearch.SelectCandidates(actual, route.Miles, 150,
      new() { Confirmed = true, TankGallons = 200, Mpg = 10, ReserveGallons = 25, FillPercent = 100 },
      new() { MinimumGallons = 25 }, options);
  ...
  public void EmptyAndCancelledSearchesDoNotProduceInventedGeometry()
  {
    ...
    cancelled.Cancel();
    Assert.Throws<OperationCanceledException>(() => new FuelSearchGeometry(route, cancelled.Token));
    Assert.Throws<OperationCanceledException>(() => search.Match(new(40, -80), ct: cancelled.Token));
    Assert.Throws<OperationCanceledException>(() => search.MatchLeg(0, new(40, -80), cancelled.Token));
    Assert.Throws<OperationCanceledException>(() => search.At(10, cancelled.Token));
  }

  private static void Equal((double Along, double Away, RoutePoint Point) exact,
