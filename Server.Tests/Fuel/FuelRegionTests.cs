...
public class FuelRegionTests
{
  ...
  {
    var later = new FuelCandidate(
      new() { StationId = Guid.NewGuid(), Name = "Nevada" },
      450,
      0,
      0,
      4,
  ...
  private static FuelCandidate Station(double price) =>
    new(
      new()
      {
  ...
  private static FuelArrivalPolicy Policy(
    double minimum,
    double target,
    double price
  ) =>
  ...
  public void RefillsEvenWhenDeliveryIsReachableToKeepFuelForExit()
  {
    ...
    var regional = FuelOptimizer.Optimize(
      100,
      40,
      p,
      [Station(3)],
  ...
  [InlineData(90)]
  [InlineData(100)]
  public void UnknownOnwardRouteInPoorAreaFillsToConfiguredMaximum(
    int fillPercent
  )
  {
    var profile = Profile();
    profile.FillPercent = fillPercent;
    var policy = Policy(20, 30, 3);
    policy.PoorArea = true;
    var plan = FuelOptimizer.Optimize(
      100,
      50,
  ...
  public void KnownNextDispatchDoesNotForceMaximumFill()
  {
    var policy = Policy(20, 30, 3);
    policy.PoorArea = true;
    policy.NextDispatchId = Guid.NewGuid();
    var plan = FuelOptimizer.Optimize(
      100,
      50,
      Profile(),
  ...
  [InlineData(175, true)]
  [InlineData(180, true)]
  public void TopsUpBeforeExpensiveDestinationOnlyWithEnoughHeadroomForMinimumPurchase(
    double laterMiles,
    bool topUp
  )
  {
    ...
    var policy = Policy(20, 20, 6);
    policy.PoorArea = true;
    policy.TopUpPriceCeilingUsd = 5.85;
    var plan = FuelOptimizer.Optimize(
      400,
      30,
      Profile(),
    ...
    if (topUp)
    {
      ...
      Assert.Equal(
        100 - plan.Stops[1].ArrivalGallons,
        plan.Stops[1].BuyGallons
      );
    }
  }
  ...
  public void DestinationTopUpAlsoAppliesWithAssignedNextDispatchAndRespectsFillLimit()
  {
    ...
    var later = new FuelCandidate(
      new() { StationId = Guid.NewGuid(), YourPrice = 4 },
      175,
      0,
      0,
      4,
  ...
  {
    var later = new FuelCandidate(
      new() { StationId = Guid.NewGuid(), YourPrice = 6 },
      150,
      0,
      0,
      6,
  ...
  public void HighFuelDoesNotAddTokenPurchaseNearDelivery()
  {
    var profile = Profile();
    profile.FillPercent = 90;
    var policy = Policy(20, 95, 6);
    policy.PoorArea = true;
    var plan = FuelOptimizer.Optimize(
      50,
      100,
      profile,
      [new(new() { StationId = Guid.NewGuid() }, 40, 0, 0, 3, 3)],
      1,
      false,
      arrivalPolicy: policy
    );
  ...
  {
    var policy = Policy(20, 30, 6);
    policy.PoorArea = true;
    policy.TopUpPriceCeilingUsd = 5.85;
    var nearDelivery = new FuelCandidate(
  ...
  public void GridSeparatesExpensiveSparseAndUnknownData()
  {
    ...
    PricedFuelStation Priced(double price) =>
      new(new() { StationId = Guid.NewGuid(), Point = point }, price, price);
    var options = new FuelRegionOptions();
    Assert.Equal(
      "unknown",
      new FuelRegionGrid([], options, 3).Cell(point).Kind
    );
    Assert.Equal(
      "sparse",
      new FuelRegionGrid([Priced(4)], options, 3).Cell(point).Kind
    );
    var cell = new FuelRegionGrid([Priced(4), Priced(4.1)], options, 3).Cell(
      point
    );
    ...
    Assert.Equal(
      "good",
      new FuelRegionGrid([Priced(3), Priced(3.1)], options, 3).Cell(point).Kind
    );
  }
