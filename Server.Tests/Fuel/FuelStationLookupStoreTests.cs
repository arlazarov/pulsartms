...
public sealed class FuelStationLookupStoreTests
{
  ...
  public async Task ReservationSurvivesContextDisposalAndImportRollbackAndRejectsStaleOwner()
  {
    ...
    {
      db.FuelStations.Add(
        new FuelStation { Id = Guid.NewGuid(), ExternalId = "rolled-back" }
      );
    ...
    Assert.Equal(
      now.AddMinutes(15),
      (await second.ReadAsync("STATION", default))!.NextAttemptAt
    );
    ...
    await db
      .SynchronizationCheckpoints.Where(x => x.Owner == "first")
