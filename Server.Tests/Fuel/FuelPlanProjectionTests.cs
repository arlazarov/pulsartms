...
public sealed class FuelPlanProjectionTests
{
  ...
  [InlineData(0)]
  [InlineData(1)]
  public void NativeToLegacySavedHorizonProjectsEveryRemainingArrival(
    int current
  )
  {
    ...
    root.ExecutionLegId = leg;
    root.AssignmentRevision = 7;
    root.ExecutionStatus = "active";
    foreach (var load in fixture.Loads)
    {
    ...
    var state = fixture.State(current);
    if (current == 0)
    {
      state.Plan!.ExecutionLegId = leg;
      state.Plan.AssignmentRevision = 7;
    }

    var result = FuelPlanProjection.Project(
      saved,
      state,
      fixture.Loads.Skip(current).ToArray(),
      fixture.Leg(current),
      fixture.Now
    );

    Assert.False(result.NeedsRefresh);
    Assert.Equal(state.Plan!.ExecutionLegId, result.ExecutionLegId);
    Assert.Equal(state.Plan.AssignmentRevision, result.AssignmentRevision);
    Assert.Equal(3 - current, result.StopArrivals.Count);
    Assert.Equal(
      fixture.Loads.Skip(current).Select(load => load.Id),
      result.StopArrivals.Select(stop => stop.DispatchId)
    );
    Assert.Equal(leg, saved.Plan.ExecutionLegId);
    Assert.Equal(7, saved.Plan.AssignmentRevision);
  }
  ...
  public void CompletedFutureStopInvalidatesThePreviouslySavedItinerary(
    string completed
  )
  {
    var fixture = new Fixture();
    ...
    if (completed == "pickup")
      stop.PickedUpAt = fixture.Now;
    if (completed == "delivery")
      stop.DeliveredAt = fixture.Now;
    if (completed == "departure")
      stop.DepartedAt = fixture.Now;
    ...
    Assert.Contains(
      ...
      reason => reason.Contains("Assigned stops", StringComparison.Ordinal)
    );
  }
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void PassedCurrentPrefixDoesNotInvalidateTheRemainingStopIdentity(
    bool confirmedCompleted
  )
  {
    var fixture = new Fixture();
    ...
    second.Sequence = 2;
    var itinerary = fixture
      .Saved.Stops.Select(
        (x, i) => i == 1 ? x with { DispatchId = load.Id } : x
      )
      .ToList();
    if (confirmedCompleted)
      first.DeliveredAt = fixture.Now;
  ...
  public void ChangedRemainingAssignmentsInvalidateTheSavedPlan(string change)
  {
    ...
    switch (change)
    {
      case "insert":
        loads.Insert(
          1,
          new() { Id = Guid.NewGuid(), TruckId = fixture.TruckId }
        );
        break;
      case "reorder":
        (loads[1], loads[2]) = (loads[2], loads[1]);
        break;
      case "changed-stop":
        loads[1].Stops[0].Address = "Changed pickup address";
        break;
      case "changed-truck":
        loads[1].TruckId = Guid.NewGuid();
        break;
      case "new-continuation":
        loads.Add(new() { Id = Guid.NewGuid(), TruckId = fixture.TruckId });
        break;
    }
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void ChangedPostHorizonPickupOrMissingSavedSignatureInvalidatesTheEscapePolicy(
    bool missingSignature
  )
  {
    var fixture = new Fixture();
    var next = new DispatchResponse
  ...
  public void LaterVisitToTheSameStationSurvivesRolloverAndOriginalSnapshotIsUnchanged()
  {
    ...
    Assert.Equal(
      fixture.Saved.Stops.Skip(1).Select(x => x.Stop.Id),
      result.StopArrivals.Select(x => x.StopId)
    );
  ...
  {
    var fixture = new Fixture();
    var state = fixture.State(1) with
    {
      FuelPercent = 50,
      FuelUpdatedAt = fixture.Now,
    };
  ...
  [InlineData(360)]
  [InlineData(2880)]
  public void ReadingAgeAloneDoesNotHideStationsOrChangePurchaseQuantities(
    int ageMinutes
  )
  {
    var fixture = new Fixture();
    ...
    var fresh = FuelPlanProjection.Project(
      fixture.Saved,
      fixture.State(0),
      fixture.Loads,
      fixture.Leg(0),
    ...
    Assert.Equal(50, result.StartingGallons);
    Assert.Equal(
      fixture.Saved.Plan.Stops.Select(x => x.VisitKey),
      result.Stops.Select(x => x.VisitKey)
    );
    Assert.All(
      result.Stops,
      stop => Assert.True(stop.ArrivalGallons >= fixture.Profile.ReserveGallons)
    );
    Assert.True(
      result.ArrivalGallons >= fixture.Saved.Plan.ArrivalPolicy!.MinimumGallons
    );
    Assert.Equal(
      fixture.Saved.Plan.Stops.Select(x => x.BuyGallons),
      result.Stops.Select(x => x.BuyGallons)
    );
    Assert.Equal(
  ...
  public void InsufficientFuelStillInvalidatesAnUnreachableFirstStation()
  {
    ...
    Assert.Contains(
      result.RefreshReasons,
      reason => reason.Contains("no longer reachable", StringComparison.Ordinal)
    );
  }
  ...
  [InlineData(45)]
  [InlineData(50)]
  public void LowFuelRecoveryRemainsVisibleBeforeAndAtTheFirstPurchase(
    double progress
  )
  {
    var fixture = new Fixture();
    ...
    Assert.All(
      result.Stops,
      x => Assert.True(x.DepartureGallons >= fixture.Profile.ReserveGallons)
    );
    Assert.True(
      result.Stops[1].ArrivalGallons >= fixture.Profile.ReserveGallons
    );
  ...
  [InlineData(0)]
  [InlineData(.9)]
  public void LowFuelProjectionNeverPublishesAnUnreachableFirstPurchase(
    double fuelPercent
  )
  {
    var fixture = new Fixture();
  ...
  public void RecoveryExceptionDoesNotExtendPastTheFirstFuelPurchase()
  {
    ...
    Assert.Contains(
      result.RefreshReasons,
      x => x.Contains("no longer reachable", StringComparison.Ordinal)
    );
  }
  ...
  public void RemainingTotalsUseNormalizedUsdPricesAndExcludePassedPurchases()
  {
    ...
    Assert.Equal(
      100 + 3d / 60 * fixture.Profile.DriverHourlyCostUsd,
      result.EconomicCostUsd,
      10
    );
  ...
  public void ManualQuantityAlsoOverridesAnOlderLiveObservation()
  {
    ...
    {
      FuelPercent = 60,
      FuelUpdatedAt = fixture.Now.AddMinutes(-3),
    };
    var result = FuelPlanProjection.Project(
      fixture.Saved,
      state,
  ...
  [InlineData(false, 65)]
  [InlineData(true, 50)]
  public void ManualQuantityKeepsPrecedenceUntilANewerSensorReadingArrives(
    bool newer,
    double expectedGallons
  )
  {
    var fixture = new Fixture();
    fixture.Saved.Plan.ManualStartingFuel = true;
    ...
    {
      FuelPercent = 50,
      FuelUpdatedAt = newer ? fixture.Now : fixture.Now.AddMinutes(-2),
    };
  ...
  public void OrdinaryProjectionRetainsOnlyTheDatedScheduleSnapshotAndOmitsRouteCheckDetails()
  {
    ...
    fixture.Saved.Plan.ScheduleImpact = new(
      fixture.Now,
      true,
      true,
      false,
  ...
  public void UnverifiedTankReadingsCannotValidateTheRemainingPurchases(
    string reading
  )
  {
    var fixture = new Fixture();
    ...
    Assert.Contains(
      result.RefreshReasons,
      reason => reason.Contains("fresh fuel reading", StringComparison.Ordinal)
    );
  }
  ...
  public void ManualFuelExpiresAfterThirtyMinutesOrAnUnconfirmedPassedPurchase(
    double minutes,
    double progress,
    bool needsRefresh
  )
  {
    ...
    var state = fixture.State(0, progress) with
    {
      FuelPercent = null,
      FuelUpdatedAt = null,
    };
    ...
    if (!needsRefresh)
      Assert.Equal(45, result.StartingGallons, 5);
    else
      Assert.Contains(
        result.RefreshReasons,
        reason =>
          reason.Contains("fresh fuel reading", StringComparison.Ordinal)
      );
  }
  ...
  {
    var fixture = new Fixture();
    fixture.Saved.Plan.StopArrivals =
    [
      new(fixture.Loads[0].Id, fixture.Saved.Stops[0].Stop.Id, 50, 50),
    ];
    ...
    Assert.Contains(
      result.RefreshReasons,
      reason => reason.Contains("GPS position", StringComparison.Ordinal)
    );
  ...
  public void ChangedFuelInputsAndOldPricesCannotReuseAValidRecommendation(
    string changed
  )
  {
    var fixture = new Fixture();
    if (changed == "profile")
      fixture.Profile.StopCostUsd += 1;
    if (changed == "date")
      fixture.Saved.Plan.PricingDate = fixture.Saved.Plan.PricingDate.AddDays(
        -1
      );
    if (changed == "selection-version")
      fixture.Saved.Plan.SelectionVersion =
        FuelOptimizer.MinimumProjectionVersion - 1;
    if (changed == "future-version")
      fixture.Saved.Plan.SelectionVersion = FuelOptimizer.SelectionVersion + 1;
  ...
  [InlineData(19)]
  [InlineData(20)]
  public void SearchOrderingUpgradeKeepsThePreviouslyCheckedPlanVisible(
    int version
  )
  {
    var fixture = new Fixture();
  ...
  [InlineData(21)]
  [InlineData(27)]
  public void OldEstimatedAccessCannotReappearAsACurrentRecommendation(
    int version
  )
  {
    var fixture = new Fixture();
    ...
    Assert.True(result.NeedsRefresh);
    Assert.Contains(
      "Fuel station eligibility changed. Recalculate fuel.",
      result.RefreshReasons
    );
    Assert.Empty(result.StopArrivals);
    Assert.Equal(fixture.Saved.Plan.Stops.Count, result.Stops.Count);
    Assert.False(fixture.Saved.Plan.NeedsRefresh);
    Assert.Equal(version, fixture.Saved.Plan.SelectionVersion);
  }

  [Fact]
  public void CountryConstrainedEstimatedAccessRemainsProjectable()
  {
    var fixture = new Fixture();
    fixture.Saved.Plan.EstimatedStationAccess = true;
    fixture.Saved.Plan.SelectionVersion = 28;
    foreach (var stop in fixture.Saved.Plan.Stops)
    {
      stop.RouteMilesAhead = stop.MilesAhead;
      stop.DetourMiles = 2;
    ...
    Assert.False(result.NeedsRefresh);
    Assert.NotEmpty(result.Stops);
    Assert.NotEmpty(result.StopArrivals);
  }
  ...
  public async Task GeometryMemoryReusesOnlyTheRequestedSavedVersion()
  {
    ...
    {
      calls++;
      return Task.FromResult<TruckFuelPlanSnapshot?>(fixture.Saved);
    }
    ...
    var changed = fixture.Saved with
    {
      CalculatedAt = fixture.Saved.CalculatedAt.AddTicks(1),
    };
  ...
  public void FullFillProjectionBuysExactlyTheConfiguredFractionalTankCapacity()
  {
    ...
    Assert.Equal(
      stop.BuyGallons * stop.CashUsdPerGallon,
      result.PurchaseCostUsd,
      9
    );
  }
  ...
  public void NearbyOffRoadPositionPaysItsInitialAccessWithoutRequiringNewGeometry()
  {
    ...
    state = state with
    {
      Progress = state.Progress! with { Position = new(40.0032, -99.75) },
    };
  ...
  public void OriginEstimateDoesNotAcceptAnUnmatchedCurrentPositionBeyondFortyMiles()
  {
    ...
    state = state with
    {
      Progress = state.Progress! with { Position = new(40.6, -99.75) },
    };
    ...
    Assert.Contains(
      result.RefreshReasons,
      reason => reason.Contains("GPS position")
    );
  }
  ...
  public void ManualOriginAllowanceIsNotLostAfterTheTruckRejoinsTheSavedRoad()
  {
    ...
    var state = fixture.State(0) with
    {
      FuelUpdatedAt = fixture.Saved.CalculatedAt.AddMinutes(-1),
    };
  ...
  public void UnchangedManualReplayRetainsItsAccessCostsAndExactQuantitiesAfterProjection()
  {
      ...
      .Saved.Plan.Stops.Select(
        (source, index) =>
          new FuelCandidate(
            ...
            3,
            3,
            source.CashUsdPerGallon,
            source.EconomicUsdPerGallon
          )
          {
            LegIndex = index,
          }
    ...
    var replay = FuelManualReplay.Evaluate(
      ...
      visits,
      edits,
      fixture.Saved.Plan.ArrivalPolicy!,
      1,
      .5
    );
    ...
    var saved = fixture.Saved with
    {
      ...
      Plan = plan,
      CheckedRoute = null,
      BaselineRoute = fixture.Saved.CheckedRoute,
    };
    var state = fixture.State(0, 0);
    state = state with
    {
      Progress = state.Progress! with { Position = new(40.0032, -100) },
    };
    ...
    Assert.Equal(
      ...
      result.ExpectedFutureFuelCostUsd,
      10
    );
    Assert.Equal(
      plan.Stops.Select(x => x.BuyGallons),
      result.Stops.Select(x => x.BuyGallons)
    );
    Assert.True(result.RemainingCostEstimate);
  }
  ...
  public void RemainingCostDropsPassedAccessAndDoesNotChargeHistoricalScheduleDelayAgain()
  {
    ...
    Assert.Equal(
      80 + 14d / 60 * fixture.Profile.DriverHourlyCostUsd,
      result.EconomicCostUsd,
      10
    );
    ...
    Assert.Equal(
      fixture.Now.AddMinutes(-30),
      result.ScheduleImpact.CalculatedAt
    );
    Assert.True(result.RemainingCostEstimate);
    Assert.Null(result.SavingsUsd);
  ...
  public void NoRemainingPurchasesChargeOnlyCurrentAccessNotPastStopsOrHistoricalWait()
  {
    var fixture = new Fixture();
    fixture.Saved.Plan.EstimatedStationAccess = true;
    fixture.Saved.Plan.Stops.ForEach(x =>
    {
    ...
    state = state with
    {
      Progress = state.Progress! with { Position = new(40.0032, -100.25) },
    };

    var result = FuelPlanProjection.Project(
      fixture.Saved,
      state,
      fixture.Loads.Skip(2).ToList(),
      fixture.Leg(2),
      fixture.Now
    );
    ...
    Assert.Equal(3, result.ExtraMinutes);
    Assert.Equal(
      3d / 60 * fixture.Profile.DriverHourlyCostUsd,
      result.EconomicCostUsd,
      10
    );
    Assert.True(result.RemainingCostEstimate);
  ...
  private sealed class Fixture
  {
    ...
    public Fixture()
    {
      var points = new[]
      {
        new RoutePoint(40, -100),
        new RoutePoint(40, -99),
        new RoutePoint(40, -100),
        new RoutePoint(40, -101),
      };
      ...
      for (var i = 0; i < 3; i++)
      {
        var stop = new DispatchStopResponse
        {
          ...
          Sequence = 1,
          Address = $"Address {i}",
          Latitude = (decimal)points[i + 1].Latitude,
          Longitude = (decimal)points[i + 1].Longitude,
        ...
        itinerary.Add(
          new(
            load.Id,
            new(stop.Id, $"Stop {i}", stop.Address, 1, points[i + 1]),
            (i + 1) * 100
          )
        );
      ...
      for (var i = 0; i < 2; i++)
        fuel.Stops.Add(
          ...
          {
            StationId = StationId,
            VisitKey = $"{StationId:N}:{i}",
            DispatchId = Loads[i].Id,
            BeforeStopId = itinerary[i].Stop.Id,
            Point = new(40, -99.5),
            MilesAhead = 50 + i * 100,
            BuyGallons = 20,
            YourPrice = 3,
    ...
    public RouteGeometry Leg(int index) =>
      new(new() { Legs = [Saved.CheckedRoute!.Legs[index]] });
