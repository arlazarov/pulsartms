...
public class FuelRouteVariantTests
{
  ...
  public void CheckedRouteChargesItsActualTimeOnceWithoutMutatingEstimatedCandidateMetadata()
  {
    ...
    var source = new FuelPlanStop
    {
      ...
      CashUsdPerGallon = 3,
      EconomicUsdPerGallon = 2.9,
      CurrentRouteMile = 50,
      StationId = Guid.NewGuid(),
      Name = "Fuel",
      Address = "100 Fuel Road",
      Point = new(40, -79.5),
      MilesAhead = 52,
      ArrivalGallons = 12,
      BuyGallons = 40,
    ...
    var candidate = new FuelCandidate(source, 50, 2, 2, 3, 3)
    {
      LegIndex = 0,
      EntryMiles = 48,
      ExitMiles = 52,
    ...
    var route = new TruckRoute
    {
      Miles = 100,
      Seconds = 7200,
      Points = [start, source.Point, stop.Point],
      Legs =
      [
        new(50, 3600, [start, source.Point]),
        new(50, 3600, [source.Point, stop.Point]),
      ],
    };
    var checkedRoute = FuelRouteVariant.Collapse(
      route,
      [
        new(start),
        new(source.Point, Fuel: candidate),
        new(stop.Point, Stop: stop),
      ]
    );
    ...
    var expected = JsonSerializer.SerializeToNode(source)!;
    expected[nameof(FuelPlanStop.DetourMinutes)] = 0;
    Assert.Equal(
      expected.ToJsonString(),
      JsonSerializer.Serialize(checkedCandidate.Station)
    );
    ...
    var plan = FuelOptimizer.Optimize(
      checkedRoute.Route.Miles,
      20,
      profile,
      checkedRoute.Stations,
      1,
      false
    );
    ...
    var delayCost = FuelScheduleRanking.DelayCost(
      impact,
      0,
      (checkedRoute.Route.Seconds - 3600) / 60,
  ...
  public void KeepsPickupBeforeDeliveryAndDoesNotReturnToOldRoad()
  {
    ...
    var reference = new RouteGeometry(
      new()
      {
        Legs =
        [
          new(100, 6000, [start, pickup.Point]),
          new(100, 6000, [pickup.Point, delivery.Point]),
        ],
      }
    );
    var station = new FuelCandidate(
      new() { StationId = Guid.NewGuid(), Point = new(39.9, -78.5) },
      150,
      0,
      0,
      3,
    ...
    Assert.Equal(
      new[] { start, pickup.Point, station.Station.Point, delivery.Point },
    ...
    var route = new TruckRoute
    {
      Miles = 202,
      Seconds = 12120,
      Points = points.Select(x => x.Point).ToList(),
      Legs =
      [
        new(100, 6000, [start, pickup.Point]),
        new(49, 2940, [pickup.Point, station.Station.Point]),
        new(53, 3180, [station.Station.Point, delivery.Point]),
      ],
    };
  ...
  public void InvalidTimingCannotBeHiddenByJoiningOrCollapsing(double seconds)
  {
    ...
    var route = new TruckRoute
    {
      Miles = 100,
      Seconds = seconds,
      Points = [start, stop.Point],
      Legs = [new(100, 60, [start, stop.Point])],
    };
    var valid = new TruckRoute
    {
      Miles = 100,
      Seconds = 60,
      Points = [stop.Point, new(40, -78)],
      Legs = [new(100, 60, [stop.Point, new(40, -78)])],
    };
    Assert.Throws<RoutePlanningException>(
      () =>
        FuelRouteVariant.Collapse(
          route,
