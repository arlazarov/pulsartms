...
public sealed class FuelStopArrivalsTests
{
  ...
  public void PurchasesAffectOnlyTheirMandatoryStopAndLaterStops(bool estimated, double delivery, double nextPickup)
  {
    var load = Guid.NewGuid();
    var next = Guid.NewGuid();
    var visits = new[] { Visit(load, 100), Visit(load, 300), Visit(next, 400) };
    var fuel = new FuelPlan { StartingGallons = 100, EstimatedStationAccess = estimated, StartAccessMiles = 5,
      Stops = [new() { DispatchId = load, BeforeStopId = visits[1].Stop.Id, BuyGallons = 30, DetourMiles = 10 }] };
    var result = FuelStopArrivals.Calculate(fuel, visits, new() { Mpg = 5, TankGallons = 250 });
    Assert.Equal(new[] { 79d, delivery - 1, nextPickup - 1 }, result.Select(x => x.Gallons));
  ...
  public void CurrentProgressAndLatestFuelDoNotCreditPassedPurchasesAgain()
  {
    ...
    var result = FuelStopArrivals.Calculate(new() { StartingGallons = 80 }, [stop],
      new() { Mpg = 5, TankGallons = 250 }, 200);
  ...
  {
    var visit = Visit(Guid.NewGuid(), condition == "invalid-distance" ? double.NaN : 100);
    var fuel = new FuelPlan { StartingGallons = condition == "unreachable" ? 1 : 100,
      NeedsRefresh = condition == "invalid-plan" };
    if (condition == "unowned-purchase") fuel.Stops.Add(new() { BuyGallons = 10 });
    var profile = new TruckRouteProfile { Mpg = condition == "unknown-mpg" ? null : 5,
