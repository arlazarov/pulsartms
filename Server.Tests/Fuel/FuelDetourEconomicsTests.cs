...
public sealed class FuelDetourEconomicsTests
{
  ...
  [InlineData(4.9, 2.1)]
  [InlineData(5, 2.1)]
  public void OneHourDetourCannotWinForTinySavingsEvenWhenExtraDistanceIsShort(double extraMiles, double price)
  {
    var profile = Profile();
    var closer = Station("On route", 100, 3);
    var cheaper = Station("Small discount", 100, price, extraMiles, 60);
    var withoutTimeCost = FuelOptimizer.Optimize(300, 40, Profile(hourlyCost: 0), [closer, cheaper], 1, false);
    Assert.Equal("Small discount", Assert.Single(withoutTimeCost.Stops).Name);
    var plan = FuelOptimizer.Optimize(300, 40, profile, [closer, cheaper], 1, false);
  ...
  public void ALongerDetourWinsWhenSavingsCoverFuelTimeAndTheStopCost()
  {
    ...
    var plan = FuelOptimizer.Optimize(300, 40, profile, [closer, worthwhile], 1, false);
  ...
  {
    var profile = Profile();
    var plan = FuelOptimizer.Optimize(350, 30, profile,
      [Station("First", 80, 3), Station("Tiny discount", 170, 2.99)], 1, false);
  ...
  private static TruckRouteProfile Profile(double hourlyCost = 35) => new()
  {
    Confirmed = true, TankGallons = 100, Mpg = 5, ReserveGallons = 10, FillPercent = 100,
    StopCostUsd = 20, DriverHourlyCostUsd = hourlyCost
  };

  private static FuelCandidate Station(string name, double mile, double price, double detour = 0, double minutes = 0) =>
    new(new() { StationId = Guid.NewGuid(), Name = name, YourPrice = price, EconomicPrice = price,
      Currency = "USD", Unit = "US gal", DetourMinutes = minutes }, mile, detour / 2, detour / 2, price, price);
}
