...
public sealed class FuelRouteOccurrencesTests
{
  ...
  private static PricedFuelStation Station(
    string name,
    RoutePoint point,
    double price = 3
  ) =>
  ...
  public void OutboundAndReturnVisitsRemainDistinctWithoutDuplicatingTheTurningBoundary()
  {
    ...
    var candidates = FuelRouteOccurrences.Create(
      Route(),
      [station, station, turn],
      new()
    );

    Assert.Equal(3, candidates.Count);
    var visits = candidates
      .Where(x => x.Station.StationId == station.Station.StationId)
      .ToList();
    ...
    Assert.Single(
      candidates,
      x => x.Station.StationId == turn.Station.StationId
    );
  }
  ...
  public void WiderNearbyStationRetainsBothDirectedVisitsAndPaysEstimatedAccessForEach()
  {
    ...
    static RoutePoint Away(double miles) =>
      new(miles / 3958.7613 * 180 / Math.PI, 2);
    ...
    Assert.Equal(
      occurrences.Select(x => x.VisitKey),
      candidates.Select(x => x.VisitKey)
    );
  }
  ...
  public void GeographicalStationBoundaryIsFortyMilesInclusive(
    double away,
    int count
  )
  {
    ...
    if (count > 0)
      Assert.Equal(away, candidates[0].ExtraInMiles, 6);
  }
  ...
  public void ChainCanBuyAtTheSameStationOnBothSidesOfTheTurn()
  {
    ...
    var selected = FuelRouteSearch.SelectCandidates(
      candidates.Concat(candidates).ToList(),
      1200,
      60,
      Profile(),
      arrival,
    ...
    var result = FuelOptimizer.OptimizeWithVisits(
      1200,
      60,
      Profile(),
      chains[0].Concat(chains[0]).ToList(),
      1,
      false,
    ...
    Assert.Equal(
      new[]
      {
        candidates[0].VisitKey,
        candidates[1].VisitKey,
        candidates[2].VisitKey,
      },
    ...
    Assert.All(
      ...
      chain =>
        Assert.Equal(
          chain.Count,
          chain.Select(x => x.VisitKey).Distinct().Count()
        )
    );
  }
  ...
  public void CheckedVariantKeepsEachVisitBeforeItsOwnMandatoryStop()
  {
    ...
    var stops = new[]
    {
      new PlanStop(Guid.NewGuid(), "Turn", "", 1, Turn),
      new PlanStop(Guid.NewGuid(), "Delivery", "", 2, Start),
    };
    var points = FuelRouteVariant.Waypoints(
      Start,
      stops,
      candidates.Concat(candidates).ToList(),
      new(Route()),
    ...
    Assert.Equal(
      new[] { Start, StationPoint, Turn, StationPoint, Start },
      points.Select(x => x.Point)
    );
    ...
    var raw = new TruckRoute
    {
      ...
      Legs =
      [
        new(200, 12000, [Start, StationPoint]),
        new(400, 24000, [StationPoint, Turn]),
        new(400, 24000, [Turn, StationPoint]),
        new(200, 12000, [StationPoint, Start]),
      ],
    };
    ...
    Assert.Equal(
      candidates.Select(x => x.VisitKey),
      collapsed.Stations.Select(x => x.VisitKey)
    );
    Assert.Equal(
      new[] { 600d, 600d },
      collapsed.Route.Legs.Select(x => x.Miles)
    );
  }
  ...
  public void ZoneChecksKeepTheOtherPurchasesNeededOnALongRoute()
  {
    var candidates = FuelRouteOccurrences.Create(
      Route(),
      [Station("Repeat", StationPoint), Station("Turn", Turn, 4)],
      new()
    );
    var seed = candidates.ToList();
    var extra = new FuelCandidate(
      new() { StationId = Guid.NewGuid(), Name = "Alternative" },
      650,
      0,
      0,
      3.5,
    ...
    var zone = Assert.Single(
      scheduled.Take(3),
      chain => chain.Any(x => x.VisitKey == extra.VisitKey)
    );
    Assert.All(
      seed,
      visit => Assert.Contains(zone, x => x.VisitKey == visit.VisitKey)
    );
    Assert.All(
      scheduled,
      chain =>
  ...
  public void CandidateSelectionRemainsBoundedAcrossRepeatedRouteLegs()
  {
    var stations = Enumerable
      .Range(1, 20)
      .Select(i => Station($"Station {i}", new(40, -100 + i * .25)))
      .ToList();
    ...
    Assert.InRange(
      FuelZoneSearch.Representatives(candidates, options).Count,
      1,
      options.ZoneRoadChecks
    );
  }
  ...
  public void CancelledOccurrenceSearchAndChainSearchStopImmediately()
  {
    ...
    Assert.Throws<OperationCanceledException>(
      () => FuelRouteOccurrences.Create(Route(), [], new(), cancellation.Token)
    );
    Assert.Throws<OperationCanceledException>(
      () =>
