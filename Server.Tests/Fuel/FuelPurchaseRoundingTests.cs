...
public sealed class FuelPurchaseRoundingTests
{
  ...
  public void FleetPolicyRemovesLegacyStopFeeAndSelectsTheBridgeToCheaperMainFuel()
  {
    ...
    Assert.Equal(
      new[] { "LOVES 435", "LOVES 790" },
      plan.Stops.Select(x => x.Name)
    );
    ...
    Assert.Equal(
      15.648311353,
      single.EconomicCostUsd
        + single.ExpectedFutureFuelCostUsd
    ...
    FuelPlan Calculate(List<FuelCandidate> candidates) =>
  ...
  public void EqualAccessStationsCannotFavorTheDearerPriceThroughPerLegRounding()
  {
    ...
    Assert.Equal(
      175 + 25 - (452.051517 + 1) / profile.Mpg.Value,
      combined.ArrivalGallons,
      9
    );
    Assert.Equal(
      25 * (5.924 - 5.745),
      dearPlan.EconomicCostUsd
        + dearPlan.ExpectedFutureFuelCostUsd
        - cheapPlan.EconomicCostUsd
        - cheapPlan.ExpectedFutureFuelCostUsd,
      9
    );
  ...
  [InlineData(99, 100)]
  [InlineData(100, 100)]
  public void PartialPurchasesRoundUpBeforeCostAndArrivalAreCalculated(
    double required,
    double expected
  )
  {
    ...
    Assert.Equal(
      profile.ReserveGallons + expected - required,
      plan.ArrivalGallons,
      9
    );
  ...
  public void RoundedPurchaseCannotOverfillAndFullTargetIsNotRoundedAboveCapacity()
  {
    ...
    var plan = FuelOptimizer.Optimize(
      ...
      {
        MinimumGallons = 25,
        TargetGallons = profile.TankGallons.Value,
    ...
    Assert.Equal(
      profile.TankGallons.Value - (114.7 - 1 / profile.Mpg!.Value),
      stop.BuyGallons,
      9
    );
  ...
  public void FullShortlistWithFractionalAccessKeepsAFeasiblePlan(double miles)
  {
    ...
    Assert.Equal(
      175.3
        ...
        - (miles + plan.Stops.Sum(x => x.DetourMiles)) / profile.Mpg.Value,
      plan.ArrivalGallons,
      8
    );
  }
