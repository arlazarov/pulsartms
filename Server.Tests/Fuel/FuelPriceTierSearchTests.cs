...
public sealed class FuelPriceTierSearchTests
{
  ...
  {
    var cheap = new[] { Station("Cheap first", 100, 3), Station("Cheap middle", 400, 3),
      Station("Cheap final", 800, 3) };
    var fewerStops = new[] { Station("Slightly dearer first", 250, 3.01), Station("Slightly dearer final", 650, 3.01) };

    var chains = FuelRouteSearch.Chains([.. cheap, .. fewerStops], 1100, 60, Profile(), Arrival());
    ...
    Assert.Equal(cheap.Select(x => x.VisitKey), scheduled[1].Select(x => x.VisitKey));
    Assert.Equal(fewerStops.Select(x => x.VisitKey), scheduled[2].Select(x => x.VisitKey));
    Assert.Equal(fewerStops.Select(x => x.VisitKey), chains[0].Select(x => x.VisitKey));
    ...
    Assert.Equal(fewerStopsPlan.PurchaseCostUsd + 2 * 20, fewerStopsPlan.EconomicCostUsd, 6);
  }

  [Fact]
  public void CheapestCompleteTierKeepsPurchasesAcrossTheWholeAssignedItinerary()
  {
    var cheap = new[] { Station("First cheap fill", 70, 3), Station("Middle cheap fill", 500, 3, leg: 1),
      Station("Final cheap fill", 900, 3, leg: 2) };
    var expensive = new[] { Station("First expensive fill", 50, 6), Station("Middle expensive fill", 450, 6, leg: 1),
      Station("Final expensive fill", 850, 6, leg: 2) };

    var chains = FuelRouteSearch.Chains([.. expensive, .. cheap], 1200, 30, Profile(), Arrival());
    var scheduled = FuelZoneSearch.Schedule(chains, []);
  ...
  public void HigherPriceIsRestoredOnlyForTheSmallBridgeNeededToReachCheapFuel()
  {
    ...
    var chains = FuelRouteSearch.Chains([cheap, later, expensive, bridge], 500, 30, Profile(), Arrival());
    var scheduled = FuelZoneSearch.Schedule(chains, []);
    var plan = Optimize(scheduled[1], 500, 30);

    Assert.Equal(new[] { bridge.VisitKey, cheap.VisitKey }, scheduled[1].Select(x => x.VisitKey));
  ...
  public void CheapOutboundAndReturnVisitsRemainDistinctWhenAHigherPriceMiddleBridgeIsRequired()
  {
    ...
    var chains = FuelRouteSearch.Chains([unneeded, outbound, middle, returning], 900, 60, Profile(), Arrival());
    var scheduled = FuelZoneSearch.Schedule(chains, []);
    var plan = Optimize(scheduled[1], 900, 60);

    Assert.Equal(new[] { outbound.VisitKey, middle.VisitKey, returning.VisitKey }, scheduled[1].Select(x => x.VisitKey));
    Assert.Equal(100, plan.Stops[0].DepartureGallons);
    Assert.Equal(2, plan.Stops.Count(x => x.StationId == outbound.Station.StationId));
  ...
  public void TwentyDollarStopCostStillRejectsATinyDiscountAfterTheRequiredBridge()
  {
    ...
    var chains = FuelRouteSearch.Chains([tinyDiscount, bridge], 350, 30, Profile(), Arrival());
    var scheduled = FuelZoneSearch.Schedule(chains, []);
  ...
  public void CheapPriceIsCheckedFirstButNearRoadAlternativeCanWinAfterActualDrivingCost()
  {
    ...
    var chains = FuelRouteSearch.Chains([cheap, nearby], 300, 40, Profile(), Arrival());

    var scheduled = FuelZoneSearch.Schedule(chains, []);
    var firstChecks = scheduled.Take(new FuelRegionOptions().CandidateRoadChecks).ToList();
    ...
    var checkedCheapCost = cheapPlan.EconomicCostUsd
      + FuelScheduleRanking.DelayCost(impact, 0, 60, Profile().DriverHourlyCostUsd);
  ...
  public void ProtectedNearestComparisonIsNotDeferredForARepeatedDistantPhysicalAnchor()
  {
    ...
    var nearerVisit = cheapest with { AlongMiles = 700, LegIndex = 2, ExtraInMiles = 6 };
    List<FuelCandidate> cheapChain = [cheapest];
    List<FuelCandidate> nearestChain = [Station("Near bridge", 200, 3, access: .1), nearerVisit];
  ...
  private static FuelPlan Optimize(List<FuelCandidate> chain, double miles, double gallons) =>
    FuelOptimizer.Optimize(miles, gallons, Profile(),
      chain.Select(x => x with { ExtraInMiles = 0, ExtraOutMiles = 0 }).ToList(), 1, false,
