...
public sealed class FuelCheckedRouteSearchTests
{
  ...
  public async Task DirectBaselineReusesEveryLegWithoutProviderCalls()
  {
    ...
    for (var index = 0; index < baseline.Legs.Count; index++) Assert.Same(baseline.Legs[index], result.Route.Legs[index]);
  ...
  public async Task OnlyChangedMandatoryLegIsRoutedAndActualOffsetsUseCanonicalLegSums()
  {
    ...
    Assert.Equal(new[] { baseline.Legs[1].Points[0], fuel.Station.Point, baseline.Legs[1].Points[^1] }, Assert.Single(provider.Requests));
  ...
  public async Task CommonRecipeIsReusedWithCurrentPricesAndReturnVisitsKeepTheirOwnLeg()
  {
    ...
    var newer = first with { PriceUsd = 2.5, Station = new() { StationId = first.Station.StationId,
      Point = first.Station.Point, Name = "Current price", YourPrice = 2.5, DetourMinutes = 11 } };
    ...
    Assert.Equal(new[] { 30d, 190d }, second.Stations.Select(fuel => fuel.AlongMiles));
  ...
  public async Task ChangedStationCoordinatesAndOrderRequireNewRecipeOnlyForThatLeg()
  {
    ...
    var moved = first with { Station = new() { StationId = first.Station.StationId, Point = new(40.1, -78.8) } };
    Assert.NotNull((await search.CheckAsync([moved, second], default)).Route);
    Assert.NotNull((await search.CheckAsync([moved with { AlongMiles = 180 }, second], default)).Route);
  ...
  public async Task EqualMileVisitsKeepTheirInputOrder()
  {
    ...
    var provider = new Provider();
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider);
    var fuels = Enumerable.Range(0, 18).Select(index => Fuel(1, point: new(40, -79 + index / 100d))).ToArray();
    var result = await search.CheckAsync(fuels, default);
    Assert.NotNull(result.Route);
    Assert.Equal(fuels.Select(fuel => fuel.VisitKey), result.Stations.Select(fuel => fuel.VisitKey));
    Assert.Equal(fuels.Select(fuel => fuel.Station.Point), Assert.Single(provider.Requests).Skip(1).Take(fuels.Length));
  }
  ...
  public async Task ExhaustedBudgetMakesNoCallsAndNeverPublishesPartialChain()
  {
    var (baseline, stops) = Baseline();
    var provider = new Provider();
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider, maximumRoadChecks: 2);
    var first = Fuel(0);
    var second = Fuel(2);
  ...
  [InlineData("fuel")]
  [InlineData("points")]
  public async Task WholeRequestFallbackRejectsInvalidRoadAnchorsFuelHitsAndPointBounds(string error)
  {
    var (baseline, stops) = Baseline();
    var provider = new Provider { Alter = route =>
    {
      if (error == "points")
      {
        var leg = route.Legs[0];
        route.Legs[0] = leg with { Points = Enumerable.Repeat(leg.Points[0], 9).Append(leg.Points[^1]).ToList() };
        return;
      }
      var index = error == "anchor" ? 1 : 0;
      var point = route.Legs[index].Points[^1] with
        { Latitude = route.Legs[index].Points[^1].Latitude + (error == "anchor" ? .06 : .6) / 69 };
      route.Legs[index] = route.Legs[index] with { Points = [route.Legs[index].Points[0], point] };
      route.Legs[index + 1] = route.Legs[index + 1] with { Points = [point, route.Legs[index + 1].Points[^1]] };
    } };
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider, maximumRoadChecks: 2, maximumGeometryPoints: 10);
  ...
  [InlineData("miles")]
  [InlineData("seconds")]
  public async Task InvalidCheckedSegmentsAreRejectedWithoutPoisoningRecipeCache(string error)
  {
    var (baseline, stops) = Baseline();
    var provider = new Provider { Alter = route =>
    {
      ...
      RoutePoint Shift(RoutePoint point, double miles) => point with { Latitude = point.Latitude + miles / 69 };
      if (error == "start") route.Legs[0] = first with { Points = [Shift(first.Points[0], .06), first.Points[^1]] };
      if (error == "end") route.Legs[^1] = last with { Points = [last.Points[0], Shift(last.Points[^1], .06)] };
      if (error == "fuel")
      {
      ...
      if (error == "join") route.Legs[^1] = last with { Points = [Shift(last.Points[0], .06), last.Points[^1]] };
      if (error == "miles") route.Miles += 1;
      if (error == "seconds") route.Seconds += 100;
    } };
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider);
    var fuel = Fuel(1);
  ...
  public async Task UsesEachLegsOwnRoadAnchorAtAValidSnappedMandatoryBoundary()
  {
    ...
    var snapped = leg.Points[0] with { Latitude = leg.Points[0].Latitude + .04 / 69 };
  ...
  public async Task RecipeCacheAndAssembledCandidateBothRespectAggregatePointLimit()
  {
    var (baseline, stops) = Baseline();
    var provider = new Provider();
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider, maximumGeometryPoints: 8);
  ...
  public async Task InvalidVisitOwnershipAndWaypointBoundsDoNotCallProvider()
  {
    ...
    var provider = new Provider();
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider);
    var invalid = new[] { Fuel(1) with { LegIndex = -1 }, Fuel(1) with { AlongMiles = 10 }, Fuel(1) with { LegIndex = 3 } };
    foreach (var fuel in invalid) Assert.Null((await search.CheckAsync([fuel], default)).Route);
    var duplicate = Fuel(1);
    Assert.Null((await search.CheckAsync([duplicate, duplicate], default)).Route);
    Assert.Null((await search.CheckAsync(Enumerable.Range(0, 47).Select(_ => Fuel(1)).ToArray(), default)).Route);
    Assert.Empty(provider.Requests);
  }
  ...
  public async Task CancelledProviderResponseCannotPublishOrCacheGeometry()
  {
    ...
    var search = new FuelCheckedRouteSearch(baseline, stops, new(), provider);
    await Assert.ThrowsAnyAsync<OperationCanceledException>(() => search.CheckAsync([Fuel(0)], cancellation.Token));
    Assert.Equal(2, search.RoadChecks);
    Assert.Equal(0, search.RetainedPoints);
  }
  ...
  {
    var (baseline, stops) = Baseline();
    baseline.Legs[^1] = baseline.Legs[^1] with { Points = [baseline.Legs[^1].Points[0], new(45, -70)] };
    var provider = new Provider();
    Assert.Throws<RoutePlanningException>(() => new FuelCheckedRouteSearch(baseline, stops, new(), provider));
    Assert.Empty(provider.Requests);
  }
  ...
  public void EmptyMileageBaselineCannotReachCommit()
  {
    var (baseline, stops) = Baseline();
    baseline.Miles = 0;
    baseline.Legs = baseline.Legs.Select(leg => leg with { Miles = 0 }).ToList();
    Assert.Throws<RoutePlanningException>(() => new FuelCheckedRouteSearch(baseline, stops, new(), new Provider()));
  }

  private static (TruckRoute Route, PlanStop[] Stops) Baseline(int count = 3)
  {
    var points = Enumerable.Range(0, count + 1).Select(index => new RoutePoint(40, -80 + index % 3)).ToArray();
    var route = new TruckRoute { Miles = count * 100, Seconds = count * 6000, CalculatedAt = DateTime.UnixEpoch,
      Legs = points.Zip(points.Skip(1), (from, to) => new RouteLeg(100, 6000, [from, to])).ToList() };
    var stops = points.Skip(1).Select((point, index) => new PlanStop(Guid.NewGuid(), "Stop", "", index + 1, point)).ToArray();
    return (route, stops);
  }

  private static FuelCandidate Fuel(int leg, Guid? id = null, RoutePoint? point = null) => new(new()
  {
    StationId = id ?? Guid.NewGuid(), Point = point ?? new(40, -79.5 + leg / 2d), Name = "Fuel", DetourMinutes = 25
  }, leg * 100 + 50, 1, 1, 3, 3) { LegIndex = leg, EntryMiles = leg * 100 + 40, ExitMiles = leg * 100 + 60 };
  ...
  private sealed class Provider : IRoutingProvider
  {
    ...
    public Task<TruckRoute> CalculateAsync(IReadOnlyList<RoutePoint> points, TruckRouteProfile profile, CancellationToken ct)
    {
      ...
      var route = new TruckRoute { Miles = (points.Count - 1) * 30, Seconds = (points.Count - 1) * 1800 + .5,
        Legs = points.Zip(points.Skip(1), (from, to) => new RouteLeg(30, 1800, [from, to])).ToList() };
      Alter?.Invoke(route);
