...
public sealed class FuelHorizonMemoryTests(ITestOutputHelper output)
{
  ...
  public void JoiningDenseLoadsReusesLegsWithoutCopyingTheFlatPath()
  {
    ...
    Assert.True(allocated < 4096, $"Route join copied detailed coordinates: {allocated:N0} bytes.");
    ...
    output.WriteLine($"Joining 50,002 leg points: {allocated:N0} allocated bytes; original legs shared, no flat coordinate copy.");
  }
  ...
  public void AggregateGeometryLimitRejectsTheJoinBeforeAllocatingAnotherPath()
  {
    ...
    var failure = Assert.Throws<RoutePlanningException>(() => FuelHorizon.Join(first, next));
    ...
    Assert.Equal(200_000, FuelHorizon.Join(first, first).Legs.Sum(leg => leg.Points.Count));
  }
  ...
  private static TruckRoute Route(int count)
  {
    ...
    return new() { Miles = 100, Seconds = 6000, Legs = [new(100, 6000, points)], Points = points };
  }
}
