...
public sealed class FuelDisplayPricesTests
{
  ...
  public void DisplayUsesTheSameEligibleMinimumAsPlanning(string currency, string unit, bool ifta)
  {
    var station = Station([
      Quote("DEF", .1m, currency, unit), Quote("Reefer Diesel", .2m, currency, unit),
      Quote("Diesel", 3.5m, currency, unit),
      Quote("ULSD Diesel", 3m, currency, unit) with { EffectiveFrom = Date.AddDays(-1) },
      Quote("Diesel", .3m, currency, unit) with { EffectiveTo = Date.AddDays(-1) },
      Quote("Diesel", .4m, currency, unit) with { EffectiveFrom = Date.AddDays(1) },
    ]);
    var selected = FuelDisplayPrices.Select(station, Date);
    var display = ifta ? selected.IftaDiscount : selected.CashDiscount;
    var planning = Assert.Single(FuelRegionGrid.Prices([station], new TruckRouteProfile { UseIfta = ifta, CadToUsd = .75 }, Date));
    Assert.NotNull(display);
    Assert.Equal("ULSD Diesel", display.Product);
    Assert.Equal((double)display.DiscountPrice, planning.Station.YourPrice);
    Assert.Equal((double)(ifta ? display.PriceAfterIfta!.Value : display.DiscountPrice), planning.Station.EconomicPrice);
  ...
  {
    var station = Station([Quote("Diesel", 3m), Quote("Diesel", 1m, "CAD", "L")]);
    var selected = FuelDisplayPrices.Select(station, Date);
  ...
  public void UnsupportedQuotesCannotDisplaceEligibleDiesel(string currency, string unit, string product, string value)
  {
    var valid = Quote("Diesel", 3m);
    var invalid = Quote(product, decimal.Parse(value, System.Globalization.CultureInfo.InvariantCulture), currency, unit);
    Assert.Same(valid, FuelDisplayPrices.Select(Station([invalid, valid]), Date).CashDiscount);
    Assert.Null(FuelDisplayPrices.Select(Station([invalid]), Date).CashDiscount);
  }
  ...
  public void EqualPricesPreferTheNewestActiveQuoteIndependentlyOfInputOrder()
  {
    ...
    Assert.Same(current, FuelDisplayPrices.Select(Station([old, current]), Date).CashDiscount);
    Assert.Same(current, FuelDisplayPrices.Select(Station([current, old]), Date).IftaDiscount);
  }

  private static FuelDiscountDto Quote(string product, decimal price, string currency = "USD", string unit = "US gal") =>
    new(currency, product, price + .5m, price, .5m, Date, Date, price - .25m, unit);

  private static FuelStationDto Station(List<FuelDiscountDto> discounts) =>
    new(Guid.NewGuid(), "test", "Station", "1 Road", "City", "NY", "", "US", 40m, -79m, discounts);
}
