...
public sealed class FuelStopEconomyTests
{
  ...
  public void AdditionalStopsMustEarnTwentyDollarsEach(double savings, int additional, bool worthwhile)
  {
    var comparison = FuelStopEconomy.Compare(1000 - savings, 2 + additional, 1000, 2);
    Assert.Equal(worthwhile, comparison < 0);
    Assert.Equal(-comparison, FuelStopEconomy.Compare(1000, 2, 1000 - savings, 2 + additional));
  }
  ...
  {
    var profile = new TruckRouteProfile { Confirmed = true, TankGallons = 250, FillPercent = 100, Mpg = 6.720416657142858,
      ReserveGallons = 25, StopCostUsd = 0, DriverHourlyCostUsd = 35 };
    var arrival = new FuelArrivalPolicy { MinimumGallons = 125, TargetGallons = 250,
      ReplacementPriceUsd = 5.653, PoorArea = true, EconomicPurchasesOnly = true };
    var candidates = new[] {
    ...
    var chains = FuelRouteSearch.Chains(candidates, miles, 102.5, profile, arrival,
      includeAccess: true, initialAccessMiles: .5);
    var best = Optimize(chains[0]);
    var cheapest = Optimize(candidates);

    Assert.Equal(new[] { "365", "310", "361", "399" }, best.Stops.Select(stop => stop.Name));
    ...
    Assert.All(best.Stops, stop => Assert.InRange(stop.ArrivalGallons, 25, 250));
    Assert.True(best.ArrivalGallons >= arrival.MinimumGallons);
    var scheduled = FuelZoneSearch.Schedule(chains, []);
    Assert.Contains(scheduled.Take(12), chain => chain.SequenceEqual(chains[0]));
    var originalScore = best.EconomicCostUsd;
    Assert.True(FuelStopEconomy.Compare(best.EconomicCostUsd + best.ExpectedFutureFuelCostUsd, best.Stops.Count,
      cheapest.EconomicCostUsd + cheapest.ExpectedFutureFuelCostUsd, cheapest.Stops.Count) < 0);
    Assert.Equal(originalScore, best.EconomicCostUsd);

    FuelPlan Optimize(IReadOnlyList<FuelCandidate> chain) => FuelOptimizer.Optimize(miles, 102.5, profile, chain,
      1, false, compare: false, arrivalPolicy: arrival, initialAccessMiles: .5);
  }

  private static FuelCandidate Station(string name, double mile, double price) => new(new()
    { StationId = Guid.NewGuid(), Name = name, Point = new(40, -80), YourPrice = price, EconomicPrice = price,
      Currency = "USD", Unit = "US gal", DetourMinutes = 4 }, mile, .5, .5, price, price);
}
