...
public sealed class TruckFuelPlanReplacementTests
{
  ...
  public async Task MissingExpectedRevisionOnlyInsertsTheFirstPlan()
  {
    ...
    Assert.False(await store.ReplaceAsync(fixture.Snapshot(original.CalculatedAt.AddMinutes(1)), null, default));
  ...
  public async Task ExpectedRevisionDoesNotInsertWhenThePlanNoLongerExists()
  {
    ...
    Assert.False(await store.ReplaceAsync(fixture.Snapshot(TruckFuelPlanFixture.Now.AddMinutes(1)), TruckFuelPlanFixture.Now, default));
  ...
  public async Task MatchingRevisionReplacesAllColumnsWithoutAccumulatingHistory()
  {
    ...
    changed = changed with { RootDispatchId = fixture.FutureId, Stops = [
      changed.Stops[0] with { DispatchId = fixture.FutureId },
      changed.Stops[1] with { DispatchId = fixture.CurrentId }] };

    Assert.True(await store.ReplaceAsync(changed, original.CalculatedAt, default));
  ...
  public async Task StaleExpectedRevisionCannotOverwriteEvenWithALaterCandidateTimestamp()
  {
    ...
    Assert.True(await store.ReplaceAsync(winner, original.CalculatedAt, default));

    Assert.False(await store.ReplaceAsync(fixture.Snapshot(original.CalculatedAt.AddMinutes(2)), original.CalculatedAt, default));
  ...
  public async Task WriterPausedBeforeItsUpdateCannotOverwriteAnInterleavedEdit()
  {
    ...
    var winner = fixture.Snapshot(original.CalculatedAt.AddMinutes(1));
    fixture.Commands.BeforeWrite = async () => Assert.True(await new TruckFuelPlanStore(competingContext)
  ...
  public async Task InitialWriterPausedBeforeItsInsertCannotOverwriteAnInterleavedFirstPlan()
  {
    ...
    Assert.False(await store.ReplaceAsync(fixture.Snapshot(winner.CalculatedAt.AddMinutes(1)), null, default));
  ...
  public async Task CandidateMustBeNewerAtDatabasePrecision(int ticks)
  {
    ...
    Assert.False(await store.ReplaceAsync(fixture.Snapshot(original.CalculatedAt.AddTicks(ticks)), original.CalculatedAt, default));
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task ReplaceParticipatesInTheCallerTransactionAndRollsBackTogether(bool existingPlan)
  {
    await using var fixture = await TruckFuelPlanFixture.CreateAsync();
    var store = new TruckFuelPlanStore(fixture.Db);
    var original = fixture.Snapshot();
    if (existingPlan) Assert.True(await store.ReplaceAsync(original, null, default));
    var changed = fixture.Snapshot(original.CalculatedAt.AddMinutes(1));
    await using (var transaction = await fixture.Db.Database.BeginTransactionAsync())
    {
      ...
      Assert.True(await store.ReplaceAsync(changed, existingPlan ? original.CalculatedAt : null, default));
      await AssertSnapshotAsync(fixture, changed);
    ...
    Assert.Equal(string.Empty, (await fixture.Db.Trucks.SingleAsync()).UnitNumber);
    if (existingPlan) await AssertSnapshotAsync(fixture, original);
    else Assert.Null(await store.ReadAsync(fixture.TruckId, true, default));
  }
  ...
  public async Task ReplacementPreservesValidationBoundsAndCancellationWithoutMutatingTheSavedPlan(string failure)
  {
    await using var fixture = await TruckFuelPlanFixture.CreateAsync();
    var store = new TruckFuelPlanStore(fixture.Db);
    var original = fixture.Snapshot();
    ...
    switch (failure)
    {
      case "non-utc-revision": expected = DateTime.SpecifyKind(expected, DateTimeKind.Unspecified); break;
      case "default-revision": expected = DateTime.SpecifyKind(default, DateTimeKind.Utc); break;
      case "identity": changed.Plan.TruckId = Guid.NewGuid(); break;
      case "geometry": changed.CheckedRoute!.Miles++; break;
      case "summary-size": changed.Plan.Notes = [new string('x', 512 * 1024)]; break;
      case "geometry-size": changed.CheckedRoute!.Warnings = [new string('x', 8 * 1024 * 1024)]; break;
      case "cancellation": cancellation.Cancel(); break;
    }

    if (failure == "cancellation")
      await Assert.ThrowsAnyAsync<OperationCanceledException>(() => store.ReplaceAsync(changed, expected, cancellation.Token));
    else await Assert.ThrowsAsync<ArgumentException>(() => store.ReplaceAsync(changed, expected, default));

    await AssertSnapshotAsync(fixture, original);
  }

  private static async Task AssertSnapshotAsync(TruckFuelPlanFixture fixture, TruckFuelPlanSnapshot expected)
  {
    ...
    Assert.Equal(expected.Plan.Stops[0].FillToTarget, actual.Plan.Stops[0].FillToTarget);
