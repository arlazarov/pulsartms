...
public sealed class FuelScheduleEvaluatorTests
{
    ...
    public async Task AppointmentWaitingAbsorbsAnEarlierDetourAcrossTheSameClockAndOrderedLoads()
    {
        ...
        stops[0] = stops[0] with { Stop = stops[0].Stop with { ScheduledTime = new(15, 0) } };
        stops[1] = stops[1] with { Stop = stops[1].Stop with { ScheduledTime = new(19, 0) } };
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
        ...
        Assert.Equal(30, (impact.Stops[0].CandidateArrival - impact.Stops[0].BaselineArrival)!.Value.TotalMinutes);
        Assert.Equal(impact.Stops[1].BaselineArrival, impact.Stops[1].CandidateArrival);
        Assert.Equal(stops.Select(stop => (stop.DispatchId, stop.Stop.Id)), impact.Stops.Select(stop => (stop.DispatchId, stop.StopId)));
    }

    [Fact]
    public async Task DetourAcrossDailyDrivingLimitIncludesOneRestAndOneFuelAllowance()
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now, drive: 1);
        ...
        Assert.InRange((impact.Stops[0].CandidateArrival!.Value - Now).TotalMinutes, 710, 711);
    }
    ...
    public async Task CrossingHomeMidnightUsesRealRecapInsteadOfSubtractingAllDetourTime()
    {
        ...
        await using var fixture = await FuelScheduleFixture.CreateAsync(now, cycle: 1, firstDayHours: 185d / 60);
        var (state, baseline, stops) = Inputs(fixture.TruckId, now, 30);
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, now.UtcDateTime, default);
        var impact = context.Evaluate(Road(stops, 90));
        Assert.True(impact.CycleKnown);
        Assert.InRange(impact.Stops[0].BaselineCycleAtArrivalMinutes!.Value, 29, 30);
        Assert.InRange(impact.Stops[0].CandidateCycleAtArrivalMinutes!.Value, 154, 155);
    ...
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now, cycle: 1);
        var (state, baseline, stops) = Inputs(fixture.TruckId, Now, 30);
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
        ...
        Assert.InRange(impact.Stops[0].CandidateCycleAtArrivalMinutes!.Value, -61, -60);
    }
    ...
    public async Task ExistingLatenessAndCycleShortageRemainVisibleWithoutBeingCountedAsNew()
    {
        ...
        stops[0] = stops[0] with { Stop = stops[0].Stop with { ScheduledTime = new(11, 0) } };
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
    ...
    public async Task MissingOrStaleInputsCannotProduceKnownCycleFeasibility(string input, bool complete)
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now);
        if (input == "missing") fixture.Hos.Clocks = null;
        if (input == "stale") fixture.Hos.Clocks!.UpdatedAt = Now.AddMinutes(-4).UtcDateTime;
        if (input == "history") fixture.Hos.History = null;
        if (input == "old-history") fixture.Hos.History = fixture.Hos.History! with { Through = Now.AddMinutes(-4) };
        var (state, baseline, stops) = Inputs(fixture.TruckId, Now, 60);
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
        var impact = context.Evaluate(Road(stops, 90));
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task ExpectedHosFailuresLeaveFuelScheduleUnknownWithoutFailingTheFuelCalculation(bool historyOnly)
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now);
        if (historyOnly) fixture.Hos.HistoryFailure = new HttpRequestException("Unavailable");
        else fixture.Hos.ClockFailure = new HttpRequestException("Unavailable");
        var (state, baseline, stops) = Inputs(fixture.TruckId, Now, 60);
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
        var impact = context.Evaluate(Road(stops, 90));
    ...
    public async Task ProviderTimeoutIsUnknownButAnUnexpectedFailureIsNotSilentlySuppressed()
    {
        ...
        fixture.Hos.ClockFailure = new InvalidOperationException("Unexpected failure");
        await Assert.ThrowsAsync<InvalidOperationException>(() => fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default));
    }
    ...
    public async Task MissingAppointmentKeepsAdditionalLatenessUnknownEvenWhenRoadTimesAreComplete()
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now);
        var (state, baseline, stops) = Inputs(fixture.TruckId, Now, 60);
        stops[0] = stops[0] with { Stop = stops[0].Stop with { ScheduledDate = null, ScheduledTime = null } };
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
        var impact = context.Evaluate(Road(stops, 90));
        Assert.True(impact.Complete);
    ...
    public async Task VariantsReuseFrozenHosInputsWithoutChangingTheOriginalPlanOrPublishedForecast()
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now);
        var (state, baseline, stops) = Inputs(fixture.TruckId, Now, 60);
        var saved = new DispatchEta(Now.UtcDateTime, Now.AddMinutes(2).UtcDateTime, [], null, []);
        ...
        for (var index = 0; index < 9; index++)
            Assert.Equal(JsonSerializer.Serialize(expected), JsonSerializer.Serialize(context.Evaluate(Road(stops, 90))));
        Assert.Equal(1, fixture.Hos.ClockCalls);
    ...
    public async Task TwelveRouteBoundIncludesThePreparedBaseline()
    {
        ...
        for (var index = 0; index < 11; index++) Assert.True(context.Evaluate(Road(stops, 90)).Complete);
    ...
    [InlineData("count")]
    [InlineData("horizon")]
    public async Task InvalidCompleteRoutesAreUnknownRatherThanPartialOptimisticForecasts(string invalid)
    {
        await using var fixture = await FuelScheduleFixture.CreateAsync(Now);
        var (state, baseline, stops) = Inputs(fixture.TruckId, Now, 60);
        var context = await fixture.Evaluator.PrepareAsync(state, baseline, stops, Now.UtcDateTime, default);
        var route = Road(stops, 90);
        if (invalid == "nan") route.Seconds = double.NaN;
        if (invalid == "boundary") route.Legs[0].Points[^1] = new(45, -90);
        if (invalid == "count") route.Legs.Clear();
        if (invalid == "horizon") route.Seconds = 91 * 24 * 3600;
    ...
    public async Task CancelledCandidateDoesNotSpendTheRemainingPreviewBudget()
    {
        ...
        cancellation.Cancel();
        Assert.Throws<OperationCanceledException>(() => context.Evaluate(Road(stops, 90), cancellation.Token));
        for (var index = 0; index < 11; index++) Assert.True(context.Evaluate(Road(stops, 90)).Complete);
    }

    private static (RoutePlanningState, TruckRoute, FuelItineraryStop[]) Inputs(Guid truck, DateTimeOffset now, params double[] miles)
    {
        double end = 0;
        var stops = miles.Select((distance, index) => new FuelItineraryStop(Guid.NewGuid(),
            new(Guid.NewGuid(), "Facility", "", index + 1, new(40, -79.8 + index * .2))
            { Job = index == 0 && miles.Length > 1 ? "Pick Up" : "Drop Off", ScheduledDate = DateOnly.FromDateTime(now.Date), ScheduledTime = new(23, 59) },
            end += distance)).ToArray();
        var road = Road(stops, miles);
        var plan = new RoutePlan { Id = Guid.NewGuid(), TruckId = truck, DispatchId = stops[0].DispatchId,
            Route = road, Stops = stops.Select(stop => stop.Stop).ToList(), Version = 1, FromCurrentPosition = true };
        return (new(new(), plan, new(0, road.Miles, road.Seconds, 0, false, false, now.UtcDateTime, Start), 50, now.UtcDateTime, true), road, stops);
    }

    private static TruckRoute Road(IReadOnlyList<FuelItineraryStop> stops, params double[] miles) => new()
    {
        Miles = miles.Sum(), Seconds = miles.Sum() * 60,
        Legs = miles.Select((distance, index) => new RouteLeg(distance, distance * 60,
            [index == 0 ? Start : stops[index - 1].Stop.Point, stops[index].Stop.Point])).ToList()
    };
}
