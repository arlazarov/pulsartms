...
public class FuelPricingDateTests
{
  ...
  public void UsesBusinessDayAcrossUtcMidnightAndDaylightSaving(string utc, string expected)
  {
    Assert.Equal(DateOnly.Parse(expected), FuelPricingDate.FromUtc(DateTime.Parse(utc).ToUniversalTime()));
  }
}
