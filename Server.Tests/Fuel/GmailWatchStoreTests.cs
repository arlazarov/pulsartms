...
public sealed class GmailWatchStoreTests
{
  ...
  public async Task DurableWatchUsesSeparateLeaseAndRejectsStaleOwnerWritesAndRelease()
  {
    ...
    await store.SaveAsync("first", new GmailWatchState
    {
      RegisteredAt = now, ExpiresAt = now.AddDays(7), NextRenewalAt = now.AddDays(1), HistoryId = 42
    }, default);
    await db.SynchronizationCheckpoints.Where(x => x.Id == GmailWatchStore.Id)
    ...
    await Assert.ThrowsAsync<InvalidOperationException>(() => store.SaveAsync("first", new(), default));
    await store.ReleaseAsync("first", default);
    Assert.Equal("second", await db.SynchronizationCheckpoints.Where(x => x.Id == GmailWatchStore.Id).Select(x => x.Owner).SingleAsync());
    ...
    Assert.Equal("fleet", await db.SynchronizationCheckpoints.Where(x => x.Id == SynchronizationStore.Id).Select(x => x.Owner).SingleAsync());
  }
