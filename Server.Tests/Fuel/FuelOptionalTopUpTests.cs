...
public sealed class FuelOptionalTopUpTests
{
  ...
  public void Captured11007UsesReachableCheaperStopAndCoversDeliveryAndExit()
  {
    ...
    var plan = FuelOptimizer.Optimize(
      ...
      [
        ...
        Station("405", 1136.698607242638, 5.579, .5),
      ],
      1,
      false,
  ...
  public void OnlySkipsDearerOptionalStopsInKnownGoodAreas(
    bool poor,
    bool knownExit,
    double laterPrice,
    double minimum,
  ...
  )
  {
    ...
    var plan = FuelOptimizer.Optimize(
      ...
      40,
      Profile(),
      [Station("Main", 50, 4), Station("Later", 500, laterPrice)],
      1,
      false,
      arrivalPolicy: policy
    );
    ...
    Assert.All(
      plan.Stops,
      stop => Assert.InRange(stop.DepartureGallons, 25, 250)
    );
    if (expectedStops == 1)
      Assert.True(plan.Stops[0].FillToTarget);
  }
