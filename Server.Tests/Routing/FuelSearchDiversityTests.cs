...
public sealed class FuelSearchDiversityTests
{
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void CrowdedShortlistKeepsCheapestEqualAccessVisitInEachSection(
    bool reverse
  )
  {
    var candidates = Enumerable
      .Range(1, 14)
      .Select(i => Station($"Road {i}", i * 100, 6, 1))
      .ToList();
    var economical = Station("Cheaper road visit", 750, 5.8, 1);
    candidates.AddRange(
      [
        Station("Cheap distant A", 720, 5.7, 50),
        Station("Cheap distant B", 730, 5.71, 45),
        economical,
        economical with
        {
          AlongMiles = 830,
    ...
    var selected = FuelRouteSearch.SelectCandidates(
      ...
      new()
      {
        MinimumGallons = 125,
        TargetGallons = 250,
        ReplacementPriceUsd = 6.13,
        PoorArea = true,
        TopUpPriceCeilingUsd = 6,
      },
      new() { CandidateShortlistLimit = 12 },
      includeAccess: true
    );

    Assert.Contains(economical, selected);
  ...
  public void DifferentDistantAnchorsEnterTheBudgetBeforeRepeatedCheapestDetourVariations()
  {
    ...
    var variants = Enumerable
      ...
      .Select(i => new List<FuelCandidate>
      {
        ...
        Station($"Return {i}", 500 + i, 4, .1),
      })
      .ToList();
    var corridor = new List<FuelCandidate> { Station("Corridor", 200, 5, .1) };
    var balanced = new List<FuelCandidate>
    {
      Station("Balanced detour", 220, 3.2, 7),
    };
    var alternative = new List<FuelCandidate>
    {
      Station("Different detour", 240, 3.3, 12),
    };

    var scheduled = FuelZoneSearch.Schedule(
      [.. variants, corridor, balanced, alternative],
      []
    );
    var firstTwelve = scheduled
      .Take(new FuelRegionOptions().CandidateRoadChecks)
      .ToList();
    var repeated = scheduled.FindIndex(
      2,
      chain => chain.Any(x => x.Station.StationId == distant.Station.StationId)
    );
    var first = scheduled.FindIndex(chain =>
      chain.Any(x => x.Station.StationId == distant.Station.StationId)
    );
    var second = scheduled.FindIndex(
      first + 1,
      chain => chain.Any(x => x.Station.StationId == distant.Station.StationId)
    );
    ...
    Assert.True(
      scheduled.FindIndex(chain => chain.SequenceEqual(alternative)) < second
    );
  ...
  public void SharedNearRoadBridgeDoesNotDeferDifferentUsefulCorridorFills()
  {
    ...
    var first = new List<FuelCandidate>
    {
      bridge,
      Station("First economical fill", 200, 3, .1),
    };
    var second = new List<FuelCandidate>
    {
      bridge,
      Station("Other economical fill", 210, 3.1, .2),
    };
    var unrelated = new List<FuelCandidate>
    {
      Station("Distant alternate", 220, 3.2, 20),
    };
  ...
  public void ReachableCheaperOutboundVisitSkipsExpensiveBridgeAndRetainsTheReturnVisit()
  {
    ...
    var selected = FuelRouteSearch.SelectCandidates(
      [bridge, outbound, turnaround, returning],
      900,
      60,
      profile,
      arrival,
    ...
    Assert.Equal(
      new[] { outbound.VisitKey, turnaround.VisitKey, returning.VisitKey },
    ...
    Assert.All(
      result.Plan.Stops,
      x => Assert.InRange(x.ArrivalGallons, 10, 100)
    );
