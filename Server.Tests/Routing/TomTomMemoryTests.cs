...
public sealed class TomTomMemoryTests(ITestOutputHelper output)
{
  ...
  public async Task SequentialLargeRouteChecksDoNotRetainAuditPayloadsInTheSharedContext()
  {
    ...
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage(HttpStatusCode.OK) { Content = new ByteArrayContent(body) }), dailyRequestLimit: 11);
    var unrelated = new DispatchSettings { Id = Guid.NewGuid(), LoadNumberPrefix = "TEST" };
    fixture.Db.DispatchSettings.Add(unrelated);
    await fixture.Db.SaveChangesAsync();
    long legacyPayloadBytes = 0;
    for (var index = 0; index < 11; index++)
    {
      var result = await fixture.CalculateAsync([new(40, -80), new(41, -80), new(42 + index * .001, -80)]);
      Assert.Equal(48001, result.Points.Count);
      Assert.Equal(2, result.Legs.Count);
      Assert.Equal(14400, result.Seconds);
      Assert.Single(result.Warnings);
      legacyPayloadBytes += JsonSerializer.Serialize(result, Json).Length * sizeof(char);
    ...
    var saved = await fixture.Db.RoutingApiCalls.AsNoTracking().Select(call => call.ResultJson!).ToListAsync();
    Assert.Equal(11, fixture.Calls);
    Assert.Equal(11, saved.Count);
    var compactPayloadBytes = saved.Sum(json => (long)json.Length * sizeof(char));
    Assert.True(compactPayloadBytes * 1.8 < legacyPayloadBytes);
    Assert.All(saved, json =>
    {
      using var document = JsonDocument.Parse(json);
      Assert.False(document.RootElement.TryGetProperty("points", out _));
      Assert.Equal(2, document.RootElement.GetProperty("legs").GetArrayLength());
    });
    output.WriteLine($"11 checks / 48,001 points each: legacy serialized payloads={legacyPayloadBytes:N0} B; compact persisted payloads={compactPayloadBytes:N0} B; tracked RoutingApiCall payloads after each check=0 B. Serialized string sizes exclude object overhead and are not process RSS.");
  }

  [Theory]
  [InlineData(false)]
  [InlineData(true)]
  public async Task CompactAndLegacyCachesReturnCompleteEquivalentGeometry(bool legacy)
  {
    string? legacyHash = null;
    await using var fixture = await TomTomProviderFixture.CreateAsync((request, _) =>
    {
      legacyHash = LegacyRequestHash(request);
      return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK) { Content = new ByteArrayContent(RouteResponse(4)) });
    });
    var points = new RoutePoint[] { new(40, -80), new(41, -80), new(42, -80) };
    var fresh = await fixture.CalculateAsync(points);
    var compactHash = await fixture.Db.RoutingApiCalls.AsNoTracking().Select(call => call.RequestHash).SingleAsync();
    ...
    Assert.Equal(JsonSerializer.Serialize(fresh, Json), JsonSerializer.Serialize(cached, Json));
  ...
  public async Task LegacyCooldownIsReusedWithoutAnotherReservationOrRequest()
  {
    ...
    var initial = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync());
    await using (var observer = fixture.CreateObserver())
    {
      var row = await observer.RoutingApiCalls.SingleAsync();
      row.RequestHash = legacyHash!;
    ...
    var cached = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync());
  ...
  public async Task UnsafeCachePayloadsFailBeforeReuseWithoutAnotherProviderCall(string scenario)
  {
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage(HttpStatusCode.OK) { Content = new ByteArrayContent(RouteResponse(4)) }));
    var points = new RoutePoint[] { new(40, -80), new(41, -80), new(42, -80) };
    var route = await fixture.CalculateAsync(points);
    string replacement;
    switch (scenario)
    {
      case "invalid-json": replacement = "{invalid-cache"; break;
      case "null-legs": replacement = "{\"legs\":null}"; break;
      case "invalid-point": route.Legs[0].Points[0] = new(0, 0); replacement = JsonSerializer.Serialize(route, Json); break;
      case "negative-miles": route.Miles = -1; replacement = JsonSerializer.Serialize(route, Json); break;
      case "too-many-flat-points":
        route.Points = Enumerable.Repeat(new RoutePoint(40, -80), 200001).ToList();
        replacement = JsonSerializer.Serialize(route, Json); break;
      case "too-many-leg-points":
        route.Legs = [new(200, 14400, Enumerable.Repeat(new RoutePoint(40, -80), 200001).ToList())];
        replacement = JsonSerializer.Serialize(route, Json); break;
      case "oversized-text": replacement = new string(' ', 32 * 1024 * 1024 + 1); break;
      case "oversized-utf8": replacement = new string('é', 16 * 1024 * 1024 + 1); break;
    ...
    var failure = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync(points));
  ...
  public async Task SuccessfulResponsesUseTheStreamWithoutBufferingAndDisposeIt()
  {
    ...
    var result = await fixture.CalculateAsync([new(40, -80), new(41, -80), new(42, -80)]);
  ...
  [InlineData(429)]
  [InlineData(503)]
  public async Task FailedResponsesDoNotReadTheBodyOrRetainTrackedReservations(int status)
  {
    using var content = new StreamOnlyContent(RouteResponse(4));
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage((HttpStatusCode)status) { Content = content }));
    await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync());
    Assert.Equal(0, content.BufferRequests);
    ...
    if (status == 400) Assert.Equal(DateTime.MaxValue, saved.ExpiresAt);
  }
  ...
  public async Task OversizedDeclaredResponseIsRejectedBeforeOpeningTheBody()
  {
    ...
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage(HttpStatusCode.OK) { Content = content }));
    var failure = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync());
  ...
  public async Task UnknownLengthOversizedBodyStopsAtTheByteLimitAndKeepsCooldown()
  {
    ...
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage(HttpStatusCode.OK) { Content = new UnbufferedContent(stream) }));
    var failure = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync());
    Assert.Contains("too large", failure.Message);
  ...
  {
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage(HttpStatusCode.OK) { Content = new ByteArrayContent(RouteResponse(100000)) }));
    var failure = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync(
  ...
  public async Task HttpTimeoutStillBoundsTheBodyAfterHeadersArrive()
  {
    ...
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(
      new HttpResponseMessage(HttpStatusCode.OK) { Content = new UnbufferedContent(stream) }), timeout: TimeSpan.FromSeconds(1));
    var failure = await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync().WaitAsync(TimeSpan.FromSeconds(5)));
  ...
  private static byte[] RouteResponse(int segmentsPerLeg) => JsonSerializer.SerializeToUtf8Bytes(new
  {
    routes = new[] { new
    {
      summary = new { lengthInMeters = 200 * 1609.344, travelTimeInSeconds = 14400 },
      legs = Enumerable.Range(0, 2).Select(leg => new
      {
        summary = new { lengthInMeters = 100 * 1609.344, travelTimeInSeconds = 7200 },
        points = Enumerable.Range(0, segmentsPerLeg + 1).Select(point => new
          { latitude = 40 + leg + point / (double)segmentsPerLeg, longitude = -80 })
      }),
      sections = new[] { new { sectionType = "TRUCK", startPointIndex = 0, endPointIndex = 1 } }
    } }
  }, Json);
  ...
  private sealed class StreamOnlyContent(byte[] bytes) : HttpContent
  {
    ...
    {
      BufferRequests++;
      throw new InvalidOperationException("The routing response must not be buffered by HttpClient.");
    }
    protected override Task<Stream> CreateContentReadStreamAsync() => OpenStream();
    protected override Task<Stream> CreateContentReadStreamAsync(CancellationToken ct) => OpenStream();
    private Task<Stream> OpenStream() { StreamRequests++; return Task.FromResult<Stream>(stream); }
    protected override void Dispose(bool disposing) { if (disposing) stream.Dispose(); base.Dispose(disposing); }
  }

  private sealed class ObservedStream(byte[] bytes) : MemoryStream(bytes, writable: false)
  {
