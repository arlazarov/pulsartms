...
public sealed class DeadheadHistoryReaderTests
{
  ...
  public void PostgreSqlPredecessorLookupTranslatesToOneParameterizedLateralBatch()
  {
    ...
    var method = typeof(DeadheadHistoryReader).GetMethod("PostgresCandidates", BindingFlags.Instance | BindingFlags.NonPublic)!;
    var query = (IQueryable)method.Invoke(reader, [new[] { Guid.NewGuid(), Guid.NewGuid() }])!;
  ...
  public async Task CandidateMaterializationIsBoundedAndRetainsUnknownAndTiedHistory()
  {
    ...
    db.Trucks.Add(truck);
    var history = Enumerable.Range(0, 100).Select(i => Load(truck.Id, i)).ToList();
    ...
    Assert.Equal(2, snapshot.Predecessors.Count);
    Assert.Equal(DeadheadConnection.Find(current, history)!.Previous.Id, DeadheadConnection.Find(snapshot)!.Previous.Id);
    var page = await reader.ReadAsync(history.TakeLast(10).Select(x => x.Id).ToArray(), default);
    Assert.Equal(10, page.Count);
    Assert.All(page.Values, value =>
    {
      Assert.Equal(2, value.Predecessors.Count);
      Assert.Equal(DeadheadConnection.Find(value.Current, history)!.Previous.Id, DeadheadConnection.Find(value)!.Previous.Id);
    });
    ...
    Assert.Equal(2, snapshot.Predecessors.Count);
    Assert.Null(DeadheadConnection.Find(snapshot));
    ...
    Assert.Equal(history[^2].Id, DeadheadConnection.Find(snapshot)!.Previous.Id);
  }

  private static Dispatch Load(Guid truck, int index)
  {
    var date = new DateOnly(2026, 1, 1).AddDays(index * 2);
    return new() { Id = Guid.NewGuid(), LoadNumber = index, TruckId = truck, Status = "assigned", Stops = [
      new() { Id = Guid.NewGuid(), Sequence = 1, Job = "Pick Up", ScheduledDate = date },
      new() { Id = Guid.NewGuid(), Sequence = 2, Job = "Drop Off", ScheduledDate = date.AddDays(1) }] };
  }
}
