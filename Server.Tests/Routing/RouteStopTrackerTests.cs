...
public class RouteStopTrackerTests
{
  ...
  [Fact]
  public void GpsDepartureCannotCompleteUnconfirmedHandoff()
  {
    var (plan, load) = Trip();
    load.Stops[0].PickedUpAt = now.AddHours(-1);
    load.Stops[1].AwaitingHandoff = true;
    ...
    Assert.Equal(load.Stops[1].Id, plan.Tracking.NextStopId);
    Assert.DoesNotContain(load.Stops[1].Id, plan.Tracking.PassedStopIds);
  }

  [Fact]
  public void ProviderCompletionCannotConfirmNativeHandoff()
  {
    var (plan, load) = Trip();
    load.Stops[1].AwaitingHandoff = true;
  ...
  public void PositionAndDirectionAloneCannotConfirmPickup(
    double lat,
    double lng,
    int heading,
    bool passed
  )
  {
    ...
    Assert.Equal(
      passed,
      plan.Tracking.PassedStopIds.Contains(load.Stops[0].Id)
    );
    Assert.Equal(
      passed ? load.Stops[1].Id : load.Stops[0].Id,
  ...
  public void StaleGpsOrFuturePickupDoesNotAdvanceStops(bool stale, bool future)
  {
    ...
    if (stale)
      truck.UpdatedAt = now.AddHours(-1);
    if (future)
      load.Stops[0].ScheduledDate = DateOnly.FromDateTime(now.AddDays(1));
    RouteStopTracker.Update(plan, load, truck, now);
  ...
  public void RecentlyObservedParkedTruckKeepsItsLastGpsPosition()
  {
    ...
    truck.Speed = 0;
    truck.EngineState = "off";
    truck.UpdatedAt = now.AddHours(-1);
    truck.ObservedAt = now;
    RouteStopTracker.Update(plan, load, truck, now);
    Assert.Empty(plan.Tracking.PassedStopIds);
  }
  ...
  private TruckLocation Location(double lat, double lng, int heading) =>
    new()
    {
  ...
  {
    var stops = new List<DispatchStop>
    {
      new()
      {
        Id = Guid.NewGuid(),
        ...
        Latitude = 40,
        Longitude = -80,
        ScheduledDate = DateOnly.FromDateTime(now.AddDays(-1)),
      },
    };
    if (intermediate)
      stops.Add(
    ...
    {
      Miles = 100,
      Points = planned.Select(x => x.Point).ToList(),
      Legs = planned
        .Zip(
          planned.Skip(1),
          (a, b) =>
            new RouteLeg(100d / (planned.Count - 1), 3600, [a.Point, b.Point])
        )
        .ToList(),
    };
    return (
      new() { Route = route, Stops = planned },
      new() { Stops = stops, Status = "in_transit" }
    );
  }
}
