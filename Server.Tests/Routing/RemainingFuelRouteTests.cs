...
public sealed class RemainingFuelRouteTests
{
  ...
  [InlineData(0)]
  [InlineData(.005)]
  public void ArrivalAtPickupKeepsZeroLengthAnchorAndUnchangedDeliveryRoad(double latitudeOffset)
  {
    ...
    var plan = new RoutePlan { FromCurrentPosition = true, Stops = [pickup, delivery], Route = new()
      { Miles = 150, Seconds = 9000, Legs = [new(50, 3000, [new(40, -80), pickup.Point]), deliveryLeg] } };

    var result = Assert.IsType<TruckRoute>(RemainingFuelRoute.TryRead(plan, plan.Stops, new(40 + latitudeOffset, -79), 2));
  ...
  public void ReusesOnlyRemainingRoadAndDoesNotMutateSavedRoute()
  {
    ...
    var plan = new RoutePlan { FromCurrentPosition = true, Stops = [stop], Route = new()
      { Miles = 100, Seconds = 6000, Legs = [new(100, 6000, [new(40, -80), new(40, -79), new(40, -78)])] } };
  ...
  public void WiderExplicitMatchReturnsOnlyTheSavedRoadAndKeepsTheLegacyDefaultStrict()
  {
    ...
    var remaining = Assert.IsType<TruckRoute>(RemainingFuelRoute.TryRead(plan, [stop], position, 2));

    Assert.InRange(RouteGeometry.Distance(position, remaining.Legs[0].Points[0]), .223, .225);
    ...
    Assert.Equal(new[] { new RoutePoint(40, -80), new RoutePoint(40, -79), new RoutePoint(40, -78) }, plan.Route.Legs[0].Points);
  ...
  {
    Assert.Throws<ArgumentOutOfRangeException>(() => RemainingFuelRoute.TryRead(new(), [], new(40, -79), allowance));
  }
}
