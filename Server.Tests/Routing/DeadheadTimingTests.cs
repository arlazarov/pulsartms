...
public sealed class DeadheadTimingTests
{
  ...
  public void SavedDestinationConnectionPreservesTravelTimeAndRejectsChangedPredecessor()
  {
    ...
    var from = new DispatchStop { Id = Guid.NewGuid(), Latitude = 35, Longitude = -81 };
    var to = new DispatchStop { Id = Guid.NewGuid(), Latitude = 36, Longitude = -80 };
    var pair = new DeadheadConnection(previous, current, from, to);
    var profile = new TruckRouteProfile();
    var route = new TruckRoute { Miles = 100, Seconds = 7200,
      Points = [new(35, -81), new(36, -80)], Legs = [new(100, 7200, [new(35, -81), new(36, -80)])] };
    var saved = new DispatchDeadhead
    {
      DispatchId = current.Id, PreviousDispatchId = previous.Id,
      InputHash = pair.Signature(profile), Miles = 100,
    ...
    Assert.Null((pair with { Previous = new Load { Id = Guid.NewGuid() } }).ReadRoute(saved, profile));
    to.Longitude = -79;
    Assert.Null(pair.ReadRoute(saved, profile));
  }
}
