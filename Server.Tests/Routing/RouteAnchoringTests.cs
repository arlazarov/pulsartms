...
public sealed class RouteAnchoringTests
{
  ...
  public void FacilityRoadSnappingKeepsTheExistingHalfMileTolerance(double offsetMiles, bool expected)
  {
    var from = new RoutePoint(40, -80);
    var to = new RoutePoint(41, -79);
    var snapped = to with { Latitude = to.Latitude + offsetMiles / 69 };
    Assert.Equal(expected, RouteAnchoring.Matches(Route(from, snapped), [from, to]));
    Assert.Equal(expected, RouteAnchoring.Matches(Route(snapped, from), [to, from]));
  }

  [Theory]
  [InlineData(.04, true)]
  [InlineData(.08, false)]
  public void AdjacentLegsMustConnectEvenWhenBothEndpointsAreWithinTheFacility(double gapMiles, bool expected)
  {
    var from = new RoutePoint(40, -80);
    ...
    var joined = new TruckRoute { Miles = 200, Seconds = 12000, Legs = [first.Legs[0], next.Legs[0]] };
    ...
    if (!expected) Assert.Throws<RoutePlanningException>(() => FuelHorizon.Join(first, next));
    else
    {
  ...
  public void MissingOrInvalidEndpointsNeverPassAnchorValidation()
  {
    ...
    Assert.False(RouteAnchoring.Matches(new() { Legs = [new(100, 6000, [])] }, [from, to]));
    Assert.False(RouteAnchoring.Matches(Route(from, new(double.NaN, -79)), [from, to]));
    Assert.False(RouteAnchoring.Continuous(Route(from, to), new()));
  }

  private static TruckRoute Route(RoutePoint from, RoutePoint to) => new()
    { Miles = 100, Seconds = 6000, Points = [from, to], Legs = [new(100, 6000, [from, to])] };
}
