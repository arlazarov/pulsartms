...
public sealed class GoogleAddressGeocoderTests
{
  ...
  public async Task RequiresAnUnambiguousMatchingStreetAddress(
    string precision,
    bool partial,
    string number,
    string city,
  ...
  )
  {
    var result = new
    {
      ...
      geometry = new
      {
        ...
        location = new { lat = 29.407257, lng = -98.3613863 },
      },
      address_components = new[]
      {
        ...
        {
          short_name = "TX",
          types = new[] { "administrative_area_level_1" },
        },
        new
        {
          short_name = city == "Houston" ? "77001" : "78222",
          types = new[] { "postal_code" },
        },
    ...
    using var handler = new Handler(
      JsonSerializer.Serialize(
        new { status = "OK", results = Enumerable.Repeat(result, count) }
      )
    );
    ...
    const string address =
      "2045 South Foster Road, San Antonio, TX, USA, 78222";
    if (!accepted)
    {
      var first = await Assert.ThrowsAsync<RoutePlanningException>(
        () => geocoder.GeocodeAsync(address, default)
      );
      var second = await Assert.ThrowsAsync<RoutePlanningException>(
        () => geocoder.GeocodeAsync(address, default)
      );
    ...
    else
    {
      Assert.Equal(
        29.407257,
        (await geocoder.GeocodeAsync(address, default)).Latitude
      );
  ...
  [Theory]
  [InlineData(
    "178 Mooresville Blvd Attn Sam Mishler",
    "178",
    "Mooresville Blvd",
    "Mooresville",
    "NC",
  ...
  [InlineData(
    "13077 SW Anthony F Sansone Sr Blvd",
    "13077",
    "SW Anthony F. Sansone Sr. Blvd",
    "Port St. Lucie",
    "FL",
    true,
    "Port Saint Lucie"
  ...
  public async Task AddressAliasesPreserveHouseAndHighwayNumbers(
    string street,
    string number,
    string route,
    string city,
  ...
  )
  {
    var result = new
    {
      geometry = new
      {
        ...
        location = new { lat = 42.9379051, lng = -74.2410061 },
      },
      address_components = new[]
      {
        new { short_name = number, types = new[] { "street_number" } },
    ...
    using var handler = new Handler(
      JsonSerializer.Serialize(
        new { status = "OK", results = new[] { result } }
      )
    );
    using var http = new HttpClient(handler);
    using var cache = new MemoryCache(new MemoryCacheOptions());
    ...
    Assert.DoesNotContain(
      "Attn",
      Uri.UnescapeDataString(handler.Query!),
  ...
  public async Task CorrectionsRequireConfirmedPremiseComponents(
    string action,
    string confirmation,
    bool replaced,
    string granularity,
  ...
  {
    var geocode = JsonSerializer.Serialize(
      new
      {
        status = "OK",
        results = new[]
        {
          new
          {
            ...
            {
              location_type = "ROOFTOP",
              location = new { lat = 27.62, lng = -99.47 },
            },
            address_components = new[]
            {
              ("street_number", "11402"),
      ...
      new
      {
        result = new
        {
          ...
          address = new
          {
            ...
            {
              ("street_number", "11402"),
              ("route", "EastPoint Drive"),
              ("locality", "Laredo"),
              ("administrative_area_level_1", "TX"),
              ("postal_code", "78045"),
            }.Select(x => new
            {
              componentType = x.Item1,
              componentName = new { text = x.Item2 },
          ...
          geocode = new
          {
            location = new { latitude = 27.62, longitude = -99.47 },
          },
        },
      }
    );
    ...
    for (var i = 0; i < 2; i++)
      if (accepted)
        Assert.Equal(
          27.62,
          (await service.GeocodeAsync(input, default)).Latitude
        );
      else
        await Assert.ThrowsAsync<RoutePlanningException>(
          () => service.GeocodeAsync(input, default)
        );
  ...
  public async Task WayAliasRequiresTheSameHouseStreetRegionAndPostalCode(
    string number,
    string street,
    string region,
    string postal,
  ...
  )
  {
    ...
    if (accepted)
    {
      ...
      await Assert.ThrowsAsync<RoutePlanningException>(
        () => service.ResolveAsync(input, default)
      );
  }
  ...
  public async Task ZipPlusFourFormattingPreservesPostalMatchingAndSharesTheCanonicalCache(
    string inputPostal,
    string canonicalPostal,
    string resultPostal,
    bool accepted
  )
  {
    ...
    if (accepted)
    {
      ...
      await service.ResolveAsync(
        $"16825 Murphy Parkway, LATHROP, CA, USA, {canonicalPostal}",
        default
      );
      Assert.Equal(1, handler.Calls);
    }
    ...
    Assert.Contains(
      $"address=16825 Murphy Parkway, LATHROP, CA, USA, {canonicalPostal}&",
      Uri.UnescapeDataString(handler.GeocodeQuery!)
    );
  }
  ...
  public async Task ZipFormattingDoesNotRewriteAStreetBuildingNumber()
  {
    ...
    Assert.Contains(
      "address=123456789 Murphy Parkway, LATHROP, CA, USA, 95330-9257&",
      Uri.UnescapeDataString(handler.GeocodeQuery!)
    );
  }

  [Fact]
  public async Task CanadianWarehouseAddressUsesThePremiseQuery()
  {
    using var handler = new Handler(
    ...
    await service.ResolveAsync(
      "30 Ironside Drive, Door 41, Brampton, ON, CAN, L7A1A2",
      default
    );

    var query = Uri.UnescapeDataString(handler.GeocodeQuery!);
    Assert.Contains(
      "address=30 Ironside Drive, Brampton, ON, Canada, L7A1A2&",
      query
    );
    Assert.DoesNotContain("Door 41", query);
  }
  ...
  public async Task RegionMatchingUsesExplicitLocalityTokensNotStreetOrCountryAliases(
    string input,
    string region,
    string country,
    bool accepted
  )
  {
    ...
    var city =
      input.Contains("Keasbey") ? "Keasbey"
      : input.Contains("Toronto") ? "Toronto"
      : "Lathrop";
    var postal =
      country == "CA" ? "M5V 1A1"
      : city == "Keasbey" ? "08832"
        ...
        result = new
        {
          ...
          address = new
          {
            ...
            addressComponents = new[]
            {
              ("street_number", "1"),
              ("route", "Arizona Way"),
              ("locality", "Woodbridge Township"),
              ("administrative_area_level_1", "WY"),
              ("postal_code", "08832"),
            }.Select(x => new
            {
              componentType = x.Item1,
          ...
          geocode = new
          {
            location = new { latitude = 40.5, longitude = -74.3 },
          },
        },
      }
    );
      ...
      new
      {
        ...
        results = new[]
        {
          new
          {
            ...
            {
              location_type = precision,
              location = new { lat = 40.5, lng = -74.3 },
            },
            address_components = new[]
            {
  ...
  private sealed class Handler(string json, string? validation = null)
    : HttpMessageHandler
  {
    public int Calls;
    public string? Query;
