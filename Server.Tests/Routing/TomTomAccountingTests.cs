...
public sealed class TomTomAccountingTests
{
  ...
  [InlineData("{\"routes\":[{}]}")]
  [InlineData("{\"routes\":{}}")]
  [InlineData("{\"routes\":[{\"summary\":{\"lengthInMeters\":\"wrong\",\"travelTimeInSeconds\":60}}]}")]
  [InlineData("[]")]
  public async Task MalformedSuccessKeepsAccountingAndCooldown(string body)
  {
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, _) => Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(body) }));
    ...
    await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync(42));
  ...
  public async Task CancellationAfterDispatchPreservesReservationAndCancellationSemantics()
  {
    ...
    await using var fixture = await TomTomProviderFixture.CreateAsync((_, ct) =>
    {
      cancellation.Cancel();
      return Task.FromCanceled<HttpResponseMessage>(ct);
    });
    await Assert.ThrowsAnyAsync<OperationCanceledException>(() => fixture.CalculateAsync(ct: cancellation.Token));
    Assert.Empty(fixture.Db.ChangeTracker.Entries<RoutingApiCall>());
    Assert.Equal(1, await fixture.Db.RoutingApiCalls.CountAsync());
    await Assert.ThrowsAsync<RoutePlanningException>(() => fixture.CalculateAsync());
    Assert.Equal(1, fixture.Calls);
  ...
  public async Task ReservationIsCommittedBeforeExternalIoAndSurvivesAnUnfinalizedAttempt()
  {
    ...
    await using var fixture = await TomTomProviderFixture.CreateAsync(async (_, ct) =>
    {
    ...
    current = fixture;
    await Assert.ThrowsAsync<SimulatedProcessInterruption>(() => fixture.CalculateAsync());
    Assert.Empty(fixture.Db.ChangeTracker.Entries<RoutingApiCall>());
