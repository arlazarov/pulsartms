...
public sealed class RoutePreparationQueueTests
{
  ...
  public void RepeatedDemandPreservesRetryBudgetAndChangedDemandBypassesIt()
  {
    ...
    queue.Retry(work, "input", null, clock.GetUtcNow().AddMinutes(10).UtcDateTime);
  ...
  public void ChangedTruckInvalidatesKnownSuccessorsButNotUnrelatedTrucks()
  {
    ...
    foreach (var work in queue.Take(10)) queue.Complete(work, work.Fingerprint!, queue.Identity(work.DispatchId).TruckId);
  ...
  public void CapacityKeepsOlderEqualPriorityAndDroppedDemandCanBeRequestedAgain()
  {
    ...
    var first = Guid.NewGuid(); var second = Guid.NewGuid(); var dropped = Guid.NewGuid();
    ...
    queue.Request(dropped, "dropped");
    Assert.Equal(new[] { second, dropped }, queue.Take(10).Select(x => x.DispatchId));
  }

  [Theory]
  [InlineData(true)]
  [InlineData(false)]
  public void DroppedRetryOfAPreviouslyCompletedLoadStillHonorsItsDeadline(bool changedDemand)
  {
    var clock = Clock();
    ...
    if (changedDemand) queue.Request(id, identity, 2);
    else
    {
      clock.Advance(TimeSpan.FromHours(6));
      queue.Observe(id, null, "old", 2);
    }
    queue.Retry(Assert.Single(queue.Take(1)), identity, null, clock.GetUtcNow().AddMinutes(10).UtcDateTime);
  ...
  public void ClearingAnAddressRetryWakesPreparationWithoutRepeatedPollingWork()
  {
    ...
    var load = new global::Domain.Entities.Dispatch.Dispatch { Id = Guid.NewGuid(), TruckId = truck,
      Status = "assigned", Stops = [new DispatchStop { Id = Guid.NewGuid(), Sequence = 1,
        Address = "1 Arizona Way", AddressRetryAfter = retry }] };
    string Signature() => RoutePreparationInputs.Signature(load, truck, 0, reads, clock.GetUtcNow().UtcDateTime);
  ...
  public void FinancialAndProfileChangesDirtyInputsButUnrelatedDispatchInvalidationDoesNot()
  {
    ...
    var truck = Guid.NewGuid();
    var load = new global::Domain.Entities.Dispatch.Dispatch { Id = Guid.NewGuid(), TruckId = truck,
      Status = "assigned", Price = 100, Currency = "USD", LoadedMiles = 50,
      Stops = [new DispatchStop { Id = Guid.NewGuid(), Sequence = 1, Latitude = 40, Longitude = -80 }] };
    string Signature() => RoutePreparationInputs.Signature(load, truck, 0, reads, Clock().GetUtcNow().UtcDateTime);
  ...
  private static ManualTimeProvider Clock() => new(new DateTimeOffset(2026, 9, 8, 12, 0, 0, TimeSpan.Zero));
  private static RoutePreparationQueue Create(ManualTimeProvider clock, int pending = 512, int state = 2048) =>
    new(Options.Create(new RoutePreparationOptions { PendingCapacity = pending, StateCapacity = state }), clock);
}
