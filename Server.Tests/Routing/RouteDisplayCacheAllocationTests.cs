...
public sealed class RouteDisplayCacheAllocationTests(ITestOutputHelper output)
{
  ...
  public void ConstructingExactGeometryUsesOnlyItsFinalSegmentBuffer()
  {
    var points = Enumerable
      .Range(0, 50_001)
      .Select(i => new RoutePoint(40, -80 + i / 50000d))
      .ToList();
    ...
    output.WriteLine(
      $"Exact geometry, 50,000 segments: {allocated:N0} allocated bytes."
    );
    Assert.True(
      allocated < 50_000L * 48 + 4096,
      $"Unexpected temporary geometry buffer: {allocated:N0} bytes."
    );
  ...
  private static WeakReference<RouteDisplayCache.Snapshot> Populate(
    RouteDisplayCache cache,
    Guid id
  )
  {
      ...
      Route = new()
      {
        ...
        [
          new(
            ...
            Enumerable
              .Range(0, 10001)
              .Select(i => new RoutePoint(40, -80 + i / 10000d))
              .ToList()
          ),
        ],
      },
      ...
      .GetAsync(
        ...
        () =>
          Task.FromResult<DispatchRoutePlan?>(
            new() { PlanJson = RoutePlanStorage.Serialize(plan) }
          ),
        default
      )
      .GetAwaiter()
  ...
  public void MatchingReadsAllocateMetadataRatherThanObjectsForEveryDisplayPoint()
  {
    ...
    {
      for (var i = 0; i < 5; i++)
        GC.KeepAlive(snapshot.ReadPlan(knownId, 1));
      var before = GC.GetAllocatedBytesForCurrentThread();
      for (var i = 0; i < 10; i++)
        GC.KeepAlive(snapshot.ReadPlan(knownId, 1));
    ...
    output.WriteLine(
      $"Per read: small metadata={smallMetadata:N0} B, large metadata={largeMetadata:N0} B, large full={largeFull:N0} B; display={large.DisplayBytes:N0} B, metadata={large.MetadataBytes:N0} B."
    );
