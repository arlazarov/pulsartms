...
public sealed class StopAddressServiceTests
{
  ...
  public async Task ConcurrentImportWinsOverAnInFlightCorrection()
  {
    ...
    var stop = new DispatchStop { Id = Guid.NewGuid(), Address = "11402 East Point Drive" };
    ...
    var geocoder = new Geocoder { BeforeResolve = () =>
    {
    ...
    await new StopAddressService(db, geocoder, cache, TestCache.Preparation()).VerifyAsync([stop], default);
  ...
  public async Task PersistsCorrectionPreservesSourceAndDoesNotRepeatLookup()
  {
    ...
    await db.Database.EnsureCreatedAsync();
    var stop = new DispatchStop { Id = Guid.NewGuid(), Address = "11402 East Point Drive", City = "Laredo", Province = "TX", Country = "USA", ZipCode = "78045" };
    db.Dispatches.Add(new Dispatch { Id = Guid.NewGuid(), Stops = [stop] });
    await db.SaveChangesAsync();
    using var cache = TestCache.Create();
    var geocoder = new Geocoder();
    var service = new StopAddressService(db, geocoder, cache, TestCache.Preparation());
    ...
    Assert.Equal(persisted.Address, (await db.Dispatches.Select(DispatchProjection.Details).SingleAsync()).Stops.Single().Address);
    await service.VerifyAsync([persisted], default);
    Assert.Equal(1, geocoder.Calls);

    var source = new ExternalDispatchStop { Address = "11402 East Point Drive", City = "Laredo", Province = "TX", Country = "USA", ZipCode = "78045",
      Latitude = 27.5m, Longitude = -99.5m };
  ...
  public async Task ExpirationRestoresSourceAndFailuresKeepItWithBoundedRetries()
  {
    ...
    await using var db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseSqlite(connection).Options);
    await db.Database.EnsureCreatedAsync();
    var source = new StopAddress("100 Original Rd", "Town", "TX", "USA", "78045");
    var stop = new DispatchStop { Id = Guid.NewGuid(), Address = "100 Corrected Rd", SourceAddressJson = source.Serialize(), AddressVerifiedAt = DateTime.UtcNow.AddDays(-30) };
    db.Dispatches.Add(new Dispatch { Id = Guid.NewGuid(), Stops = [stop] });
    await db.SaveChangesAsync();
    using var cache = TestCache.Create();
  ...
  public async Task VerificationPropagatesProvenanceToTheDetachedPreparationInput()
  {
    ...
    await using var db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseSqlite(connection).Options);
    await db.Database.EnsureCreatedAsync();
    var stop = new DispatchStop { Id = Guid.NewGuid(), Address = "11402 East Point Drive", City = "Laredo", Province = "TX" };
    db.Dispatches.Add(new Dispatch { Id = Guid.NewGuid(), Stops = [stop] });
    await db.SaveChangesAsync();
    var detached = await db.DispatchStops.AsNoTracking().SingleAsync();
    using var cache = TestCache.Create();
    var geocoder = new Geocoder();
    await new StopAddressService(db, geocoder, cache, TestCache.Preparation()).VerifyAsync([detached], default);
    ...
    Assert.Equal(new RoutePoint(27.62, -99.47), StopLocation.VerifiedPoint(detached, DateTime.UtcNow));
    await new StopAddressService(db, geocoder, cache, TestCache.Preparation()).VerifyAsync([detached], default);
    Assert.Equal(1, geocoder.Calls);
  }
  ...
  public async Task MalformedExpiredSourceCannotBlockOtherRowsOrBecomeTrustedCoordinates()
  {
    ...
    await using var db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseSqlite(connection).Options);
    await db.Database.EnsureCreatedAsync();
    var valid = new DispatchStop { Id = Guid.NewGuid(), Address = "Corrected", AddressVerifiedAt = DateTime.UtcNow.AddDays(-30),
      SourceAddressJson = new StopAddress("Original", "Town", "TX", "US", "78045").Serialize() };
    var invalid = new DispatchStop { Id = Guid.NewGuid(), Address = "Preserved", AddressVerifiedAt = DateTime.UtcNow.AddDays(-30),
      SourceAddressJson = "broken-source", Latitude = 40, Longitude = -80 };
    db.Dispatches.Add(new Dispatch { Id = Guid.NewGuid(), Stops = [valid, invalid] });
    await db.SaveChangesAsync();
    using var cache = TestCache.Create();
    var geocoder = new Geocoder();
    var service = new StopAddressService(db, geocoder, cache, TestCache.Preparation());
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task CommittedPredecessorAddressChangesImmediatelyDirtyKnownSuccessors(bool expire)
  {
    await using var connection = new SqliteConnection("Data Source=:memory:");
    await connection.OpenAsync();
    await using var db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseSqlite(connection).Options);
    await db.Database.EnsureCreatedAsync();
    var truck = new global::Domain.Entities.Fleet.Truck { Id = Guid.NewGuid(), ExternalId = "address-owner" };
    db.Trucks.Add(truck);
    var stop = new DispatchStop { Id = Guid.NewGuid(), Address = "11402 East Point Drive",
      AddressVerifiedAt = expire ? DateTime.UtcNow.AddDays(-30) : null,
      SourceAddressJson = new StopAddress("11402 East Point Drive", "Laredo", "TX", "US", "78045").Serialize() };
    db.Dispatches.Add(new Dispatch { Id = Guid.NewGuid(), TruckId = truck.Id, Stops = [stop] });
    await db.SaveChangesAsync();
    using var cache = TestCache.Create();
    ...
    if (expire) await service.ExpireAsync(default);
    else await service.VerifyAsync([stop], default);
    Assert.Contains(queue.Take(10), work => work.DispatchId == successor && work.ConnectionVersion == 1);
  }
  ...
  private sealed class Geocoder : IAddressGeocoder
  {
    ...
    public Task<ResolvedAddress> ResolveAsync(string address, CancellationToken ct)
    {
      Calls++;
      BeforeResolve?.Invoke();
      if (Fail) throw new RoutePlanningException("Address needs confirmation.");
      return Task.FromResult(new ResolvedAddress(new(27.62, -99.47), "11402 EastPoint Dr", "Laredo", "TX", "US", "78045"));
    }
  }
}
