...
public class PlanningSettingsTests
{
  ...
  public async Task PreferencesPersistAcrossContextsAndUnchangedSaveKeepsTheRevision()
  {
    ...
    await using (var db = new AppDbContext(options))
    {
      ...
      var preferences = new PlanningPreferences { UseIfta = false, MaxDetourMinutes = 8, FillPercent = 95, ReserveGallons = 30, CadToUsd = .73 };
      var saved = await service.SaveAsync(new(preferences, defaults.Revision), default);
      Assert.Equal(1, saved.Revision);
      var repeated = await service.SaveAsync(new(preferences, saved.Revision), default);
  ...
  public async Task InvalidSettingsDoNotReachStorage(double detour)
  {
    ...
    await Assert.ThrowsAsync<RoutePlanningException>(() => services.Settings.SaveAsync(new(new() { MaxDetourMinutes = detour }, 0), default));
  ...
  {
    var profile = new TruckRouteProfile { TankGallons = 200, Mpg = 7.5, HeightFeet = 13.5, LengthFeet = 72 };
    new PlanningPreferences { UseIfta = false, MaxDetourMinutes = 5 }.ApplyTo(profile);
  ...
  {
    Assert.NotNull(typeof(SettingsController).GetCustomAttribute<AuthorizeAttribute>());
    Assert.Null(typeof(SettingsController).GetMethod(nameof(SettingsController.Save))!.GetCustomAttribute<AllowAnonymousAttribute>());
  }
}
