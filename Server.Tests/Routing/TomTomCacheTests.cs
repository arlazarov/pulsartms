...
public class TomTomCacheTests
{
  ...
  [InlineData(true)]
  [InlineData(false)]
  public async Task RepeatedRequestsUseCacheWithoutAnExtraTransactionOrPaidCall(
    bool success
  )
  {
    ...
    if (success)
      Assert.Equal(40, (await Calculate()).Points[0].Latitude);
    else
  ...
  public async Task FailurePersistsReasonAndRetryPolicy(
    int status,
    bool permanent
  )
  {
    ...
    var error = await Assert.ThrowsAsync<RoutePlanningException>(
      () =>
        provider.CalculateAsync([new(40, -80), new(41, -80)], profile, default)
    );
    ...
    await Assert.ThrowsAsync<RoutePlanningException>(
      () =>
        provider.CalculateAsync([new(40, -80), new(42, -80)], profile, default)
    );
    Assert.Equal(2, handler.Calls);
    config["TomTom:DailyRequestLimit"] = "2";
    var limited = await Assert.ThrowsAsync<RoutePlanningException>(
      () =>
        provider.CalculateAsync([new(40, -80), new(43, -80)], profile, default)
    );
  ...
  private sealed class Handler(
    bool success,
    HttpStatusCode failureStatus = HttpStatusCode.ServiceUnavailable
  ) : HttpMessageHandler
  {
    public int Calls;
    ...
    )
    {
      ...
      return Task.FromResult(
        new HttpResponseMessage(success ? HttpStatusCode.OK : failureStatus)
        {
