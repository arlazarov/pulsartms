...
public class TruckCameraTests
{
  ...
  public async Task EmptyMediaDoesNotFailOpeningOrPolling(string body)
  {
    ...
    var provider = new SamsaraTruckCameraProvider(new SamsaraApiService(client, new StubProviderCredentials(("apiKey", "test"))), cache);
  ...
  public async Task RefreshCreatesNewCaptureAndCannotBeReadForAnotherTruck()
  {
    ...
    var truck = new Truck { Id = Guid.NewGuid(), ExternalId = "vehicle", UnitNumber = "123", IsActive = true };
    ...
    Assert.True((await latest.Handle(new GetLatestTruckCameraQuery(truck.Id), default)).Success);
    Assert.Equal(0, provider.Requests);
    Assert.False((await latest.Handle(new GetLatestTruckCameraQuery(Guid.NewGuid()), default)).Success);
    var first = await handler.Handle(new RequestTruckCameraCommand(truck.Id), default);
    var second = await handler.Handle(new RequestTruckCameraCommand(truck.Id), default);
    Assert.True(first.Success); Assert.NotEqual(first.Response, second.Response); Assert.Equal(2, provider.Requests);
    Assert.False((await retrieval.Handle(new GetTruckCameraQuery(Guid.NewGuid(), first.Response), default)).Success);
    Assert.Equal("pending", (await retrieval.Handle(new GetTruckCameraQuery(truck.Id, first.Response), default)).Response!.Status);
    Assert.False((await handler.Handle(new RequestTruckCameraCommand(Guid.NewGuid()), default)).Success);
  }
  ...
  public async Task ProviderRequestsRoadImageAndUsesActualCaptureTime()
  {
    ...
    using var cache = new MemoryCache(new MemoryCacheOptions());
    var provider = new SamsaraTruckCameraProvider(new SamsaraApiService(client, new StubProviderCredentials(("apiKey", "test"))), cache);
    Assert.Equal("retrieval", await provider.RequestAsync("vehicle", DateTimeOffset.UtcNow, default));
    var image = await provider.GetAsync("vehicle", "retrieval", default);
    Assert.Equal("https://example.com/image.jpg", image.Url);
    Assert.Equal(DateTimeOffset.Parse("2026-09-07T12:00:00Z"), image.CapturedAt);
  }
  [Fact]
  ...
  {
    public int Requests;
    public Task<CameraImage> LatestAsync(string vehicleId, CancellationToken ct) => Task.FromResult(new CameraImage("pending"));
    public Task<string> RequestAsync(string vehicleId, DateTimeOffset time, CancellationToken ct) { Requests++; return Task.FromResult("retrieval"); }
    public Task<CameraImage> GetAsync(string vehicleId, string retrievalId, CancellationToken ct) => Task.FromResult(new CameraImage("pending"));
  }
  private sealed class CameraHttp(bool pending = false) : HttpMessageHandler
  {
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken ct)
    {
      if (request.Method == HttpMethod.Post)
      {
        ...
        Assert.Equal("dashcamRoadFacing", data.GetProperty("inputs")[0].GetString());
        Assert.Equal(data.GetProperty("startTime").GetString(), data.GetProperty("endTime").GetString());
      }
      var isRetrieval = request.RequestUri!.AbsolutePath.EndsWith("/retrieval");
      if (!isRetrieval) ListCalls++;
      if (pending && isRetrieval && request.Method == HttpMethod.Get)
        return new(HttpStatusCode.OK) { Content = new StringContent("""{"data":{"media":[{"vehicleId":"vehicle","input":"dashcamRoadFacing","mediaType":"image","status":"pending"}]}}""") };
