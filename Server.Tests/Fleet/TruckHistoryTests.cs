...
public class TruckHistoryTests
{
  ...
  public void SimplificationKeepsTurnAndGapButRemovesStraightPoints()
  {
    ...
    var points = Enumerable.Range(0, 100).Select(i => new VehicleLocationPoint {
      Latitude = 40, Longitude = -80m + i * .0001m, UpdatedAt = time.AddSeconds(i)
    }).ToList();
    points.Add(new() { Latitude = 40.01m, Longitude = -79.9901m, UpdatedAt = time.AddSeconds(110) });
    points.Add(new() { Latitude = 41, Longitude = -79, UpdatedAt = time.AddMinutes(20) });
  ...
  {
    public int Calls;
    public Task<IReadOnlyList<VehicleTelemetry>> GetVehicleTelemetryAsync(CancellationToken cancellationToken = default) => throw new NotImplementedException();
    public Task<VehicleLocationStream> GetLocationStreamAsync(IReadOnlyCollection<string> vehicleIds, DateTime startTime, DateTime endTime, string? cursor = null, CancellationToken cancellationToken = default)
    { Calls++; return Task.FromResult(new VehicleLocationStream()); }
  }
}
