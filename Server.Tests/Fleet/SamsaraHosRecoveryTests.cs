...
public sealed class SamsaraHosRecoveryTests
{
  ...
  public async Task FailureReturnsUnavailableButRecoveryKeepsTheIncrementalBaselineAcrossProviderScopes()
  {
    ...
    fixture.Read = (_, _) => Task.FromResult(new HttpResponseMessage(HttpStatusCode.ServiceUnavailable));
    ...
    fixture.Read = (request, _) => Task.FromResult(SamsaraProviderFixture.Reply(request));
    var restored = (await fixture.Provider().GetAsync("driver", default))!;
    Assert.Equal(TimeSpan.FromDays(2), fixture.Requests[^1].To - fixture.Requests[^1].From);
  ...
  public async Task RecoveryDoesNotPostponeTheFifteenMinuteFullReconciliation()
  {
    ...
    Assert.Equal(TimeSpan.FromDays(16), fixture.Requests[^1].To - fixture.Requests[^1].From);
  ...
  public async Task CallerCancellationDoesNotEraseTheBaselineOrCreateAFailureCooldown()
  {
    ...
    fixture.Read = (_, ct) => { cancellation.Cancel(); ct.ThrowIfCancellationRequested(); throw new InvalidOperationException(); };
    await Assert.ThrowsAnyAsync<OperationCanceledException>(() => fixture.Provider().GetAsync("driver", cancellation.Token));
    fixture.Read = (request, _) => Task.FromResult(SamsaraProviderFixture.Reply(request));
    Assert.NotNull(await fixture.Provider().GetAsync("driver", default));
  ...
  {
    using var fixture = new SamsaraProviderFixture();
    var other = Enumerable.Range(0, 1000).Select(i => $"other-{i}")
      .First(id => !ReferenceEquals(fixture.History.Gate("driver"), fixture.History.Gate(id)));
    ...
    {
      if (request.Driver == "driver") { entered.TrySetResult(); await release.Task.WaitAsync(ct); }
    ...
    try { Assert.NotNull(await fixture.Provider().GetAsync(other, default).WaitAsync(TimeSpan.FromSeconds(5))); }
  ...
  public async Task ExplicitCatalogRefreshUpdatesTheSharedSnapshotWithoutLeakingMutableMetadata()
  {
    ...
    Assert.Equal("Etc/UTC", (await fixture.Catalog.GetAsync(fixture.Api, default))[0].Timezone);
    fixture.Drivers = ["replacement"];
    await new Infrastructure.Integrations.Samsara.SamsaraFleetProvider(fixture.Api, fixture.Catalog).GetDriversAsync();
    Assert.Equal("replacement", Assert.Single(await fixture.Catalog.GetAsync(fixture.Api, default)).Id);
    Assert.Equal(2, fixture.CatalogCalls);
  }
  ...
  public async Task FailedExplicitRefreshDoesNotInvalidateAStillFreshCatalogOrExtendItsLifetime()
  {
    ...
    fixture.Clock.Advance(TimeSpan.FromMinutes(1));
    fixture.CatalogStatus = HttpStatusCode.ServiceUnavailable;
    await Assert.ThrowsAsync<HttpRequestException>(() => fixture.Catalog.GetAsync(fixture.Api, default, forceRefresh: true));
    Assert.Single(await fixture.Catalog.GetAsync(fixture.Api, default));
    Assert.Equal(2, fixture.CatalogCalls);
    fixture.Clock.Advance(TimeSpan.FromMinutes(14));
    await Assert.ThrowsAsync<HttpRequestException>(() => fixture.Catalog.GetAsync(fixture.Api, default));
    Assert.Equal(3, fixture.CatalogCalls);
  }
}
