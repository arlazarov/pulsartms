...
public class SamsaraHosHistoryTests
{
  ...
  public async Task ActualSleeperBedPayloadProducesUsableHistoryAndReusesTheCache()
  {
    ...
    var provider = new SamsaraHosHistoryProvider(new(client, new StubProviderCredentials(("apiKey", "test"))), cache, catalog, clock, NullLogger<SamsaraHosHistoryProvider>.Instance);
    ...
    Assert.Equal(300, HosDutyStatus.Read(history, null, history.Through)!.RestMinutes);
  ...
  private sealed class Handler : HttpMessageHandler
  {
    ...
    protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
      ...
      if (request.RequestUri!.AbsolutePath.EndsWith("/drivers"))
        return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("""{"data":[{"id":"test","timezone":"Etc/UTC"}]}""") });
      ...
      var periods = new[] {
        new { logStartTime = start, logEndTime = end.AddHours(-5), hosStatusType = "onDuty" },
        new { logStartTime = end.AddHours(-5), logEndTime = end.AddHours(-4), hosStatusType = "offDuty" },
        new { logStartTime = end.AddHours(-4), logEndTime = end.AddHours(-3), hosStatusType = "personalConveyance" },
        new { logStartTime = end.AddHours(-3), logEndTime = end, hosStatusType = "sleeperBed" }
      };
      return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(JsonSerializer.Serialize(new
        { data = new[] { new { driver = new { id = "test" }, hosLogs = periods } } })) });
    }
  }
}
