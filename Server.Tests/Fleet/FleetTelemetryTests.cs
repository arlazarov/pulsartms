...
public class FleetTelemetryTests
{
  ...
  public async Task StreamReadsAllPagesWithTheSameStartTime()
  {
    ...
    var result = await stream.GetAsync(provider,
    ...
    Assert.Equal(TimeSpan.FromSeconds(60), provider.EndTimes[0] - provider.StartTimes[0]);
  }
  ...
  {
    using var stream = new FleetLocationStream(TimeProvider.System);
    await Assert.ThrowsAsync<InvalidOperationException>(() => stream.GetAsync(
  ...
  public async Task ConcurrentClientsShareOneLoadAndFailuresCanRetry()
  {
    ...
    await Assert.ThrowsAsync<HttpRequestException>(() => cache.GetAsync(
      _ => Task.FromException<FleetLocationsResponse>(new HttpRequestException()), default));
    var release = new TaskCompletionSource<FleetLocationsResponse>(TaskCreationOptions.RunContinuationsAsynchronously);
    var calls = 0;
    ...
    var requests = Enumerable.Range(0, 10).Select(_ => cache.GetAsync(Load, default)).ToArray();
  ...
  public async Task ACancelledReaderDoesNotReceiveAWarmTelemetrySnapshot()
  {
    using var memory = new MemoryCache(new MemoryCacheOptions());
    using var cache = new FleetTelemetryCache(memory);
    await cache.GetAsync(_ => Task.FromResult(new FleetLocationsResponse()), default);
    using var cancellation = new CancellationTokenSource();
    cancellation.Cancel();
    await Assert.ThrowsAnyAsync<OperationCanceledException>(() => cache.GetAsync(_ => throw new InvalidOperationException(), cancellation.Token));
  }
  ...
  public async Task PeriodicFullWindowRecoversOlderLatePointsAndPrunesExpiredOnes()
  {
    ...
    var initial = clock.GetUtcNow().UtcDateTime;
    using var stream = new FleetLocationStream(clock);
    var fleet = new[] { new FleetTruckInfo { TruckExternalId = "truck", IsActive = true } };
    var provider = new StubFleetTelemetryProvider { Read = (_, _) => Task.FromResult(new VehicleLocationStream
      { Data = [new() { ExternalId = "truck", UpdatedAt = initial.AddSeconds(-50) }] }) };
    ...
    provider.Read = (_, _) => Task.FromResult(new VehicleLocationStream
      { Data = [new() { ExternalId = "truck", UpdatedAt = initial.AddSeconds(-20) }] });
  ...
  public async Task FullReconciliationRemovesPointsNoLongerPresentInTheProviderWindow()
  {
    ...
    using var stream = new FleetLocationStream(clock);
    var fleet = new[] { new FleetTruckInfo { TruckExternalId = "truck", IsActive = true } };
    var provider = new StubFleetTelemetryProvider { Read = (request, _) => Task.FromResult(new VehicleLocationStream
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task FailedOrCancelledPaginationDoesNotAdvanceTheWatermark(bool cancel)
  {
    var clock = new ManualTimeProvider();
    var initial = clock.GetUtcNow().UtcDateTime;
    using var stream = new FleetLocationStream(clock);
    ...
    {
      if (request.Cursor is null) return Task.FromResult(new VehicleLocationStream { HasNextPage = true, EndCursor = "next" });
      if (cancel) { cancellation.Cancel(); ct.ThrowIfCancellationRequested(); }
      throw new HttpRequestException("Test failure");
    };
    if (cancel) await Assert.ThrowsAnyAsync<OperationCanceledException>(() => stream.GetAsync(provider, fleet, cancellation.Token));
    else await Assert.ThrowsAsync<HttpRequestException>(() => stream.GetAsync(provider, fleet));
  ...
  [InlineData("gap")]
  [InlineData("clock")]
  public async Task FleetChangesGapsAndClockRollbackBootstrapAFullWindow(string change)
  {
    var clock = new ManualTimeProvider();
    using var stream = new FleetLocationStream(clock);
    var truck = new FleetTruckInfo { TruckExternalId = "truck", IsActive = true };
    var provider = new StubFleetTelemetryProvider();
    await stream.GetAsync(provider, [truck]);
    clock.Advance(change == "gap" ? TimeSpan.FromMinutes(2) : change == "clock" ? TimeSpan.FromSeconds(-5) : TimeSpan.FromSeconds(10));
    if (change == "fleet") truck.TruckExternalId = "another";
    await stream.GetAsync(provider, [truck]);
    Assert.Equal(TimeSpan.FromSeconds(60), provider.Requests[^1].To - provider.Requests[^1].From);
  }

  [Fact]
  public async Task OversizedWindowIsReturnedButNotRetainedAsACompleteIncrementalBaseline()
  {
    var clock = new ManualTimeProvider();
    using var stream = new FleetLocationStream(clock);
    var fleet = new[] { new FleetTruckInfo { TruckExternalId = "truck", IsActive = true } };
    var provider = new StubFleetTelemetryProvider { Read = (request, _) => Task.FromResult(new VehicleLocationStream
      { Data = Enumerable.Range(0, 65537).Select(i => new VehicleLocationPoint
        { ExternalId = "truck", UpdatedAt = request.To.AddTicks(-i) }).ToArray() }) };
    Assert.Equal(65537, (await stream.GetAsync(provider, fleet)).Count);
    clock.Advance(TimeSpan.FromSeconds(10));
    provider.Read = (_, _) => Task.FromResult(new VehicleLocationStream());
    Assert.Empty(await stream.GetAsync(provider, fleet));
    Assert.Equal(TimeSpan.FromSeconds(60), provider.Requests[^1].To - provider.Requests[^1].From);
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task FallbackHonorsHighFrequencyFlagAndStillRefreshesFleetStats(bool highFrequency)
  {
    using var memory = new MemoryCache(new MemoryCacheOptions());
    memory.Set(FleetCache.CacheKey, new FleetTruckInfo[] { new() { TruckExternalId = "truck", IsActive = true } });
    var provider = new StubFleetTelemetryProvider { Vehicles = [new() { ExternalId = "truck", UpdatedAt = DateTime.UtcNow }] };
    using var stream = new FleetLocationStream(TimeProvider.System);
    using var telemetry = new FleetTelemetryCache(memory);
    var handler = new GetFleetLocationsHandler(null!, provider, new(memory), telemetry, stream, new(),
      Options.Create(new SynchronizationOptions { Enabled = false, HighFrequencyLocations = highFrequency }));
  ...
  private sealed class Provider : IFleetTelemetryProvider
  {
      ...
      DateTime startTime, DateTime endTime, string? cursor = null, CancellationToken cancellationToken = default)
    {
      ...
      {
        Data = [new VehicleLocationPoint { ExternalId = "truck", UpdatedAt = startTime.AddSeconds(Cursors.Count) }],
