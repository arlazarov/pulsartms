...
public class ReadCacheAllocationTests(ITestOutputHelper output)
{
  ...
  public async Task WarmLargeSnapshotAvoidsJsonSizedAllocations()
  {
    ...
    var entity = new DispatchRoutePlan { PlanJson = "{\"points\":[" + string.Join(',', Enumerable.Repeat("{\"latitude\":43.123456,\"longitude\":-79.123456}", 60_000)) + "]}" };
    ...
    for (var i = 0; i < count; i++)
      Assert.Equal(entity.PlanJson.Length, JsonSerializer.Deserialize<DispatchRoutePlan>(wrapped)!.PlanJson.Length);
    watch.Stop();
    ...
    watch.Restart();
    for (var i = 0; i < count; i++)
      Assert.Same(entity.PlanJson, (await cache.GetAsync("route", "benchmark", Load)).PlanJson);
    watch.Stop();
    var currentBytes = GC.GetAllocatedBytesForCurrentThread() - before;
    output.WriteLine($"Payload characters: {entity.PlanJson.Length}; reads: {count}");
    output.WriteLine($"Previous snapshot cloning: {previousBytes:N0} allocated bytes, {previousMs:F2} ms");
    output.WriteLine($"Current snapshot cloning: {currentBytes:N0} allocated bytes, {watch.Elapsed.TotalMilliseconds:F2} ms");
