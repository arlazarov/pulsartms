...
public sealed class StubProviderCredentials(
  params (string Field, string Value)[] fields
) : IIntegrationCredentials
{
  public IntegrationCredentialValues Values { get; set; } =
    new(fields.Select(field => KeyValuePair.Create(field.Field, field.Value)));
