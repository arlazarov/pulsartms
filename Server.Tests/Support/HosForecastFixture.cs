...
{
  public static HosHistory History(
    DateTimeOffset now,
    double cycleHours = 0,
  ...
  )
  {
    ...
    for (var day = -7; day <= 0; day++)
    {
      ...
      var end = new DateTimeOffset(
        now.Date.AddDays(day).AddHours(endHour),
        now.Offset
      );
      var begin = end.AddHours(-hours);
      if (cursor < begin)
        periods.Add(new(cursor, begin, "offDuty"));
  ...
  public static DriverHosClocks Clocks(
    DateTimeOffset now,
    HosHistory history,
    double drive = 11,
    double shift = 14
  )
  {
    ...
    return new()
    {
      ...
      CycleMs = (long)Math.Round((rule.Hours - used) * 3600000),
      BreakMs = 8 * 3600000L,
      UpdatedAt = now.UtcDateTime,
    };
  }

  public static HosTravelClock Clock(
    DateTimeOffset now,
    HosCycleMode mode,
    double firstDayHours = 0,
    double ongoingRestHours = 0,
  ...
  )
  {
    var history = History(
      ...
      ongoingRestHours: ongoingRestHours
    );
