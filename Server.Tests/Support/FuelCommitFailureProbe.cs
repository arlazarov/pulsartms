...
public sealed class FuelCommitFailureProbe : DbCommandInterceptor
{
  ...
  {
    if (FailNextSnapshotWrite && command.CommandText.Contains("UPDATE \"DispatchRoutePlans\"", StringComparison.Ordinal))
    ...
    CommandEventData eventData, InterceptionResult<int> result, CancellationToken cancellationToken = default)
  {
    if (FailNextSnapshotWrite && IsSnapshotWrite(command))
    {
      ...
      throw new InvalidOperationException("Injected truck fuel snapshot write failure.");
    }
    return ValueTask.FromResult(result);
  }
  ...
  private static bool IsSnapshotWrite(DbCommand command) =>
    command.CommandText.Contains("INSERT INTO \"TruckFuelPlans\"", StringComparison.Ordinal)
    || command.CommandText.Contains("UPDATE \"TruckFuelPlans\"", StringComparison.Ordinal);
}
