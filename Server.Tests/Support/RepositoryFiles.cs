...
internal static class RepositoryFiles
{
  private static readonly HashSet<string> ExcludedDirectories =
    ["bin", "obj", "node_modules", ".git", ".cache", "artifacts", "test-results"];
  ...
  public static IEnumerable<string> Sources(string directory, string pattern)
  {
    ...
    {
      // Prune outputs before traversal; release artifacts can contain entire dependency trees.
      if (ExcludedDirectories.Contains(child.Name) || child.Attributes.HasFlag(FileAttributes.ReparsePoint)) continue;
      foreach (var file in Sources(child.FullName, pattern)) yield return file;
    }
  }
}
