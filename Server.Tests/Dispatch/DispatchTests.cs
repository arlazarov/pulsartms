...
public class DispatchTests
{
  ...
  public async Task RepeatedSyncResolvesStopAssignmentsWithoutReplacingUnchangedStops()
  {
    ...
    var preparation = new RoutePreparationQueue(
    ...
    var detail = await new GetDispatchByIdHandler(
      db,
      planning.Forecasts
    ).Handle(new(list.Response.Items.Single().Id), default);
    Assert.Single(detail.Response!.Stops);
    Assert.Equal(truck.Id, detail.Response.TruckId);
    var missing = await new GetDispatchByIdHandler(
      db,
      planning.Forecasts
