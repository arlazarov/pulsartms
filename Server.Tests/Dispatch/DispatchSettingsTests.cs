...
public sealed class DispatchSettingsTests
{
  ...
  public async Task DefaultReadDoesNotWriteAndBlankPrefixSurvivesAnotherContext()
  {
    ...
    await using var reloaded = new AppDbContext(
      fixture.Services.GetRequiredService<DbContextOptions<AppDbContext>>()
    );
    var actual = await new GetDispatchSettingsHandler(reloaded).Handle(
      new(),
      default
    );
  ...
  public async Task NormalizationPreservesCaseAndAnUnchangedSaveKeepsRevision()
  {
    ...
    var saved = await sender.Send(
      new UpdateDispatchSettingsCommand("  Ab-  ", 0)
    );
    Assert.True(saved.Success);
    Assert.Equal("Ab-", saved.Response!.LoadNumberPrefix);
    var repeated = await sender.Send(
      new UpdateDispatchSettingsCommand("Ab- ", saved.Response.Revision)
    );
    Assert.Equal(saved.Response, repeated.Response);
    var changed = await sender.Send(
      new UpdateDispatchSettingsCommand("X", saved.Response.Revision)
    );
    ...
    Assert.Equal(
      "X",
      (await sender.Send(new GetDispatchSettingsQuery()))
        .Response!
        .LoadNumberPrefix
    );
  }
  ...
  public async Task InvalidUpdatesAreRejectedBeforeStorage(
    string? prefix,
    long revision
  )
  {
    ...
    var result = await fixture
      ...
      .Send(new UpdateDispatchSettingsCommand(prefix, revision));
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task ConcurrentInsertOrUpdateCannotOverwriteTheWinningSession(
    bool existing
  )
  {
    await using var fixture = await PlanningPipelineFixture.CreateAsync();
    ...
    var winnerHandler = new UpdateDispatchSettingsHandler(
      winner,
      TimeProvider.System
    );
    if (existing)
      Assert.True(
        (await winnerHandler.Handle(new("Original", 0), default)).Success
      );
    var revision = existing ? 1 : 0;
    var interceptor = new BeforeSave(
      async () =>
        Assert.True(
          (await winnerHandler.Handle(new("Winner", revision), default)).Success
        )
    );
    await using var losing = new AppDbContext(
      new DbContextOptionsBuilder<AppDbContext>(options)
        .AddInterceptors(interceptor)
        .Options
    );

    var result = await new UpdateDispatchSettingsHandler(
      losing,
      TimeProvider.System
    ).Handle(new("Loser", revision), default);

    Assert.False(result.Success);
    Assert.Equal(409, result.StatusCode);
    Assert.Empty(losing.ChangeTracker.Entries<DispatchSettings>());
    var saved = (
      await new GetDispatchSettingsHandler(losing).Handle(new(), default)
    ).Response!;
