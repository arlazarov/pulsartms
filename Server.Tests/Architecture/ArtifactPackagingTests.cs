...
public sealed class ArtifactPackagingTests
{
  ...
  {
    var patterns = File.ReadAllLines(Path.Combine(RepositoryFiles.Root(), ".gcloudignore"))
  ...
  public void BuildInputsExcludeRawCapturesAndRootOrNestedEnvironmentFiles(string file)
  {
    string[] required = ["docs/archive/**/*.jsonl", ".env", ".env.*", "**/.env", "**/.env.*"];
  ...
  public void CloudSourceIncludesMaintainedToolsForArchitectureAuditButDockerExcludesThem()
  {
    ...
    var cloud = File.ReadAllLines(Path.Combine(root, ".gcloudignore")).Select(line => line.Trim()).ToArray();
    var docker = File.ReadAllLines(Path.Combine(root, ".dockerignore")).Select(line => line.Trim()).ToArray();
    ...
    {
      "tools/LoadProbe/Program.cs", "tools/RouteMemoryProbe/Program.cs",
      "tools/MapLoadProbe/probe.js", "tools/map-performance-probe.mjs"
    })
      Assert.True(File.Exists(Path.Combine(root, file)), $"Missing maintained tool source: {file}");
  }
  ...
  public async Task ApiContentEvaluationExcludesCredentialFiles()
  {
    ...
    foreach (var group in json.RootElement.GetProperty("Items").EnumerateObject())
      foreach (var item in group.Value.EnumerateArray())
      {
        ...
        Assert.False(name == ".env" || name.StartsWith(".env.", StringComparison.Ordinal));
      }
  }
  ...
  {
    var directory = Directory.CreateTempSubdirectory("amftms-publish-guard-");
    try
    {
      var args = new[] { "-t:RejectSecretPublishArtifacts", $"-p:PublishDir={directory.FullName}/" };
      ...
      Assert.Contains("Publish output contains credential files", result.Output);
    }
    finally { directory.Delete(recursive: true); }
  }

  private static async Task<(int ExitCode, string Output)> EvaluateAsync(params string[] args)
  {
    var start = new ProcessStartInfo("dotnet")
    {
      WorkingDirectory = RepositoryFiles.Root(), RedirectStandardOutput = true,
      RedirectStandardError = true, UseShellExecute = false
    };
    foreach (var argument in new[] { "msbuild", "Server/API/API.csproj", "-verbosity:quiet", "-nologo" }.Concat(args))
