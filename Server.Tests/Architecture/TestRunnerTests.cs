...
public sealed class TestRunnerTests
{
  ...
  public async Task EveryFeatureRunsBothDotNetAssembliesAndArchitecture(string category)
  {
    var result = await RunAsync(category);
    Assert.Equal(0, result.ExitCode);
    Assert.Contains("DOTNET:test AMFTMS.slnx", result.Output);
    Assert.Contains($"--artifacts-path {Path.Combine(RepositoryFiles.Root(), "artifacts", "tests")}", result.Output);
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task FullSuiteRunsUnfilteredDotNetAndAllJavaScript(bool explicitAll)
  {
  ...
  private static async Task<(int ExitCode, string Output)> RunAsync(params string[] categories)
  {
    ...
    start.ArgumentList.Add("dotnet() { printf 'DOTNET:%s\\n' \"$*\"; }; npm() { printf 'NPM:%s\\n' \"$*\"; }; source ./test.sh \"$@\"");
    start.ArgumentList.Add("test-runner-check");
    foreach (var category in categories) start.ArgumentList.Add(category);
