...
BEGIN
  IF current_setting('pulsr.reset_database', true)
    ...
    OR current_setting('pulsr.reset_ack', true)
      IS DISTINCT FROM '20260923144550_SeparateTrailerCatalogFromAssignment'
  ...
  IF (SELECT count(*) FROM "__EFMigrationsHistory") <> 59
    OR (SELECT max("MigrationId") FROM "__EFMigrationsHistory")
      IS DISTINCT FROM '20260923144550_SeparateTrailerCatalogFromAssignment' THEN
