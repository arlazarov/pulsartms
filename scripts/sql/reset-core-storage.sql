...
BEGIN
  IF current_setting('pulsr.reset_database', true)
    ...
    OR current_setting('pulsr.reset_ack', true)
      IS DISTINCT FROM '20260919203514_RecordFuelStationPlaceId'
  ...
  IF (SELECT count(*) FROM "__EFMigrationsHistory") <> 50
    OR (SELECT max("MigrationId") FROM "__EFMigrationsHistory")
      IS DISTINCT FROM '20260919203514_RecordFuelStationPlaceId' THEN
