...
BEGIN
  IF current_setting('pulsr.reset_database', true)
    ...
    OR current_setting('pulsr.reset_ack', true)
      IS DISTINCT FROM '20260919192056_RecordFuelStationHours'
  ...
  IF (SELECT count(*) FROM "__EFMigrationsHistory") <> 49
    OR (SELECT max("MigrationId") FROM "__EFMigrationsHistory")
      IS DISTINCT FROM '20260919192056_RecordFuelStationHours' THEN
