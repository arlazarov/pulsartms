...
public sealed class TestConventionsTests
{
  ...
  public void ClientTestsUseTheCompiledClientAssemblyWithoutServerDependencies()
  {
    ...
    Assert.DoesNotContain(tests.GetReferencedAssemblies(), assembly => assembly.Name is "API" or "Application" or "Infrastructure" or "Domain");
  }
  ...
  public void EveryClientTestClassHasAFeatureAndTestKind()
  {
    ...
    string[] kinds = ["Unit", "Integration", "Component", "Architecture", "Allocation"];
    var types = typeof(TestConventionsTests).Assembly.GetTypes().Where(type =>
      type.GetMethods().Any(method => method.GetCustomAttributes<FactAttribute>().Any()));
    foreach (var type in types)
    {
      var traits = type.GetCustomAttributesData().Where(trait => trait.AttributeType == typeof(TraitAttribute)).ToArray();
      Assert.StartsWith("Client.Tests.", type.Namespace);
      Assert.Contains(traits, trait => trait.ConstructorArguments[0].Value as string == "Category");
      Assert.Contains(traits, trait => trait.ConstructorArguments[0].Value as string == "Kind");
      foreach (var trait in traits)
      {
        if (trait.ConstructorArguments[0].Value as string == "Category")
          Assert.Contains(trait.ConstructorArguments[1].Value as string, categories);
