...
public sealed class FleetMapQuietEtaTests
{
  ...
  public async Task VisibleSavedRouteHidesOnlyMaintenanceNoiseAndKeepsAddressWarnings()
  {
    ...
    fixture.Planning = fixture.Planning with
    {
      Message =
    ...
    fixture.Planning = fixture.Planning with
    {
      Message = "Delivery address needs verified coordinates.",
    };
    ...
    component.WaitForAssertion(
      () =>
        Assert.Contains(
          "Delivery address needs verified coordinates.",
          component.Markup
        )
    );
  }
  ...
  public async Task OlderForecastWithoutPendingDispatchMetadataCanRenderTheSelectedFutureStop()
  {
    ...
    fixture.Planning = fixture.Planning with
    {
      State = fixture.Planning.State! with
      {
  ...
  public async Task InFlightPlanningAcrossValidityDeadlineRetainsCurrentAndFutureEtaUntilReplacement()
  {
    ...
    await component.InvokeAsync(
      () => fixture.Clock.Advance(TimeSpan.FromSeconds(111))
    );
    component.Render();
    ...
    QuietEtaMapFixture.Reply(
      pending,
      original with
      {
        State = original.State with { Eta = null },
      }
    );
    component.WaitForAssertion(() =>
    {
      Assert.False(
        PublishedState(fixture).GetProperty("Refreshing").GetBoolean()
      );
    ...
    QuietEtaMapFixture.Reply(
      await fixture.ReadPendingAsync(),
      original with
      {
        State = original.State with { Eta = updated },
      }
    );
    component.WaitForAssertion(() =>
    {
  ...
  public async Task VersionOnlyRerouteRetainsEtaAndDistanceButExplicitUnavailableReplacementClearsThem()
  {
    ...
    var currentHtml = CurrentEta(component);
    var futureHtml = FutureEta(component);
    var replacement = JsonSerializer.Deserialize<AutomaticPlanningResult>(
      JsonSerializer.Serialize(fixture.Planning)
    )!;
    ...
    replacement = replacement with
    {
      ...
      {
        Eta = null,
        Progress = replacement.State.Progress! with
        {
          ProgressMiles = 20,
          RemainingMiles = 80,
        },
    ...
    using var payload = JsonDocument.Parse(
      (byte[])
        fixture.Js.Calls.Last(call => call.Name == "setRouteBytes").Args![0]!
    );
    Assert.Equal(
      replacement.State.Plan.Version,
      payload.RootElement.GetProperty("version").GetInt32()
    );
    var geometryProgress = Assert.IsType<RouteProgress>(
      fixture.Js.Calls.Last(call => call.Name == "setRouteBytes").Args![1]
    );
    Assert.Equal(20, geometryProgress.ProgressMiles);
    Assert.Equal(80, geometryProgress.RemainingMiles);
    var now = fixture.Clock.GetUtcNow().UtcDateTime;
    fixture.Planning = replacement with
    {
      State = replacement.State with
      {
        Eta = new(
    ...
    component.WaitForAssertion(() =>
    {
      ...
      Assert.Empty(
        component.FindAll("[aria-label='Selected next load'] .arrival-estimate")
      );
  ...
  public async Task PartialCurrentForecastRetainsOnlyThePendingFutureWithinItsOriginalGrace()
  {
    ...
    var currentHtml = CurrentEta(component);
    var futureHtml = FutureEta(component);
    var partial = previous with
    {
      CalculatedAt = previous.CalculatedAt.AddSeconds(10),
      ValidUntil = previous.ValidUntil.AddSeconds(10),
      Stops =
      [
        previous.Stops[0] with
        {
          Arrival = previous.Stops[0].Arrival.AddMinutes(5),
        },
      ],
      PendingDispatches = new Dictionary<Guid, string>
      {
        [fixture.Future.Id] = "Waiting for the saved connection.",
      },
    };
    Assert.False(partial.RouteUpdatePending);
    fixture.Planning = original with
    {
      State = original.State with { Eta = partial },
    };
    await component.InvokeAsync(
      () => fixture.Clock.Advance(TimeSpan.FromSeconds(10))
    );
    component.WaitForAssertion(
      () =>
        Assert.Equal(partial.CalculatedAt, PublishedEta(fixture).CalculatedAt)
    );
    Assert.NotEqual(currentHtml, CurrentEta(component));
    Assert.Equal(futureHtml, FutureEta(component));
    Assert.Contains(
      fixture.Future.Id,
      PublishedEta(fixture).PendingDispatches.Keys
    );
    Assert.DoesNotContain("Updating", component.Markup);
    fixture.HoldPlanning = true;
    await component.InvokeAsync(
    ...
    var fresh = previous with
    {
      ...
      Stops = previous
        .Stops.Select(stop =>
          stop with
          {
            Arrival = stop.Arrival.AddMinutes(10),
          }
        )
        .ToArray(),
    };
    QuietEtaMapFixture.Reply(
      ...
      original with
      {
        State = original.State with { Eta = fresh },
      }
    );
    component.WaitForAssertion(
      () =>
