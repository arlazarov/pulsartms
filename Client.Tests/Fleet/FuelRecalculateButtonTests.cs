...
public sealed class FuelRecalculateButtonTests
{
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task SwitchingAwayAndBackRejectsOldCompletionWithoutClearingNewBusyState(
    bool failure
  )
  {
    ...
    await component.InvokeAsync(
      () => component.Render(p => p.Add(x => x.DispatchId, Guid.NewGuid()))
    );
    Assert.True(old.Token.IsCancellationRequested);
    Assert.False(component.Find("button").HasAttribute("disabled"));
    await component.InvokeAsync(
      () => component.Render(p => p.Add(x => x.DispatchId, fixture.Dispatch))
    );
  ...
  private sealed class Pending(CancellationToken token)
  {
    ...
    public void Reply(AutomaticPlanningResult result, bool failure = false) =>
      Completion.SetResult(
        new(HttpStatusCode.OK)
        {
          Content = JsonContent.Create(
            ...
            {
              Success = !failure,
              Response = failure ? null : result,
              Errors = failure ? ["Fixture failure"] : [],
            }
          ),
        }
      );
  ...
  private sealed class Fixture : IDisposable
  {
    ...
    public Channel<Pending> Requests { get; } =
    ...
    public IRenderedComponent<FuelRecalculateButton> Render() =>
      Context.Render<FuelRecalculateButton>(p =>
        p.Add(x => x.DispatchId, Dispatch)
          .Add(
            x => x.Recalculated,
            (AutomaticPlanningResult result) => Results.Add(result)
          )
          .Add(x => x.BusyChanged, (bool busy) => Busy.Add(busy))
          .Add(x => x.Failed, () => Failures++)
      );

    public Task<Pending> NextAsync() =>
      Requests.Reader.ReadAsync().AsTask().WaitAsync(TimeSpan.FromSeconds(5));

    public AutomaticPlanningResult Result() =>
      new(Truck, Dispatch, 1, null, null);
