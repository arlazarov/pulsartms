...
public sealed class FleetRouteDisplayMemoryTests
{
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void InvalidatedInputsDiscardForecastAndProgressEvenWhenOldStopIdentityIsUnchanged(
    bool oldForecastReturned
  )
  {
    ...
    saved.Plan!.InputsChanged = true;
    var invalidated = saved with
    {
      Eta = oldForecastReturned ? saved.Eta : Pending(),
    };

    Assert.False(
      memory.IsRetaining(invalidated, Now.AddSeconds(10), refreshing: true)
    );
    memory.Update(invalidated, Now.AddSeconds(10));
    var display = memory.Display(
      invalidated,
      Now.AddSeconds(10),
      refreshing: true
    )!;
    Assert.Null(display.Eta);
    Assert.Null(display.Progress);
    Assert.Null(memory.RetainedRemainingMiles);
    Assert.Null(memory.RetainedNextStopMiles);
    Assert.Same(saved.Plan, display.Plan);
    Assert.NotNull(invalidated.Progress);

    saved.Plan.InputsChanged = false;
    var pending = saved with { Eta = Pending(), Progress = null };
    memory.Update(pending, Now.AddSeconds(20));
    Assert.Empty(memory.Display(pending, Now.AddSeconds(20))!.Eta!.Stops);
    Assert.False(memory.IsRetaining(pending, Now.AddSeconds(20)));
    var ready = saved with
    {
      Eta = saved.Eta! with { CalculatedAt = Now.AddSeconds(30) },
    };
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void PendingAndNullForecastsKeepTheLastDisplayedPairWithoutMutatingTheResponse(
    bool missing
  )
  {
    var memory = new FleetRouteDisplayMemory();
    ...
    var ready = saved with
    {
      ...
      Eta = saved.Eta with
      {
        CalculatedAt = Now.AddMinutes(4),
        ValidUntil = Now.AddMinutes(6),
      },
    };
  ...
  public void RepeatedPendingResponsesCannotSlideTheOriginalGraceDeadline()
  {
    ...
    memory.Update(pending, Now.AddMinutes(3));
    var repeated = pending with
    {
      Eta = Pending() with
      {
        CalculatedAt = Now.AddMinutes(16),
        ValidUntil = Now.AddMinutes(18),
      },
    };
    ...
    Assert.False(
      FleetRouteDisplayMemory.CanDisplay(before.Eta, Now.AddMinutes(17))
    );
  }
  ...
  public void ExplicitUnavailableForecastDoesNotReuseEarlierEstimates()
  {
    ...
    var saved = State();
    memory.Update(saved, Now);
    var unavailable = saved with
    {
      Progress = null,
      Eta = Pending() with { RouteUpdatePending = false },
    };
  ...
  public void ChangedRouteIdentityNeverReceivesThePreviousPair(string changed)
  {
    ...
    switch (changed)
    {
      ...
      case "dispatch":
        ...
        break;
      case "stop":
        plan.Tracking.NextStopId = Guid.NewGuid();
        break;
      case "passed":
        plan.Tracking.PassedStopIds.Add(Guid.NewGuid());
        break;
      case "complete":
        plan.Tracking.AllStopsPassed = true;
        break;
      case "address":
        plan.Stops[0] = plan.Stops[0] with { Address = "Changed destination" };
        break;
      case "point":
        plan.Stops[0] = plan.Stops[0] with { Point = new(43, -78) };
        break;
      case "appointment":
        plan.Stops[0] = plan.Stops[0] with { ScheduledDate = new(2026, 9, 10) };
        break;
    }
    var pending = saved with { Progress = null, Eta = Pending() };
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void GeometryOnlyChangesRetainEtaAndVisibleDistancesWithoutReusingOldProgressCoordinates(
    bool newPlan
  )
  {
    var memory = new FleetRouteDisplayMemory();
  ...
  public void InFlightReadUsesOriginalGraceBeforeAnyPendingResponseArrives()
  {
    ...
    Assert.Equal(saved.Eta!.ValidUntil, display.Eta.ValidUntil);
    Assert.True(
      FleetRouteDisplayMemory.CanDisplay(display.Eta, Now.AddMinutes(3))
    );
    Assert.False(
      memory.IsRetaining(saved, Now.AddMinutes(17), refreshing: true)
    );
    Assert.False(
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void ChangedDestinationRejectsTheResentOldForecastUntilACompleteNewCalculation(
    bool pending
  )
  {
    var memory = new FleetRouteDisplayMemory();
    var saved = State();
    memory.Update(saved, Now);
    saved.Plan!.Stops[0] = saved.Plan.Stops[0] with
    {
      Address = "New destination",
    };
    var outdated = saved with
    {
      Eta = saved.Eta! with { RouteUpdatePending = pending },
    };
  ...
  public void PersistedPendingForecastUsesItsOriginalBoundWithoutNeedingAnEarlierRender()
  {
    ...
    var pending = saved with
    {
      Eta = saved.Eta! with { RouteUpdatePending = true },
    };
    memory.Update(pending, Now.AddMinutes(3));
    Assert.Equal(
      saved.Eta!.Stops,
      memory.Display(pending, Now.AddMinutes(3))!.Eta!.Stops
    );
    Assert.Null(memory.Display(pending, Now.AddMinutes(17))!.Eta);
    Assert.False(
      FleetRouteDisplayMemory.CanDisplay(saved.Eta, Now.AddMinutes(2))
    );
  }
  ...
  private static DispatchEta Pending() =>
    new(Now, Now.AddMinutes(2), [], "Route updating", [])
    {
  ...
  private static RoutePlanningState State()
  {
    ...
    )
    {
      Eta = new(
        ...
        Now.AddMinutes(2),
        [
          new(stop, Now.AddHours(10), "UTC", null, 0, 600, 0)
          {
            DispatchId = dispatch,
          },
        ],
