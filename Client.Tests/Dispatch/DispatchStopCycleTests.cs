...
public sealed class DispatchStopCycleTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task EachStopOwnsASeparateArrivalCycleSectionWithoutDepartureBalance(bool current)
    {
        await using var context = new ClientComponentContext((_, _) =>
            throw new InvalidOperationException("Cycle sections must use the board forecast."));
        context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
        var load = Load();
        load.Eta = load.Eta! with { Stops = load.Eta.Stops.Select((stop, index) => stop with
        {
            Hours = new StopHoursForecast(index == 0 ? 600 : 300, index == 0 ? 480 : -120,
                null, null, true, [], null)
        }).Reverse().ToArray() };
        var component = context.Render<DispatchLoadCard>(p => p.Add(x => x.Load, load).Add(x => x.Current, current));
        ...
        {
            var cycle = Assert.Single(stop.QuerySelectorAll("section.stop-hours__cycle"));
            Assert.Equal("Cycle remaining", cycle.QuerySelector("h3")!.TextContent);
            Assert.Null(cycle.QuerySelector(".stop-hours__road"));
        }
        Assert.Equal("+10h 00m", stops[0].QuerySelector(".stop-hours__arrival-cycle strong")!.TextContent);
        Assert.Null(stops[0].QuerySelector(".stop-hours__departure-cycle"));
        Assert.Equal("+5h 00m", stops[1].QuerySelector(".stop-hours__arrival-cycle strong")!.TextContent);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task CurrentAndFutureCardsShowEachStopsOwnCycleWithoutRequests(bool current)
    {
        var requests = 0;
        await using var context = new ClientComponentContext((_, _) =>
        {
            requests++;
            throw new InvalidOperationException("Stop cycle presentation must use the board forecast.");
        });
        context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
        var load = Load();
        load.Eta = load.Eta! with { Stops = load.Eta.Stops.Reverse().ToArray() };
        var component = context.Render<DispatchLoadCard>(parameters => parameters
            .Add(card => card.Load, load).Add(card => card.Current, current));

        Assert.Empty(component.FindAll(".dispatch-load__stop-cycle, details"));
        await OpenDetailsAsync(context, component);
        var rows = component.FindAll(".dispatch-load-dialog .dispatch-load__stop-cycle");
        Assert.Equal(2, rows.Count);
        Assert.All(rows, row => Assert.Contains("Cycle after stop", row.TextContent));
    ...
    public void WrongOrEmptyIdentityCannotShowAnotherStopsCycle(string mismatch)
    {
        ...
        var load = Load();
        if (mismatch == "dispatch") load.Id = Guid.NewGuid();
        if (mismatch == "stop") load.Stops[0].Id = Guid.NewGuid();
        if (mismatch == "empty-dispatch")
        {
            load.Id = Guid.Empty;
            load.Eta = load.Eta! with { Stops = load.Eta.Stops.Select(stop => stop with { DispatchId = Guid.Empty }).ToArray() };
        }
        if (mismatch == "empty-stop")
        {
            load.Stops[0].Id = Guid.Empty;
            load.Eta = load.Eta! with { Stops = [load.Eta.Stops[0] with { StopId = Guid.Empty }, load.Eta.Stops[1]] };
        }
    ...
    public void MatchingStopIsSelectedByBothIdsEvenWhenWrongDispatchAppearsFirst()
    {
        ...
        {
            Stops = [load.Eta.Stops[0] with { DispatchId = Guid.NewGuid(), CycleAfterDeparture = Cycle(5) }, .. load.Eta.Stops]
        };
    ...
    [InlineData(-1, null)]
    [InlineData(0, "~0h 00m")]
    public void MissingInvalidAndExhaustedCycleAreDistinct(int? remaining, string? expected)
    {
        using var context = Context();
        var load = Load();
        load.Eta = load.Eta! with
        {
            Stops = [load.Eta.Stops[0] with { CycleAfterDeparture = remaining is { } minutes ? Cycle(minutes) : null }]
        };

        var component = Render(context, load);
        if (expected is null) Assert.Empty(component.FindAll(".dispatch-load__stop-cycle"));
        else Assert.Equal(expected, Value(component));
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void ChangedStopOrDispatchCannotInheritPendingCycle(bool changeDispatch)
    {
        using var context = Context();
        var load = Load();
        var component = Render(context, load);
        if (changeDispatch) load.Id = Guid.NewGuid();
        else load.Stops[0].Id = Guid.NewGuid();
        load.Eta = load.Eta! with { Stops = [], RouteUpdatePending = true };
        Update(component, load);
    ...
    public void PendingMatchingForecastKeepsTheCompleteCycleUntilAReadySnapshotReplacesIt()
    {
        ...
        load.Eta = load.Eta! with
        {
            ...
            Stops = [load.Eta.Stops[0] with { CycleAfterDeparture = null }], RouteUpdatePending = true
        };
        Update(component, load);
    ...
    private static IRenderedComponent<DispatchLoadStop> Render(BunitContext context, DispatchResponse load) =>
        context.Render<DispatchLoadStop>(parameters => parameters.Add(stop => stop.DispatchId, load.Id)
            .Add(stop => stop.Stop, load.Stops[0]).Add(stop => stop.Number, 1).Add(stop => stop.Eta, load.Eta));
    ...
    private static StopCycleForecast Cycle(int remaining) => new(remaining, null, null, null, false);
    ...
    private static DispatchResponse Load()
    {
        var load = new DispatchResponse
        {
            ...
            Stops = Enumerable.Range(1, 2).Select(index => new DispatchStopResponse
            {
                ...
                Name = $"Facility {index}", City = "City", Province = "CA"
            }).ToList()
        };
        load.Eta = new(Start.UtcDateTime, Start.AddMinutes(2).UtcDateTime,
            load.Stops.Select((stop, index) => new StopEta(stop.Id, Start.AddHours(index * 2),
                "America/Los_Angeles", null, 0, 60, 0)
            {
                DispatchId = load.Id, Departure = Start.AddHours(index * 2 + 2),
