...
public sealed class DispatchPlanningRetentionTests
{
  ...
  public void BoardHeaderGroupsTheDrivingStatusAndPumpReadingWithoutAnotherDataRequest()
  {
    ...
    {
      State = source.State! with { FuelPercent = 28 },
    };
    var reads = 0;
    ...
    var component = context.Render<DispatchPlanning>(p =>
      p.Add(x => x.TruckId, load.TruckId)
        .Add(x => x.Load, load)
  ...
  public void CompactHeaderShowsAuthoritativeTruckRecapBesideStatusEvenWhileRouteIsPending()
  {
    ...
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    var snapshot = new DriverCycleSnapshot(
      Start.UtcDateTime,
    ...
    var component = context.Render<DispatchPlanning>(p =>
      p.Add(x => x.TruckId, load.TruckId)
        .Add(x => x.Load, load)
        .Add(x => x.Compact, true)
        .Add(x => x.CurrentCycle, snapshot)
        .Add(
          x => x.Hos,
          new() { CurrentDutyStatus = "driving", UpdatedAt = Start.UtcDateTime }
        )
    );
    ...
    component.WaitForAssertion(() =>
    {
      Assert.Equal(
        "Sep 11",
        component.Find(".driver-next-recap time").TextContent
      );
      Assert.Contains(
        "+3h 05m",
      ...
      Assert.Contains(
        "Driving",
        component.Find(".dispatch-planning__driver .driver-duty").TextContent
      );
      Assert.Empty(
        component.FindAll(
          ".driver-hours-panel .driver-duty, .dispatch-planning__details"
        )
      );
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void MaintenanceMessageIsQuietOnlyForAValidRouteAndDoesNotHideProviderWarnings(
    bool inputsChanged
  )
  {
    var load = Load();
    ...
    [
      "Route includes a restricted approach.",
    ];
    using var context = new ClientComponentContext(
      (_, _) => Task.FromResult(Json(result))
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    var component = context.Render<DispatchPlanning>(p =>
      p.Add(x => x.TruckId, load.TruckId).Add(x => x.Load, load)
    );
    ...
    component.WaitForAssertion(() =>
    {
      ...
      if (inputsChanged)
        Assert.Contains("Route update pending.", component.Markup);
      else
        Assert.DoesNotContain("Route update pending.", component.Markup);
    });
  }
  ...
  public void TransientRefreshFailureIsQuietOnlyWhileThePreviousEstimateIsWithinItsGrace()
  {
    ...
    context
      ...
      .Store($"api/fleet/trucks/{load.TruckId}/planning", Result(load));
    var component = context.Render<DispatchPlanning>(p =>
      p.Add(x => x.TruckId, load.TruckId).Add(x => x.Load, load)
    );
    component.WaitForAssertion(() =>
    ...
    Assert.Contains(
      ...
      component.Find(".dispatch-planning__hint").TextContent
    );
  }
  ...
  public void LatestLoadStopChangeCannotUseTheOlderCachedPlanForecast(
    string changedField
  )
  {
    var load = Load();
    ...
    if (changedField == "address")
      load.Stops[0].Address = "200 Other St";
    else
      load.Stops[0].ScheduledTime = new(15, 0);
  ...
  public void MissingPlanDuringRecalculationKeepsSummaryAndArrivalOnlyWithinTheExistingGrace()
  {
    ...
    var reads = 0;
    using var context = new ClientComponentContext(
      (_, _) => ++reads == 1 ? Task.FromResult(Json(pending)) : held.Task
    );
    context.Services.AddSingleton<TimeProvider>(clock);
    context
      .Services.GetRequiredService<PlanningDisplayCache>()
      .Store($"api/fleet/trucks/{load.TruckId}/planning", complete);
    var component = context.Render<DispatchPlanning>(p =>
      p.Add(x => x.TruckId, load.TruckId).Add(x => x.Load, load)
    );
    component.WaitForAssertion(() =>
    {
      Assert.True(
        component
          .FindComponent<ArrivalEstimate>()
          .Instance.Eta?.RouteUpdatePending
      );
      Assert.Contains(
        "100 mi",
        component.Find(".dispatch-planning__metrics").TextContent
      );
      Assert.Contains(
        "75 mi",
        component.Find(".dispatch-planning__metrics").TextContent
  ...
  public async Task ReadyReplacementAtomicallyReplacesTheRetainedSummaryAndEstimate()
  {
    ...
    using var context = new ClientComponentContext(
      ...
      {
        if (++reads == 1)
          return Task.FromResult(Json(Pending(complete)));
  ...
  public void CompletingTheRetainedNextStopRemovesItsEstimateAndPendingSummaryImmediately()
  {
    ...
    var complete = Result(load);
    using var context = new ClientComponentContext(
      (_, _) => Task.FromResult(Json(Pending(complete)))
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    context
      .Services.GetRequiredService<PlanningDisplayCache>()
  ...
  public void NewCurrentLoadOnTheSameTruckCannotShowOrRetainThePreviousLoad()
  {
    ...
    var reads = 0;
    using var context = new ClientComponentContext(
      (_, _) => ++reads == 1 ? Task.FromResult(Json(Result(first))) : held.Task
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    var component = context.Render<DispatchPlanning>(p =>
      p.Add(x => x.TruckId, first.TruckId).Add(x => x.Load, first)
    );
    component.WaitForAssertion(
      () =>
        Assert.Contains(
  ...
  public void NonPendingUnavailableResultDoesNotRetainTheOldSummary()
  {
    ...
    {
      State = unavailable.State! with
      {
        Eta = unavailable.State!.Eta! with { RouteUpdatePending = false },
      },
    };
    using var context = new ClientComponentContext(
      (_, _) => Task.FromResult(Json(unavailable))
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    context
      .Services.GetRequiredService<PlanningDisplayCache>()
  ...
  private static DispatchResponse Load(Guid? truck = null) =>
    new()
    {
      Id = Guid.NewGuid(),
      TruckId = truck ?? Guid.NewGuid(),
      Stops =
      [
        new()
        {
  ...
  )
  {
    ...
    var plan = new RoutePlan
    {
      ...
      Version = 1,
      OriginalPlannedMiles = miles,
      Stops = [new(stop.Id, "Delivery", "Warehouse", 1, new(40, -80))],
      Tracking = new()
      {
        NextStopId = stop.Id,
        NextStopLabel = "Delivery · Warehouse",
      },
    };
    var eta = new DispatchEta(
      Start.AddSeconds(hour).UtcDateTime,
      Start.AddMinutes(2).UtcDateTime,
      [
        new(stop.Id, Start.AddHours(hour - 12), "UTC", null, 0, 60, 0)
        {
          DispatchId = load.Id,
        },
      ],
  ...
  private static AutomaticPlanningResult Pending(
