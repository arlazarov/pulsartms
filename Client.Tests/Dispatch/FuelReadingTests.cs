...
public sealed class FuelReadingTests
{
    ...
    public void PumpPillFormatsTheGivenReadingWithoutInventingAMissingValue(double? percent, string value, string tone)
    {
        using var context = new BunitContext();
        var component = context.Render<FuelReading>(p => p.Add(reading => reading.Percent, percent));
        ...
        Assert.Equal("true", component.Find(".fuel-reading__icon").GetAttribute("aria-hidden"));
        Assert.Empty(component.FindAll("button, a, input"));
        Assert.DoesNotContain("fuel-reading--metric", component.Find(".fuel-reading").ClassList);
    }
    ...
    [InlineData(100d, "100%", "is-normal")]
    [InlineData(null, "—", "is-unknown")]
    public void MetricVariantPreservesTheReadingAndToneWithASeparateLabel(double? percent, string value, string tone)
    {
        using var context = new BunitContext();
        var component = context.Render<FuelReading>(p => p.Add(reading => reading.Percent, percent)
            .Add(reading => reading.Variant, "metric"));
        Assert.Contains("fuel-reading--metric", component.Find(".fuel-reading").ClassList);
        Assert.Contains(tone, component.Find(".fuel-reading").ClassList);
