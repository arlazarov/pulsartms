...
public sealed class DispatchRefreshRetentionTests
{
  ...
  public async Task DelayedBoardFailureAndPendingRecoveryKeepMountedCardsUntilAtomicReplacement()
  {
    ...
    component.WaitForAssertion(() => Assert.Equal(2, component.FindAll(".arrival-estimate__ontime").Count));
    ...
    component.WaitForAssertion(() => Assert.True(component.FindComponent<DispatchLoadCard>().Instance.Refreshing));
    ...
    fixture.Load.Eta = fixture.Forecast() with { Stops = [], RouteUpdatePending = true };
    recovering.SetResult(fixture.Board());
    component.WaitForAssertion(() => Assert.True(component.FindComponent<DispatchLoadCard>().Instance.Load.Eta!.RouteUpdatePending));
    Assert.Same(card, component.FindComponent<DispatchLoadCard>().Instance);
    Assert.Equal(previous, component.FindComponent<DispatchLoadCard>().Markup);
    ...
    component.WaitForAssertion(() => Assert.Equal(2, component.FindAll(".arrival-estimate__late").Count));
    Assert.Empty(component.FindAll(".arrival-estimate__ontime, .dispatch-load__eta-missing"));
    Assert.Same(card, component.FindComponent<DispatchLoadCard>().Instance);
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task NullForecastAfterItsDeadlineRetainsQuietlyButExplicitUnavailabilityDoesNot(bool unavailable)
  {
    using var fixture = new DispatchRefreshFixture();
    ...
    await component.InvokeAsync(() => fixture.Clock.Advance(TimeSpan.FromMinutes(2)));
    fixture.Load.Eta = unavailable ? original with { CalculatedAt = original.CalculatedAt.AddSeconds(-1),
      Stops = [], RouteUpdatePending = false, UnavailableReason = "Position unavailable" } : null;
    pending.SetResult(fixture.Board());
    component.WaitForAssertion(() => Assert.False(component.FindComponent<DispatchLoadCard>().Instance.Refreshing));
    ...
    if (unavailable)
    {
      component.WaitForAssertion(() =>
      {
        ...
        Assert.False(component.FindComponent<DispatchLoadCard>().Instance.Load.Eta!.RouteUpdatePending);
      });
      return;
    }
    Assert.Equal(previous, component.FindComponent<DispatchLoadCard>().Markup);
    var retained = component.FindComponent<DispatchLoadCard>().Instance.Load.Eta!;
    ...
    await component.InvokeAsync(() => fixture.Clock.Advance(TimeSpan.FromMinutes(15)));
    component.FindComponent<DispatchLoadCard>().Render();
    Assert.Empty(component.FindAll(".arrival-estimate__ontime"));
  }
  ...
  public async Task AccessDeniedClearsThePreviousBoardInsteadOfRetainingProtectedInformation(HttpStatusCode status)
  {
    using var fixture = new DispatchRefreshFixture();
    var component = fixture.Render();
    ...
    component.WaitForAssertion(() => Assert.Empty(component.FindComponents<DispatchLoadCard>()));
  ...
  public async Task FailedChangedSearchDoesNotShowThePreviousBoardAsTheNewSearchResult()
  {
    ...
    component.WaitForAssertion(() => Assert.Single(component.FindComponents<DispatchLoadCard>()));
    fixture.DeferBoard = true;
    var search = component.Find("#dispatch-search").InputAsync(new ChangeEventArgs { Value = "another truck" });
    await component.InvokeAsync(() => fixture.Clock.Advance(TimeSpan.FromMilliseconds(300)));
    (await fixture.NextRequest()).SetResult(new(HttpStatusCode.ServiceUnavailable));
    await search;
    component.WaitForAssertion(() => Assert.Empty(component.FindComponents<DispatchLoadCard>()));
    Assert.Single(component.FindAll("[role=alert]"));
  }
