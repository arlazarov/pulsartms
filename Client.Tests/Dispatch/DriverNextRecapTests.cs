...
public sealed class DriverNextRecapTests
{
  ...
  public void VerifiedCurrentDriverRecapShowsOnlyHomeDateAndCreditedHours()
  {
    ...
    var component = context.Render<DriverNextRecap>(p =>
      p.Add(x => x.Snapshot, Snapshot())
    );
  ...
  public void UnavailableSnapshotKeepsOneQuietLabeledPlaceholder(string invalid)
  {
    ...
    {
      "missing" => null,
      "unverified" => snapshot with
      {
        Cycle = snapshot.Cycle with { RecapVerified = false },
      },
      "no-time" => snapshot with
      {
        Cycle = snapshot.Cycle with { NextRecapAt = null },
      },
      "no-hours" => snapshot with
      {
        Cycle = snapshot.Cycle with { NextRecapMinutes = null },
      },
      "zero" => snapshot with
      {
        Cycle = snapshot.Cycle with { NextRecapMinutes = 0 },
      },
      "returned" => snapshot with
      {
        Cycle = snapshot.Cycle with { NextRecapAt = Start },
      },
      "expired" => snapshot with
      {
        CalculatedAt = Start.AddMinutes(-20).UtcDateTime,
    ...
    Assert.Equal(
      "Next recap",
      component.Find(".driver-next-recap > span").TextContent
    );
    ...
    component.Render(p =>
      p.Add(
        ...
        snapshot with
        {
          ...
          Cycle = snapshot.Cycle with { NextRecapMinutes = 60 },
        }
      )
    );
  ...
  public void ReturnedRecapOrRemovedTruckSnapshotCannotKeepThePreviousValue()
  {
    ...
    {
      Cycle = snapshot.Cycle with
      {
        NextRecapAt = Start.AddMinutes(5),
        NextRecapMinutes = 60,
      },
    };
  ...
  private static DriverCycleSnapshot Snapshot() =>
    new(
      Start.UtcDateTime,
