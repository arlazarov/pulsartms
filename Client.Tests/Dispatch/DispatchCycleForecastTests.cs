...
public sealed class DispatchCycleForecastTests
{
    ...
    public async Task PerStopHoursKeepPickupDeliveryCyclesInTheDialogWithoutRepeatingTheTruckRecap()
    {
        ...
        {
            Stops = load.Eta.Stops.Select(stop => stop with { Hours = new(870, 750, 0, null, true, [], null) }).ToArray()
        };
        var component = context.Render<DispatchLoadCard>(p => p.Add(x => x.Load, load));
        Assert.Empty(component.FindAll(".stop-hours__arrival-cycle, .dispatch-load__more-details"));
        await OpenDetailsAsync(context, component);
        Assert.Equal(2, component.FindAll(".dispatch-load-dialog .stop-hours__arrival-cycle").Count);
        Assert.Empty(component.FindAll(".stop-hours__departure-cycle"));
        Assert.Empty(component.FindAll(".dispatch-load__cycle"));
        Assert.Empty(component.FindAll(".dispatch-load__more-details"));
    ...
    public async Task FutureCardKeepsLegacyFinalDepartureCycleInTheDialogWithoutRepeatingRecapOrMakingRequests()
    {
        ...
        {
            requests++;
            throw new InvalidOperationException("Cycle presentation must use the board forecast.");
        });
        ...
        await OpenDetailsAsync(context, component);
        var summary = component.Find(".dispatch-load-dialog .dispatch-load__cycle");
        Assert.Equal(2, component.FindAll(".dispatch-load-dialog details:not([open])").Count);
    ...
    public void CurrentCardDoesNotShowTheFutureLoadSummary()
    {
        ...
        var component = context.Render<DispatchLoadCard>(parameters => parameters
            .Add(card => card.Load, Load()).Add(card => card.Current, true));
    ...
    public void MatchingFinalStopMustBelongToTheExactDispatch()
    {
        ...
        var wrongDispatch = final with { DispatchId = Guid.NewGuid(), CycleAfterDeparture = Cycle(999) };
        load.Eta = load.Eta with { Stops = [wrongDispatch, .. load.Eta.Stops] };
        var component = context.Render<DispatchCycleForecast>(parameters => parameters.Add(card => card.Load, load));
    ...
    public void VerifiedHorizonWithoutPositiveRecapIsDistinctFromUnknown()
    {
        using var context = Context();
        var load = Load();
        SetBaselineCycle(load, BaselineCycle() with { NextRecapAt = null, NextRecapMinutes = null });
        var component = context.Render<DispatchCycleForecast>(parameters => parameters.Add(card => card.Load, load));
    ...
    public void PendingUpdateRetainsTheSameForecastOnlyWithinExistingDisplayGrace()
    {
        ...
        clock.Advance(TimeSpan.FromMinutes(3));
        load.Eta = load.Eta! with { Stops = [], RouteUpdatePending = true, CycleAtCalculation = null };
        component.Render(parameters => parameters.Add(card => card.Load, load));
        ...
        load.Eta = complete with { CalculatedAt = clock.GetUtcNow().UtcDateTime,
    ...
    public void InitialPendingForecastShowsOnlyCompactEtaWithoutTechnicalReasonsOrEmptyCycleSections()
    {
        using var context = Context();
        var load = Load();
        const string reason = "ETA unavailable: waiting for the saved preceding connection.";
        load.Eta = load.Eta! with { Stops = [], RouteUpdatePending = true, CycleAtCalculation = null,
            UnavailableReason = reason };
        var component = context.Render<DispatchLoadCard>(parameters => parameters.Add(card => card.Load, load));

        Assert.Equal(2, component.FindAll(".dispatch-load__eta-missing").Count);
        Assert.All(component.FindAll(".dispatch-load__eta-missing"), value => Assert.Null(value.GetAttribute("title")));
        Assert.Empty(component.FindAll(".dispatch-load__stop-cycle, .dispatch-load__cycle"));
    ...
    public void PendingForecastDoesNotKeepRecapWhoseReturnTimeHasPassed()
    {
        ...
        using var context = Context(clock);
        var load = Load();
        SetBaselineCycle(load, BaselineCycle() with { NextRecapAt = Start.AddMinutes(5).ToOffset(TimeSpan.FromHours(-4)) });
        var component = context.Render<DispatchCycleForecast>(parameters => parameters.Add(card => card.Load, load));
        Assert.Empty(component.FindAll("time"));
    ...
    public void AnotherFinalStopOrLoadCannotInheritRetainedCycle()
    {
        ...
        var load = Load();
        var component = context.Render<DispatchCycleForecast>(parameters => parameters.Add(card => card.Load, load));
        load.Stops.Add(new() { Id = Guid.NewGuid(), Sequence = 3, Job = "Delivery" });
    ...
    private static StopCycleForecast Cycle(int remaining = 750) =>
        new(remaining, new(2026, 9, 13, 0, 0, 0, TimeSpan.FromHours(-4)), 798, "America/New_York", true);
    ...
    private static void SetBaselineCycle(DispatchResponse load, StopCycleForecast? cycle) =>
    ...
    private static DispatchResponse Load()
    {
        ...
        load.Eta = new(Start.UtcDateTime, Start.AddMinutes(2).UtcDateTime,
            ...
            }).ToArray(), null, []) { CycleAtCalculation = BaselineCycle() };
        return load;
    }
}
