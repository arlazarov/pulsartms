...
public sealed class DispatchLoadCardTests
{
  ...
  public void FooterShowsOnlyEligibleLivePlanningValues(
    bool current,
    bool completed,
    bool remainingVisible,
    bool fuelVisible
  )
  {
    ...
    Assert.Equal(
      remainingVisible ? 1 : 0,
      component.FindAll(".dispatch-load__remaining").Count
    );
    Assert.Equal(
      fuelVisible ? 1 : 0,
      component.FindAll(".dispatch-load__fuel-count").Count
    );
    if (remainingVisible)
      Assert.Equal(
        "302\u00a0mi · 486\u00a0km remaining",
        component.Find(".dispatch-load__remaining").TextContent
      );
    if (fuelVisible)
      Assert.Contains(
        "Fuel stops 2",
        component.Find(".dispatch-load__fuel-count").TextContent
      );
  ...
  public void UnknownFooterPlanningIsNotShownAsZeroOrReplacedByTheTotalLoadDistance()
  {
    ...
    component.Render(p =>
      p.Add(card => card.RemainingMiles, double.NaN)
        .Add(card => card.FuelStopCount, 0)
    );
  ...
  public async Task CompactStopsKeepPrimaryInformationVisibleAndOpenSupplementalDetailsWithoutRequests()
  {
    ...
    await using var context = new ClientComponentContext(
      (_, _) =>
      {
        ...
        throw new InvalidOperationException(
          "Load details must use the existing board data."
        );
      }
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    ...
    foreach (var stop in component.FindAll(".dispatch-load__stop"))
    {
      Assert.Null(
        stop.QuerySelector(
          "details, .dispatch-load__stop-detail-content, .dispatch-load__reference-number, .dispatch-load__appointment-reference"
        )
      );
      Assert.NotNull(stop.QuerySelector(".dispatch-load__location"));
      if (stop.ClassList.Contains("dispatch-load__stop--completed"))
      {
        Assert.NotNull(stop.QuerySelector(".dispatch-load__history-time"));
        Assert.Null(stop.QuerySelector(".dispatch-load__street"));
        continue;
      }
    ...
    Assert.Equal(
      2,
      component
        .FindAll(".dispatch-load__stop-times .stop-hours__road time")
        .Count
    );
    Assert.Single(
    ...
    Assert.Equal(
      "Current",
      dialog.Find(".dispatch-load-dialog .dispatch-paper__phase").TextContent
    );
    foreach (
      var stop in dialog.FindAll(".dispatch-load-dialog .dispatch-load__stop")
    )
    {
      ...
      Assert.NotNull(stop.QuerySelector(".dispatch-load__more-details"));
      Assert.NotNull(
        stop.QuerySelector("[hidden] .dispatch-load__stop-detail-content")
      );
      Assert.NotNull(stop.QuerySelector(":scope > .dispatch-load__street"));
      Assert.NotNull(stop.QuerySelector(".dispatch-load__reference-number"));
    }
    Assert.Equal(
      2,
      dialog.FindAll(".dispatch-load-dialog .arrival-estimate__timezone").Count
    );
    Assert.Single(
      dialog.FindAll(".dispatch-load-dialog .dispatch-load__stop--completed")
    );
    Assert.Empty(
      dialog.FindAll(
  ...
  [Fact]
  public void LaneCardKeepsEveryStopButShowsMileageAndPricesOnlyInDetails()
  {
    using var context = new BunitContext();
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    var load = Load();
    ...
    Assert.Equal(
      3,
      component
        .FindAll(".dispatch-load__route > .dispatch-load__route-stop")
        .Count
    );
    Assert.Equal(
      new[] { "1", "2", "3" },
    ...
    Assert.Equal(
      new[] { "2 Warehouse Road", "3 Warehouse Road" },
      component
        .FindAll(".dispatch-load__street")
        .Select(item => item.TextContent)
    );
    Assert.Single(component.FindAll(".dispatch-load__stop--completed"));
    Assert.Empty(
    ...
    var dialog = RenderDetails(context, component);
    var financials = dialog.Find(".dispatch-paper__financials").TextContent;
    foreach (
      var value in new[]
      {
        "900\u00a0mi",
  ...
  [Fact]
  public void UnknownEmptyDistanceKeepsADashInDetailsWithoutInternalRoutePreparationTooltips()
  {
    using var context = new BunitContext();
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    var load = Load();
    ...
    var dialog = RenderDetails(context, component);
    var distance = dialog.Find(".dispatch-paper__empty-miles");
    Assert.Equal("Empty 43\u00a0mi · 69\u00a0km", distance.TextContent);
    Assert.Equal(
      "Planned road miles from the previous load's delivery to this pickup",
      distance.GetAttribute("title")
    );
    ...
    foreach (var status in new[] { "pending", "unknown", "" })
    {
      ...
      dialog.Render(p => p.Add(x => x.Load, load));
      distance = dialog.Find(".dispatch-paper__empty-miles");
      Assert.Equal("Empty —", distance.TextContent);
      Assert.Null(distance.GetAttribute("title"));
      Assert.DoesNotContain("The server is preparing", dialog.Markup);
      Assert.DoesNotContain("unambiguous previous load", dialog.Markup);
    }
  }
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task CurrentAndFutureLoadCardsShowAndCopyTheExactOrderNumber(
    bool current
  )
  {
    using var context = new BunitContext();
    ...
    Assert.StartsWith(
      "Open load ",
      component.Find(".dispatch-load__number").GetAttribute("aria-label")
    );
    ...
    Assert.Equal(
      "Copied",
      component
        .Find("button[aria-label='Copy order number']")
        .GetAttribute("title")
    );
  ...
  public async Task FailedOrderCopyShowsRecoverableFeedbackAndAnotherLoadClearsIt()
  {
    ...
    context
      .JSInterop.SetupVoid("navigator.clipboard.writeText", load.OrderNumber)
      .SetException(new JSException("Clipboard unavailable"));
    var component = context.Render<DispatchLoadCard>(parameters =>
    ...
    Assert.Empty(
      component.FindAll("[role='alert'], .dispatch-load__copy-icon.is-copied")
    );
    Assert.Contains(
      next.OrderNumber,
      component.Find("button[aria-label='Copy order number']").TextContent
    );
    next.OrderNumber = " ";
    component.Render(parameters => parameters.Add(card => card.Load, next));
  ...
  public async Task EveryStopShowsItsOwnAppointmentLocalEtaAndLatenessInSequence()
  {
    ...
    {
      Stops = new[] { wrongLoad }.Concat(load.Eta.Stops.Reverse()).ToArray(),
    };
    ...
    Assert.All(
      stops,
      stop => Assert.DoesNotContain("Arrival time zone", stop.TextContent)
    );
    ...
    Assert.Contains(
      "900\u00a0mi",
      dialog.Find(".dispatch-paper__financials").TextContent
    );
    Assert.Contains(
      "2,500.00 CAD",
      dialog.Find(".dispatch-paper__financials").TextContent
    );
    var detailedStops = dialog.FindAll(
      ".dispatch-load-dialog .dispatch-load__stop"
    );
    Assert.Equal(
      expected.Select(stop => stop.Id.ToString()),
      detailedStops.Select(stop => stop.GetAttribute("data-stop-id"))
    );
    Assert.All(
      detailedStops,
  ...
  public void MissingOrWrongLoadAndStopIdentitiesNeverUseTheFirstForecast()
  {
    ...
    load.Eta = load.Eta! with
    {
      ...
      [
        ...
        new(load.Stops[1].Id, Start, "UTC", null, 0, 0, 0)
        {
          DispatchId = Guid.NewGuid(),
        },
        new(load.Stops[2].Id, Start, "UTC", null, 0, 0, 0)
        {
          DispatchId = Guid.Empty,
        },
        new(Guid.NewGuid(), Start, "UTC", null, 0, 0, 0)
        {
          DispatchId = load.Id,
        },
      ],
  ...
  public void CompletedStopsKeepTheirStatusWithoutActualDatesOrRetainedEta()
  {
    ...
    Assert.All(
      component.FindAll(".dispatch-load__stop"),
      stop => Assert.NotNull(stop.QuerySelector("[aria-label='Completed']"))
    );
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task CurrentAndFutureCardsShowOnlyTheMatchingPickupAndDeliveryReferencesWithoutRequests(
    bool current
  )
  {
    var requests = 0;
    await using var context = new ClientComponentContext(
      (_, _) =>
      {
        requests++;
        throw new InvalidOperationException(
          "Appointment references must use the existing stop notes."
        );
      }
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    ...
    const string notes =
      "Shipper appointment confirmation number: PU123456. Receiver appointment confirmation number: DL654321. Shipper BOL: 42845601. Commodity: STEEL COILS. Service for Load sentinel.";
    foreach (var stop in load.Stops)
      stop.Notes = notes;
    ...
    Assert.Empty(
      component.FindAll(
        ".dispatch-load__appointment-reference, .dispatch-load__reference-number"
      )
    );
    var dialog = RenderDetails(context, component);
    ...
    Assert.Equal(
      "Appt # PU123456",
      stops[0]
        .QuerySelector(".dispatch-load__appointment-reference")!
        .TextContent.Trim()
    );
    Assert.Equal(
      "Appt # DL654321",
    ...
    Assert.Equal(
      "Ref # PU-REF / 123",
      stops[0]
        .QuerySelector(".dispatch-load__reference-number")!
        .TextContent.Trim()
    );
    Assert.Equal(
      "Ref # DL-REF / 456",
    ...
    Assert.DoesNotContain(
      "DL-REF / 456",
      stops[0].QuerySelector(".dispatch-load__reference-number")!.TextContent
    );
    Assert.DoesNotContain(
      "PU-REF / 123",
    ...
    var referenceText = string.Join(
      ...
      stops
        .SelectMany(stop =>
          stop.QuerySelectorAll(
            ".dispatch-load__appointment-reference, .dispatch-load__reference-number"
          )
        )
        .Select(reference => reference.TextContent)
    );
    ...
    Assert.Contains(
      notes,
      dialog.Find(".dispatch-load-dialog .dispatch-paper__notes").TextContent
    );
    Assert.DoesNotContain("Actual pickup", dialog.Markup);
    Assert.Equal(
      current,
      stops[0].ClassList.Contains("dispatch-load__stop--completed")
    );
    Assert.Equal(0, requests);
  }
  ...
  public async Task ChangedNotesAndJobRefreshReferencesAndMissingNotesClearThem()
  {
    ...
    load.Stops = [stop];
    stop.Notes =
      "Shipper appointment confirmation number: PU123456. Receiver appointment confirmation number: DL654321.";
    var component = context.Render<DispatchLoadCard>(parameters =>
      parameters.Add(card => card.Load, load)
    );
    var dialog = RenderDetails(context, component);
    Assert.Contains(
      "PU123456",
      dialog.Find(".dispatch-load__appointment-reference").TextContent
    );

    stop.Notes =
      "Shipper appointment confirmation number: PU987654. Receiver appointment confirmation number: DL123456.";
    dialog.Render(parameters => parameters.Add(card => card.Load, load));
    Assert.Contains(
      "PU987654",
      dialog.Find(".dispatch-load__appointment-reference").TextContent
    );
    Assert.DoesNotContain("PU123456", dialog.Markup);
    ...
    Assert.DoesNotContain(
      "PU987654",
      dialog
        .Find(".dispatch-load-dialog .dispatch-load__appointment-reference")
        .TextContent
    );

    stop.Job = "unknown";
    dialog.Render(parameters => parameters.Add(card => card.Load, load));
    Assert.Empty(dialog.FindAll(".dispatch-load__appointment-reference"));
    ...
    dialog.Render(parameters => parameters.Add(card => card.Load, load));
    Assert.Empty(dialog.FindAll(".dispatch-load__appointment-reference"));
    Assert.DoesNotContain("DL123456", dialog.Markup);
    Assert.Single(
      dialog.FindAll(".dispatch-load-dialog .dispatch-load__stop-number")
    );
  }
  ...
  public async Task StructuredStopReferencesUpdateAndClearWithoutBecomingAppointmentNumbers()
  {
    ...
    Assert.Equal(
      "Ref # PU-REF / 123",
      dialog.Find(".dispatch-load__reference-number").TextContent.Trim()
    );
    Assert.Empty(dialog.FindAll(".dispatch-load__appointment-reference"));
    ...
    Assert.DoesNotContain("PU-REF / 123", dialog.Markup);

    stop.StopNo = " ";
    dialog.Render(parameters => parameters.Add(card => card.Load, load));
    Assert.Empty(dialog.FindAll(".dispatch-load__reference-number"));
    Assert.DoesNotContain("PU-REF / 789", dialog.Markup);
    Assert.Empty(dialog.FindAll(".dispatch-load__appointment-reference"));
  }
  ...
  public void PresentationMakesNoRequestsAndCannotRetainAnotherLoadsEstimate()
  {
    ...
    using var context = new ClientComponentContext(
      (_, _) =>
      {
        requests++;
        throw new InvalidOperationException(
          "Dispatch Cards must use the board forecast."
        );
      }
    );
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
  ...
  [Fact]
  public void CompletedStopsLinkToTheirExactVisitInTheLoadWorkspace()
  {
    using var context = new BunitContext();
    context.Services.AddSingleton<TimeProvider>(new FakeTimeProvider(Start));
    var load = Load();
    ...
    var links = component.FindAll("a.dispatch-load__history-stop");
    Assert.Equal(load.Stops.Count, links.Count);
    Assert.Equal(
      load.Stops.Select(stop => $"/dispatch/{load.Id}?stopId={stop.Id}"),
      links.Select(link => link.GetAttribute("href"))
    );
    Assert.DoesNotContain("StopUrl(", component.Markup);
  }
  ...
  private static DispatchResponse Load()
  {
    var load = new DispatchResponse
    {
      ...
      Price = 2500,
      Currency = "CAD",
      LoadedRatePerMile = 2.78m,
      TotalRatePerMile = 2.5m,
      Stops = Enumerable
        .Range(1, 3)
        .Select(index => new DispatchStopResponse
        {
          ...
          Name = index == 2 ? "Intermediate warehouse" : $"Facility {index}",
          City = $"City {index}",
          Province = "ON",
          ScheduledDate = new(2026, 9, 9),
    ...
    load.Eta = new(
      ...
      load.Stops.Select(
          (stop, index) =>
            new StopEta(
              stop.Id,
              Start.AddHours(index * 2).ToOffset(TimeSpan.FromHours(-4)),
              "America/Toronto",
              null,
              index == 1 ? 25 : 0,
              60,
