...
public sealed class DispatchNumberSettingsTests
{
    ...
    [InlineData("   ", "")]
    [InlineData(" TMS- ", "TMS-")]
    public async Task SavingUsesTheRevisionAndPreservesExplicitEmptyPrefix(string input, string expected)
    {
        ...
        using var context = new ClientComponentContext(async (request, ct) =>
        {
            ...
            if (request.Method == HttpMethod.Get) return Response(new("AMF", 3, null));
            writes.Add((await request.Content!.ReadFromJsonAsync<DispatchSettingsUpdate>(ct))!);
            return Response(new(writes[^1].LoadNumberPrefix, 4, null));
        });
        var component = context.Render<CascadingValue<Action<DispatchSettingsState>>>(parameters => parameters
        ...
        Assert.Equal(expected, component.Find("#settings-load-prefix").GetAttribute("value") ?? "");
        Assert.Contains("Load numbering saved", component.Markup);
    }
    ...
    public async Task ConflictPreservesDraftUntilExplicitReload()
    {
        ...
        using var context = new ClientComponentContext((request, _) => Task.FromResult(
            request.Method == HttpMethod.Get ? Response(++reads == 1 ? new("AMF", 3, null) : new("OTHER-", 4, null))
            : new HttpResponseMessage(HttpStatusCode.Conflict)
            {
                Content = JsonContent.Create(new RequestResponseDTO<DispatchSettingsState>
                    { Success = false, Errors = ["Settings changed. Reload before saving."] })
            }));
        var component = context.Render<DispatchNumberSettings>();
        component.WaitForElement("#settings-load-prefix").Change("DRAFT-");
        await component.Find("form").SubmitAsync(EventArgs.Empty);
        Assert.Equal("DRAFT-", component.Find("#settings-load-prefix").GetAttribute("value"));
        Assert.Empty(component.FindAll(".settings-page__saved"));
        await component.Find("[role=alert] button").ClickAsync(new());
        Assert.Equal("OTHER-", component.Find("#settings-load-prefix").GetAttribute("value"));
    }
    ...
    public async Task InvalidLengthDoesNotWrite()
    {
        ...
        var component = context.Render<DispatchNumberSettings>();
        component.WaitForElement("#settings-load-prefix").Change(new string('A', 17));
        await component.Find("form").SubmitAsync(EventArgs.Empty);
        Assert.Equal(0, writes);
        Assert.Contains("Use at most 16 characters", component.Markup);
    }

    internal static HttpResponseMessage Response(DispatchSettingsState state) => new(HttpStatusCode.OK)
        { Content = JsonContent.Create(new RequestResponseDTO<DispatchSettingsState> { Success = true, Response = state }) };
}
