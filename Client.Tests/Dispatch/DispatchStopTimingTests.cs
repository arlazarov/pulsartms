...
public sealed class DispatchStopTimingTests
{
  ...
  public void StopUsesItsOwnSavedRoadEtaAndHoursWithoutRequestsOrExplanationBlocks()
  {
    ...
    using var context = new ClientComponentContext(
      (_, _) =>
      {
        ...
        throw new InvalidOperationException(
          "Stop timing must use the existing board forecast."
        );
      }
    );
    ...
    Assert.Equal(
      "ETA",
      timing.QuerySelector(".stop-hours__road .stop-hours__label")!.TextContent
    );
    Assert.Contains(
      "+14h 30m",
    ...
    Assert.Empty(
      component.FindAll(".dispatch-load__timing, .dispatch-load__stop-cycle")
    );
  ...
  public void OnTimeOrUnknownArrivalDoesNotClaimADelayReason(int? lateMinutes)
  {
    ...
    var component = Render(context, load);
    Assert.Empty(
      component.FindAll(".dispatch-load__timing, .stop-hours__status--danger")
    );
    Assert.Equal(
      lateMinutes == 0 ? 1 : 0,
      component.FindAll(".stop-hours__status--success").Count
    );
  }
    ...
    load.Eta = load.Eta with
    {
      ...
      [
        exact with
        {
          ...
          Hours = exact.Hours! with { CycleAtArrivalMinutes = 6000 },
        },
        .. load.Eta.Stops,
      ],
    };
  ...
  public void PendingPartialForecastRetainsRoadArrivalAndArrivalCycleUntilReplacementIsComplete()
  {
    ...
    var load = Load();
    var component = Render(context, load);
    var previousArrivalCycle = component
      .Find(".stop-hours__arrival-cycle")
      .TextContent;
    Assert.Empty(component.FindAll(".stop-hours__departure-cycle"));
    ...
    load.Eta = load.Eta! with
    {
      ...
      Stops =
      [
        load.Eta.Stops[^1] with
        {
          Arrival = Start.AddDays(5),
          DrivingMinutes = 1,
          RestMinutes = 0,
    ...
    Assert.Equal(
      previousArrivalCycle,
      component.Find(".stop-hours__arrival-cycle").TextContent
    );
    Assert.Empty(component.FindAll(".stop-hours__departure-cycle"));
    Assert.Contains(
      "Sep 10 · 07:05 AM",
      component.Find(".dispatch-load__stop").TextContent
    );
    ...
    load.Eta = load.Eta with
    {
      ...
      [
        load.Eta.Stops[0] with
        {
          DrivingMinutes = 600,
          RestMinutes = 1200,
          CycleAfterDeparture = new(500, null, null, null, false),
          Hours = new(620, 500, 0, null, true, [], null),
        },
      ],
    };
    Update(component, load);
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void AnotherStopOrDispatchCannotInheritThePreviousPlan(
    bool changeDispatch
  )
  {
    using var context = Context();
  ...
  private static IRenderedComponent<DispatchLoadStop> Render(
    ...
    context.Render<DispatchLoadStop>(parameters =>
      parameters
        ...
        .Add(stop => stop.Stop, load.Stops[^1])
        .Add(stop => stop.Number, 2)
    ...
    load.Eta = new(
      ...
      load.Stops.Select(
          (stop, index) =>
            new StopEta(
              stop.Id,
              new(2026, 9, 10, 7, 5, 0, TimeSpan.FromHours(-4)),
              "America/New_York",
              null,
              65,
              index == 0 ? 135 : 780,
