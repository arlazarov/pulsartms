...
public class ClientRefreshTests
{
  ...
  public async Task ConcurrentUnauthorizedRequestsShareRefresh(
  ...
  )
  {
    ...
    using var client = new HttpClient(
      new AuthHeaderHandler(storage, new(storage, services))
      {
    ...
    var responses = await Task.WhenAll(
      Enumerable
        .Range(0, 8)
        .Select(_ => client.GetAsync("https://example.test/api/data"))
    );
  ...
  public async Task TemporaryRefreshFailureKeepsTokens()
  {
    ...
    using var client = new HttpClient(
      new AuthHeaderHandler(storage, new(storage, services))
      {
        InnerHandler = new Transport(HttpStatusCode.ServiceUnavailable),
      }
    );
    await Assert.ThrowsAsync<HttpRequestException>(
      () => client.GetAsync("https://example.test/api/data")
    );
      ...
      if (request.RequestUri!.AbsolutePath == "/api/auth/refresh")
      {
        ...
        return new(status)
        {
          Content = JsonContent.Create(
            new { accessToken = "new", refreshToken = "new-refresh" }
          ),
        };
      }
      return new(
        request.Headers.Authorization?.Parameter == "new"
          ? HttpStatusCode.OK
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task ARequestFromAnOldLoginIsNeverReplayedWithAnotherUsersToken(
    bool switchDuringRefresh
  )
  {
    var storage = new TokenStorageService(new LocalStorageJsRuntime());
    ...
    var pending = client.PostAsync(
      "https://example.test/api/data",
      JsonContent.Create(new { value = "account-a-update" })
    );
    await started.Task;
  ...
  public async Task LegacyTokensAreUpgradedTogetherAndOnlyLoginChangesSessionIdentity()
  {
    ...
    var original = Assert.IsType<TokenStorageService.Session>(
      await storage.GetSessionAsync()
    );
    Assert.Equal(original, await storage.GetSessionAsync());
    Assert.Null(
      await js.InvokeAsync<string?>("localStorage.getItem", "access_token")
    );
  ...
  private sealed class SwitchTransport(
    bool switchDuringRefresh,
  ...
  ) : HttpMessageHandler
  {
    ...
    )
    {
      ...
      if (!refreshing)
        ResourceTokens.Add(request.Headers.Authorization?.Parameter);
