...
public sealed class LoginComponentTests
{
  ...
  public async Task AuthenticatedVisitorGoesDirectlyToFleetMapWithoutShowingTheForm()
  {
    ...
    await context.Services.GetRequiredService<TokenStorageService>().SetTokensAsync("access", "refresh");
    var navigation = context.Services.GetRequiredService<NavigationManager>();
    navigation.NavigateTo("/login");
    var component = context.Render<Login>();
    component.WaitForAssertion(() => Assert.EndsWith("/fleet/map", navigation.Uri));
  ...
  {
    await using var context = new ClientComponentContext((_, _) => Task.FromResult(new HttpResponseMessage(status)));
    context.AddAuthenticationServices();
    await context.Services.GetRequiredService<TokenStorageService>().SetTokensAsync("access", "refresh");
    var navigation = context.Services.GetRequiredService<NavigationManager>();
    navigation.NavigateTo("/login");
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task DelayedSessionCheckCannotRedirectAfterLeavingLogin(bool dispose)
  {
    var release = new TaskCompletionSource<HttpResponseMessage>(TaskCreationOptions.RunContinuationsAsynchronously);
    await using var context = new ClientComponentContext((_, ct) => release.Task.WaitAsync(ct));
    context.AddAuthenticationServices();
    await context.Services.GetRequiredService<TokenStorageService>().SetTokensAsync("access", "refresh");
    var navigation = context.Services.GetRequiredService<NavigationManager>();
    navigation.NavigateTo("/login");
    ...
    else navigation.NavigateTo("/settings");
  ...
  private static HttpResponseMessage Profile() => new(HttpStatusCode.OK)
  {
    Content = JsonContent.Create(new { id = Guid.NewGuid(), name = "Dispatcher", email = "dispatcher@example.test", isAdmin = false })
  };
  ...
  public sealed class ObservedLogin : Login
  {
    ...
    {
      try { await base.OnInitializedAsync(); }
  ...
  public async Task PendingLoginDisablesSubmissionAndRejectedCredentialsRemainEditable()
  {
    ...
    component.WaitForAssertion(() => Assert.True(component.Find("button[type=submit]").HasAttribute("disabled")));
    release.SetResult(new(HttpStatusCode.Unauthorized));
    await submitting;
    Assert.Equal("Invalid email or password.", component.Find(".login-page__error").TextContent.Trim());
    Assert.False(component.Find("button[type=submit]").HasAttribute("disabled"));
    Assert.Equal("dispatcher@example.test", component.Find("#email").GetAttribute("value"));
    Assert.Null(await context.Services.GetRequiredService<TokenStorageService>().GetAccessTokenAsync());
  }
  ...
  public async Task ATemporaryFailureCanBeRetriedWithoutLosingTheForm()
  {
    ...
    await using var context = new ClientComponentContext((request, _) =>
    {
      if (request.RequestUri!.AbsolutePath == "/api/auth/me")
        return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK)
          { Content = JsonContent.Create(new { id = Guid.NewGuid(), name = "Dispatcher", email = "dispatcher@example.test", isAdmin = false }) });
      logins++;
      return Task.FromResult(logins == 1
        ? new HttpResponseMessage(HttpStatusCode.ServiceUnavailable)
        : new HttpResponseMessage(HttpStatusCode.OK)
          { Content = JsonContent.Create(new { accessToken = "access", refreshToken = "refresh" }) });
    });
    context.AddAuthenticationServices();
    var component = context.Render<Login>();
    component.Find("#email").Change("dispatcher@example.test");
    ...
    Assert.Contains("Could not reach the server", component.Find(".login-page__error").TextContent);
    await component.Find("form").SubmitAsync(EventArgs.Empty);
    Assert.Equal(2, logins);
    Assert.Equal("access", await context.Services.GetRequiredService<TokenStorageService>().GetAccessTokenAsync());
    Assert.EndsWith("/fleet/map", context.Services.GetRequiredService<NavigationManager>().Uri);
