...
public sealed class SessionLifecycleTests
{
  ...
  {
    await using var storage = new TokenStorageService(
      new LocalStorageJsRuntime()
    );
    await storage.SetTokensAsync("first", "refresh-first");
    var first = Assert.IsType<TokenStorageService.Session>(
          ...
          (request, _) =>
          {
            ...
            return Task.FromResult(
              new HttpResponseMessage(
                request.Headers.Authorization?.Parameter == "rotated"
                  ? HttpStatusCode.OK
  ...
  [InlineData("null")]
  [InlineData(
    "{\"Id\":\"invalid\",\"AccessToken\":\"a\",\"RefreshToken\":\"r\"}"
  )]
  public async Task InvalidStoredEnvelopesAreAnonymous(string json)
  {
    ...
    Assert.False(
      (
        await new AppAuthenticationStateProvider(
          storage,
          services
