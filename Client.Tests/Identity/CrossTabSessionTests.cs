...
public sealed class CrossTabSessionTests
{
  ...
  public async Task AnOldTabsNewMutationNeverUsesAnotherTabsLogin()
  {
    ...
    var calls = 0;
    using var client = new HttpClient(new AuthHeaderHandler(oldTab, authentication)
    {
    ...
    var error = await Assert.ThrowsAsync<HttpRequestException>(() => client.PostAsJsonAsync(
      "https://example.test/api/data", new { value = "first-login-draft" }));
  ...
  public async Task SameLoginRotationFromAnotherTabRemainsUsable()
  {
    ...
    var original = Assert.IsType<TokenStorageService.Session>(await secondTab.GetSessionAsync());
    Assert.True(await secondTab.ReplaceAsync(original, original with { AccessToken = "rotated", RefreshToken = "refresh-rotated" }));
    using var services = new ServiceCollection().BuildServiceProvider();
    var tokens = new List<string?>();
    using var client = new HttpClient(new AuthHeaderHandler(firstTab, new(firstTab, services))
    {
  ...
  public async Task AnAnonymousTabRejectsRemoteLoginUntilItsOwnExplicitLogin()
  {
    ...
    using var client = new HttpClient(new AuthHeaderHandler(oldTab, new(oldTab, services))
    {
      InnerHandler = new StubHttpMessageHandler((request, _) =>
      {
        requests.Add((request.RequestUri!.AbsolutePath, request.Headers.Authorization?.Parameter));
        return Task.FromResult(request.RequestUri.AbsolutePath.EndsWith("/login")
          ? new HttpResponseMessage(HttpStatusCode.OK)
            { Content = JsonContent.Create(new { accessToken = "local", refreshToken = "refresh-local" }) }
          : new HttpResponseMessage(HttpStatusCode.OK));
      })
    }) { BaseAddress = new("https://example.test/") };
    await Assert.ThrowsAsync<HttpRequestException>(() => client.PostAsJsonAsync("api/data", new { value = "old-draft" }));
    Assert.Empty(requests);
    await new AuthService(client, oldTab, new(new(client))).LoginAsync("local@example.test", "synthetic-password");
    Assert.Equal("local", await oldTab.GetAccessTokenAsync());
    Assert.False(oldTab.SessionChanged);
    using var response = await client.PostAsJsonAsync("api/data", new { value = "new-draft" });
    Assert.Equal(new[] { ("/api/auth/login", (string?)null), ("/api/data", "local") }, requests);
  }
