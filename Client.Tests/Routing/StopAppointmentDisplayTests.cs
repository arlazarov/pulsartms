...
public sealed class StopAppointmentDisplayTests
{
  ...
  public void ClockUsesTwelveHourTimeWithoutChangingTheLocalHour(
    int hour,
    string expected
  ) =>
    Assert.Equal(
      expected,
      StopAppointmentDisplay.Format(Stop with { ScheduledTime = new(hour, 0) })
    );
  ...
  public void SameDayWindowShowsItsDateOnceAndCollapsesIdenticalDisplayTimes()
  {
    ...
    Assert.Equal(
      ...
      StopAppointmentDisplay.Format(stop)
    );
    Assert.Equal(
      "Sep 9 · 07:00 AM – 02:00 PM",
      StopAppointmentDisplay.Format(stop with { ScheduledDate2 = null })
    );
    Assert.Equal(
      "Sep 9 · 07:00 AM",
  ...
  public void CrossDayAndCrossYearWindowsKeepTheirCalendarBoundaries()
  {
    Assert.Equal(
      ...
      StopAppointmentDisplay.Format(
        ...
        {
          ScheduledDate = new(2026, 9, 9),
          ScheduledTime = new(0, 0),
          ScheduledDate2 = new(2026, 9, 10),
          ScheduledTime2 = new(5, 15, 59),
        }
      )
    );
    Assert.Equal(
      ...
      StopAppointmentDisplay.Format(
        Stop with
        {
          ScheduledDate = new(2026, 12, 31),
          ScheduledTime = new(23, 0),
          ScheduledDate2 = new(2027, 1, 1),
          ScheduledTime2 = new(1, 0),
        }
      )
    );
  }
  ...
  public void UnknownPartsRemainUnknownAndMidnightIsNotTreatedAsAnUnsetTime()
  {
    ...
    Assert.Equal(
      "07:00 AM",
      StopAppointmentDisplay.Format(Stop with { ScheduledTime = new(7, 0) })
    );
    Assert.Equal(
      "Jan 1 · 12:00 AM",
      StopAppointmentDisplay.Format(
        Stop with
        {
          ScheduledDate = new(2026, 1, 1),
          ScheduledTime = new(0, 0, 59),
        }
      )
    );
  }
  ...
  [InlineData("ar-SA")]
  [InlineData("tr-TR")]
  public void FormattingUsesTheSuppliedLocalCalendarAndStableEnglishLabels(
    string culture
  )
  {
