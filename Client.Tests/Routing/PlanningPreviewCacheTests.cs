...
public sealed class PlanningPreviewCacheTests
{
  ...
  public async Task LatePreviewCannotReplaceNewerPlanningOrRestoreClearedSessionData(string change)
  {
    ...
    using var handler = new StubHttpMessageHandler((request, _) =>
    {
      ...
      Assert.Equal($"/api/fleet/trucks/{truck}/planning/preview", request.RequestUri!.AbsolutePath);
      return reply.Task;
    });
    ...
    switch (change)
    {
      ...
      case "newer-plan": cache.Store(url, latest); break;
      case "newer-empty": cache.Store(url, latest with { State = null, DispatchId = null }); break;
    }
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task AuthoritativeIdentityWithoutGeometryIsReturnedButNotCachedAsAPlan(bool noCurrent)
  {
    var truck = Guid.NewGuid();
    var expected = Result(truck, 1) with { State = null };
    if (noCurrent) expected = expected with { DispatchId = null, LoadNumber = null };
    using var handler = new StubHttpMessageHandler((_, _) => Task.FromResult(Ok(expected)));
    using var client = new HttpClient(handler) { BaseAddress = new("https://local.test/") };
    var cache = new PlanningDisplayCache(new ApiService(client));
  ...
  public async Task WritesForAnotherTruckAndItsDispatchAliasDoNotDiscardAValidColdPreview()
  {
    ...
    cache.Store($"api/dispatch/{latestA.DispatchId}/planning/automatic", latestA);
  ...
  public async Task BulkPreloadOverflowDoesNotBlockAnExplicitTruckPreview()
  {
    ...
    var bulkReply = new TaskCompletionSource<HttpResponseMessage>(TaskCreationOptions.RunContinuationsAsynchronously);
    using var handler = new StubHttpMessageHandler((request, _) =>
      request.RequestUri!.AbsolutePath.EndsWith("/planning/preview", StringComparison.Ordinal)
        ? Task.FromResult(Ok(saved)) : bulkReply.Task);
    using var client = new HttpClient(handler) { BaseAddress = new("https://local.test/") };
    var cache = new PlanningDisplayCache(new ApiService(client));
    ...
    bulkReply.SetResult(new(HttpStatusCode.OK)
      { Content = JsonContent.Create(new { success = true, response = new[] { saved } }) });
    await bulk;
    Assert.Same(result, cache.Get($"api/fleet/trucks/{truck}/planning"));
  }
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task EarlierBulkReplyCannotSupersedeAnActiveExplicitPreview(bool noCurrent)
  {
    var truck = Guid.NewGuid();
    var old = Result(truck, 1);
    var latest = Result(truck, 2);
    if (noCurrent) latest = latest with { DispatchId = null, LoadNumber = null, State = null };
    var bulkReply = new TaskCompletionSource<HttpResponseMessage>(TaskCreationOptions.RunContinuationsAsynchronously);
    var directReply = new TaskCompletionSource<HttpResponseMessage>(TaskCreationOptions.RunContinuationsAsynchronously);
    using var handler = new StubHttpMessageHandler((request, _) =>
      request.RequestUri!.AbsolutePath.EndsWith("/planning/preview", StringComparison.Ordinal)
        ? directReply.Task : bulkReply.Task);
    using var client = new HttpClient(handler) { BaseAddress = new("https://local.test/") };
    var cache = new PlanningDisplayCache(new ApiService(client));
    var bulk = cache.PreloadAsync();
    var direct = cache.ReadPreviewAsync(truck, default);
    bulkReply.SetResult(new(HttpStatusCode.OK)
      { Content = JsonContent.Create(new { success = true, response = new[] { old } }) });
    await bulk;
    var url = $"api/fleet/trucks/{truck}/planning";
    Assert.Null(cache.Get(url));
  ...
  private static AutomaticPlanningResult Result(Guid truck, int version) => new(truck, Guid.NewGuid(), 1358,
    new(new(), new() { Id = Guid.NewGuid(), TruckId = truck, Version = version }, null, null, null, true), null);

  private static HttpResponseMessage Ok(AutomaticPlanningResult response) => new(HttpStatusCode.OK)
