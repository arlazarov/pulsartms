...
public class PlanningDisplayCacheTests
{
  ...
  {
    var calculatedAt = new DateTime(2026, 9, 8, 12, 1, 6, 268, DateTimeKind.Utc);
    ...
    var pending = state with { Progress = null, Eta = state.Eta! with { RouteUpdatePending = true } };
    route.Update(pending, pendingAt);
    arrival.Update(saved.DispatchId, stop, pending.Eta);
    Assert.Same(state.Eta, arrival.Display(pending.Eta, pendingAt, refreshing: true));
    ...
    (await handler.Next()).SetResult(ControlledHandler.Reply(saved with { State = state with { Progress = null } }));
    ...
    Assert.Same(state.Eta, arrival.Display(late.State.Eta, receivedAt, refreshing: false));
    ...
    var ready = saved with { State = state with { Eta = state.Eta with { CalculatedAt = readyAt,
      ValidUntil = readyAt.AddMinutes(2), Stops = [state.Eta.Stops[0] with { Arrival = state.Eta.Stops[0].Arrival.AddMinutes(5) }] } } };
    var replacementAt = new DateTime(2026, 9, 8, 12, 3, 26, 382, DateTimeKind.Utc);
    ...
    Assert.Same(replacement.State.Eta, arrival.Display(replacement.State.Eta, replacementAt));
    Assert.Same(replacement.State, route.Display(replacement.State, replacementAt));
  }
  ...
  public async Task NewerButExpiredRepliesCannotOverwriteAFresherDisplayOrRenewTheOriginalGraceDeadline()
  {
    ...
    foreach (var minute in new[] { 1, 16 })
    {
      ...
      var expired = saved with { State = state with { Eta = state.Eta! with { CalculatedAt = now.AddMinutes(minute).AddSeconds(-30),
        ValidUntil = now.AddMinutes(minute).AddSeconds(-10), Stops = [state.Eta.Stops[0] with { Arrival = now.AddHours(20) }] } } };
      var read = cache.RefreshAsync("planning", default);
      ...
      Assert.Same(state.Eta, arrival.Display(late.State.Eta, clock.GetUtcNow().UtcDateTime));
      Assert.Equal(state.Eta!.ValidUntil, route.Display(late.State, clock.GetUtcNow().UtcDateTime)!.Eta!.ValidUntil);
    }
    Assert.Same(state.Eta, arrival.Display(late!.State!.Eta, now.AddMinutes(17).AddTicks(-1)));
    Assert.Null(arrival.Display(late.State.Eta, now.AddMinutes(17)));
    Assert.False(route.IsRetaining(late.State, now.AddMinutes(17)));
    Assert.False(FleetRouteDisplayMemory.CanDisplay(route.Display(late.State, now.AddMinutes(17))!.Eta, now.AddMinutes(17)));
  }
  ...
  public void PassiveCachedRerendersDoNotClassifyAnExpiredForecastAsANewPendingResponse()
  {
    var now = DateTime.UnixEpoch;
    var clock = new FakeTimeProvider(new DateTimeOffset(now));
    using var client = new HttpClient { BaseAddress = new("https://local.test/") };
    var cache = new PlanningDisplayCache(new ApiService(client), clock);
    var saved = ForecastResult(now);
    var arrival = new ArrivalDisplayMemory();
    var route = new FleetRouteDisplayMemory();
    cache.Store("planning", saved);
    arrival.Update(saved.DispatchId, saved.State!.Plan!.Stops[0], saved.State.Eta);
    ...
    arrival.Update(cached.DispatchId, cached.State.Plan!.Stops[0], cached.State.Eta with { Stops = cached.State.Eta.Stops.ToArray() });
    route.Update(cached.State, clock.GetUtcNow().UtcDateTime);
    Assert.Null(arrival.Display(cached.State.Eta, clock.GetUtcNow().UtcDateTime));
    Assert.False(route.IsRetaining(cached.State, clock.GetUtcNow().UtcDateTime));
    Assert.False(FleetRouteDisplayMemory.CanDisplay(route.Display(cached.State, clock.GetUtcNow().UtcDateTime)!.Eta,
  ...
  public async Task OnlyValidExpiredIncompleteRouteForecastsAreClassifiedAsPending(string scenario)
  {
    var now = DateTime.UnixEpoch;
    var clock = new FakeTimeProvider(new DateTimeOffset(now.AddMinutes(3)));
    var result = ForecastResult(now);
    if (scenario == "fresh") result = result with { State = result.State! with { Eta = result.State.Eta! with { ValidUntil = now.AddMinutes(5) } } };
    if (scenario == "unavailable") result = result with { State = result.State! with { Eta = result.State.Eta! with { Stops = [], UnavailableReason = "GPS unavailable" } } };
    if (scenario == "invalid") result = result with { State = result.State! with { Eta = result.State.Eta! with { CalculatedAt = result.State.Eta.ValidUntil } } };
    if (scenario == "completed") result.State!.Plan!.Tracking.AllStopsPassed = true;
    ...
    Assert.Equal(result.State!.Eta!.ValidUntil, response.Response.State.Eta.ValidUntil);
  }
  ...
  private static AutomaticPlanningResult ForecastResult(DateTime now)
  {
    ...
    return new(truck, dispatch, 1, new(new(), new() { Id = Guid.NewGuid(), TruckId = truck,
      DispatchId = dispatch, Stops = [stop], Tracking = new() { NextStopId = stop.Id } },
      new(200, 800, 1000, 0, false, false, now, null), null, null, true)
    {
      Eta = new(now, now.AddMinutes(2), [new(stop.Id, now.AddHours(1), "UTC", null, 0, 60, 0) { DispatchId = dispatch }], null, [])
    }, null);
  }
  ...
  public async Task ForcedRevalidationSupersedesOnlyItsInflightReadAndKeepsOtherTrucksCached()
  {
    ...
    AutomaticPlanningResult Result(int version) => new(Guid.NewGuid(), Guid.NewGuid(), 1,
      new(new(), new() { Id = Guid.NewGuid(), Version = version }, null, null, null, true), null);
  ...
  {
    private readonly System.Threading.Channels.Channel<TaskCompletionSource<HttpResponseMessage>> requests =
      System.Threading.Channels.Channel.CreateUnbounded<TaskCompletionSource<HttpResponseMessage>>();
    public Task<TaskCompletionSource<HttpResponseMessage>> Next() => requests.Reader.ReadAsync().AsTask();
    ...
    public static HttpResponseMessage Reply(AutomaticPlanningResult result) => new(HttpStatusCode.OK)
  ...
  {
    var truck = Guid.NewGuid(); var dispatch = Guid.NewGuid();
    AutomaticPlanningResult Result(int version) => new(truck, dispatch, 1,
      new(new(), new() {Id = dispatch, Version = version}, null, null, null, true), null);
    ...
    Assert.Equal(1, cache.Get($"api/dispatch/{dispatch}/planning/automatic")!.State!.Plan!.Version);
  ...
  public async Task LatePreloadCannotRestoreNewerNoPlanKeysButStillLoadsUnrelatedTrucks(bool truckChanged, bool dispatchChanged)
  {
    var truckA = Guid.NewGuid();
    var truckB = Guid.NewGuid();
    var unrelatedTruck = Guid.NewGuid();
    AutomaticPlanningResult Result(Guid truck) => new(truck, Guid.NewGuid(), 1,
      new(new(), new() { Id = Guid.NewGuid(), TruckId = truck, Version = 1 }, null, null, null, true), null);
    ...
    if (truckChanged) cache.Store(truckUrl, oldA with { DispatchId = null, State = null });
    if (dispatchChanged) cache.Store(dispatchUrl, oldA with { State = null });
    ...
    Assert.Equal(oldB.DispatchId, cache.Get($"api/fleet/trucks/{truckB}/planning")!.DispatchId);
    Assert.NotNull(cache.Get($"api/dispatch/{oldB.DispatchId}/planning/automatic"));
  ...
  {
    var truck = Guid.NewGuid();
    var result = new AutomaticPlanningResult(truck, Guid.NewGuid(), 1,
      new(new(), new() { Id = Guid.NewGuid(), Version = 1 }, null, null, null, true), null);
    using var handler = new PreviewHandler(result);
    using var client = new HttpClient(handler) { BaseAddress = new("https://local.test/") };
    var cache = new PlanningDisplayCache(new ApiService(client));
    var pending = cache.PreloadAsync();
    for (var i = 0; i < 257; i++) cache.Store($"api/fleet/trucks/{Guid.NewGuid()}/planning", null);
    handler.Release.SetResult();
    await pending;
  ...
  private sealed class PreviewHandler(params AutomaticPlanningResult[] results) : HttpMessageHandler
  {
    public int Calls;
    public TaskCompletionSource Release = new(TaskCreationOptions.RunContinuationsAsynchronously);
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
      Calls++; await Release.Task.WaitAsync(cancellationToken);
      return new(HttpStatusCode.OK) {Content = JsonContent.Create(new RequestResponseDTO<List<AutomaticPlanningResult>>
        {Success = true, Response = results.ToList()})};
    }
  }
  ...
  private sealed class DelayedHandler : HttpMessageHandler
  {
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
      ...
      return new(HttpStatusCode.OK) { Content = JsonContent.Create(new RequestResponseDTO<AutomaticPlanningResult>
      { Success = true, Response = new(Guid.NewGuid(), Guid.NewGuid(), 1, new(new(), new() { Id = Guid.NewGuid() }, null, null, null, true), null) }) };
    }
  }
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task RefreshRestoresOnlyMatchingGeometryAndKeepsFreshMetadata(bool changed)
  {
    var id = Guid.NewGuid();
    var previous = new RoutePlan { Id = id, Version = 1,
      Route = new() { Legs = [new(10, 600, [new(40, -80), new(41, -80)])] } };
    var next = new RoutePlan { Id = id, Version = changed ? 2 : 1, GeometryOmitted = !changed,
      Route = new() { Legs = [new(11, 650, changed ? [new(42, -80), new(43, -80)] : [])] },
      Stops = [new(Guid.NewGuid(), "Updated stop", "Address", 1, new(41, -80))] };
    AutomaticPlanningResult Result(RoutePlan plan) => new(Guid.NewGuid(), Guid.NewGuid(), 1, new(new(), plan, null, 50, null, true), null);
