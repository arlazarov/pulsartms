...
public sealed class DriverDutySummaryComponentTests
{
  ...
  public void MissingDutyStatusUsesOneNeutralPlaceholder()
  {
    ...
    Assert.Equal(
      "—",
      component.Find(".driver-duty__current strong").TextContent
    );
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void MissingRestHistoryAndStaleReadingsStayQuietWithoutInventingCountdowns(
    bool stale
  )
  {
    using var context = new BunitContext();
  ...
  public void CurrentStatusExplicitlyLabelsElapsedTime(
    int minutes,
    string duration
  )
  {
    ...
    component.Render(p =>
      p.Add(x => x.CurrentStatus, "driving")
  ...
  public void DisplaysServerResetCountdownBesideDailyRest(
    string country,
    int hours,
    int remaining,
    string text
  )
  {
    ...
    Assert.Equal(
      $"{text} left to complete {hours}h reset · {country}",
      component.Find(".driver-duty__cycle-reset").TextContent
    );
  }
  ...
  public void MissingStaleOrMismatchedStatusHidesResetCountdown()
  {
    ...
    status = status with
    {
      ...
      CycleResetRemainingMinutes = 1680,
    };
    component.Render(p =>
      p.Add(x => x.Status, status).Add(x => x.CurrentStatus, "driving")
    );
    Assert.Empty(component.FindAll(".driver-duty__cycle-reset"));
    component.Render(p =>
      p.Add(x => x.CurrentStatus, "sleeperBerth")
        .Add(x => x.Status, status with { ObservedAt = now.AddMinutes(-4) })
    );
    Assert.Empty(component.FindAll(".driver-duty__cycle-reset"));
  }
  ...
  [Fact]
  public void StatusOnlyPresentationKeepsElapsedTimeWithoutDetailedRestCountdowns()
  {
    using var context = new BunitContext();
    var now = DateTimeOffset.UtcNow;
    var status = new DriverDutyStatus(
    ...
    Assert.Single(component.FindAll(".driver-duty > *"));
    component.Render(p => p.Add(x => x.ShowRestDetails, true));
    Assert.Contains("3h 52m left to complete 10h rest", component.Markup);
    Assert.Contains("27h 52m left to complete 34h reset", component.Markup);
  }
