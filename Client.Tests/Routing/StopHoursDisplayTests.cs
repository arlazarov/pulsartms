...
public sealed class StopHoursDisplayTests
{
  ...
  public void SignedCycleFormatsTheServerValueWithoutClamping(
    int? minutes,
    string expected
  ...
  public void AlternativeColorDependsOnKnownLatenessNotRestKind(
    string kind,
    int? late,
    string expected
  )
  {
    var alternative = new StopHoursAlternative(
      kind,
  ...
  public void TimestampKeepsEnglishLabelsAndTheStopsLocalOffset()
  {
    ...
    try
    {
      ...
      Assert.Equal(
        "Sep 11 · 12:05 AM",
        StopHoursDisplay.Timestamp(
          new(2026, 9, 11, 0, 5, 0, TimeSpan.FromHours(-4))
        )
      );
    }
    finally
  ...
  public void RecapDateUsesTheProvidedHomeOffsetWithoutShowingTime(
    string at,
    string expected
  )
  {
  ...
  public void ArrivalFeasibilityIncludesEarlierDrivingButNotDestinationServiceDebt(
    ...
    int departure,
    int shortfall,
    bool earlierShortage,
    bool expected
  )
  {
    var hours = new StopHoursForecast(
      arrival,
      departure,
      shortfall,
      earlierShortage ? DateTimeOffset.UnixEpoch : null,
      true,
      [],
      null
    );
    ...
    Assert.False(
      StopHoursDisplay.CycleShort(hours with { CycleVerified = false })
    );
  }
}
