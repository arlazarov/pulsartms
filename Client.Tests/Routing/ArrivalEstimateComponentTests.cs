...
public sealed class ArrivalEstimateComponentTests
{
  ...
  public void SummaryAndDetailsCanBeSeparatedWithoutChangingTheDefaultDisplay()
  {
    ...
    var eta = new DispatchEta(
      ...
      [
        new(
          stop.Id,
          clock.GetUtcNow().AddHours(1).ToOffset(TimeSpan.FromHours(-4)),
          "America/Toronto",
          null,
          25,
          60,
    ...
    Assert.Equal(
      "Arrival time zone UTC-04:00",
      component.Find(".arrival-estimate__timezone").TextContent
    );
  ...
  public void PassiveRerenderAtTheDeadlineDoesNotTurnTheSameForecastIntoAPendingResponse()
  {
    ...
    component.Render(p =>
      p.Add(x => x.Eta, eta with { Stops = eta.Stops.ToArray() })
    );
    Assert.Empty(component.FindAll(".arrival-estimate"));
  ...
  public void TechnicalUnavailableReasonsNeverBecomeVisibleTextOrAnEmptyWrapper(
    string reason,
    bool pending
  )
  {
    ...
    var eta = new DispatchEta(now, now.AddMinutes(2), [], reason, [])
    {
  ...
  public void InFlightRefreshKeepsVisibleEtaThroughExpiryAndReplacesAllValuesTogether()
  {
    ...
    component.Render(p =>
      p.Add(x => x.Eta, refreshed).Add(x => x.Refreshing, false)
    );
    Assert.Equal(
      "Sep 8 · 02:00 PM",
      component.Find(".stop-hours__road time").TextContent.Trim()
    );
    ...
    component.Render(p =>
      p.Add(
        ...
        eta with
        {
          ...
          DutyStatus = null,
          Stops = [eta.Stops[0] with { Arrival = now.AddHours(2) }],
        }
      )
    );
    Assert.Equal(
  ...
  public void PreviousLatenessKeepsItsPresentationUntilTheNewEstimateIsReady()
  {
    ...
    var complete = new DispatchEta(
      ...
      now.AddMinutes(2),
      [
        new(stop.Id, now.AddHours(1), "UTC", null, 125, 60, 0)
        {
          DispatchId = dispatch,
        },
      ],
  ...
  [InlineData(0)]
  [InlineData(30)]
  public void UnexpiredPendingForecastKeepsItsResultWithinTheDisplayGrace(
    int lateMinutes
  )
  {
    using var context = new BunitContext();
    ...
    var eta = new DispatchEta(
      now,
      now.AddMinutes(2),
      [new(stop.Id, now.AddHours(1), "UTC", null, lateMinutes, 60, 0)],
      null,
      []
    )
    {
  ...
  public void HidingAppointmentKeepsEtaAndItsBoundedRecalculationGrace()
  {
    ...
    )
    {
      ...
      ScheduledDate2 = new(2026, 9, 9),
      ScheduledTime2 = new(14, 0),
    };
    var eta = new DispatchEta(
      now,
      now.AddMinutes(2),
    ...
    Assert.Contains(
      ...
      component.Find(".arrival-estimate__appointment").TextContent
    );
  ...
  public void HiddenAppointmentWithoutAnEstimateDoesNotLeaveAnEmptyWrapper()
  {
    ...
    var component = context.Render<ArrivalEstimate>(p =>
      p.Add(x => x.Stop, stop).Add(x => x.ShowAppointment, false)
    );
    Assert.Empty(component.FindAll(".arrival-estimate"));
  }
  ...
  public void OldEstimateKeepsItsPresentationDuringRecalculationButDisappearsAfterGrace()
  {
    ...
    var eta = new DispatchEta(
      now,
      now.AddMinutes(2),
      [new(stop.Id, now, "UTC", null, 0, 60, 0)],
      null,
      []
    );
    var component = context.Render<ArrivalEstimate>(p =>
