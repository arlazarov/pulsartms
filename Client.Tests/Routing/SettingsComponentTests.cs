...
using SettingsPage = Client.Pages.Settings.FleetSettings;
...
public sealed class SettingsComponentTests
{
  ...
  public async Task ConflictPreservesDraftAndReloadUsesTheNewRevision()
  {
    ...
    using var context = new ClientComponentContext(
      async (request, ct) =>
      {
        if (request.RequestUri!.AbsolutePath == "/api/settings/dispatch")
          return NumberingResponse();
        if (request.RequestUri.AbsolutePath == "/api/settings/integrations")
          return IntegrationSettingsFixture.ListResponse();
        if (request.Method == HttpMethod.Get)
        {
          loads++;
          return Response(
            new(new() { UseIfta = true }, loads == 1 ? 3 : 4, null)
          );
        }
        saves.Add(
          (
            await request.Content!.ReadFromJsonAsync<PlanningSettingsUpdate>(ct)
          )!
        );
        return saves.Count == 1
          ? new(HttpStatusCode.Conflict)
          {
            Content = JsonContent.Create(
              new RequestResponseDTO<PlanningSettingsState>
              {
                Success = false,
                Errors = ["Settings changed. Reload settings and try again."],
              }
            ),
          }
    ...
    Assert.Contains(
      "Settings saved",
      component.Find(".settings-page__saved").TextContent
    );
  }
  ...
  public void FuelStopControlsAndBroadRestoreActionAreAbsentWithoutAnAutomaticWrite()
  {
    ...
    using var context = new ClientComponentContext(
      ...
      {
        if (request.RequestUri!.AbsolutePath == "/api/settings/dispatch")
          return Task.FromResult(NumberingResponse());
        if (request.RequestUri.AbsolutePath == "/api/settings/integrations")
          return Task.FromResult(IntegrationSettingsFixture.ListResponse());
        if (request.Method != HttpMethod.Get)
          writes++;
    ...
    Assert.Empty(
      component.FindAll(
        "#settings-driving-cost, #settings-reserve, #settings-fill, #settings-detour"
      )
    );
    ...
    Assert.Contains("Load numbering", component.Markup);
  ...
  [InlineData(false)]
  [InlineData(true)]
  public async Task SavingPriceBasisPreservesTheServerProvidedHiddenPreferences(
    bool useIfta
  )
  {
  ...
  [InlineData(-1)]
  [InlineData(61)]
  public async Task InvalidServerProvidedPreferencesStillDoNotReachTheServer(
    double detour
  )
  {
    var writes = 0;
    using var context = new ClientComponentContext(
      (request, _) =>
      {
        ...
        return Task.FromResult(
          Response(new(new() { MaxDetourMinutes = detour }, 1, null))
        );
      }
    );
    var component = context.Render<SettingsPage>();
    ...
    Assert.Equal(0, writes);
    Assert.Contains(
      "Maximum detour must be between 1 and 60 minutes.",
      component.Markup
    );
    Assert.Empty(component.FindAll(".settings-page__saved"));
  }
  ...
  public async Task SavingFuelPreferencesDoesNotSubmitOrResetAnIntegrationDraft()
  {
    ...
    using var context = new ClientComponentContext(
      async (request, ct) =>
      {
        ...
        if (request.RequestUri.AbsolutePath == "/api/settings/integrations")
          return IntegrationSettingsFixture.ListResponse();
        if (request.Method == HttpMethod.Get)
          return Response(new(new(), 3, null));
    ...
    await component
      ...
      .ClickAsync(new());
    component
      .Find("#integration-samsara-apiKey")
      .Input("independent-secret-draft");
    component.WaitForElement("#settings-ifta").Change(false);
    await component
      .Find(".planning-settings__form")
      .SubmitAsync(EventArgs.Empty);
    ...
    Assert.Equal(
      "independent-secret-draft",
      component.Find("#integration-samsara-apiKey").GetAttribute("value")
    );
    Assert.Empty(
      component.FindAll(".integration-settings .settings-page__saved")
    );
  }

  private static HttpResponseMessage Response(PlanningSettingsState state) =>
  ...
  private static HttpResponseMessage NumberingResponse() =>
    new(HttpStatusCode.OK)
    {
      Content = JsonContent.Create(
