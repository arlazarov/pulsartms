...
public sealed class StopHoursComponentTests
{
  ...
  [InlineData(true)]
  [InlineData(false)]
  public void ArrivalSeparatesDateAndClockWithoutChangingTimestampOrOffset(
    bool withHours
  )
  {
    ...
    if (!withHours)
      estimate = estimate with { Hours = null };
    ...
    var component = context.Render<ArrivalEstimate>(p =>
      p.Add(x => x.DispatchId, estimate.DispatchId)
        .Add(x => x.Eta, eta)
        .Add(
          x => x.Stop,
          new PlanStop(estimate.StopId, "Plant", "Address", 1, new(40, -80))
        )
    );
    var timestamp = component.Find(".stop-hours__road time");
    Assert.Equal(
      StopHoursDisplay.Timestamp(estimate.Arrival),
      timestamp.TextContent.Trim()
    );
    Assert.Equal(
      estimate.Arrival.ToString("O", CultureInfo.InvariantCulture),
      timestamp.GetAttribute("datetime")
    );
    Assert.Equal(
      StopHoursDisplay.RecapDate(estimate.Arrival),
      timestamp.QuerySelector(".stop-hours__date")!.TextContent
    );
    Assert.Equal(
      estimate.Arrival.ToString("hh:mm tt", CultureInfo.InvariantCulture),
      timestamp.QuerySelector(".stop-hours__clock")!.TextContent
    );
  }

  [Theory]
  [InlineData(3837, "+63h 57m")]
  [InlineData(0, "0h 00m")]
  [InlineData(-60, "−1h 00m")]
  public void ObservedStopUsesCurrentCycleWithoutBackdatingIt(
    int minutes,
    string expected
  )
  {
    using var context = Context();
    var estimate = Estimate(100, 200);
    estimate = estimate with
    {
      Hours = estimate.Hours! with
      {
        CycleAtArrivalMinutes = null,
        CurrentCycleMinutes = minutes,
      },
    };
    ...
    Assert.DoesNotContain("Cycle unknown", component.Markup);
    Assert.Equal(
      "Current cycle remaining at this stop",
      component.Find(".stop-hours__heading").GetAttribute("title")
    );
    Assert.Null(estimate.Hours.CycleAtArrivalMinutes);
  }
  ...
  public void CompactSummaryPreservesTheCycleWarningWhileDetailedBalancesCanBeRenderedSeparately()
  {
    ...
    var component = context.Render<StopHours>(p =>
      p.Add(x => x.Estimate, Estimate(-80, -200)).Add(x => x.ShowDetails, false)
    );
    Assert.Contains(
      "Cycle short",
      component.Find(".stop-hours__road").TextContent
    );
    Assert.Empty(component.FindAll(".stop-hours__cycle"));

    component.Render(p =>
      p.Add(x => x.ShowSummary, false).Add(x => x.ShowDetails, true)
    );
  ...
  [InlineData(20, -100)]
  [InlineData(20, 0)]
  public void DepartureBalanceIsNeverDisplayedEvenWhenItDiffers(
    int arrival,
    int departure
  )
  {
    ...
    var component = Render(context, estimate);
    Assert.Equal(
      StopHoursDisplay.Signed(arrival),
      component.Find(".stop-hours__arrival-cycle strong").TextContent
    );
    Assert.Empty(component.FindAll(".stop-hours__departure-cycle"));
  ...
  public void CycleHasItsOwnSectionSeparateFromRoadEta()
  {
    ...
    Assert.Equal(
      "Cycle remaining",
      cycle.QuerySelector(".stop-hours__arrival-cycle h3")!.TextContent
    );
  ...
  public void EarlierDrivingShortageRemainsVisibleAfterRecapRestoresCycle()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with { FirstCycleShortageAt = Start },
    };
    var component = Render(context, estimate);
    Assert.Contains(
      "+3h 20m",
  ...
  [InlineData(false)]
  [InlineData(true)]
  public void UnknownCycleHidesUnverifiedHoursAndAlternativesButNotKnownRoadLateness(
    bool late
  )
  {
    using var context = Context();
    ...
    estimate = estimate with
    {
      ...
      {
        CycleVerified = false,
        Alternatives = [Alternative("recap", 0)],
      },
    };
    var component = Render(context, estimate);
    Assert.Contains(
    ...
    Assert.Empty(
      component.FindAll(
        ".stop-hours__alternative, .stop-hours__status--success"
      )
    );
    Assert.Equal(
      late ? 1 : 0,
  ...
  public void VerifiedHistoryWithoutArrivalCycleDoesNotClaimArrivalFeasibility()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with { CycleAtArrivalMinutes = null },
    };
    var component = Render(context, estimate);
    Assert.Contains(
      "Cycle unknown",
  ...
  public void CycleShortAndRoadLatenessAreShownIndependently()
  {
    ...
    Assert.Equal(
      2,
      component.FindAll(".stop-hours__road .stop-hours__status--danger").Count
    );
  ...
  public void RecapIsExplicitAndResetAlternativesAreNotDisplayed()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with
      {
        ...
        [
          Alternative("recap", 0),
          Alternative("restart", 0),
          Alternative("restart", 10),
          Alternative("unknown", 0),
        ],
      },
    };
    var component = Render(context, estimate);
    ...
    Assert.Equal(
      "With recap",
      component.Find("[data-kind=recap] .stop-hours__label").TextContent
    );
    Assert.Empty(component.FindAll("[data-kind=restart]"));
    Assert.Contains(
      "On time with recap",
      component
        .Find("[data-kind=recap] .stop-hours__status--success")
        .TextContent
    );
    Assert.DoesNotContain("On time if reset", component.Markup);
    Assert.Empty(
      component.FindAll(
        ".stop-hours__alternative .stop-hours__status--conditional"
      )
    );
    Assert.Empty(
      component.FindAll(".stop-hours__road .stop-hours__status--success")
    );
  }
  ...
  public void ARecapAlternativeThatStillMissesTheAppointmentShowsItsOwnLateness()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with
      {
        Alternatives = [Alternative("recap", 130)],
      },
    };
    var component = Render(context, estimate);
    Assert.Contains(
      "With recap",
      component.Find("[data-kind=recap]").TextContent
    );
    Assert.Contains(
      "Late by 2h 10m",
      component
        .Find("[data-kind=recap] .stop-hours__status--danger")
        .TextContent
    );
  ...
  [InlineData(null)]
  [InlineData(-1)]
  public void AlternativeWithoutKnownLatenessCannotPromiseAppointmentFeasibility(
    int? late
  )
  {
    using var context = Context();
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with
      {
        Alternatives = [Alternative("recap", late)],
      },
    };
  ...
  public void MissingAppointmentCannotClaimOnTimeWithRecap()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with { Alternatives = [Alternative("recap", 0)] },
    };
    Assert.Empty(Render(context, estimate).FindAll(".stop-hours__alternative"));
  }
  ...
  public void DuplicateAlternativeKindsDoNotExpandTheCompactStopCard()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with
      {
        Alternatives = [Alternative("recap", 0), Alternative("recap", 20)],
      },
    };
    Assert.Single(
      Render(context, estimate).FindAll(".stop-hours__alternative")
    );
  }
  ...
  public void PendingRetainsEveryForecastFieldStatusAndColorWithinTheDisplayGrace()
  {
    ...
    estimate = estimate with
    {
      Hours = estimate.Hours! with
      {
        Alternatives = [Alternative("recap", 0), Alternative("restart", 0)],
      },
    };
    ...
    Assert.Empty(
      component.FindAll(
        ".stop-hours__status--danger, .stop-hours__value--danger, .stop-hours__alternative"
      )
    );
  }
  ...
  public void ChangedIdentityOrCompletionNeverInheritsHours(string changed)
  {
    ...
    component.Render(p =>
      p.Add(
          x => x.DispatchId,
          changed == "load" ? Guid.NewGuid() : estimate.DispatchId
        )
        .Add(
          x => x.Stop,
  ...
  public void RecapShowsOnlyTheHomeDateAndAmountWhilePreservingItsExactInstant(
    string value,
    string zone,
    string expected
  )
  {
    using var context = Context();
    var at = DateTimeOffset.Parse(value, CultureInfo.InvariantCulture);
    var component = context.Render<StopHours>(p =>
      p.Add(x => x.Estimate, Estimate(300, 180))
        .Add(x => x.ShowRecap, true)
        .Add(x => x.Recap, new StopCycleForecast(300, at, 185, zone, true))
    );
    var recap = component.Find(".stop-hours__recap");
    Assert.Equal(
      "Next recap",
      recap.QuerySelector(".stop-hours__label")!.TextContent
    );
    Assert.Equal(expected, recap.QuerySelector("time")!.TextContent);
    Assert.Equal(
      at,
      DateTimeOffset.Parse(
        recap.QuerySelector("time")!.GetAttribute("datetime")!,
  ...
  public void UnknownOrExpiredNextRecapDoesNotShowAnUnusableDate(bool expired)
  {
    ...
    var component = context.Render<StopHours>(p =>
      p.Add(x => x.Estimate, Estimate(300, 180))
        ...
        .Add(
          x => x.Recap,
          new StopCycleForecast(
            300,
            expired ? Start.AddHours(-1) : Start.AddDays(1),
            185,
            "UTC",
            expired
          )
  ...
  public void VerifiedHorizonWithoutNextRecapShowsKnownNoneInsteadOfUnknown()
  {
    ...
    var component = context.Render<StopHours>(p =>
      p.Add(x => x.Estimate, Estimate(300, 180))
        .Add(x => x.ShowRecap, true)
        .Add(x => x.Recap, new StopCycleForecast(300, null, null, "UTC", true))
    );
    Assert.Equal(
      "—",
      component.Find(".stop-hours__recap .stop-hours__value").TextContent.Trim()
    );
  ...
  public void ExpiredRecapShowsUnavailableWithoutAnOldDateOrCredit()
  {
    ...
    var component = context.Render<StopHours>(p =>
      p.Add(x => x.Estimate, Estimate(300, 180))
        ...
        .Add(
          x => x.Recap,
          new StopCycleForecast(300, Start.AddMinutes(-1), 185, "UTC", true)
        )
    );
    Assert.Equal(
      "—",
  ...
  private static IRenderedComponent<StopHours> Render(
    BunitContext context,
    StopEta estimate
  ) => context.Render<StopHours>(p => p.Add(x => x.Estimate, estimate));

  private static StopEta Estimate(int arrival, int departure) =>
    new(Guid.NewGuid(), Start.AddHours(1), "UTC", Start.AddHours(2), 0, 60, 0)
    {
      DispatchId = Guid.NewGuid(),
      Hours = new(arrival, departure, 0, null, true, [], null),
    };
