...
public sealed class RouteMessageDisplayTests
{
  ...
  public void ExactMaintenanceMessagesAreQuietOnlyWithAValidVisibleRoute(
    string message
  )
  {
  ...
  public void AddressFailuresRemainActionableWithoutTechnicalFallbackExplanations(
    string message
  )
  {
    Assert.Equal(
      "Check the stop address.",
      RouteMessageDisplay.For(message, true)
    );
    Assert.Equal(
      "Check the stop address.",
      RouteMessageDisplay.For(message, false)
    );
    Assert.Equal(
      "Check the stop address.",
      RouteMessageDisplay.Concise(message)
    );
  }
