...
public sealed class ArrivalDisplayMemoryTests
{
  ...
  public void FirstForecastCanArriveWithHydratedStopDetailsAfterAnEmptyPlaceholder()
  {
    ...
    var detailed = placeholder with
    {
      Address = "200 Receiving Street",
      ScheduledDate = new(2026, 9, 10),
    };
    var eta = new DispatchEta(
      now,
      now.AddMinutes(2),
      [
        new(detailed.Id, now.AddHours(1), "UTC", null, 0, 60, 0)
        {
          DispatchId = dispatch,
        },
      ],
  ...
  public void QuietRefreshKeepsTheOriginalCompleteSnapshotWithoutExtendingItsDeadline()
  {
    ...
    var eta = new DispatchEta(
      ...
      now.AddMinutes(2),
      [
        new(stop.Id, now.AddHours(1), "UTC", null, 45, 60, 0)
        {
          DispatchId = dispatch,
        },
      ],
    ...
    Assert.NotSame(
      eta,
      memory.Display(unavailable, now.AddMinutes(1), refreshing: true)
    );
  }
  ...
  public void ChangedSameIdStopRejectsTheOldTimestampUntilANewerCompleteForecastArrives(
    string change
  )
  {
    var memory = new ArrivalDisplayMemory();
    ...
    var pending = eta with
    {
      CalculatedAt = now.AddSeconds(1),
  ...
  public void NewerPendingStopCannotReplaceAnyPartOfTheCompleteSnapshot()
  {
    ...
    var cycle = new StopCycleForecast(
      217,
      now.AddDays(1),
      185,
      "America/New_York",
    ...
    var pending = complete with
    {
      ...
      ValidUntil = now.AddMinutes(3),
      RouteUpdatePending = true,
      Stops =
      [
        complete.Stops[0] with
        {
          Arrival = now.AddHours(2),
          CycleAfterDeparture = null,
        },
      ],
    };
    ...
    Assert.Same(
      cycle,
      memory.Display(pending, now.AddMinutes(3))!.Stops[0].CycleAfterDeparture
    );
    ...
    var refreshed = pending with
    {
      ...
      Stops =
      [
        pending.Stops[0] with
        {
          CycleAfterDeparture = cycle with { RemainingMinutes = 200 },
        },
      ],
    };
  ...
  public void MatchingFreshDataCannotBypassCompletionAndDifferentIdentityCannotBeDisplayed()
  {
    ...
    var eta = new DispatchEta(
      now,
      now.AddMinutes(2),
      [new(stop.Id, now, "UTC", null, 0, 60, 0) { DispatchId = dispatch }],
      null,
      []
    );
    ...
    memory.Update(dispatch, stop, eta);
    var differentLoad = eta with
    {
      Stops = [eta.Stops[0] with { DispatchId = Guid.NewGuid() }],
    };
    Assert.Null(memory.Display(differentLoad, now));
    Assert.Null(
      memory.Display(differentLoad with { RouteUpdatePending = true }, now)
    );
    var unknownLoad = eta with
    {
      Stops = [eta.Stops[0] with { DispatchId = Guid.Empty }],
    };
    Assert.Null(memory.Display(unknownLoad, now));
    Assert.Null(
      memory.Display(unknownLoad with { RouteUpdatePending = true }, now)
    );
    var differentStop = eta with
    {
      Stops = [eta.Stops[0] with { StopId = Guid.NewGuid() }],
    };
    Assert.Null(memory.Display(differentStop, now));
    Assert.Null(
      memory.Display(differentStop with { RouteUpdatePending = true }, now)
    );
  }
  ...
  public void OutOfOrderFreshSnapshotCannotRevertTheDisplayedCalculation()
  {
    ...
    {
      CalculatedAt = now.AddSeconds(30),
      Stops = [older.Stops[0] with { Arrival = now.AddHours(1) }],
    };
