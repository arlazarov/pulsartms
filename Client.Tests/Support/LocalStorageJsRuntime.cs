...
internal sealed class LocalStorageJsRuntime : IJSRuntime
{
  ...
  )
  {
    ...
    lock (values)
    {
      ...
      {
        if (!Equals(args![0], "./js/generated/shared/authStorage.js"))
          throw new InvalidOperationException("Unexpected module import.");
        return ValueTask.FromResult((TValue)(object)new Module(this));
      }
      if (identifier == "readSession")
        return Result<TValue>(ReadSession());
      if (identifier == "setSession")
        WriteSession((string)args![0]!);
      else if (identifier == "replaceSession")
      {
        var current = Parse(ReadSession());
        var expected = Parse((string)args![0]!);
        if (current != expected || expected is null)
      ...
      {
        if (Parse(ReadSession())?.Id.ToString() != (string)args![0]!)
          return Result<TValue>(false);
        WriteSession(null);
        return Result<TValue>(true);
      }
        ...
        switch (identifier)
        {
          ...
          case "localStorage.getItem":
            return ValueTask.FromResult(
              (TValue)(object?)values.GetValueOrDefault(key)!
            );
          default:
            throw new InvalidOperationException(
              $"Unexpected JS call: {identifier}"
            );
        }
      }
  ...
  {
    if (values.TryGetValue("auth_session", out var json))
      return json;
    ...
    json = JsonSerializer.Serialize(
      new TokenStorageService.Session(Guid.NewGuid(), access, refresh)
    );
    WriteSession(json);
    return json;
  }
  ...
  private static TokenStorageService.Session? Parse(string? json)
  {
    try
    {
      return json is null
        ? null
        : JsonSerializer.Deserialize<TokenStorageService.Session>(json);
    }
    catch (JsonException)
    {
      return null;
  ...
  private sealed class Module(LocalStorageJsRuntime runtime)
    : IJSObjectReference
  {
    public ValueTask<TValue> InvokeAsync<TValue>(
      string identifier,
      object?[]? args
    ) => runtime.InvokeAsync<TValue>(identifier, args);
