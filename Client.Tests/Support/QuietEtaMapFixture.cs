...
internal sealed class QuietEtaMapFixture : IDisposable
{
  ...
  private readonly Channel<TaskCompletionSource<HttpResponseMessage>> pending =
    Channel.CreateUnbounded<TaskCompletionSource<HttpResponseMessage>>();
  public Guid TruckId { get; } = Guid.NewGuid();
  public FakeTimeProvider Clock { get; } =
    new(new DateTimeOffset(2026, 9, 10, 12, 0, 0, TimeSpan.Zero));
  ...
  public QuietEtaMapFixture()
  {
    ...
    Future = new(
      ...
      202,
      "ready",
      [new(10, 600, [new(41, -79), new(42, -78)])],
      [
        new(41, -79, "Future pickup") { Id = Guid.NewGuid() },
        new(42, -78, "Future delivery") { Id = Guid.NewGuid() },
      ]
    )
    {
      Deadhead = new(15, []),
    ...
    var forecast = new DispatchEta(
      ...
      now.AddMinutes(2),
      [
        new(current.Id, now.AddHours(1), "UTC", now.AddHours(2), 0, 60, 0)
        {
    ...
    Planning = new(
      ...
      new(
        ...
        new(10, 90, 5400, 0, false, false, now, new(40.1, -79.9)),
        null,
        null,
        true
      )
    ...
    Js.Respond = (name, _) =>
      Task.FromResult<object?>(
        name is "import" or "createFleetMap" ? Js
        : name is "setRouteBytes" or "focusTruck" ? true
        : null
      );
  }
  ...
  public async Task<IRenderedComponent<FleetMap>> SelectAsync()
  {
    ...
    await component.InvokeAsync(
      () => component.Instance.OnRouteProgress(TruckId.ToString(), 90, 10)
    );
    await component
      .FindAll("label")
      .Single(label => label.TextContent.Trim() == "Next loads")
      .QuerySelector("input")!
    ...
    await component.InvokeAsync(
      () =>
        component.Instance.OnNextLoadSelected(
          TruckId.ToString(),
          Planning.DispatchId!.Value.ToString(),
          Future.Id.ToString(),
          1
        )
    );
  ...
  public static void Reply(
    TaskCompletionSource<HttpResponseMessage> request,
    AutomaticPlanningResult response
  ...
  )
  {
    ...
    if (path == "/api/fleet/locations")
      return Task.FromResult(
        Ok(
          new
          {
            trucks = new[] { new { truckId = TruckId, unitNumber = "54777" } },
    ...
    {
      if (!HoldPlanning)
        return Task.FromResult(Ok(Planning));
    ...
    if (path.EndsWith("/planning/preview", StringComparison.Ordinal))
      return Task.FromResult(Ok(Planning));
    if (path.EndsWith("/next-routes", StringComparison.Ordinal))
      return Task.FromResult(
        Ok(
          new NextLoadRoutesResponse(
            "future",
    ...
    if (path == $"/api/dispatch/{Future.Id}")
      return Task.FromResult(
        Ok(new { id = Future.Id, loadNumber = Future.LoadNumber })
      );
    if (path == $"/api/dispatch/{Planning.DispatchId}")
      return Task.FromResult(
        Ok(new { id = Planning.DispatchId, loadNumber = Planning.LoadNumber })
      );
